...
| `gleaner/`       | A Python directory that contains code for your Dagster repository                 |
| `gleaner_tests/` | A Python directory that contains tests for `gleaner`                      |
...
pyenv virtualenv $PYTHON_VERSION gleaner
pyenv activate gleaner
```

**Conda**
```bash
export PYTHON_VERSION=X.Y.Z
conda create --name gleaner python=PYTHON_VERSION
conda activate gleaner
```
...
Tests can be found in `gleaner_tests` and are run with the following command:

```bash
pytest gleaner_tests
```

As you create Dagster ops and graphs, add tests in `gleaner_tests/` to check that your
