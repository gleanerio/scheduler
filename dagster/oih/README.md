...
| `oih/`       | A Python directory that contains code for your Dagster repository                 |
| `oih_tests/` | A Python directory that contains tests for `oih`                      |
...
pyenv virtualenv $PYTHON_VERSION oih
pyenv activate oih
```
...
```bash
export PYTHON_VERSION=X.Y.Z
conda create --name oih python=PYTHON_VERSION
conda activate oih
```
...
Tests can be found in `oih_tests` and are run with the following command:

```bash
pytest oih_tests
```

As you create Dagster ops and graphs, add tests in `oih_tests/` to check that your
