...
@op
def nwisgw19_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw19")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw19_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw19")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw19_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw19")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw19_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw19")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw19_uploadrelease(context, msg: str):
...
@op
def nwisgw19_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw19")
    ...
    return msg + r
@op
def nwisgw19_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw19")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw19_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw19")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw19_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw19")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw19"
...
def nwisgw19_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw19"
...
def harvest_nwisgw19():
    harvest = nwisgw19_gleaner()

    report_ms3 = nwisgw19_missingreport_s3(harvest)
    report_idstat = nwisgw19_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw19_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw19")
    load_release = nwisgw19_naburelease(harvest)
    load_uploadrelease = nwisgw19_uploadrelease(load_release)

    load_prune = nwisgw19_nabu_prune(load_uploadrelease)
    load_prov = nwisgw19_nabuprov(load_prune)
    load_org = nwisgw19_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw19_missingreport_graph(load_org)
    report_graph=nwisgw19_graph_reports(report_msgraph)
