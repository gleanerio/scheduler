...
@op
def pws0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "pws0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def pws0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "pws0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def pws0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "pws0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def pws0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "pws0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def pws0_uploadrelease(context, msg: str):
...
@op
def pws0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="pws0")
    ...
    return msg + r
@op
def pws0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="pws0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def pws0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="pws0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def pws0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="pws0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "pws0"
...
def pws0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "pws0"
...
@graph
def harvest_pws0():
    harvest = pws0_gleaner()

    report_ms3 = pws0_missingreport_s3(harvest)
    report_idstat = pws0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = pws0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="pws0")
    load_release = pws0_naburelease(harvest)
    load_uploadrelease = pws0_uploadrelease(load_release)

    load_prune = pws0_nabu_prune(load_uploadrelease)
    load_prov = pws0_nabuprov(load_prune)
    load_org = pws0_nabuorg(load_prov)

# run after load
    report_msgraph=pws0_missingreport_graph(load_org)
    report_graph=pws0_graph_reports(report_msgraph)
