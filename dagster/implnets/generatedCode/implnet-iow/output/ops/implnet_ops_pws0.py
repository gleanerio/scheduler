...
def pws0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "pws0")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def pws0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "pws0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def pws0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "pws0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def pws0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "pws0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def pws0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "pws0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def pws0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "pws0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def pws0_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def pws0_uploadrelease(context):
#     returned_value = post_to_graph("pws0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def pws0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="pws0"
    )
...
@op(ins={"start": In(Nothing)})
def pws0_upload_summarize(context):
...
def harvest_pws0():
    ...
    # prov branch
    prov_release = pws0_nabu_provrelease(start=harvest)
    prov_clear = pws0_nabu_provclear(start=prov_release)
    prov_object = pws0_nabu_provobject(start=prov_clear)
    prov_drain = pws0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = pws0_nabu_orgsrelease(start=harvest)
    load_org = pws0_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = pws0_summarize(start=load_uploadrelease)
    # upload_summarize = pws0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = pws0_missingreport_graph(start=summarize)
