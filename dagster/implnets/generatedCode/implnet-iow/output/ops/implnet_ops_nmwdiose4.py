...
@op
def nmwdiose4_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nmwdiose4")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nmwdiose4_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nmwdiose4")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nmwdiose4_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nmwdiose4")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nmwdiose4_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nmwdiose4")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nmwdiose4_uploadrelease(context, msg: str):
...
@op
def nmwdiose4_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nmwdiose4")
    ...
    return msg + r
@op
def nmwdiose4_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nmwdiose4")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nmwdiose4_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nmwdiose4")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nmwdiose4_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nmwdiose4")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nmwdiose4"
...
def nmwdiose4_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nmwdiose4"
...
def harvest_nmwdiose4():
    harvest = nmwdiose4_gleaner()

    report_ms3 = nmwdiose4_missingreport_s3(harvest)
    report_idstat = nmwdiose4_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nmwdiose4_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nmwdiose4")
    load_release = nmwdiose4_naburelease(harvest)
    load_uploadrelease = nmwdiose4_uploadrelease(load_release)

    load_prune = nmwdiose4_nabu_prune(load_uploadrelease)
    load_prov = nmwdiose4_nabuprov(load_prune)
    load_org = nmwdiose4_nabuorg(load_prov)

# run after load
    report_msgraph=nmwdiose4_missingreport_graph(load_org)
    report_graph=nmwdiose4_graph_reports(report_msgraph)
