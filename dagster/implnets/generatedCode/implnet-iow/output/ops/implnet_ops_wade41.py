...
@op(ins={"start": In(Nothing)})
def wade41_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade41")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade41_nabuorg(context):
...
def wade41_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade41")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade41")
    ...
    load_prov = wade41_nabuprov(start=load_prune)
    load_org = wade41_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade41_missingreport_graph(start=load_org)
