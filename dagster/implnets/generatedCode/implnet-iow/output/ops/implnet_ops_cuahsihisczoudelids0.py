...
@op
def cuahsihisczoudelids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisczoudelids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisczoudelids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisczoudelids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisczoudelids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisczoudelids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisczoudelids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisczoudelids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisczoudelids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisczoudelids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisczoudelids0")
    ...
    return msg + r
@op
def cuahsihisczoudelids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisczoudelids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisczoudelids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisczoudelids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisczoudelids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisczoudelids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisczoudelids0"
...
def cuahsihisczoudelids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisczoudelids0"
...
def harvest_cuahsihisczoudelids0():
    harvest = cuahsihisczoudelids0_gleaner()

    report_ms3 = cuahsihisczoudelids0_missingreport_s3(harvest)
    report_idstat = cuahsihisczoudelids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisczoudelids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisczoudelids0")
    load_release = cuahsihisczoudelids0_naburelease(harvest)
    load_uploadrelease = cuahsihisczoudelids0_uploadrelease(load_release)

    load_prune = cuahsihisczoudelids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisczoudelids0_nabuprov(load_prune)
    load_org = cuahsihisczoudelids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisczoudelids0_missingreport_graph(load_org)
    report_graph=cuahsihisczoudelids0_graph_reports(report_msgraph)
