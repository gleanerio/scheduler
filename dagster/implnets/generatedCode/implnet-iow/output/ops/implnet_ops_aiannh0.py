...
def aiannh0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "aiannh0")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def aiannh0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "aiannh0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def aiannh0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "aiannh0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def aiannh0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "aiannh0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def aiannh0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "aiannh0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def aiannh0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "aiannh0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def aiannh0_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def aiannh0_uploadrelease(context):
#     returned_value = post_to_graph("aiannh0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def aiannh0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="aiannh0"
    )
...
@op(ins={"start": In(Nothing)})
def aiannh0_upload_summarize(context):
...
def harvest_aiannh0():
    ...
    # prov branch
    prov_release = aiannh0_nabu_provrelease(start=harvest)
    prov_clear = aiannh0_nabu_provclear(start=prov_release)
    prov_object = aiannh0_nabu_provobject(start=prov_clear)
    prov_drain = aiannh0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = aiannh0_nabu_orgsrelease(start=harvest)
    load_org = aiannh0_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = aiannh0_summarize(start=load_uploadrelease)
    # upload_summarize = aiannh0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = aiannh0_missingreport_graph(start=summarize)
