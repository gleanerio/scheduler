...
@op
def aiannh0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "aiannh0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def aiannh0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "aiannh0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def aiannh0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "aiannh0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def aiannh0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "aiannh0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def aiannh0_uploadrelease(context, msg: str):
...
@op
def aiannh0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="aiannh0")
    ...
    return msg + r
@op
def aiannh0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="aiannh0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def aiannh0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="aiannh0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def aiannh0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="aiannh0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "aiannh0"
...
def aiannh0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "aiannh0"
...
@graph
def harvest_aiannh0():
    harvest = aiannh0_gleaner()

    report_ms3 = aiannh0_missingreport_s3(harvest)
    report_idstat = aiannh0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = aiannh0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="aiannh0")
    load_release = aiannh0_naburelease(harvest)
    load_uploadrelease = aiannh0_uploadrelease(load_release)

    load_prune = aiannh0_nabu_prune(load_uploadrelease)
    load_prov = aiannh0_nabuprov(load_prune)
    load_org = aiannh0_nabuorg(load_prov)

# run after load
    report_msgraph=aiannh0_missingreport_graph(load_org)
    report_graph=aiannh0_graph_reports(report_msgraph)
