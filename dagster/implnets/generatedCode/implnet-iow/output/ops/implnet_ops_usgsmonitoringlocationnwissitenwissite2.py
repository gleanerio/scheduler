...
# #########################  Ops Section
@op
def usgsmonitoringlocationnwissitenwissite2_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def usgsmonitoringlocationnwissitenwissite2_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "usgsmonitoringlocationnwissitenwissite2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwissitenwissite2_naburelease(context):
    returned_value = gleanerio(context, ("release"), "usgsmonitoringlocationnwissitenwissite2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwissitenwissite2_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "usgsmonitoringlocationnwissitenwissite2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwissitenwissite2_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "usgsmonitoringlocationnwissitenwissite2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwissitenwissite2_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "usgsmonitoringlocationnwissitenwissite2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwissitenwissite2_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "usgsmonitoringlocationnwissitenwissite2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwissitenwissite2_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "usgsmonitoringlocationnwissitenwissite2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwissitenwissite2_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "usgsmonitoringlocationnwissitenwissite2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwissitenwissite2_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "usgsmonitoringlocationnwissitenwissite2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwissitenwissite2_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "usgsmonitoringlocationnwissitenwissite2")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def usgsmonitoringlocationnwissitenwissite2_uploadrelease(context):
#     returned_value = post_to_graph("usgsmonitoringlocationnwissitenwissite2", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def usgsmonitoringlocationnwissitenwissite2_uploadrelease(context):
#     returned_value = postRelease("usgsmonitoringlocationnwissitenwissite2")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwissitenwissite2_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="usgsmonitoringlocationnwissitenwissite2"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "usgsmonitoringlocationnwissitenwissite2"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwissitenwissite2_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="usgsmonitoringlocationnwissitenwissite2"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwissitenwissite2_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="usgsmonitoringlocationnwissitenwissite2"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwissitenwissite2_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="usgsmonitoringlocationnwissitenwissite2"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwissitenwissite2_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwissitenwissite2_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwissitenwissite2_upload_summarize(context):
    returned_value = post_to_graph(
        "usgsmonitoringlocationnwissitenwissite2",
        path=SUMMARY_PATH,
        extension="ttl",
...
@graph
def harvest_usgsmonitoringlocationnwissitenwissite2():
    # check for containers
    containers = usgsmonitoringlocationnwissitenwissite2_getImage()

    # conduct the harvest
    harvest = usgsmonitoringlocationnwissitenwissite2_gleaner(start=containers)

    # data branch
    load_release = usgsmonitoringlocationnwissitenwissite2_naburelease(start=harvest)
    load_uploadrelease = usgsmonitoringlocationnwissitenwissite2_uploadrelease(start=load_release)
    load_prune = usgsmonitoringlocationnwissitenwissite2_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = usgsmonitoringlocationnwissitenwissite2_nabu_provrelease(start=harvest)
    prov_clear = usgsmonitoringlocationnwissitenwissite2_nabu_provclear(start=prov_release)
    prov_object = usgsmonitoringlocationnwissitenwissite2_nabu_provobject(start=prov_clear)
    prov_drain = usgsmonitoringlocationnwissitenwissite2_nabu_provdrain(start=prov_object)

    # org branch
    org_release = usgsmonitoringlocationnwissitenwissite2_nabu_orgsrelease(start=harvest)
    load_org = usgsmonitoringlocationnwissitenwissite2_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = usgsmonitoringlocationnwissitenwissite2_missingreport_s3(start=harvest)
    # report_idstat = usgsmonitoringlocationnwissitenwissite2_identifier_stats(start=report_ms3)
    # report_bucketurl = usgsmonitoringlocationnwissitenwissite2_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = usgsmonitoringlocationnwissitenwissite2_summarize(start=load_uploadrelease)
    # upload_summarize = usgsmonitoringlocationnwissitenwissite2_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = usgsmonitoringlocationnwissitenwissite2_missingreport_graph(start=summarize)
    # report_graph = usgsmonitoringlocationnwissitenwissite2_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="usgsmonitoringlocationnwissitenwissite2"):
#
