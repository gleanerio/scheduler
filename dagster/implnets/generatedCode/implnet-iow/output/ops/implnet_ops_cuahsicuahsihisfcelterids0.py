...
# #########################  Ops Section
@op
def cuahsicuahsihisfcelterids0_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def cuahsicuahsihisfcelterids0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "cuahsicuahsihisfcelterids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisfcelterids0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "cuahsicuahsihisfcelterids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisfcelterids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsicuahsihisfcelterids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisfcelterids0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "cuahsicuahsihisfcelterids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisfcelterids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsicuahsihisfcelterids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisfcelterids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsicuahsihisfcelterids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisfcelterids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsicuahsihisfcelterids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisfcelterids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsicuahsihisfcelterids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisfcelterids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsicuahsihisfcelterids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisfcelterids0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "cuahsicuahsihisfcelterids0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihisfcelterids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsicuahsihisfcelterids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihisfcelterids0_uploadrelease(context):
#     returned_value = postRelease("cuahsicuahsihisfcelterids0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisfcelterids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisfcelterids0"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsicuahsihisfcelterids0"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisfcelterids0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisfcelterids0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisfcelterids0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisfcelterids0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisfcelterids0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisfcelterids0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisfcelterids0_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisfcelterids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisfcelterids0_upload_summarize(context):
    returned_value = post_to_graph(
        "cuahsicuahsihisfcelterids0",
        path=SUMMARY_PATH,
        extension="ttl",
...
@graph
def harvest_cuahsicuahsihisfcelterids0():
    # check for containers
    containers = cuahsicuahsihisfcelterids0_getImage()

    # conduct the harvest
    harvest = cuahsicuahsihisfcelterids0_gleaner(start=containers)

    # data branch
    load_release = cuahsicuahsihisfcelterids0_naburelease(start=harvest)
    load_uploadrelease = cuahsicuahsihisfcelterids0_uploadrelease(start=load_release)
    load_prune = cuahsicuahsihisfcelterids0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = cuahsicuahsihisfcelterids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsicuahsihisfcelterids0_nabu_provclear(start=prov_release)
    prov_object = cuahsicuahsihisfcelterids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsicuahsihisfcelterids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsicuahsihisfcelterids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsicuahsihisfcelterids0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = cuahsicuahsihisfcelterids0_missingreport_s3(start=harvest)
    # report_idstat = cuahsicuahsihisfcelterids0_identifier_stats(start=report_ms3)
    # report_bucketurl = cuahsicuahsihisfcelterids0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = cuahsicuahsihisfcelterids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsicuahsihisfcelterids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsicuahsihisfcelterids0_missingreport_graph(start=summarize)
    # report_graph = cuahsicuahsihisfcelterids0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="cuahsicuahsihisfcelterids0"):
#
