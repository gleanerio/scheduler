...
@op
def cuahsihiscalvinhhsids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihiscalvinhhsids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihiscalvinhhsids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihiscalvinhhsids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihiscalvinhhsids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihiscalvinhhsids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihiscalvinhhsids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihiscalvinhhsids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihiscalvinhhsids0_uploadrelease(context, msg: str):
...
@op
def cuahsihiscalvinhhsids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihiscalvinhhsids0")
    ...
    return msg + r
@op
def cuahsihiscalvinhhsids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihiscalvinhhsids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihiscalvinhhsids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihiscalvinhhsids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihiscalvinhhsids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihiscalvinhhsids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihiscalvinhhsids0"
...
def cuahsihiscalvinhhsids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihiscalvinhhsids0"
...
def harvest_cuahsihiscalvinhhsids0():
    harvest = cuahsihiscalvinhhsids0_gleaner()

    report_ms3 = cuahsihiscalvinhhsids0_missingreport_s3(harvest)
    report_idstat = cuahsihiscalvinhhsids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihiscalvinhhsids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihiscalvinhhsids0")
    load_release = cuahsihiscalvinhhsids0_naburelease(harvest)
    load_uploadrelease = cuahsihiscalvinhhsids0_uploadrelease(load_release)

    load_prune = cuahsihiscalvinhhsids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihiscalvinhhsids0_nabuprov(load_prune)
    load_org = cuahsihiscalvinhhsids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihiscalvinhhsids0_missingreport_graph(load_org)
    report_graph=cuahsihiscalvinhhsids0_graph_reports(report_msgraph)
