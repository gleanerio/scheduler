...
@op
def nwisgw11_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw11")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw11_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw11")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw11_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw11")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw11_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw11")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw11_uploadrelease(context, msg: str):
...
@op
def nwisgw11_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw11")
    ...
    return msg + r
@op
def nwisgw11_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw11")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw11_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw11")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw11_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw11")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw11"
...
def nwisgw11_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw11"
...
def harvest_nwisgw11():
    harvest = nwisgw11_gleaner()

    report_ms3 = nwisgw11_missingreport_s3(harvest)
    report_idstat = nwisgw11_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw11_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw11")
    load_release = nwisgw11_naburelease(harvest)
    load_uploadrelease = nwisgw11_uploadrelease(load_release)

    load_prune = nwisgw11_nabu_prune(load_uploadrelease)
    load_prov = nwisgw11_nabuprov(load_prune)
    load_org = nwisgw11_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw11_missingreport_graph(load_org)
    report_graph=nwisgw11_graph_reports(report_msgraph)
