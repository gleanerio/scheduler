...
@op
def nwisgw26_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw26")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw26_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw26")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw26_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw26")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw26_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw26")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw26_uploadrelease(context, msg: str):
...
@op
def nwisgw26_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw26")
    ...
    return msg + r
@op
def nwisgw26_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw26")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw26_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw26")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw26_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw26")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw26"
...
def nwisgw26_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw26"
...
def harvest_nwisgw26():
    harvest = nwisgw26_gleaner()

    report_ms3 = nwisgw26_missingreport_s3(harvest)
    report_idstat = nwisgw26_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw26_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw26")
    load_release = nwisgw26_naburelease(harvest)
    load_uploadrelease = nwisgw26_uploadrelease(load_release)

    load_prune = nwisgw26_nabu_prune(load_uploadrelease)
    load_prov = nwisgw26_nabuprov(load_prune)
    load_org = nwisgw26_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw26_missingreport_graph(load_org)
    report_graph=nwisgw26_graph_reports(report_msgraph)
