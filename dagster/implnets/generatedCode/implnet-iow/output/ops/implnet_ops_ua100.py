...
def ua100_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "ua100")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def ua100_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "ua100")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ua100_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "ua100")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ua100_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "ua100")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ua100_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "ua100")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ua100_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "ua100")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ua100_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def ua100_uploadrelease(context):
#     returned_value = post_to_graph("ua100", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def ua100_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="ua100"
    )
...
@op(ins={"start": In(Nothing)})
def ua100_upload_summarize(context):
...
def harvest_ua100():
    ...
    # prov branch
    prov_release = ua100_nabu_provrelease(start=harvest)
    prov_clear = ua100_nabu_provclear(start=prov_release)
    prov_object = ua100_nabu_provobject(start=prov_clear)
    prov_drain = ua100_nabu_provdrain(start=prov_object)

    # org branch
    org_release = ua100_nabu_orgsrelease(start=harvest)
    load_org = ua100_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = ua100_summarize(start=load_uploadrelease)
    # upload_summarize = ua100_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = ua100_missingreport_graph(start=summarize)
