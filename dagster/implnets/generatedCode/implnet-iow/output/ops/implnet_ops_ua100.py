...
@op
def ua100_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "ua100")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def ua100_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "ua100")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def ua100_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "ua100")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def ua100_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "ua100")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def ua100_uploadrelease(context, msg: str):
...
@op
def ua100_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="ua100")
    ...
    return msg + r
@op
def ua100_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="ua100")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def ua100_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="ua100")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def ua100_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="ua100")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "ua100"
...
def ua100_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "ua100"
...
@graph
def harvest_ua100():
    harvest = ua100_gleaner()

    report_ms3 = ua100_missingreport_s3(harvest)
    report_idstat = ua100_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = ua100_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="ua100")
    load_release = ua100_naburelease(harvest)
    load_uploadrelease = ua100_uploadrelease(load_release)

    load_prune = ua100_nabu_prune(load_uploadrelease)
    load_prov = ua100_nabuprov(load_prune)
    load_org = ua100_nabuorg(load_prov)

# run after load
    report_msgraph=ua100_missingreport_graph(load_org)
    report_graph=ua100_graph_reports(report_msgraph)
