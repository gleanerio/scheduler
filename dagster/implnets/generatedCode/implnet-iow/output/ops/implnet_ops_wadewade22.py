...
# #########################  Ops Section
@op
def wadewade22_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def wadewade22_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "wadewade22")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade22_naburelease(context):
    returned_value = gleanerio(context, ("release"), "wadewade22")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade22_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wadewade22")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade22_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "wadewade22")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade22_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wadewade22")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade22_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wadewade22")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade22_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wadewade22")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade22_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wadewade22")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade22_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wadewade22")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade22_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "wadewade22")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wadewade22_uploadrelease(context):
#     returned_value = post_to_graph("wadewade22", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wadewade22_uploadrelease(context):
#     returned_value = postRelease("wadewade22")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def wadewade22_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade22"
    )
...
@op(ins={"start": In(Nothing)})
def wadewade22_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade22"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def wadewade22_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade22"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def wadewade22_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade22"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def wadewade22_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wadewade22_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wadewade22_upload_summarize(context):
...
@graph
def harvest_wadewade22():
    # check for containers
    containers = wadewade22_getImage()

    # conduct the harvest
    harvest = wadewade22_gleaner(start=containers)

    # data branch
    load_release = wadewade22_naburelease(start=harvest)
    load_uploadrelease = wadewade22_uploadrelease(start=load_release)
    load_prune = wadewade22_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = wadewade22_nabu_provrelease(start=harvest)
    prov_clear = wadewade22_nabu_provclear(start=prov_release)
    prov_object = wadewade22_nabu_provobject(start=prov_clear)
    prov_drain = wadewade22_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wadewade22_nabu_orgsrelease(start=harvest)
    load_org = wadewade22_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = wadewade22_missingreport_s3(start=harvest)
    # report_idstat = wadewade22_identifier_stats(start=report_ms3)
    # report_bucketurl = wadewade22_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = wadewade22_summarize(start=load_uploadrelease)
    # upload_summarize = wadewade22_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wadewade22_missingreport_graph(start=summarize)
    # report_graph = wadewade22_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="wadewade22"):
#
