...
@op
def refgage1_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "refgage1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def refgage1_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "refgage1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def refgage1_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "refgage1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def refgage1_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "refgage1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def refgage1_uploadrelease(context, msg: str):
...
@op
def refgage1_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="refgage1")
    ...
    return msg + r
@op
def refgage1_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="refgage1")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def refgage1_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="refgage1")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def refgage1_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="refgage1")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "refgage1"
...
def refgage1_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "refgage1"
...
def harvest_refgage1():
    harvest = refgage1_gleaner()

    report_ms3 = refgage1_missingreport_s3(harvest)
    report_idstat = refgage1_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = refgage1_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="refgage1")
    load_release = refgage1_naburelease(harvest)
    load_uploadrelease = refgage1_uploadrelease(load_release)

    load_prune = refgage1_nabu_prune(load_uploadrelease)
    load_prov = refgage1_nabuprov(load_prune)
    load_org = refgage1_nabuorg(load_prov)

# run after load
    report_msgraph=refgage1_missingreport_graph(load_org)
    report_graph=refgage1_graph_reports(report_msgraph)
