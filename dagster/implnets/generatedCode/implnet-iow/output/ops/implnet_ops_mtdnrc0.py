...
@op(ins={"start": In(Nothing)})
def mtdnrc0_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "mtdnrc0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def mtdnrc0_nabuorg(context):
...
def mtdnrc0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="mtdnrc0")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="mtdnrc0")
    ...
    load_prov = mtdnrc0_nabuprov(start=load_prune)
    load_org = mtdnrc0_nabuorg(start=load_prov)

# run after load
    report_msgraph=mtdnrc0_missingreport_graph(start=load_org)
