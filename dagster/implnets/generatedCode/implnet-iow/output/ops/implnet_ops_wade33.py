...
def wade33_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "wade33")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade33_naburelease(context):
    returned_value = gleanerio(context, ("release"), "wade33")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade33_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wade33")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade33_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "wade33")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade33_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wade33")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade33_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wade33")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade33_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wade33")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade33_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wade33")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade33_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wade33")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade33_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "wade33")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wade33_uploadrelease(context):
#     returned_value = post_to_graph("wade33", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wade33_uploadrelease(context):
#     returned_value = postRelease("wade33")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def wade33_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade33"
    )
...
@op(ins={"start": In(Nothing)})
def wade33_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade33"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def wade33_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade33"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def wade33_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade33"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def wade33_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wade33_upload_summarize(context):
...
def harvest_wade33():
    ...
    containers = wade33_getImage()

    # conduct the harvest
    harvest = wade33_gleaner(start=containers)

    # data branch
    load_release = wade33_naburelease(start=harvest)
    load_uploadrelease = wade33_uploadrelease(start=load_release)
    load_prune = wade33_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = wade33_nabu_provrelease(start=harvest)
    prov_clear = wade33_nabu_provclear(start=prov_release)
    prov_object = wade33_nabu_provobject(start=prov_clear)
    prov_drain = wade33_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wade33_nabu_orgsrelease(start=harvest)
    load_org = wade33_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = wade33_missingreport_s3(start=harvest)
    # report_idstat = wade33_identifier_stats(start=report_ms3)
    # report_bucketurl = wade33_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = wade33_summarize(start=load_uploadrelease)
    # upload_summarize = wade33_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wade33_missingreport_graph(start=summarize)
    # report_graph = wade33_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="wade33"):
#
