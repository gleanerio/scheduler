...
@op(ins={"start": In(Nothing)})
def wade33_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade33")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade33_nabuorg(context):
...
def wade33_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade33")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade33")
    ...
    load_prov = wade33_nabuprov(start=load_prune)
    load_org = wade33_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade33_missingreport_graph(start=load_org)
