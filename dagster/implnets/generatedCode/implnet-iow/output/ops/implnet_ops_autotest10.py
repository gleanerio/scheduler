...
@op
def autotest10_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "autotest10")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def autotest10_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "autotest10")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def autotest10_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "autotest10")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def autotest10_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "autotest10")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def autotest10_uploadrelease(context, msg: str):
...
@op
def autotest10_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="autotest10")
    ...
    return msg + r
@op
def autotest10_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="autotest10")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def autotest10_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="autotest10")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def autotest10_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="autotest10")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "autotest10"
...
def autotest10_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "autotest10"
...
def harvest_autotest10():
    harvest = autotest10_gleaner()

    report_ms3 = autotest10_missingreport_s3(harvest)
    report_idstat = autotest10_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = autotest10_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="autotest10")
    load_release = autotest10_naburelease(harvest)
    load_uploadrelease = autotest10_uploadrelease(load_release)

    load_prune = autotest10_nabu_prune(load_uploadrelease)
    load_prov = autotest10_nabuprov(load_prune)
    load_org = autotest10_nabuorg(load_prov)

# run after load
    report_msgraph=autotest10_missingreport_graph(load_org)
    report_graph=autotest10_graph_reports(report_msgraph)
