...
def princiaq0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "princiaq0")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def princiaq0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "princiaq0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def princiaq0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "princiaq0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def princiaq0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "princiaq0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def princiaq0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "princiaq0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def princiaq0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "princiaq0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def princiaq0_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def princiaq0_uploadrelease(context):
#     returned_value = post_to_graph("princiaq0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def princiaq0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="princiaq0"
    )
...
@op(ins={"start": In(Nothing)})
def princiaq0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def princiaq0_upload_summarize(context):
...
def harvest_princiaq0():
    ...
    # prov branch
    prov_release = princiaq0_nabu_provrelease(start=harvest)
    prov_clear = princiaq0_nabu_provclear(start=prov_release)
    prov_object = princiaq0_nabu_provobject(start=prov_clear)
    prov_drain = princiaq0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = princiaq0_nabu_orgsrelease(start=harvest)
    load_org = princiaq0_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = princiaq0_summarize(start=load_uploadrelease)
    # upload_summarize = princiaq0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = princiaq0_missingreport_graph(start=summarize)
