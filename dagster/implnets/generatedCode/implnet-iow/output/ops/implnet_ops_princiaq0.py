...
@op
def princiaq0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "princiaq0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def princiaq0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "princiaq0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def princiaq0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "princiaq0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def princiaq0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "princiaq0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def princiaq0_uploadrelease(context, msg: str):
...
@op
def princiaq0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="princiaq0")
    ...
    return msg + r
@op
def princiaq0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="princiaq0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def princiaq0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="princiaq0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def princiaq0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="princiaq0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "princiaq0"
...
def princiaq0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "princiaq0"
...
def harvest_princiaq0():
    harvest = princiaq0_gleaner()

    report_ms3 = princiaq0_missingreport_s3(harvest)
    report_idstat = princiaq0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = princiaq0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="princiaq0")
    load_release = princiaq0_naburelease(harvest)
    load_uploadrelease = princiaq0_uploadrelease(load_release)

    load_prune = princiaq0_nabu_prune(load_uploadrelease)
    load_prov = princiaq0_nabuprov(load_prune)
    load_org = princiaq0_nabuorg(load_prov)

# run after load
    report_msgraph=princiaq0_missingreport_graph(load_org)
    report_graph=princiaq0_graph_reports(report_msgraph)
