...
@op
def refgage3_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "refgage3")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def refgage3_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "refgage3")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def refgage3_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "refgage3")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def refgage3_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "refgage3")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def refgage3_uploadrelease(context, msg: str):
...
@op
def refgage3_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="refgage3")
    ...
    return msg + r
@op
def refgage3_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="refgage3")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def refgage3_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="refgage3")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def refgage3_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="refgage3")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "refgage3"
...
def refgage3_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "refgage3"
...
def harvest_refgage3():
    harvest = refgage3_gleaner()

    report_ms3 = refgage3_missingreport_s3(harvest)
    report_idstat = refgage3_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = refgage3_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="refgage3")
    load_release = refgage3_naburelease(harvest)
    load_uploadrelease = refgage3_uploadrelease(load_release)

    load_prune = refgage3_nabu_prune(load_uploadrelease)
    load_prov = refgage3_nabuprov(load_prune)
    load_org = refgage3_nabuorg(load_prov)

# run after load
    report_msgraph=refgage3_missingreport_graph(load_org)
    report_graph=refgage3_graph_reports(report_msgraph)
