...
def states0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "states0")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def states0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "states0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def states0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "states0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def states0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "states0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def states0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "states0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def states0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "states0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def states0_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def states0_uploadrelease(context):
#     returned_value = post_to_graph("states0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def states0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="states0"
    )
...
@op(ins={"start": In(Nothing)})
def states0_upload_summarize(context):
...
def harvest_states0():
    ...
    # prov branch
    prov_release = states0_nabu_provrelease(start=harvest)
    prov_clear = states0_nabu_provclear(start=prov_release)
    prov_object = states0_nabu_provobject(start=prov_clear)
    prov_drain = states0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = states0_nabu_orgsrelease(start=harvest)
    load_org = states0_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = states0_summarize(start=load_uploadrelease)
    # upload_summarize = states0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = states0_missingreport_graph(start=summarize)
