...
@op
def states0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "states0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def states0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "states0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def states0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "states0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def states0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "states0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def states0_uploadrelease(context, msg: str):
...
@op
def states0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="states0")
    ...
    return msg + r
@op
def states0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="states0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def states0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="states0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def states0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="states0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "states0"
...
def states0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "states0"
...
@graph
def harvest_states0():
    harvest = states0_gleaner()

    report_ms3 = states0_missingreport_s3(harvest)
    report_idstat = states0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = states0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="states0")
    load_release = states0_naburelease(harvest)
    load_uploadrelease = states0_uploadrelease(load_release)

    load_prune = states0_nabu_prune(load_uploadrelease)
    load_prov = states0_nabuprov(load_prune)
    load_org = states0_nabuorg(load_prov)

# run after load
    report_msgraph=states0_missingreport_graph(load_org)
    report_graph=states0_graph_reports(report_msgraph)
