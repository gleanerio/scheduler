...
@op(ins={"start": In(Nothing)})
def wade38_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade38")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade38_nabuorg(context):
...
def wade38_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade38")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade38")
    ...
    load_prov = wade38_nabuprov(start=load_prune)
    load_org = wade38_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade38_missingreport_graph(start=load_org)
