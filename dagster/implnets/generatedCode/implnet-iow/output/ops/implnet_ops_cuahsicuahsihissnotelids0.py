...
# #########################  Ops Section
@op
def cuahsicuahsihissnotelids0_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def cuahsicuahsihissnotelids0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "cuahsicuahsihissnotelids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihissnotelids0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "cuahsicuahsihissnotelids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihissnotelids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsicuahsihissnotelids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihissnotelids0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "cuahsicuahsihissnotelids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihissnotelids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsicuahsihissnotelids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihissnotelids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsicuahsihissnotelids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihissnotelids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsicuahsihissnotelids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihissnotelids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsicuahsihissnotelids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihissnotelids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsicuahsihissnotelids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihissnotelids0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "cuahsicuahsihissnotelids0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihissnotelids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsicuahsihissnotelids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihissnotelids0_uploadrelease(context):
#     returned_value = postRelease("cuahsicuahsihissnotelids0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihissnotelids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihissnotelids0"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsicuahsihissnotelids0"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihissnotelids0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihissnotelids0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihissnotelids0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihissnotelids0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihissnotelids0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihissnotelids0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihissnotelids0_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihissnotelids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihissnotelids0_upload_summarize(context):
    returned_value = post_to_graph(
        "cuahsicuahsihissnotelids0",
        path=SUMMARY_PATH,
        extension="ttl",
...
@graph
def harvest_cuahsicuahsihissnotelids0():
    # check for containers
    containers = cuahsicuahsihissnotelids0_getImage()

    # conduct the harvest
    harvest = cuahsicuahsihissnotelids0_gleaner(start=containers)

    # data branch
    load_release = cuahsicuahsihissnotelids0_naburelease(start=harvest)
    load_uploadrelease = cuahsicuahsihissnotelids0_uploadrelease(start=load_release)
    load_prune = cuahsicuahsihissnotelids0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = cuahsicuahsihissnotelids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsicuahsihissnotelids0_nabu_provclear(start=prov_release)
    prov_object = cuahsicuahsihissnotelids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsicuahsihissnotelids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsicuahsihissnotelids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsicuahsihissnotelids0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = cuahsicuahsihissnotelids0_missingreport_s3(start=harvest)
    # report_idstat = cuahsicuahsihissnotelids0_identifier_stats(start=report_ms3)
    # report_bucketurl = cuahsicuahsihissnotelids0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = cuahsicuahsihissnotelids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsicuahsihissnotelids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsicuahsihissnotelids0_missingreport_graph(start=summarize)
    # report_graph = cuahsicuahsihissnotelids0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="cuahsicuahsihissnotelids0"):
#
