...
# #########################  Ops Section
@op
def wadewade11_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def wadewade11_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "wadewade11")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade11_naburelease(context):
    returned_value = gleanerio(context, ("release"), "wadewade11")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade11_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wadewade11")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade11_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "wadewade11")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade11_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wadewade11")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade11_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wadewade11")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade11_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wadewade11")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade11_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wadewade11")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade11_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wadewade11")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade11_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "wadewade11")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wadewade11_uploadrelease(context):
#     returned_value = post_to_graph("wadewade11", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wadewade11_uploadrelease(context):
#     returned_value = postRelease("wadewade11")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def wadewade11_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade11"
    )
...
@op(ins={"start": In(Nothing)})
def wadewade11_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade11"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def wadewade11_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade11"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def wadewade11_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade11"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def wadewade11_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wadewade11_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wadewade11_upload_summarize(context):
...
@graph
def harvest_wadewade11():
    # check for containers
    containers = wadewade11_getImage()

    # conduct the harvest
    harvest = wadewade11_gleaner(start=containers)

    # data branch
    load_release = wadewade11_naburelease(start=harvest)
    load_uploadrelease = wadewade11_uploadrelease(start=load_release)
    load_prune = wadewade11_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = wadewade11_nabu_provrelease(start=harvest)
    prov_clear = wadewade11_nabu_provclear(start=prov_release)
    prov_object = wadewade11_nabu_provobject(start=prov_clear)
    prov_drain = wadewade11_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wadewade11_nabu_orgsrelease(start=harvest)
    load_org = wadewade11_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = wadewade11_missingreport_s3(start=harvest)
    # report_idstat = wadewade11_identifier_stats(start=report_ms3)
    # report_bucketurl = wadewade11_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = wadewade11_summarize(start=load_uploadrelease)
    # upload_summarize = wadewade11_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wadewade11_missingreport_graph(start=summarize)
    # report_graph = wadewade11_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="wadewade11"):
#
