...
@op
def cuahsihislittlebearriverids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihislittlebearriverids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihislittlebearriverids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihislittlebearriverids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihislittlebearriverids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihislittlebearriverids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihislittlebearriverids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihislittlebearriverids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihislittlebearriverids0_uploadrelease(context, msg: str):
...
@op
def cuahsihislittlebearriverids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihislittlebearriverids0")
    ...
    return msg + r
@op
def cuahsihislittlebearriverids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihislittlebearriverids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihislittlebearriverids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihislittlebearriverids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihislittlebearriverids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihislittlebearriverids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihislittlebearriverids0"
...
def cuahsihislittlebearriverids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihislittlebearriverids0"
...
def harvest_cuahsihislittlebearriverids0():
    harvest = cuahsihislittlebearriverids0_gleaner()

    report_ms3 = cuahsihislittlebearriverids0_missingreport_s3(harvest)
    report_idstat = cuahsihislittlebearriverids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihislittlebearriverids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihislittlebearriverids0")
    load_release = cuahsihislittlebearriverids0_naburelease(harvest)
    load_uploadrelease = cuahsihislittlebearriverids0_uploadrelease(load_release)

    load_prune = cuahsihislittlebearriverids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihislittlebearriverids0_nabuprov(load_prune)
    load_org = cuahsihislittlebearriverids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihislittlebearriverids0_missingreport_graph(load_org)
    report_graph=cuahsihislittlebearriverids0_graph_reports(report_msgraph)
