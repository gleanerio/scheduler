...
@op
def cuahsihisczoarizids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisczoarizids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisczoarizids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisczoarizids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisczoarizids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisczoarizids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisczoarizids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisczoarizids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisczoarizids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisczoarizids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisczoarizids0")
    ...
    return msg + r
@op
def cuahsihisczoarizids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisczoarizids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisczoarizids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisczoarizids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisczoarizids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisczoarizids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisczoarizids0"
...
def cuahsihisczoarizids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisczoarizids0"
...
def harvest_cuahsihisczoarizids0():
    harvest = cuahsihisczoarizids0_gleaner()

    report_ms3 = cuahsihisczoarizids0_missingreport_s3(harvest)
    report_idstat = cuahsihisczoarizids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisczoarizids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisczoarizids0")
    load_release = cuahsihisczoarizids0_naburelease(harvest)
    load_uploadrelease = cuahsihisczoarizids0_uploadrelease(load_release)

    load_prune = cuahsihisczoarizids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisczoarizids0_nabuprov(load_prune)
    load_org = cuahsihisczoarizids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisczoarizids0_missingreport_graph(load_org)
    report_graph=cuahsihisczoarizids0_graph_reports(report_msgraph)
