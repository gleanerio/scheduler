...
@op
def chyldpilotids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "chyldpilotids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def chyldpilotids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "chyldpilotids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def chyldpilotids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "chyldpilotids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def chyldpilotids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "chyldpilotids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def chyldpilotids0_uploadrelease(context, msg: str):
...
@op
def chyldpilotids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="chyldpilotids0")
    ...
    return msg + r
@op
def chyldpilotids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="chyldpilotids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def chyldpilotids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="chyldpilotids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def chyldpilotids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="chyldpilotids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "chyldpilotids0"
...
def chyldpilotids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "chyldpilotids0"
...
def harvest_chyldpilotids0():
    harvest = chyldpilotids0_gleaner()

    report_ms3 = chyldpilotids0_missingreport_s3(harvest)
    report_idstat = chyldpilotids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = chyldpilotids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="chyldpilotids0")
    load_release = chyldpilotids0_naburelease(harvest)
    load_uploadrelease = chyldpilotids0_uploadrelease(load_release)

    load_prune = chyldpilotids0_nabu_prune(load_uploadrelease)
    load_prov = chyldpilotids0_nabuprov(load_prune)
    load_org = chyldpilotids0_nabuorg(load_prov)

# run after load
    report_msgraph=chyldpilotids0_missingreport_graph(load_org)
    report_graph=chyldpilotids0_graph_reports(report_msgraph)
