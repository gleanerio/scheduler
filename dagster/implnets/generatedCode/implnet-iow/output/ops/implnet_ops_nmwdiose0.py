...
@op
def nmwdiose0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nmwdiose0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nmwdiose0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nmwdiose0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nmwdiose0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nmwdiose0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nmwdiose0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nmwdiose0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nmwdiose0_uploadrelease(context, msg: str):
...
@op
def nmwdiose0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nmwdiose0")
    ...
    return msg + r
@op
def nmwdiose0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nmwdiose0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nmwdiose0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nmwdiose0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nmwdiose0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nmwdiose0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nmwdiose0"
...
def nmwdiose0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nmwdiose0"
...
def harvest_nmwdiose0():
    harvest = nmwdiose0_gleaner()

    report_ms3 = nmwdiose0_missingreport_s3(harvest)
    report_idstat = nmwdiose0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nmwdiose0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nmwdiose0")
    load_release = nmwdiose0_naburelease(harvest)
    load_uploadrelease = nmwdiose0_uploadrelease(load_release)

    load_prune = nmwdiose0_nabu_prune(load_uploadrelease)
    load_prov = nmwdiose0_nabuprov(load_prune)
    load_org = nmwdiose0_nabuorg(load_prov)

# run after load
    report_msgraph=nmwdiose0_missingreport_graph(load_org)
    report_graph=nmwdiose0_graph_reports(report_msgraph)
