...
@op
def cuahsihiscrwaids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihiscrwaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihiscrwaids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihiscrwaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihiscrwaids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihiscrwaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihiscrwaids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihiscrwaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihiscrwaids0_uploadrelease(context, msg: str):
...
@op
def cuahsihiscrwaids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihiscrwaids0")
    ...
    return msg + r
@op
def cuahsihiscrwaids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihiscrwaids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihiscrwaids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihiscrwaids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihiscrwaids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihiscrwaids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihiscrwaids0"
...
def cuahsihiscrwaids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihiscrwaids0"
...
def harvest_cuahsihiscrwaids0():
    harvest = cuahsihiscrwaids0_gleaner()

    report_ms3 = cuahsihiscrwaids0_missingreport_s3(harvest)
    report_idstat = cuahsihiscrwaids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihiscrwaids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihiscrwaids0")
    load_release = cuahsihiscrwaids0_naburelease(harvest)
    load_uploadrelease = cuahsihiscrwaids0_uploadrelease(load_release)

    load_prune = cuahsihiscrwaids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihiscrwaids0_nabuprov(load_prune)
    load_org = cuahsihiscrwaids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihiscrwaids0_missingreport_graph(load_org)
    report_graph=cuahsihiscrwaids0_graph_reports(report_msgraph)
