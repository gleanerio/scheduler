...
@op
def wade8_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "wade8")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
...
def wade8_naburelease(context, msg: str):
    ...
    return msg + r
@op
def wade8_uploadrelease(context, msg: str):
...
@op
def wade8_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade8")
    ...
    return msg + r
@op
def wade8_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade8")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def wade8_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade8")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def wade8_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade8")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade8"
...
def wade8_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade8"
...
def harvest_wade8():
    ...
    report_ms3 = wade8_missingreport_s3(harvest)
    report_idstat = wade8_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = wade8_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="wade8")
    load_release = wade8_naburelease(harvest)
    load_uploadrelease = wade8_uploadrelease(load_release)

    load_prune = wade8_nabu_prune(load_uploadrelease)
    load_prov = wade8_nabuprov(load_prune)
    load_org = wade8_nabuorg(load_prov)

# run after load
    report_msgraph=wade8_missingreport_graph(load_org)
    report_graph=wade8_graph_reports(report_msgraph)
