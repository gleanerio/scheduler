...
def wade8_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wade8")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def wade8_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wade8")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade8_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wade8")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade8_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wade8")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade8_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wade8")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade8_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wade8")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade8_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def wade8_uploadrelease(context):
#     returned_value = post_to_graph("wade8", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def wade8_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade8"
    )
...
@op(ins={"start": In(Nothing)})
def wade8_upload_summarize(context):
...
def harvest_wade8():
    ...
    # prov branch
    prov_release = wade8_nabu_provrelease(start=harvest)
    prov_clear = wade8_nabu_provclear(start=prov_release)
    prov_object = wade8_nabu_provobject(start=prov_clear)
    prov_drain = wade8_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wade8_nabu_orgsrelease(start=harvest)
    load_org = wade8_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = wade8_summarize(start=load_uploadrelease)
    # upload_summarize = wade8_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wade8_missingreport_graph(start=summarize)
