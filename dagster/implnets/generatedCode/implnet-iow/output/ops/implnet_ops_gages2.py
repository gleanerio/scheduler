...
def gages2_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "gages2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages2_naburelease(context):
    returned_value = gleanerio(context, ("release"), "gages2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages2_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "gages2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages2_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "gages2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages2_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "gages2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages2_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "gages2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages2_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "gages2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages2_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "gages2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages2_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "gages2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages2_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "gages2")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def gages2_uploadrelease(context):
#     returned_value = post_to_graph("gages2", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def gages2_uploadrelease(context):
#     returned_value = postRelease("gages2")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def gages2_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="gages2"
    )
...
@op(ins={"start": In(Nothing)})
def gages2_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="gages2"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def gages2_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="gages2"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def gages2_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="gages2"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def gages2_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def gages2_upload_summarize(context):
...
def harvest_gages2():
    ...
    containers = gages2_getImage()

    # conduct the harvest
    harvest = gages2_gleaner(start=containers)

    # data branch
    load_release = gages2_naburelease(start=harvest)
    load_uploadrelease = gages2_uploadrelease(start=load_release)
    load_prune = gages2_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = gages2_nabu_provrelease(start=harvest)
    prov_clear = gages2_nabu_provclear(start=prov_release)
    prov_object = gages2_nabu_provobject(start=prov_clear)
    prov_drain = gages2_nabu_provdrain(start=prov_object)

    # org branch
    org_release = gages2_nabu_orgsrelease(start=harvest)
    load_org = gages2_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = gages2_missingreport_s3(start=harvest)
    # report_idstat = gages2_identifier_stats(start=report_ms3)
    # report_bucketurl = gages2_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = gages2_summarize(start=load_uploadrelease)
    # upload_summarize = gages2_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = gages2_missingreport_graph(start=summarize)
    # report_graph = gages2_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="gages2"):
#
