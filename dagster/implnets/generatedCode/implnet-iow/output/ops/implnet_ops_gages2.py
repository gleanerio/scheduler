...
@op(ins={"start": In(Nothing)})
def gages2_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "gages2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages2_nabuorg(context):
...
def gages2_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="gages2")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="gages2")
    ...
    load_prov = gages2_nabuprov(start=load_prune)
    load_org = gages2_nabuorg(start=load_prov)

# run after load
    report_msgraph=gages2_missingreport_graph(start=load_org)
