...
# #########################  Ops Section
@op
def cuahsicuahsihishassbergeids0_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def cuahsicuahsihishassbergeids0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "cuahsicuahsihishassbergeids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihishassbergeids0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "cuahsicuahsihishassbergeids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihishassbergeids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsicuahsihishassbergeids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihishassbergeids0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "cuahsicuahsihishassbergeids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihishassbergeids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsicuahsihishassbergeids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihishassbergeids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsicuahsihishassbergeids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihishassbergeids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsicuahsihishassbergeids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihishassbergeids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsicuahsihishassbergeids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihishassbergeids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsicuahsihishassbergeids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihishassbergeids0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "cuahsicuahsihishassbergeids0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihishassbergeids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsicuahsihishassbergeids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihishassbergeids0_uploadrelease(context):
#     returned_value = postRelease("cuahsicuahsihishassbergeids0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihishassbergeids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihishassbergeids0"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsicuahsihishassbergeids0"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihishassbergeids0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihishassbergeids0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihishassbergeids0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihishassbergeids0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihishassbergeids0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihishassbergeids0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihishassbergeids0_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihishassbergeids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihishassbergeids0_upload_summarize(context):
    returned_value = post_to_graph(
        "cuahsicuahsihishassbergeids0",
        path=SUMMARY_PATH,
        extension="ttl",
...
@graph
def harvest_cuahsicuahsihishassbergeids0():
    # check for containers
    containers = cuahsicuahsihishassbergeids0_getImage()

    # conduct the harvest
    harvest = cuahsicuahsihishassbergeids0_gleaner(start=containers)

    # data branch
    load_release = cuahsicuahsihishassbergeids0_naburelease(start=harvest)
    load_uploadrelease = cuahsicuahsihishassbergeids0_uploadrelease(start=load_release)
    load_prune = cuahsicuahsihishassbergeids0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = cuahsicuahsihishassbergeids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsicuahsihishassbergeids0_nabu_provclear(start=prov_release)
    prov_object = cuahsicuahsihishassbergeids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsicuahsihishassbergeids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsicuahsihishassbergeids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsicuahsihishassbergeids0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = cuahsicuahsihishassbergeids0_missingreport_s3(start=harvest)
    # report_idstat = cuahsicuahsihishassbergeids0_identifier_stats(start=report_ms3)
    # report_bucketurl = cuahsicuahsihishassbergeids0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = cuahsicuahsihishassbergeids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsicuahsihishassbergeids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsicuahsihishassbergeids0_missingreport_graph(start=summarize)
    # report_graph = cuahsicuahsihishassbergeids0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="cuahsicuahsihishassbergeids0"):
#
