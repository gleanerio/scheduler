...
@op
def hu040_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "hu040")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def hu040_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "hu040")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def hu040_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "hu040")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def hu040_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "hu040")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def hu040_uploadrelease(context, msg: str):
...
@op
def hu040_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hu040")
    ...
    return msg + r
@op
def hu040_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hu040")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def hu040_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hu040")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def hu040_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hu040")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "hu040"
...
def hu040_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "hu040"
...
@graph
def harvest_hu040():
    harvest = hu040_gleaner()

    report_ms3 = hu040_missingreport_s3(harvest)
    report_idstat = hu040_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = hu040_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="hu040")
    load_release = hu040_naburelease(harvest)
    load_uploadrelease = hu040_uploadrelease(load_release)

    load_prune = hu040_nabu_prune(load_uploadrelease)
    load_prov = hu040_nabuprov(load_prune)
    load_org = hu040_nabuorg(load_prov)

# run after load
    report_msgraph=hu040_missingreport_graph(load_org)
    report_graph=hu040_graph_reports(report_msgraph)
