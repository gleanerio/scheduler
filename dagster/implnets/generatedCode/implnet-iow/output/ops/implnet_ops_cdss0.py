...
def cdss0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "cdss0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cdss0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "cdss0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cdss0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cdss0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cdss0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "cdss0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cdss0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cdss0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cdss0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cdss0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cdss0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cdss0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cdss0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cdss0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cdss0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cdss0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cdss0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "cdss0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cdss0_uploadrelease(context):
#     returned_value = post_to_graph("cdss0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cdss0_uploadrelease(context):
#     returned_value = postRelease("cdss0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def cdss0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cdss0"
    )
...
@op(ins={"start": In(Nothing)})
def cdss0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cdss0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def cdss0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cdss0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def cdss0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cdss0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def cdss0_upload_summarize(context):
...
def harvest_cdss0():
    ...
    containers = cdss0_getImage()

    # conduct the harvest
    harvest = cdss0_gleaner(start=containers)

    # data branch
    load_release = cdss0_naburelease(start=harvest)
    load_uploadrelease = cdss0_uploadrelease(start=load_release)
    load_prune = cdss0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = cdss0_nabu_provrelease(start=harvest)
    prov_clear = cdss0_nabu_provclear(start=prov_release)
    prov_object = cdss0_nabu_provobject(start=prov_clear)
    prov_drain = cdss0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cdss0_nabu_orgsrelease(start=harvest)
    load_org = cdss0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = cdss0_missingreport_s3(start=harvest)
    # report_idstat = cdss0_identifier_stats(start=report_ms3)
    # report_bucketurl = cdss0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = cdss0_summarize(start=load_uploadrelease)
    # upload_summarize = cdss0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cdss0_missingreport_graph(start=summarize)
    # report_graph = cdss0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="cdss0"):
#
