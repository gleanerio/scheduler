...
@op(ins={"start": In(Nothing)})
def cdss0_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "cdss0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cdss0_nabuorg(context):
...
def cdss0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="cdss0")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="cdss0")
    ...
    load_prov = cdss0_nabuprov(start=load_prune)
    load_org = cdss0_nabuorg(start=load_prov)

# run after load
    report_msgraph=cdss0_missingreport_graph(start=load_org)
