...
@op
def cuahsihisghcnids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisghcnids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisghcnids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisghcnids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisghcnids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisghcnids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisghcnids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisghcnids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisghcnids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisghcnids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisghcnids0")
    ...
    return msg + r
@op
def cuahsihisghcnids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisghcnids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisghcnids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisghcnids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisghcnids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisghcnids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisghcnids0"
...
def cuahsihisghcnids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisghcnids0"
...
def harvest_cuahsihisghcnids0():
    harvest = cuahsihisghcnids0_gleaner()

    report_ms3 = cuahsihisghcnids0_missingreport_s3(harvest)
    report_idstat = cuahsihisghcnids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisghcnids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisghcnids0")
    load_release = cuahsihisghcnids0_naburelease(harvest)
    load_uploadrelease = cuahsihisghcnids0_uploadrelease(load_release)

    load_prune = cuahsihisghcnids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisghcnids0_nabuprov(load_prune)
    load_org = cuahsihisghcnids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisghcnids0_missingreport_graph(load_org)
    report_graph=cuahsihisghcnids0_graph_reports(report_msgraph)
