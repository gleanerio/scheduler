...
@op
def cuahsihisdrwiids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisdrwiids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisdrwiids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisdrwiids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisdrwiids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisdrwiids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisdrwiids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisdrwiids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisdrwiids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisdrwiids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisdrwiids0")
    ...
    return msg + r
@op
def cuahsihisdrwiids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisdrwiids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisdrwiids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisdrwiids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisdrwiids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisdrwiids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisdrwiids0"
...
def cuahsihisdrwiids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisdrwiids0"
...
def harvest_cuahsihisdrwiids0():
    harvest = cuahsihisdrwiids0_gleaner()

    report_ms3 = cuahsihisdrwiids0_missingreport_s3(harvest)
    report_idstat = cuahsihisdrwiids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisdrwiids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisdrwiids0")
    load_release = cuahsihisdrwiids0_naburelease(harvest)
    load_uploadrelease = cuahsihisdrwiids0_uploadrelease(load_release)

    load_prune = cuahsihisdrwiids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisdrwiids0_nabuprov(load_prune)
    load_org = cuahsihisdrwiids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisdrwiids0_missingreport_graph(load_org)
    report_graph=cuahsihisdrwiids0_graph_reports(report_msgraph)
