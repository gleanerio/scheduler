...
@op
def hydrologicunit0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "hydrologicunit0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def hydrologicunit0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "hydrologicunit0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def hydrologicunit0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "hydrologicunit0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def hydrologicunit0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "hydrologicunit0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def hydrologicunit0_uploadrelease(context, msg: str):
...
@op
def hydrologicunit0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hydrologicunit0")
    ...
    return msg + r
@op
def hydrologicunit0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hydrologicunit0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def hydrologicunit0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hydrologicunit0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def hydrologicunit0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hydrologicunit0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "hydrologicunit0"
...
def hydrologicunit0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "hydrologicunit0"
...
def harvest_hydrologicunit0():
    harvest = hydrologicunit0_gleaner()

    report_ms3 = hydrologicunit0_missingreport_s3(harvest)
    report_idstat = hydrologicunit0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = hydrologicunit0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="hydrologicunit0")
    load_release = hydrologicunit0_naburelease(harvest)
    load_uploadrelease = hydrologicunit0_uploadrelease(load_release)

    load_prune = hydrologicunit0_nabu_prune(load_uploadrelease)
    load_prov = hydrologicunit0_nabuprov(load_prune)
    load_org = hydrologicunit0_nabuorg(load_prov)

# run after load
    report_msgraph=hydrologicunit0_missingreport_graph(load_org)
    report_graph=hydrologicunit0_graph_reports(report_msgraph)
