...
@op
def wade17_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "wade17")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade17_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "wade17")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade17_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "wade17")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade17_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "wade17")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def wade17_uploadrelease(context, msg: str):
...
@op
def wade17_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade17")
    ...
    return msg + r
@op
def wade17_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade17")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def wade17_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade17")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def wade17_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade17")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade17"
...
def wade17_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade17"
...
@graph
def harvest_wade17():
    harvest = wade17_gleaner()

    report_ms3 = wade17_missingreport_s3(harvest)
    report_idstat = wade17_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = wade17_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="wade17")
    load_release = wade17_naburelease(harvest)
    load_uploadrelease = wade17_uploadrelease(load_release)

    load_prune = wade17_nabu_prune(load_uploadrelease)
    load_prov = wade17_nabuprov(load_prune)
    load_org = wade17_nabuorg(load_prov)

# run after load
    report_msgraph=wade17_missingreport_graph(load_org)
    report_graph=wade17_graph_reports(report_msgraph)
