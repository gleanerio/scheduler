...
@op
def cuahsihissnotelids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihissnotelids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihissnotelids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihissnotelids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihissnotelids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihissnotelids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihissnotelids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihissnotelids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihissnotelids0_uploadrelease(context, msg: str):
...
@op
def cuahsihissnotelids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihissnotelids0")
    ...
    return msg + r
@op
def cuahsihissnotelids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihissnotelids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihissnotelids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihissnotelids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihissnotelids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihissnotelids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihissnotelids0"
...
def cuahsihissnotelids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihissnotelids0"
...
def harvest_cuahsihissnotelids0():
    harvest = cuahsihissnotelids0_gleaner()

    report_ms3 = cuahsihissnotelids0_missingreport_s3(harvest)
    report_idstat = cuahsihissnotelids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihissnotelids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihissnotelids0")
    load_release = cuahsihissnotelids0_naburelease(harvest)
    load_uploadrelease = cuahsihissnotelids0_uploadrelease(load_release)

    load_prune = cuahsihissnotelids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihissnotelids0_nabuprov(load_prune)
    load_org = cuahsihissnotelids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihissnotelids0_missingreport_graph(load_org)
    report_graph=cuahsihissnotelids0_graph_reports(report_msgraph)
