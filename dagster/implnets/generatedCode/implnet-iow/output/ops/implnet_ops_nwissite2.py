...
@op
def nwissite2_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwissite2")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwissite2_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwissite2")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwissite2_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwissite2")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwissite2_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwissite2")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwissite2_uploadrelease(context, msg: str):
...
@op
def nwissite2_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwissite2")
    ...
    return msg + r
@op
def nwissite2_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwissite2")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwissite2_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwissite2")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwissite2_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwissite2")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwissite2"
...
def nwissite2_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwissite2"
...
def harvest_nwissite2():
    harvest = nwissite2_gleaner()

    report_ms3 = nwissite2_missingreport_s3(harvest)
    report_idstat = nwissite2_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwissite2_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwissite2")
    load_release = nwissite2_naburelease(harvest)
    load_uploadrelease = nwissite2_uploadrelease(load_release)

    load_prune = nwissite2_nabu_prune(load_uploadrelease)
    load_prov = nwissite2_nabuprov(load_prune)
    load_org = nwissite2_nabuorg(load_prov)

# run after load
    report_msgraph=nwissite2_missingreport_graph(load_org)
    report_graph=nwissite2_graph_reports(report_msgraph)
