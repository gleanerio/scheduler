...
@op
def cuahsihisskcmilltownids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisskcmilltownids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisskcmilltownids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisskcmilltownids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisskcmilltownids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisskcmilltownids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisskcmilltownids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisskcmilltownids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisskcmilltownids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisskcmilltownids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisskcmilltownids0")
    ...
    return msg + r
@op
def cuahsihisskcmilltownids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisskcmilltownids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisskcmilltownids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisskcmilltownids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisskcmilltownids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisskcmilltownids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisskcmilltownids0"
...
def cuahsihisskcmilltownids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisskcmilltownids0"
...
def harvest_cuahsihisskcmilltownids0():
    harvest = cuahsihisskcmilltownids0_gleaner()

    report_ms3 = cuahsihisskcmilltownids0_missingreport_s3(harvest)
    report_idstat = cuahsihisskcmilltownids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisskcmilltownids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisskcmilltownids0")
    load_release = cuahsihisskcmilltownids0_naburelease(harvest)
    load_uploadrelease = cuahsihisskcmilltownids0_uploadrelease(load_release)

    load_prune = cuahsihisskcmilltownids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisskcmilltownids0_nabuprov(load_prune)
    load_org = cuahsihisskcmilltownids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisskcmilltownids0_missingreport_graph(load_org)
    report_graph=cuahsihisskcmilltownids0_graph_reports(report_msgraph)
