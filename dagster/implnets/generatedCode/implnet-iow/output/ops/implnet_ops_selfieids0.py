...
@op
def selfieids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "selfieids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def selfieids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "selfieids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def selfieids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "selfieids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def selfieids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "selfieids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def selfieids0_uploadrelease(context, msg: str):
...
@op
def selfieids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="selfieids0")
    ...
    return msg + r
@op
def selfieids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="selfieids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def selfieids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="selfieids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def selfieids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="selfieids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "selfieids0"
...
def selfieids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "selfieids0"
...
def harvest_selfieids0():
    harvest = selfieids0_gleaner()

    report_ms3 = selfieids0_missingreport_s3(harvest)
    report_idstat = selfieids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = selfieids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="selfieids0")
    load_release = selfieids0_naburelease(harvest)
    load_uploadrelease = selfieids0_uploadrelease(load_release)

    load_prune = selfieids0_nabu_prune(load_uploadrelease)
    load_prov = selfieids0_nabuprov(load_prune)
    load_org = selfieids0_nabuorg(load_prov)

# run after load
    report_msgraph=selfieids0_missingreport_graph(load_org)
    report_graph=selfieids0_graph_reports(report_msgraph)
