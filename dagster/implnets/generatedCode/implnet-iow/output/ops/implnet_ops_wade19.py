...
def wade19_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wade19")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def wade19_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wade19")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade19_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wade19")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade19_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wade19")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade19_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wade19")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade19_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wade19")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade19_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def wade19_uploadrelease(context):
#     returned_value = post_to_graph("wade19", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def wade19_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade19"
    )
...
@op(ins={"start": In(Nothing)})
def wade19_upload_summarize(context):
...
def harvest_wade19():
    ...
    # prov branch
    prov_release = wade19_nabu_provrelease(start=harvest)
    prov_clear = wade19_nabu_provclear(start=prov_release)
    prov_object = wade19_nabu_provobject(start=prov_clear)
    prov_drain = wade19_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wade19_nabu_orgsrelease(start=harvest)
    load_org = wade19_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = wade19_summarize(start=load_uploadrelease)
    # upload_summarize = wade19_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wade19_missingreport_graph(start=summarize)
