...
@op
def wade19_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "wade19")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade19_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "wade19")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade19_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "wade19")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade19_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "wade19")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def wade19_uploadrelease(context, msg: str):
...
@op
def wade19_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade19")
    ...
    return msg + r
@op
def wade19_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade19")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def wade19_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade19")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def wade19_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade19")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade19"
...
def wade19_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade19"
...
@graph
def harvest_wade19():
    harvest = wade19_gleaner()

    report_ms3 = wade19_missingreport_s3(harvest)
    report_idstat = wade19_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = wade19_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="wade19")
    load_release = wade19_naburelease(harvest)
    load_uploadrelease = wade19_uploadrelease(load_release)

    load_prune = wade19_nabu_prune(load_uploadrelease)
    load_prov = wade19_nabuprov(load_prune)
    load_org = wade19_nabuorg(load_prov)

# run after load
    report_msgraph=wade19_missingreport_graph(load_org)
    report_graph=wade19_graph_reports(report_msgraph)
