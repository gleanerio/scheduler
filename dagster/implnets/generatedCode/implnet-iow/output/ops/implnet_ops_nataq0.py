...
def nataq0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "nataq0")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def nataq0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "nataq0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nataq0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "nataq0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nataq0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "nataq0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nataq0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "nataq0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nataq0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "nataq0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nataq0_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def nataq0_uploadrelease(context):
#     returned_value = post_to_graph("nataq0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def nataq0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nataq0"
    )
...
@op(ins={"start": In(Nothing)})
def nataq0_upload_summarize(context):
...
def harvest_nataq0():
    ...
    # prov branch
    prov_release = nataq0_nabu_provrelease(start=harvest)
    prov_clear = nataq0_nabu_provclear(start=prov_release)
    prov_object = nataq0_nabu_provobject(start=prov_clear)
    prov_drain = nataq0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = nataq0_nabu_orgsrelease(start=harvest)
    load_org = nataq0_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = nataq0_summarize(start=load_uploadrelease)
    # upload_summarize = nataq0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = nataq0_missingreport_graph(start=summarize)
