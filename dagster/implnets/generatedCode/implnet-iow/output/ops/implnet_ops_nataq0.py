...
@op
def nataq0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nataq0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nataq0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nataq0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nataq0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nataq0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nataq0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nataq0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nataq0_uploadrelease(context, msg: str):
...
@op
def nataq0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nataq0")
    ...
    return msg + r
@op
def nataq0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nataq0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nataq0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nataq0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nataq0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nataq0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nataq0"
...
def nataq0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nataq0"
...
@graph
def harvest_nataq0():
    harvest = nataq0_gleaner()

    report_ms3 = nataq0_missingreport_s3(harvest)
    report_idstat = nataq0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nataq0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nataq0")
    load_release = nataq0_naburelease(harvest)
    load_uploadrelease = nataq0_uploadrelease(load_release)

    load_prune = nataq0_nabu_prune(load_uploadrelease)
    load_prov = nataq0_nabuprov(load_prune)
    load_org = nataq0_nabuorg(load_prov)

# run after load
    report_msgraph=nataq0_missingreport_graph(load_org)
    report_graph=nataq0_graph_reports(report_msgraph)
