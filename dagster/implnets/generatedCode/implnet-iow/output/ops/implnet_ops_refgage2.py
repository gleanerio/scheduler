...
@op
def refgage2_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "refgage2")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def refgage2_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "refgage2")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def refgage2_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "refgage2")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def refgage2_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "refgage2")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def refgage2_uploadrelease(context, msg: str):
...
@op
def refgage2_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="refgage2")
    ...
    return msg + r
@op
def refgage2_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="refgage2")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def refgage2_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="refgage2")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def refgage2_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="refgage2")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "refgage2"
...
def refgage2_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "refgage2"
...
def harvest_refgage2():
    harvest = refgage2_gleaner()

    report_ms3 = refgage2_missingreport_s3(harvest)
    report_idstat = refgage2_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = refgage2_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="refgage2")
    load_release = refgage2_naburelease(harvest)
    load_uploadrelease = refgage2_uploadrelease(load_release)

    load_prune = refgage2_nabu_prune(load_uploadrelease)
    load_prov = refgage2_nabuprov(load_prune)
    load_org = refgage2_nabuorg(load_prov)

# run after load
    report_msgraph=refgage2_missingreport_graph(load_org)
    report_graph=refgage2_graph_reports(report_msgraph)
