...
# #########################  Ops Section
@op
def wadewade43_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def wadewade43_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "wadewade43")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade43_naburelease(context):
    returned_value = gleanerio(context, ("release"), "wadewade43")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade43_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wadewade43")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade43_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "wadewade43")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade43_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wadewade43")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade43_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wadewade43")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade43_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wadewade43")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade43_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wadewade43")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade43_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wadewade43")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade43_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "wadewade43")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wadewade43_uploadrelease(context):
#     returned_value = post_to_graph("wadewade43", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wadewade43_uploadrelease(context):
#     returned_value = postRelease("wadewade43")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def wadewade43_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade43"
    )
...
@op(ins={"start": In(Nothing)})
def wadewade43_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade43"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def wadewade43_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade43"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def wadewade43_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade43"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def wadewade43_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wadewade43_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wadewade43_upload_summarize(context):
...
@graph
def harvest_wadewade43():
    # check for containers
    containers = wadewade43_getImage()

    # conduct the harvest
    harvest = wadewade43_gleaner(start=containers)

    # data branch
    load_release = wadewade43_naburelease(start=harvest)
    load_uploadrelease = wadewade43_uploadrelease(start=load_release)
    load_prune = wadewade43_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = wadewade43_nabu_provrelease(start=harvest)
    prov_clear = wadewade43_nabu_provclear(start=prov_release)
    prov_object = wadewade43_nabu_provobject(start=prov_clear)
    prov_drain = wadewade43_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wadewade43_nabu_orgsrelease(start=harvest)
    load_org = wadewade43_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = wadewade43_missingreport_s3(start=harvest)
    # report_idstat = wadewade43_identifier_stats(start=report_ms3)
    # report_bucketurl = wadewade43_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = wadewade43_summarize(start=load_uploadrelease)
    # upload_summarize = wadewade43_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wadewade43_missingreport_graph(start=summarize)
    # report_graph = wadewade43_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="wadewade43"):
#
