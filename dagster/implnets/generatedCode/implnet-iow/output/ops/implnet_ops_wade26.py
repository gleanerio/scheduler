...
@op(ins={"start": In(Nothing)})
def wade26_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade26")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade26_nabuorg(context):
...
def wade26_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade26")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade26")
    ...
    load_prov = wade26_nabuprov(start=load_prune)
    load_org = wade26_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade26_missingreport_graph(start=load_org)
