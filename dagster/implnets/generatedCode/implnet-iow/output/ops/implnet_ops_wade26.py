...
def wade26_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "wade26")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade26_naburelease(context):
    returned_value = gleanerio(context, ("release"), "wade26")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade26_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wade26")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade26_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "wade26")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade26_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wade26")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade26_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wade26")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade26_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wade26")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade26_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wade26")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade26_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wade26")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade26_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "wade26")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wade26_uploadrelease(context):
#     returned_value = post_to_graph("wade26", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wade26_uploadrelease(context):
#     returned_value = postRelease("wade26")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def wade26_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade26"
    )
...
@op(ins={"start": In(Nothing)})
def wade26_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade26"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def wade26_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade26"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def wade26_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade26"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def wade26_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wade26_upload_summarize(context):
...
def harvest_wade26():
    ...
    containers = wade26_getImage()

    # conduct the harvest
    harvest = wade26_gleaner(start=containers)

    # data branch
    load_release = wade26_naburelease(start=harvest)
    load_uploadrelease = wade26_uploadrelease(start=load_release)
    load_prune = wade26_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = wade26_nabu_provrelease(start=harvest)
    prov_clear = wade26_nabu_provclear(start=prov_release)
    prov_object = wade26_nabu_provobject(start=prov_clear)
    prov_drain = wade26_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wade26_nabu_orgsrelease(start=harvest)
    load_org = wade26_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = wade26_missingreport_s3(start=harvest)
    # report_idstat = wade26_identifier_stats(start=report_ms3)
    # report_bucketurl = wade26_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = wade26_summarize(start=load_uploadrelease)
    # upload_summarize = wade26_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wade26_missingreport_graph(start=summarize)
    # report_graph = wade26_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="wade26"):
#
