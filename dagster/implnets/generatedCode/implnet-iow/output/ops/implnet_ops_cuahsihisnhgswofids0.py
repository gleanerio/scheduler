...
@op
def cuahsihisnhgswofids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisnhgswofids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisnhgswofids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisnhgswofids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisnhgswofids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisnhgswofids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisnhgswofids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisnhgswofids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisnhgswofids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisnhgswofids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisnhgswofids0")
    ...
    return msg + r
@op
def cuahsihisnhgswofids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisnhgswofids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisnhgswofids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisnhgswofids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisnhgswofids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisnhgswofids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisnhgswofids0"
...
def cuahsihisnhgswofids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisnhgswofids0"
...
def harvest_cuahsihisnhgswofids0():
    ...
    report_ms3 = cuahsihisnhgswofids0_missingreport_s3(harvest)
    report_idstat = cuahsihisnhgswofids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisnhgswofids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisnhgswofids0")
    load_release = cuahsihisnhgswofids0_naburelease(harvest)
    load_uploadrelease = cuahsihisnhgswofids0_uploadrelease(load_release)

    load_prune = cuahsihisnhgswofids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisnhgswofids0_nabuprov(load_prune)
    load_org = cuahsihisnhgswofids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisnhgswofids0_missingreport_graph(load_org)
    report_graph=cuahsihisnhgswofids0_graph_reports(report_msgraph)
