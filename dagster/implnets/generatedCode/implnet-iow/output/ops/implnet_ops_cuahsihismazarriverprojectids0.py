...
@op
def cuahsihismazarriverprojectids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihismazarriverprojectids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismazarriverprojectids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihismazarriverprojectids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismazarriverprojectids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihismazarriverprojectids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismazarriverprojectids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihismazarriverprojectids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihismazarriverprojectids0_uploadrelease(context, msg: str):
...
@op
def cuahsihismazarriverprojectids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismazarriverprojectids0")
    ...
    return msg + r
@op
def cuahsihismazarriverprojectids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismazarriverprojectids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihismazarriverprojectids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismazarriverprojectids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihismazarriverprojectids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismazarriverprojectids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihismazarriverprojectids0"
...
def cuahsihismazarriverprojectids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihismazarriverprojectids0"
...
def harvest_cuahsihismazarriverprojectids0():
    harvest = cuahsihismazarriverprojectids0_gleaner()

    report_ms3 = cuahsihismazarriverprojectids0_missingreport_s3(harvest)
    report_idstat = cuahsihismazarriverprojectids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihismazarriverprojectids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihismazarriverprojectids0")
    load_release = cuahsihismazarriverprojectids0_naburelease(harvest)
    load_uploadrelease = cuahsihismazarriverprojectids0_uploadrelease(load_release)

    load_prune = cuahsihismazarriverprojectids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihismazarriverprojectids0_nabuprov(load_prune)
    load_org = cuahsihismazarriverprojectids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihismazarriverprojectids0_missingreport_graph(load_org)
    report_graph=cuahsihismazarriverprojectids0_graph_reports(report_msgraph)
