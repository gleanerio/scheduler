...
def cuahsihismazarriverprojectids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsihismazarriverprojectids0")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def cuahsihismazarriverprojectids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsihismazarriverprojectids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihismazarriverprojectids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsihismazarriverprojectids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihismazarriverprojectids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsihismazarriverprojectids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihismazarriverprojectids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsihismazarriverprojectids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihismazarriverprojectids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsihismazarriverprojectids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihismazarriverprojectids0_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def cuahsihismazarriverprojectids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsihismazarriverprojectids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def cuahsihismazarriverprojectids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsihismazarriverprojectids0"
    )
...
@op(ins={"start": In(Nothing)})
def cuahsihismazarriverprojectids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsihismazarriverprojectids0_upload_summarize(context):
    returned_value = post_to_graph(
        "cuahsihismazarriverprojectids0",
        path=SUMMARY_PATH,
        extension="ttl",
...
def harvest_cuahsihismazarriverprojectids0():
    ...
    # prov branch
    prov_release = cuahsihismazarriverprojectids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsihismazarriverprojectids0_nabu_provclear(start=prov_release)
    prov_object = cuahsihismazarriverprojectids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsihismazarriverprojectids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsihismazarriverprojectids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsihismazarriverprojectids0_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = cuahsihismazarriverprojectids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsihismazarriverprojectids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsihismazarriverprojectids0_missingreport_graph(start=summarize)
