...
@op
def cuahsihismuddyriverids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihismuddyriverids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismuddyriverids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihismuddyriverids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismuddyriverids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihismuddyriverids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismuddyriverids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihismuddyriverids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihismuddyriverids0_uploadrelease(context, msg: str):
...
@op
def cuahsihismuddyriverids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismuddyriverids0")
    ...
    return msg + r
@op
def cuahsihismuddyriverids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismuddyriverids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihismuddyriverids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismuddyriverids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihismuddyriverids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismuddyriverids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihismuddyriverids0"
...
def cuahsihismuddyriverids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihismuddyriverids0"
...
def harvest_cuahsihismuddyriverids0():
    harvest = cuahsihismuddyriverids0_gleaner()

    report_ms3 = cuahsihismuddyriverids0_missingreport_s3(harvest)
    report_idstat = cuahsihismuddyriverids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihismuddyriverids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihismuddyriverids0")
    load_release = cuahsihismuddyriverids0_naburelease(harvest)
    load_uploadrelease = cuahsihismuddyriverids0_uploadrelease(load_release)

    load_prune = cuahsihismuddyriverids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihismuddyriverids0_nabuprov(load_prune)
    load_org = cuahsihismuddyriverids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihismuddyriverids0_missingreport_graph(load_org)
    report_graph=cuahsihismuddyriverids0_graph_reports(report_msgraph)
