...
def cuahsihismuddyriverids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsihismuddyriverids0")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def cuahsihismuddyriverids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsihismuddyriverids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihismuddyriverids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsihismuddyriverids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihismuddyriverids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsihismuddyriverids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihismuddyriverids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsihismuddyriverids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihismuddyriverids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsihismuddyriverids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihismuddyriverids0_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def cuahsihismuddyriverids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsihismuddyriverids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def cuahsihismuddyriverids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsihismuddyriverids0"
    )
...
@op(ins={"start": In(Nothing)})
def cuahsihismuddyriverids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsihismuddyriverids0_upload_summarize(context):
    returned_value = post_to_graph(
        "cuahsihismuddyriverids0",
        path=SUMMARY_PATH,
        extension="ttl",
...
def harvest_cuahsihismuddyriverids0():
    ...
    # prov branch
    prov_release = cuahsihismuddyriverids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsihismuddyriverids0_nabu_provclear(start=prov_release)
    prov_object = cuahsihismuddyriverids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsihismuddyriverids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsihismuddyriverids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsihismuddyriverids0_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = cuahsihismuddyriverids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsihismuddyriverids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsihismuddyriverids0_missingreport_graph(start=summarize)
