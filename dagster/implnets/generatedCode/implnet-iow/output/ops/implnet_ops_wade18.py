...
@op
def wade18_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "wade18")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade18_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "wade18")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade18_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "wade18")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade18_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "wade18")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def wade18_uploadrelease(context, msg: str):
...
@op
def wade18_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade18")
    ...
    return msg + r
@op
def wade18_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade18")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def wade18_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade18")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def wade18_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade18")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade18"
...
def wade18_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade18"
...
@graph
def harvest_wade18():
    harvest = wade18_gleaner()

    report_ms3 = wade18_missingreport_s3(harvest)
    report_idstat = wade18_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = wade18_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="wade18")
    load_release = wade18_naburelease(harvest)
    load_uploadrelease = wade18_uploadrelease(load_release)

    load_prune = wade18_nabu_prune(load_uploadrelease)
    load_prov = wade18_nabuprov(load_prune)
    load_org = wade18_nabuorg(load_prov)

# run after load
    report_msgraph=wade18_missingreport_graph(load_org)
    report_graph=wade18_graph_reports(report_msgraph)
