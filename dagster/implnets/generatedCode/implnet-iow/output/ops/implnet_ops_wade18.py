...
def wade18_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wade18")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def wade18_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wade18")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade18_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wade18")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade18_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wade18")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade18_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wade18")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade18_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wade18")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade18_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def wade18_uploadrelease(context):
#     returned_value = post_to_graph("wade18", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def wade18_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade18"
    )
...
@op(ins={"start": In(Nothing)})
def wade18_upload_summarize(context):
...
def harvest_wade18():
    ...
    # prov branch
    prov_release = wade18_nabu_provrelease(start=harvest)
    prov_clear = wade18_nabu_provclear(start=prov_release)
    prov_object = wade18_nabu_provobject(start=prov_clear)
    prov_drain = wade18_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wade18_nabu_orgsrelease(start=harvest)
    load_org = wade18_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = wade18_summarize(start=load_uploadrelease)
    # upload_summarize = wade18_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wade18_missingreport_graph(start=summarize)
