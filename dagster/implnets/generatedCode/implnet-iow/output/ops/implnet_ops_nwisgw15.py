...
@op
def nwisgw15_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw15")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw15_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw15")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw15_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw15")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw15_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw15")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw15_uploadrelease(context, msg: str):
...
@op
def nwisgw15_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw15")
    ...
    return msg + r
@op
def nwisgw15_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw15")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw15_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw15")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw15_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw15")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw15"
...
def nwisgw15_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw15"
...
def harvest_nwisgw15():
    harvest = nwisgw15_gleaner()

    report_ms3 = nwisgw15_missingreport_s3(harvest)
    report_idstat = nwisgw15_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw15_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw15")
    load_release = nwisgw15_naburelease(harvest)
    load_uploadrelease = nwisgw15_uploadrelease(load_release)

    load_prune = nwisgw15_nabu_prune(load_uploadrelease)
    load_prov = nwisgw15_nabuprov(load_prune)
    load_org = nwisgw15_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw15_missingreport_graph(load_org)
    report_graph=nwisgw15_graph_reports(report_msgraph)
