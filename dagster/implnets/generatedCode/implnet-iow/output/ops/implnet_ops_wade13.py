...
@op
def wade13_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "wade13")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade13_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "wade13")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade13_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "wade13")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade13_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "wade13")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def wade13_uploadrelease(context, msg: str):
...
@op
def wade13_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade13")
    ...
    return msg + r
@op
def wade13_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade13")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def wade13_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade13")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def wade13_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade13")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade13"
...
def wade13_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade13"
...
@graph
def harvest_wade13():
    harvest = wade13_gleaner()

    report_ms3 = wade13_missingreport_s3(harvest)
    report_idstat = wade13_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = wade13_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="wade13")
    load_release = wade13_naburelease(harvest)
    load_uploadrelease = wade13_uploadrelease(load_release)

    load_prune = wade13_nabu_prune(load_uploadrelease)
    load_prov = wade13_nabuprov(load_prune)
    load_org = wade13_nabuorg(load_prov)

# run after load
    report_msgraph=wade13_missingreport_graph(load_org)
    report_graph=wade13_graph_reports(report_msgraph)
