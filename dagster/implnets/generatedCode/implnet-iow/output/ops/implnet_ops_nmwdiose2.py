...
@op
def nmwdiose2_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nmwdiose2")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nmwdiose2_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nmwdiose2")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nmwdiose2_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nmwdiose2")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nmwdiose2_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nmwdiose2")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nmwdiose2_uploadrelease(context, msg: str):
...
@op
def nmwdiose2_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nmwdiose2")
    ...
    return msg + r
@op
def nmwdiose2_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nmwdiose2")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nmwdiose2_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nmwdiose2")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nmwdiose2_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nmwdiose2")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nmwdiose2"
...
def nmwdiose2_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nmwdiose2"
...
def harvest_nmwdiose2():
    harvest = nmwdiose2_gleaner()

    report_ms3 = nmwdiose2_missingreport_s3(harvest)
    report_idstat = nmwdiose2_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nmwdiose2_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nmwdiose2")
    load_release = nmwdiose2_naburelease(harvest)
    load_uploadrelease = nmwdiose2_uploadrelease(load_release)

    load_prune = nmwdiose2_nabu_prune(load_uploadrelease)
    load_prov = nmwdiose2_nabuprov(load_prune)
    load_org = nmwdiose2_nabuorg(load_prov)

# run after load
    report_msgraph=nmwdiose2_missingreport_graph(load_org)
    report_graph=nmwdiose2_graph_reports(report_msgraph)
