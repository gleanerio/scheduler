...
@op
def cuahsihisnewnids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisnewnids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisnewnids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisnewnids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisnewnids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisnewnids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisnewnids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisnewnids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisnewnids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisnewnids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisnewnids0")
    ...
    return msg + r
@op
def cuahsihisnewnids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisnewnids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisnewnids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisnewnids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisnewnids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisnewnids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisnewnids0"
...
def cuahsihisnewnids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisnewnids0"
...
def harvest_cuahsihisnewnids0():
    harvest = cuahsihisnewnids0_gleaner()

    report_ms3 = cuahsihisnewnids0_missingreport_s3(harvest)
    report_idstat = cuahsihisnewnids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisnewnids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisnewnids0")
    load_release = cuahsihisnewnids0_naburelease(harvest)
    load_uploadrelease = cuahsihisnewnids0_uploadrelease(load_release)

    load_prune = cuahsihisnewnids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisnewnids0_nabuprov(load_prune)
    load_org = cuahsihisnewnids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisnewnids0_missingreport_graph(load_org)
    report_graph=cuahsihisnewnids0_graph_reports(report_msgraph)
