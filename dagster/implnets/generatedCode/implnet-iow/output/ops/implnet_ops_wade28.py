...
@op(ins={"start": In(Nothing)})
def wade28_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade28")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade28_nabuorg(context):
...
def wade28_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade28")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade28")
    ...
    load_prov = wade28_nabuprov(start=load_prune)
    load_org = wade28_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade28_missingreport_graph(start=load_org)
