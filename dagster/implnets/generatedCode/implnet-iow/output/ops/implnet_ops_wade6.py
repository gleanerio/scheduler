...
def wade6_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wade6")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def wade6_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wade6")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade6_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wade6")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade6_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wade6")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade6_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wade6")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade6_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wade6")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade6_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def wade6_uploadrelease(context):
#     returned_value = post_to_graph("wade6", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def wade6_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade6"
    )
...
@op(ins={"start": In(Nothing)})
def wade6_upload_summarize(context):
...
def harvest_wade6():
    ...
    # prov branch
    prov_release = wade6_nabu_provrelease(start=harvest)
    prov_clear = wade6_nabu_provclear(start=prov_release)
    prov_object = wade6_nabu_provobject(start=prov_clear)
    prov_drain = wade6_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wade6_nabu_orgsrelease(start=harvest)
    load_org = wade6_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = wade6_summarize(start=load_uploadrelease)
    # upload_summarize = wade6_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wade6_missingreport_graph(start=summarize)
