...
@op
def wade6_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "wade6")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade6_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "wade6")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade6_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "wade6")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade6_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "wade6")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def wade6_uploadrelease(context, msg: str):
...
@op
def wade6_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade6")
    ...
    return msg + r
@op
def wade6_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade6")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def wade6_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade6")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def wade6_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade6")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade6"
...
def wade6_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade6"
...
@graph
def harvest_wade6():
    harvest = wade6_gleaner()

    report_ms3 = wade6_missingreport_s3(harvest)
    report_idstat = wade6_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = wade6_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="wade6")
    load_release = wade6_naburelease(harvest)
    load_uploadrelease = wade6_uploadrelease(load_release)

    load_prune = wade6_nabu_prune(load_uploadrelease)
    load_prov = wade6_nabuprov(load_prune)
    load_org = wade6_nabuorg(load_prov)

# run after load
    report_msgraph=wade6_missingreport_graph(load_org)
    report_graph=wade6_graph_reports(report_msgraph)
