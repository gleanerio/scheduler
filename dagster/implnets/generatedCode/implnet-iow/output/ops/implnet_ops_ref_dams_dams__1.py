...
# #########################  Ops Section
@op
def ref_dams_dams__1_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def ref_dams_dams__1_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "ref_dams_dams__1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ref_dams_dams__1_naburelease(context):
    returned_value = gleanerio(context, ("release"), "ref_dams_dams__1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ref_dams_dams__1_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "ref_dams_dams__1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ref_dams_dams__1_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "ref_dams_dams__1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ref_dams_dams__1_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "ref_dams_dams__1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ref_dams_dams__1_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "ref_dams_dams__1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ref_dams_dams__1_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "ref_dams_dams__1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ref_dams_dams__1_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "ref_dams_dams__1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ref_dams_dams__1_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "ref_dams_dams__1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ref_dams_dams__1_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "ref_dams_dams__1")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def ref_dams_dams__1_uploadrelease(context):
#     returned_value = post_to_graph("ref_dams_dams__1", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def ref_dams_dams__1_uploadrelease(context):
#     returned_value = postRelease("ref_dams_dams__1")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def ref_dams_dams__1_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="ref_dams_dams__1"
    )
...
@op(ins={"start": In(Nothing)})
def ref_dams_dams__1_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="ref_dams_dams__1"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def ref_dams_dams__1_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="ref_dams_dams__1"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def ref_dams_dams__1_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="ref_dams_dams__1"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def ref_dams_dams__1_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def ref_dams_dams__1_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def ref_dams_dams__1_upload_summarize(context):
...
@graph
def harvest_ref_dams_dams__1():
    # check for containers
    containers = ref_dams_dams__1_getImage()

    # conduct the harvest
    harvest = ref_dams_dams__1_gleaner(start=containers)

    # data branch
    load_release = ref_dams_dams__1_naburelease(start=harvest)
    load_uploadrelease = ref_dams_dams__1_uploadrelease(start=load_release)
    load_prune = ref_dams_dams__1_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = ref_dams_dams__1_nabu_provrelease(start=harvest)
    prov_clear = ref_dams_dams__1_nabu_provclear(start=prov_release)
    prov_object = ref_dams_dams__1_nabu_provobject(start=prov_clear)
    prov_drain = ref_dams_dams__1_nabu_provdrain(start=prov_object)

    # org branch
    org_release = ref_dams_dams__1_nabu_orgsrelease(start=harvest)
    load_org = ref_dams_dams__1_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = ref_dams_dams__1_missingreport_s3(start=harvest)
    # report_idstat = ref_dams_dams__1_identifier_stats(start=report_ms3)
    # report_bucketurl = ref_dams_dams__1_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = ref_dams_dams__1_summarize(start=load_uploadrelease)
    # upload_summarize = ref_dams_dams__1_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = ref_dams_dams__1_missingreport_graph(start=summarize)
    # report_graph = ref_dams_dams__1_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="ref_dams_dams__1"):
#
