...
@op
def gfv11pois0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "gfv11pois0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def gfv11pois0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "gfv11pois0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def gfv11pois0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "gfv11pois0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def gfv11pois0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "gfv11pois0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def gfv11pois0_uploadrelease(context, msg: str):
...
@op
def gfv11pois0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="gfv11pois0")
    ...
    return msg + r
@op
def gfv11pois0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="gfv11pois0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def gfv11pois0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="gfv11pois0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def gfv11pois0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="gfv11pois0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "gfv11pois0"
...
def gfv11pois0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "gfv11pois0"
...
def harvest_gfv11pois0():
    harvest = gfv11pois0_gleaner()

    report_ms3 = gfv11pois0_missingreport_s3(harvest)
    report_idstat = gfv11pois0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = gfv11pois0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="gfv11pois0")
    load_release = gfv11pois0_naburelease(harvest)
    load_uploadrelease = gfv11pois0_uploadrelease(load_release)

    load_prune = gfv11pois0_nabu_prune(load_uploadrelease)
    load_prov = gfv11pois0_nabuprov(load_prune)
    load_org = gfv11pois0_nabuorg(load_prov)

# run after load
    report_msgraph=gfv11pois0_missingreport_graph(load_org)
    report_graph=gfv11pois0_graph_reports(report_msgraph)
