...
def nwisgw1_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "nwisgw1")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def nwisgw1_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "nwisgw1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw1_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "nwisgw1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw1_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "nwisgw1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw1_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "nwisgw1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw1_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "nwisgw1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw1_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def nwisgw1_uploadrelease(context):
#     returned_value = post_to_graph("nwisgw1", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def nwisgw1_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nwisgw1"
    )
...
@op(ins={"start": In(Nothing)})
def nwisgw1_upload_summarize(context):
...
def harvest_nwisgw1():
    ...
    # prov branch
    prov_release = nwisgw1_nabu_provrelease(start=harvest)
    prov_clear = nwisgw1_nabu_provclear(start=prov_release)
    prov_object = nwisgw1_nabu_provobject(start=prov_clear)
    prov_drain = nwisgw1_nabu_provdrain(start=prov_object)

    # org branch
    org_release = nwisgw1_nabu_orgsrelease(start=harvest)
    load_org = nwisgw1_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = nwisgw1_summarize(start=load_uploadrelease)
    # upload_summarize = nwisgw1_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = nwisgw1_missingreport_graph(start=summarize)
