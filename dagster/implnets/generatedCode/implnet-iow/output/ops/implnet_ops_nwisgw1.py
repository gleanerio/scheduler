...
@op
def nwisgw1_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw1_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw1_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw1_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw1_uploadrelease(context, msg: str):
...
@op
def nwisgw1_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw1")
    ...
    return msg + r
@op
def nwisgw1_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw1")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw1_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw1")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw1_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw1")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw1"
...
def nwisgw1_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw1"
...
@graph
def harvest_nwisgw1():
    harvest = nwisgw1_gleaner()

    report_ms3 = nwisgw1_missingreport_s3(harvest)
    report_idstat = nwisgw1_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw1_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw1")
    load_release = nwisgw1_naburelease(harvest)
    load_uploadrelease = nwisgw1_uploadrelease(load_release)

    load_prune = nwisgw1_nabu_prune(load_uploadrelease)
    load_prov = nwisgw1_nabuprov(load_prune)
    load_org = nwisgw1_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw1_missingreport_graph(load_org)
    report_graph=nwisgw1_graph_reports(report_msgraph)
