...
@op(ins={"start": In(Nothing)})
def sta0_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "sta0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def sta0_nabuorg(context):
...
def sta0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="sta0")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="sta0")
    ...
    load_prov = sta0_nabuprov(start=load_prune)
    load_org = sta0_nabuorg(start=load_prov)

# run after load
    report_msgraph=sta0_missingreport_graph(start=load_org)
