...
def sta0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "sta0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def sta0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "sta0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def sta0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "sta0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def sta0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "sta0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def sta0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "sta0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def sta0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "sta0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def sta0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "sta0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def sta0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "sta0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def sta0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "sta0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def sta0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "sta0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def sta0_uploadrelease(context):
#     returned_value = post_to_graph("sta0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def sta0_uploadrelease(context):
#     returned_value = postRelease("sta0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def sta0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="sta0"
    )
...
@op(ins={"start": In(Nothing)})
def sta0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="sta0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def sta0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="sta0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def sta0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="sta0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def sta0_upload_summarize(context):
...
def harvest_sta0():
    ...
    containers = sta0_getImage()

    # conduct the harvest
    harvest = sta0_gleaner(start=containers)

    # data branch
    load_release = sta0_naburelease(start=harvest)
    load_uploadrelease = sta0_uploadrelease(start=load_release)
    load_prune = sta0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = sta0_nabu_provrelease(start=harvest)
    prov_clear = sta0_nabu_provclear(start=prov_release)
    prov_object = sta0_nabu_provobject(start=prov_clear)
    prov_drain = sta0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = sta0_nabu_orgsrelease(start=harvest)
    load_org = sta0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = sta0_missingreport_s3(start=harvest)
    # report_idstat = sta0_identifier_stats(start=report_ms3)
    # report_bucketurl = sta0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = sta0_summarize(start=load_uploadrelease)
    # upload_summarize = sta0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = sta0_missingreport_graph(start=summarize)
    # report_graph = sta0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="sta0"):
#
