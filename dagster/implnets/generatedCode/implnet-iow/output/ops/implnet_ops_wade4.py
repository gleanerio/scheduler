...
@op
def wade4_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "wade4")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade4_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "wade4")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade4_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "wade4")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade4_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "wade4")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def wade4_uploadrelease(context, msg: str):
...
@op
def wade4_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade4")
    ...
    return msg + r
@op
def wade4_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade4")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def wade4_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade4")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def wade4_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade4")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade4"
...
def wade4_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade4"
...
@graph
def harvest_wade4():
    harvest = wade4_gleaner()

    report_ms3 = wade4_missingreport_s3(harvest)
    report_idstat = wade4_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = wade4_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="wade4")
    load_release = wade4_naburelease(harvest)
    load_uploadrelease = wade4_uploadrelease(load_release)

    load_prune = wade4_nabu_prune(load_uploadrelease)
    load_prov = wade4_nabuprov(load_prune)
    load_org = wade4_nabuorg(load_prov)

# run after load
    report_msgraph=wade4_missingreport_graph(load_org)
    report_graph=wade4_graph_reports(report_msgraph)
