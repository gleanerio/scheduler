...
# #########################  Ops Section
@op
def wadewade8_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def wadewade8_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "wadewade8")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade8_naburelease(context):
    returned_value = gleanerio(context, ("release"), "wadewade8")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade8_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wadewade8")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade8_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "wadewade8")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade8_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wadewade8")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade8_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wadewade8")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade8_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wadewade8")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade8_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wadewade8")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade8_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wadewade8")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade8_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "wadewade8")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wadewade8_uploadrelease(context):
#     returned_value = post_to_graph("wadewade8", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wadewade8_uploadrelease(context):
#     returned_value = postRelease("wadewade8")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def wadewade8_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade8"
    )
...
@op(ins={"start": In(Nothing)})
def wadewade8_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade8"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def wadewade8_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade8"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def wadewade8_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade8"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def wadewade8_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wadewade8_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wadewade8_upload_summarize(context):
...
@graph
def harvest_wadewade8():
    # check for containers
    containers = wadewade8_getImage()

    # conduct the harvest
    harvest = wadewade8_gleaner(start=containers)

    # data branch
    load_release = wadewade8_naburelease(start=harvest)
    load_uploadrelease = wadewade8_uploadrelease(start=load_release)
    load_prune = wadewade8_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = wadewade8_nabu_provrelease(start=harvest)
    prov_clear = wadewade8_nabu_provclear(start=prov_release)
    prov_object = wadewade8_nabu_provobject(start=prov_clear)
    prov_drain = wadewade8_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wadewade8_nabu_orgsrelease(start=harvest)
    load_org = wadewade8_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = wadewade8_missingreport_s3(start=harvest)
    # report_idstat = wadewade8_identifier_stats(start=report_ms3)
    # report_bucketurl = wadewade8_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = wadewade8_summarize(start=load_uploadrelease)
    # upload_summarize = wadewade8_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wadewade8_missingreport_graph(start=summarize)
    # report_graph = wadewade8_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="wadewade8"):
#
