...
# #########################  Ops Section
@op
def ref_states_states__0_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def ref_states_states__0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "ref_states_states__0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ref_states_states__0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "ref_states_states__0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ref_states_states__0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "ref_states_states__0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ref_states_states__0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "ref_states_states__0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ref_states_states__0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "ref_states_states__0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ref_states_states__0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "ref_states_states__0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ref_states_states__0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "ref_states_states__0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ref_states_states__0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "ref_states_states__0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ref_states_states__0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "ref_states_states__0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ref_states_states__0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "ref_states_states__0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def ref_states_states__0_uploadrelease(context):
#     returned_value = post_to_graph("ref_states_states__0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def ref_states_states__0_uploadrelease(context):
#     returned_value = postRelease("ref_states_states__0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def ref_states_states__0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="ref_states_states__0"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "ref_states_states__0"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def ref_states_states__0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="ref_states_states__0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def ref_states_states__0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="ref_states_states__0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def ref_states_states__0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="ref_states_states__0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def ref_states_states__0_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def ref_states_states__0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def ref_states_states__0_upload_summarize(context):
    returned_value = post_to_graph(
        "ref_states_states__0",
        path=SUMMARY_PATH,
        extension="ttl",
...
@graph
def harvest_ref_states_states__0():
    # check for containers
    containers = ref_states_states__0_getImage()

    # conduct the harvest
    harvest = ref_states_states__0_gleaner(start=containers)

    # data branch
    load_release = ref_states_states__0_naburelease(start=harvest)
    load_uploadrelease = ref_states_states__0_uploadrelease(start=load_release)
    load_prune = ref_states_states__0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = ref_states_states__0_nabu_provrelease(start=harvest)
    prov_clear = ref_states_states__0_nabu_provclear(start=prov_release)
    prov_object = ref_states_states__0_nabu_provobject(start=prov_clear)
    prov_drain = ref_states_states__0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = ref_states_states__0_nabu_orgsrelease(start=harvest)
    load_org = ref_states_states__0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = ref_states_states__0_missingreport_s3(start=harvest)
    # report_idstat = ref_states_states__0_identifier_stats(start=report_ms3)
    # report_bucketurl = ref_states_states__0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = ref_states_states__0_summarize(start=load_uploadrelease)
    # upload_summarize = ref_states_states__0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = ref_states_states__0_missingreport_graph(start=summarize)
    # report_graph = ref_states_states__0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="ref_states_states__0"):
#
