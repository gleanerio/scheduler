...
@op
def wade10_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "wade10")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade10_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "wade10")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade10_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "wade10")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade10_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "wade10")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def wade10_uploadrelease(context, msg: str):
...
@op
def wade10_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade10")
    ...
    return msg + r
@op
def wade10_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade10")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def wade10_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade10")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def wade10_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade10")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade10"
...
def wade10_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade10"
...
@graph
def harvest_wade10():
    harvest = wade10_gleaner()

    report_ms3 = wade10_missingreport_s3(harvest)
    report_idstat = wade10_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = wade10_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="wade10")
    load_release = wade10_naburelease(harvest)
    load_uploadrelease = wade10_uploadrelease(load_release)

    load_prune = wade10_nabu_prune(load_uploadrelease)
    load_prov = wade10_nabuprov(load_prune)
    load_org = wade10_nabuorg(load_prov)

# run after load
    report_msgraph=wade10_missingreport_graph(load_org)
    report_graph=wade10_graph_reports(report_msgraph)
