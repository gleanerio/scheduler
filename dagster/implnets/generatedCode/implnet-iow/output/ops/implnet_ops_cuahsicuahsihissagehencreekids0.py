...
# #########################  Ops Section
@op
def cuahsicuahsihissagehencreekids0_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def cuahsicuahsihissagehencreekids0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "cuahsicuahsihissagehencreekids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihissagehencreekids0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "cuahsicuahsihissagehencreekids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihissagehencreekids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsicuahsihissagehencreekids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihissagehencreekids0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "cuahsicuahsihissagehencreekids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihissagehencreekids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsicuahsihissagehencreekids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihissagehencreekids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsicuahsihissagehencreekids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihissagehencreekids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsicuahsihissagehencreekids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihissagehencreekids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsicuahsihissagehencreekids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihissagehencreekids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsicuahsihissagehencreekids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihissagehencreekids0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "cuahsicuahsihissagehencreekids0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihissagehencreekids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsicuahsihissagehencreekids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihissagehencreekids0_uploadrelease(context):
#     returned_value = postRelease("cuahsicuahsihissagehencreekids0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihissagehencreekids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihissagehencreekids0"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsicuahsihissagehencreekids0"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihissagehencreekids0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihissagehencreekids0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihissagehencreekids0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihissagehencreekids0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihissagehencreekids0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihissagehencreekids0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihissagehencreekids0_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihissagehencreekids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihissagehencreekids0_upload_summarize(context):
    returned_value = post_to_graph(
        "cuahsicuahsihissagehencreekids0",
        path=SUMMARY_PATH,
        extension="ttl",
...
@graph
def harvest_cuahsicuahsihissagehencreekids0():
    # check for containers
    containers = cuahsicuahsihissagehencreekids0_getImage()

    # conduct the harvest
    harvest = cuahsicuahsihissagehencreekids0_gleaner(start=containers)

    # data branch
    load_release = cuahsicuahsihissagehencreekids0_naburelease(start=harvest)
    load_uploadrelease = cuahsicuahsihissagehencreekids0_uploadrelease(start=load_release)
    load_prune = cuahsicuahsihissagehencreekids0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = cuahsicuahsihissagehencreekids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsicuahsihissagehencreekids0_nabu_provclear(start=prov_release)
    prov_object = cuahsicuahsihissagehencreekids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsicuahsihissagehencreekids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsicuahsihissagehencreekids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsicuahsihissagehencreekids0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = cuahsicuahsihissagehencreekids0_missingreport_s3(start=harvest)
    # report_idstat = cuahsicuahsihissagehencreekids0_identifier_stats(start=report_ms3)
    # report_bucketurl = cuahsicuahsihissagehencreekids0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = cuahsicuahsihissagehencreekids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsicuahsihissagehencreekids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsicuahsihissagehencreekids0_missingreport_graph(start=summarize)
    # report_graph = cuahsicuahsihissagehencreekids0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="cuahsicuahsihissagehencreekids0"):
#
