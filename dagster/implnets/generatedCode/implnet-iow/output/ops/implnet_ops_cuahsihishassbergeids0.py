...
@op
def cuahsihishassbergeids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihishassbergeids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihishassbergeids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihishassbergeids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihishassbergeids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihishassbergeids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihishassbergeids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihishassbergeids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihishassbergeids0_uploadrelease(context, msg: str):
...
@op
def cuahsihishassbergeids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihishassbergeids0")
    ...
    return msg + r
@op
def cuahsihishassbergeids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihishassbergeids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihishassbergeids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihishassbergeids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihishassbergeids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihishassbergeids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihishassbergeids0"
...
def cuahsihishassbergeids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihishassbergeids0"
...
def harvest_cuahsihishassbergeids0():
    harvest = cuahsihishassbergeids0_gleaner()

    report_ms3 = cuahsihishassbergeids0_missingreport_s3(harvest)
    report_idstat = cuahsihishassbergeids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihishassbergeids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihishassbergeids0")
    load_release = cuahsihishassbergeids0_naburelease(harvest)
    load_uploadrelease = cuahsihishassbergeids0_uploadrelease(load_release)

    load_prune = cuahsihishassbergeids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihishassbergeids0_nabuprov(load_prune)
    load_org = cuahsihishassbergeids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihishassbergeids0_missingreport_graph(load_org)
    report_graph=cuahsihishassbergeids0_graph_reports(report_msgraph)
