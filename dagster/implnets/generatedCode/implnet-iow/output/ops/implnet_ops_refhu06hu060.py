...
# #########################  Ops Section
@op
def refhu06hu060_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def refhu06hu060_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "refhu06hu060")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu06hu060_naburelease(context):
    returned_value = gleanerio(context, ("release"), "refhu06hu060")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu06hu060_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "refhu06hu060")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu06hu060_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "refhu06hu060")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu06hu060_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "refhu06hu060")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu06hu060_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "refhu06hu060")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu06hu060_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "refhu06hu060")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu06hu060_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "refhu06hu060")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu06hu060_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "refhu06hu060")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu06hu060_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "refhu06hu060")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def refhu06hu060_uploadrelease(context):
#     returned_value = post_to_graph("refhu06hu060", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def refhu06hu060_uploadrelease(context):
#     returned_value = postRelease("refhu06hu060")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def refhu06hu060_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refhu06hu060"
    )
...
@op(ins={"start": In(Nothing)})
def refhu06hu060_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refhu06hu060"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def refhu06hu060_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refhu06hu060"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def refhu06hu060_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refhu06hu060"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def refhu06hu060_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def refhu06hu060_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def refhu06hu060_upload_summarize(context):
...
@graph
def harvest_refhu06hu060():
    # check for containers
    containers = refhu06hu060_getImage()

    # conduct the harvest
    harvest = refhu06hu060_gleaner(start=containers)

    # data branch
    load_release = refhu06hu060_naburelease(start=harvest)
    load_uploadrelease = refhu06hu060_uploadrelease(start=load_release)
    load_prune = refhu06hu060_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = refhu06hu060_nabu_provrelease(start=harvest)
    prov_clear = refhu06hu060_nabu_provclear(start=prov_release)
    prov_object = refhu06hu060_nabu_provobject(start=prov_clear)
    prov_drain = refhu06hu060_nabu_provdrain(start=prov_object)

    # org branch
    org_release = refhu06hu060_nabu_orgsrelease(start=harvest)
    load_org = refhu06hu060_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = refhu06hu060_missingreport_s3(start=harvest)
    # report_idstat = refhu06hu060_identifier_stats(start=report_ms3)
    # report_bucketurl = refhu06hu060_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = refhu06hu060_summarize(start=load_uploadrelease)
    # upload_summarize = refhu06hu060_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = refhu06hu060_missingreport_graph(start=summarize)
    # report_graph = refhu06hu060_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="refhu06hu060"):
#
