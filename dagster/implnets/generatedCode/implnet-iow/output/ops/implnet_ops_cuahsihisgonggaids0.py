...
@op
def cuahsihisgonggaids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisgonggaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisgonggaids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisgonggaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisgonggaids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisgonggaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisgonggaids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisgonggaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisgonggaids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisgonggaids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisgonggaids0")
    ...
    return msg + r
@op
def cuahsihisgonggaids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisgonggaids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisgonggaids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisgonggaids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisgonggaids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisgonggaids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisgonggaids0"
...
def cuahsihisgonggaids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisgonggaids0"
...
def harvest_cuahsihisgonggaids0():
    harvest = cuahsihisgonggaids0_gleaner()

    report_ms3 = cuahsihisgonggaids0_missingreport_s3(harvest)
    report_idstat = cuahsihisgonggaids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisgonggaids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisgonggaids0")
    load_release = cuahsihisgonggaids0_naburelease(harvest)
    load_uploadrelease = cuahsihisgonggaids0_uploadrelease(load_release)

    load_prune = cuahsihisgonggaids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisgonggaids0_nabuprov(load_prune)
    load_org = cuahsihisgonggaids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisgonggaids0_missingreport_graph(load_org)
    report_graph=cuahsihisgonggaids0_graph_reports(report_msgraph)
