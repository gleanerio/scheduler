...
def cuahsihislczoodm2ids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsihislczoodm2ids0")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def cuahsihislczoodm2ids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsihislczoodm2ids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihislczoodm2ids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsihislczoodm2ids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihislczoodm2ids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsihislczoodm2ids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihislczoodm2ids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsihislczoodm2ids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihislczoodm2ids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsihislczoodm2ids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihislczoodm2ids0_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def cuahsihislczoodm2ids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsihislczoodm2ids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def cuahsihislczoodm2ids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsihislczoodm2ids0"
    )
...
@op(ins={"start": In(Nothing)})
def cuahsihislczoodm2ids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsihislczoodm2ids0_upload_summarize(context):
    returned_value = post_to_graph(
        "cuahsihislczoodm2ids0",
        path=SUMMARY_PATH,
        extension="ttl",
...
def harvest_cuahsihislczoodm2ids0():
    ...
    # prov branch
    prov_release = cuahsihislczoodm2ids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsihislczoodm2ids0_nabu_provclear(start=prov_release)
    prov_object = cuahsihislczoodm2ids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsihislczoodm2ids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsihislczoodm2ids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsihislczoodm2ids0_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = cuahsihislczoodm2ids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsihislczoodm2ids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsihislczoodm2ids0_missingreport_graph(start=summarize)
