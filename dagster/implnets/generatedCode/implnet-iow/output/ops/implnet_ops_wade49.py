...
def wade49_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "wade49")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade49_naburelease(context):
    returned_value = gleanerio(context, ("release"), "wade49")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade49_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wade49")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade49_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "wade49")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade49_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wade49")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade49_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wade49")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade49_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wade49")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade49_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wade49")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade49_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wade49")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade49_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "wade49")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wade49_uploadrelease(context):
#     returned_value = post_to_graph("wade49", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wade49_uploadrelease(context):
#     returned_value = postRelease("wade49")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def wade49_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade49"
    )
...
@op(ins={"start": In(Nothing)})
def wade49_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade49"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def wade49_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade49"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def wade49_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade49"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def wade49_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wade49_upload_summarize(context):
...
def harvest_wade49():
    ...
    containers = wade49_getImage()

    # conduct the harvest
    harvest = wade49_gleaner(start=containers)

    # data branch
    load_release = wade49_naburelease(start=harvest)
    load_uploadrelease = wade49_uploadrelease(start=load_release)
    load_prune = wade49_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = wade49_nabu_provrelease(start=harvest)
    prov_clear = wade49_nabu_provclear(start=prov_release)
    prov_object = wade49_nabu_provobject(start=prov_clear)
    prov_drain = wade49_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wade49_nabu_orgsrelease(start=harvest)
    load_org = wade49_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = wade49_missingreport_s3(start=harvest)
    # report_idstat = wade49_identifier_stats(start=report_ms3)
    # report_bucketurl = wade49_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = wade49_summarize(start=load_uploadrelease)
    # upload_summarize = wade49_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wade49_missingreport_graph(start=summarize)
    # report_graph = wade49_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="wade49"):
#
