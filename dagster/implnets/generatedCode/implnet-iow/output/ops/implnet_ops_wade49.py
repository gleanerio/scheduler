...
@op(ins={"start": In(Nothing)})
def wade49_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade49")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade49_nabuorg(context):
...
def wade49_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade49")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade49")
    ...
    load_prov = wade49_nabuprov(start=load_prune)
    load_org = wade49_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade49_missingreport_graph(start=load_org)
