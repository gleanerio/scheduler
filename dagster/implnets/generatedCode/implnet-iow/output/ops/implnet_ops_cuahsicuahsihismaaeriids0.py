...
# #########################  Ops Section
@op
def cuahsicuahsihismaaeriids0_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def cuahsicuahsihismaaeriids0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "cuahsicuahsihismaaeriids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihismaaeriids0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "cuahsicuahsihismaaeriids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihismaaeriids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsicuahsihismaaeriids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihismaaeriids0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "cuahsicuahsihismaaeriids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihismaaeriids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsicuahsihismaaeriids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihismaaeriids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsicuahsihismaaeriids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihismaaeriids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsicuahsihismaaeriids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihismaaeriids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsicuahsihismaaeriids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihismaaeriids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsicuahsihismaaeriids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihismaaeriids0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "cuahsicuahsihismaaeriids0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihismaaeriids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsicuahsihismaaeriids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihismaaeriids0_uploadrelease(context):
#     returned_value = postRelease("cuahsicuahsihismaaeriids0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihismaaeriids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihismaaeriids0"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsicuahsihismaaeriids0"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihismaaeriids0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihismaaeriids0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihismaaeriids0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihismaaeriids0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihismaaeriids0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihismaaeriids0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihismaaeriids0_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihismaaeriids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihismaaeriids0_upload_summarize(context):
    returned_value = post_to_graph(
        "cuahsicuahsihismaaeriids0",
        path=SUMMARY_PATH,
        extension="ttl",
...
@graph
def harvest_cuahsicuahsihismaaeriids0():
    # check for containers
    containers = cuahsicuahsihismaaeriids0_getImage()

    # conduct the harvest
    harvest = cuahsicuahsihismaaeriids0_gleaner(start=containers)

    # data branch
    load_release = cuahsicuahsihismaaeriids0_naburelease(start=harvest)
    load_uploadrelease = cuahsicuahsihismaaeriids0_uploadrelease(start=load_release)
    load_prune = cuahsicuahsihismaaeriids0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = cuahsicuahsihismaaeriids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsicuahsihismaaeriids0_nabu_provclear(start=prov_release)
    prov_object = cuahsicuahsihismaaeriids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsicuahsihismaaeriids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsicuahsihismaaeriids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsicuahsihismaaeriids0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = cuahsicuahsihismaaeriids0_missingreport_s3(start=harvest)
    # report_idstat = cuahsicuahsihismaaeriids0_identifier_stats(start=report_ms3)
    # report_bucketurl = cuahsicuahsihismaaeriids0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = cuahsicuahsihismaaeriids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsicuahsihismaaeriids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsicuahsihismaaeriids0_missingreport_graph(start=summarize)
    # report_graph = cuahsicuahsihismaaeriids0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="cuahsicuahsihismaaeriids0"):
#
