...
@op
def cuahsihisodmkentstateids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisodmkentstateids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisodmkentstateids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisodmkentstateids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisodmkentstateids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisodmkentstateids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisodmkentstateids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisodmkentstateids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisodmkentstateids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisodmkentstateids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisodmkentstateids0")
    ...
    return msg + r
@op
def cuahsihisodmkentstateids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisodmkentstateids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisodmkentstateids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisodmkentstateids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisodmkentstateids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisodmkentstateids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisodmkentstateids0"
...
def cuahsihisodmkentstateids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisodmkentstateids0"
...
def harvest_cuahsihisodmkentstateids0():
    harvest = cuahsihisodmkentstateids0_gleaner()

    report_ms3 = cuahsihisodmkentstateids0_missingreport_s3(harvest)
    report_idstat = cuahsihisodmkentstateids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisodmkentstateids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisodmkentstateids0")
    load_release = cuahsihisodmkentstateids0_naburelease(harvest)
    load_uploadrelease = cuahsihisodmkentstateids0_uploadrelease(load_release)

    load_prune = cuahsihisodmkentstateids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisodmkentstateids0_nabuprov(load_prune)
    load_org = cuahsihisodmkentstateids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisodmkentstateids0_missingreport_graph(load_org)
    report_graph=cuahsihisodmkentstateids0_graph_reports(report_msgraph)
