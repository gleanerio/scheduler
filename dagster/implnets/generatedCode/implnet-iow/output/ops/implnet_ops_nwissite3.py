...
def nwissite3_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "nwissite3")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def nwissite3_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "nwissite3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwissite3_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "nwissite3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwissite3_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "nwissite3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwissite3_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "nwissite3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwissite3_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "nwissite3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwissite3_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def nwissite3_uploadrelease(context):
#     returned_value = post_to_graph("nwissite3", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def nwissite3_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nwissite3"
    )
...
@op(ins={"start": In(Nothing)})
def nwissite3_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def nwissite3_upload_summarize(context):
...
def harvest_nwissite3():
    ...
    # prov branch
    prov_release = nwissite3_nabu_provrelease(start=harvest)
    prov_clear = nwissite3_nabu_provclear(start=prov_release)
    prov_object = nwissite3_nabu_provobject(start=prov_clear)
    prov_drain = nwissite3_nabu_provdrain(start=prov_object)

    # org branch
    org_release = nwissite3_nabu_orgsrelease(start=harvest)
    load_org = nwissite3_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = nwissite3_summarize(start=load_uploadrelease)
    # upload_summarize = nwissite3_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = nwissite3_missingreport_graph(start=summarize)
