...
@op
def nwissite3_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwissite3")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwissite3_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwissite3")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwissite3_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwissite3")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwissite3_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwissite3")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwissite3_uploadrelease(context, msg: str):
...
@op
def nwissite3_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwissite3")
    ...
    return msg + r
@op
def nwissite3_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwissite3")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwissite3_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwissite3")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwissite3_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwissite3")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwissite3"
...
def nwissite3_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwissite3"
...
def harvest_nwissite3():
    harvest = nwissite3_gleaner()

    report_ms3 = nwissite3_missingreport_s3(harvest)
    report_idstat = nwissite3_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwissite3_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwissite3")
    load_release = nwissite3_naburelease(harvest)
    load_uploadrelease = nwissite3_uploadrelease(load_release)

    load_prune = nwissite3_nabu_prune(load_uploadrelease)
    load_prov = nwissite3_nabuprov(load_prune)
    load_org = nwissite3_nabuorg(load_prov)

# run after load
    report_msgraph=nwissite3_missingreport_graph(load_org)
    report_graph=nwissite3_graph_reports(report_msgraph)
