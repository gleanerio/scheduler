...
@op(ins={"start": In(Nothing)})
def wade45_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade45")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade45_nabuorg(context):
...
def wade45_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade45")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade45")
    ...
    load_prov = wade45_nabuprov(start=load_prune)
    load_org = wade45_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade45_missingreport_graph(start=load_org)
