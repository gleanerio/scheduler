...
@op
def cuahsihisfcelterids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisfcelterids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisfcelterids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisfcelterids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisfcelterids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisfcelterids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisfcelterids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisfcelterids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisfcelterids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisfcelterids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisfcelterids0")
    ...
    return msg + r
@op
def cuahsihisfcelterids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisfcelterids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisfcelterids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisfcelterids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisfcelterids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisfcelterids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisfcelterids0"
...
def cuahsihisfcelterids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisfcelterids0"
...
def harvest_cuahsihisfcelterids0():
    harvest = cuahsihisfcelterids0_gleaner()

    report_ms3 = cuahsihisfcelterids0_missingreport_s3(harvest)
    report_idstat = cuahsihisfcelterids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisfcelterids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisfcelterids0")
    load_release = cuahsihisfcelterids0_naburelease(harvest)
    load_uploadrelease = cuahsihisfcelterids0_uploadrelease(load_release)

    load_prune = cuahsihisfcelterids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisfcelterids0_nabuprov(load_prune)
    load_org = cuahsihisfcelterids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisfcelterids0_missingreport_graph(load_org)
    report_graph=cuahsihisfcelterids0_graph_reports(report_msgraph)
