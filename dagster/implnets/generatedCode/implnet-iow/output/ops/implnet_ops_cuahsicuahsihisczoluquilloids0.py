...
# #########################  Ops Section
@op
def cuahsicuahsihisczoluquilloids0_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def cuahsicuahsihisczoluquilloids0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "cuahsicuahsihisczoluquilloids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoluquilloids0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "cuahsicuahsihisczoluquilloids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoluquilloids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsicuahsihisczoluquilloids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoluquilloids0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "cuahsicuahsihisczoluquilloids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoluquilloids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsicuahsihisczoluquilloids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoluquilloids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsicuahsihisczoluquilloids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoluquilloids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsicuahsihisczoluquilloids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoluquilloids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsicuahsihisczoluquilloids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoluquilloids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsicuahsihisczoluquilloids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoluquilloids0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "cuahsicuahsihisczoluquilloids0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihisczoluquilloids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsicuahsihisczoluquilloids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihisczoluquilloids0_uploadrelease(context):
#     returned_value = postRelease("cuahsicuahsihisczoluquilloids0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoluquilloids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisczoluquilloids0"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsicuahsihisczoluquilloids0"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoluquilloids0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisczoluquilloids0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoluquilloids0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisczoluquilloids0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoluquilloids0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisczoluquilloids0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoluquilloids0_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoluquilloids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoluquilloids0_upload_summarize(context):
    returned_value = post_to_graph(
        "cuahsicuahsihisczoluquilloids0",
        path=SUMMARY_PATH,
        extension="ttl",
...
@graph
def harvest_cuahsicuahsihisczoluquilloids0():
    # check for containers
    containers = cuahsicuahsihisczoluquilloids0_getImage()

    # conduct the harvest
    harvest = cuahsicuahsihisczoluquilloids0_gleaner(start=containers)

    # data branch
    load_release = cuahsicuahsihisczoluquilloids0_naburelease(start=harvest)
    load_uploadrelease = cuahsicuahsihisczoluquilloids0_uploadrelease(start=load_release)
    load_prune = cuahsicuahsihisczoluquilloids0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = cuahsicuahsihisczoluquilloids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsicuahsihisczoluquilloids0_nabu_provclear(start=prov_release)
    prov_object = cuahsicuahsihisczoluquilloids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsicuahsihisczoluquilloids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsicuahsihisczoluquilloids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsicuahsihisczoluquilloids0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = cuahsicuahsihisczoluquilloids0_missingreport_s3(start=harvest)
    # report_idstat = cuahsicuahsihisczoluquilloids0_identifier_stats(start=report_ms3)
    # report_bucketurl = cuahsicuahsihisczoluquilloids0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = cuahsicuahsihisczoluquilloids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsicuahsihisczoluquilloids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsicuahsihisczoluquilloids0_missingreport_graph(start=summarize)
    # report_graph = cuahsicuahsihisczoluquilloids0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="cuahsicuahsihisczoluquilloids0"):
#
