...
@op
def cuahsihispanolaodmids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihispanolaodmids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihispanolaodmids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihispanolaodmids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihispanolaodmids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihispanolaodmids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihispanolaodmids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihispanolaodmids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihispanolaodmids0_uploadrelease(context, msg: str):
...
@op
def cuahsihispanolaodmids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihispanolaodmids0")
    ...
    return msg + r
@op
def cuahsihispanolaodmids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihispanolaodmids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihispanolaodmids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihispanolaodmids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihispanolaodmids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihispanolaodmids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihispanolaodmids0"
...
def cuahsihispanolaodmids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihispanolaodmids0"
...
def harvest_cuahsihispanolaodmids0():
    harvest = cuahsihispanolaodmids0_gleaner()

    report_ms3 = cuahsihispanolaodmids0_missingreport_s3(harvest)
    report_idstat = cuahsihispanolaodmids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihispanolaodmids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihispanolaodmids0")
    load_release = cuahsihispanolaodmids0_naburelease(harvest)
    load_uploadrelease = cuahsihispanolaodmids0_uploadrelease(load_release)

    load_prune = cuahsihispanolaodmids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihispanolaodmids0_nabuprov(load_prune)
    load_org = cuahsihispanolaodmids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihispanolaodmids0_missingreport_graph(load_org)
    report_graph=cuahsihispanolaodmids0_graph_reports(report_msgraph)
