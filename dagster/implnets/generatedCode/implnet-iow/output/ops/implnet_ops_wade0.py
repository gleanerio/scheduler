...
@op
def wade0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "wade0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "wade0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "wade0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "wade0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def wade0_uploadrelease(context, msg: str):
...
@op
def wade0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade0")
    ...
    return msg + r
@op
def wade0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def wade0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def wade0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade0"
...
def wade0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade0"
...
@graph
def harvest_wade0():
    harvest = wade0_gleaner()

    report_ms3 = wade0_missingreport_s3(harvest)
    report_idstat = wade0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = wade0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="wade0")
    load_release = wade0_naburelease(harvest)
    load_uploadrelease = wade0_uploadrelease(load_release)

    load_prune = wade0_nabu_prune(load_uploadrelease)
    load_prov = wade0_nabuprov(load_prune)
    load_org = wade0_nabuorg(load_prov)

# run after load
    report_msgraph=wade0_missingreport_graph(load_org)
    report_graph=wade0_graph_reports(report_msgraph)
