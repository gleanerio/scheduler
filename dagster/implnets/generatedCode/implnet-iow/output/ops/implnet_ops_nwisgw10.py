...
@op
def nwisgw10_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw10")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw10_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw10")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw10_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw10")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw10_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw10")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw10_uploadrelease(context, msg: str):
...
@op
def nwisgw10_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw10")
    ...
    return msg + r
@op
def nwisgw10_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw10")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw10_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw10")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw10_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw10")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw10"
...
def nwisgw10_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw10"
...
def harvest_nwisgw10():
    harvest = nwisgw10_gleaner()

    report_ms3 = nwisgw10_missingreport_s3(harvest)
    report_idstat = nwisgw10_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw10_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw10")
    load_release = nwisgw10_naburelease(harvest)
    load_uploadrelease = nwisgw10_uploadrelease(load_release)

    load_prune = nwisgw10_nabu_prune(load_uploadrelease)
    load_prov = nwisgw10_nabuprov(load_prune)
    load_org = nwisgw10_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw10_missingreport_graph(load_org)
    report_graph=nwisgw10_graph_reports(report_msgraph)
