...
def nwisgw10_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "nwisgw10")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def nwisgw10_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "nwisgw10")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw10_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "nwisgw10")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw10_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "nwisgw10")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw10_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "nwisgw10")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw10_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "nwisgw10")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw10_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def nwisgw10_uploadrelease(context):
#     returned_value = post_to_graph("nwisgw10", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def nwisgw10_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nwisgw10"
    )
...
@op(ins={"start": In(Nothing)})
def nwisgw10_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def nwisgw10_upload_summarize(context):
...
def harvest_nwisgw10():
    ...
    # prov branch
    prov_release = nwisgw10_nabu_provrelease(start=harvest)
    prov_clear = nwisgw10_nabu_provclear(start=prov_release)
    prov_object = nwisgw10_nabu_provobject(start=prov_clear)
    prov_drain = nwisgw10_nabu_provdrain(start=prov_object)

    # org branch
    org_release = nwisgw10_nabu_orgsrelease(start=harvest)
    load_org = nwisgw10_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = nwisgw10_summarize(start=load_uploadrelease)
    # upload_summarize = nwisgw10_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = nwisgw10_missingreport_graph(start=summarize)
