...
def gfv11pois1_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "gfv11pois1")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def gfv11pois1_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "gfv11pois1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gfv11pois1_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "gfv11pois1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gfv11pois1_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "gfv11pois1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gfv11pois1_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "gfv11pois1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gfv11pois1_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "gfv11pois1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gfv11pois1_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def gfv11pois1_uploadrelease(context):
#     returned_value = post_to_graph("gfv11pois1", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def gfv11pois1_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="gfv11pois1"
    )
...
@op(ins={"start": In(Nothing)})
def gfv11pois1_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def gfv11pois1_upload_summarize(context):
...
def harvest_gfv11pois1():
    ...
    # prov branch
    prov_release = gfv11pois1_nabu_provrelease(start=harvest)
    prov_clear = gfv11pois1_nabu_provclear(start=prov_release)
    prov_object = gfv11pois1_nabu_provobject(start=prov_clear)
    prov_drain = gfv11pois1_nabu_provdrain(start=prov_object)

    # org branch
    org_release = gfv11pois1_nabu_orgsrelease(start=harvest)
    load_org = gfv11pois1_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = gfv11pois1_summarize(start=load_uploadrelease)
    # upload_summarize = gfv11pois1_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = gfv11pois1_missingreport_graph(start=summarize)
