...
@op(ins={"start": In(Nothing)})
def wyseo0_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wyseo0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wyseo0_nabuorg(context):
...
def wyseo0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wyseo0")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wyseo0")
    ...
    load_prov = wyseo0_nabuprov(start=load_prune)
    load_org = wyseo0_nabuorg(start=load_prov)

# run after load
    report_msgraph=wyseo0_missingreport_graph(start=load_org)
