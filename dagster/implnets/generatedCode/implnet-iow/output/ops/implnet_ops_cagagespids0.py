...
@op
def cagagespids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cagagespids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cagagespids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cagagespids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cagagespids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cagagespids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cagagespids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cagagespids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cagagespids0_uploadrelease(context, msg: str):
...
@op
def cagagespids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cagagespids0")
    ...
    return msg + r
@op
def cagagespids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cagagespids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cagagespids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cagagespids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cagagespids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cagagespids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cagagespids0"
...
def cagagespids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cagagespids0"
...
def harvest_cagagespids0():
    harvest = cagagespids0_gleaner()

    report_ms3 = cagagespids0_missingreport_s3(harvest)
    report_idstat = cagagespids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cagagespids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cagagespids0")
    load_release = cagagespids0_naburelease(harvest)
    load_uploadrelease = cagagespids0_uploadrelease(load_release)

    load_prune = cagagespids0_nabu_prune(load_uploadrelease)
    load_prov = cagagespids0_nabuprov(load_prune)
    load_org = cagagespids0_nabuorg(load_prov)

# run after load
    report_msgraph=cagagespids0_missingreport_graph(load_org)
    report_graph=cagagespids0_graph_reports(report_msgraph)
