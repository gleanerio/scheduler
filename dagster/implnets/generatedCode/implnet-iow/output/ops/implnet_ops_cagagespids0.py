...
def cagagespids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cagagespids0")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def cagagespids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cagagespids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cagagespids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cagagespids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cagagespids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cagagespids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cagagespids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cagagespids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cagagespids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cagagespids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cagagespids0_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def cagagespids0_uploadrelease(context):
#     returned_value = post_to_graph("cagagespids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def cagagespids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cagagespids0"
    )
...
@op(ins={"start": In(Nothing)})
def cagagespids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cagagespids0_upload_summarize(context):
...
def harvest_cagagespids0():
    ...
    # prov branch
    prov_release = cagagespids0_nabu_provrelease(start=harvest)
    prov_clear = cagagespids0_nabu_provclear(start=prov_release)
    prov_object = cagagespids0_nabu_provobject(start=prov_clear)
    prov_drain = cagagespids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cagagespids0_nabu_orgsrelease(start=harvest)
    load_org = cagagespids0_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = cagagespids0_summarize(start=load_uploadrelease)
    # upload_summarize = cagagespids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cagagespids0_missingreport_graph(start=summarize)
