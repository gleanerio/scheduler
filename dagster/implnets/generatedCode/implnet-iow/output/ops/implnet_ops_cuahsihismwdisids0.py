...
@op
def cuahsihismwdisids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihismwdisids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismwdisids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihismwdisids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismwdisids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihismwdisids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismwdisids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihismwdisids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihismwdisids0_uploadrelease(context, msg: str):
...
@op
def cuahsihismwdisids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismwdisids0")
    ...
    return msg + r
@op
def cuahsihismwdisids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismwdisids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihismwdisids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismwdisids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihismwdisids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismwdisids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihismwdisids0"
...
def cuahsihismwdisids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihismwdisids0"
...
def harvest_cuahsihismwdisids0():
    harvest = cuahsihismwdisids0_gleaner()

    report_ms3 = cuahsihismwdisids0_missingreport_s3(harvest)
    report_idstat = cuahsihismwdisids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihismwdisids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihismwdisids0")
    load_release = cuahsihismwdisids0_naburelease(harvest)
    load_uploadrelease = cuahsihismwdisids0_uploadrelease(load_release)

    load_prune = cuahsihismwdisids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihismwdisids0_nabuprov(load_prune)
    load_org = cuahsihismwdisids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihismwdisids0_missingreport_graph(load_org)
    report_graph=cuahsihismwdisids0_graph_reports(report_msgraph)
