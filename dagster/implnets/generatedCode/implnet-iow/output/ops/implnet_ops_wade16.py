...
@op
def wade16_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "wade16")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade16_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "wade16")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade16_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "wade16")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade16_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "wade16")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def wade16_uploadrelease(context, msg: str):
...
@op
def wade16_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade16")
    ...
    return msg + r
@op
def wade16_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade16")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def wade16_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade16")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def wade16_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade16")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade16"
...
def wade16_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade16"
...
@graph
def harvest_wade16():
    harvest = wade16_gleaner()

    report_ms3 = wade16_missingreport_s3(harvest)
    report_idstat = wade16_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = wade16_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="wade16")
    load_release = wade16_naburelease(harvest)
    load_uploadrelease = wade16_uploadrelease(load_release)

    load_prune = wade16_nabu_prune(load_uploadrelease)
    load_prov = wade16_nabuprov(load_prune)
    load_org = wade16_nabuorg(load_prov)

# run after load
    report_msgraph=wade16_missingreport_graph(load_org)
    report_graph=wade16_graph_reports(report_msgraph)
