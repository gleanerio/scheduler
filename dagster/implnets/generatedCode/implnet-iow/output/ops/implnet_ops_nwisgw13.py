...
@op
def nwisgw13_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw13")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw13_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw13")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw13_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw13")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw13_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw13")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw13_uploadrelease(context, msg: str):
...
@op
def nwisgw13_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw13")
    ...
    return msg + r
@op
def nwisgw13_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw13")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw13_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw13")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw13_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw13")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw13"
...
def nwisgw13_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw13"
...
def harvest_nwisgw13():
    harvest = nwisgw13_gleaner()

    report_ms3 = nwisgw13_missingreport_s3(harvest)
    report_idstat = nwisgw13_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw13_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw13")
    load_release = nwisgw13_naburelease(harvest)
    load_uploadrelease = nwisgw13_uploadrelease(load_release)

    load_prune = nwisgw13_nabu_prune(load_uploadrelease)
    load_prov = nwisgw13_nabuprov(load_prune)
    load_org = nwisgw13_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw13_missingreport_graph(load_org)
    report_graph=nwisgw13_graph_reports(report_msgraph)
