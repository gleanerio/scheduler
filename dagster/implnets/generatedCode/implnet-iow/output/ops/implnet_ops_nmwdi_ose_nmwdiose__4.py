...
# #########################  Ops Section
@op
def nmwdi_ose_nmwdiose__4_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def nmwdi_ose_nmwdiose__4_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "nmwdi_ose_nmwdiose__4")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdi_ose_nmwdiose__4_naburelease(context):
    returned_value = gleanerio(context, ("release"), "nmwdi_ose_nmwdiose__4")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdi_ose_nmwdiose__4_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "nmwdi_ose_nmwdiose__4")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdi_ose_nmwdiose__4_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "nmwdi_ose_nmwdiose__4")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdi_ose_nmwdiose__4_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "nmwdi_ose_nmwdiose__4")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdi_ose_nmwdiose__4_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "nmwdi_ose_nmwdiose__4")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdi_ose_nmwdiose__4_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "nmwdi_ose_nmwdiose__4")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdi_ose_nmwdiose__4_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "nmwdi_ose_nmwdiose__4")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdi_ose_nmwdiose__4_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "nmwdi_ose_nmwdiose__4")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdi_ose_nmwdiose__4_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "nmwdi_ose_nmwdiose__4")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def nmwdi_ose_nmwdiose__4_uploadrelease(context):
#     returned_value = post_to_graph("nmwdi_ose_nmwdiose__4", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def nmwdi_ose_nmwdiose__4_uploadrelease(context):
#     returned_value = postRelease("nmwdi_ose_nmwdiose__4")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def nmwdi_ose_nmwdiose__4_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nmwdi_ose_nmwdiose__4"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nmwdi_ose_nmwdiose__4"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def nmwdi_ose_nmwdiose__4_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nmwdi_ose_nmwdiose__4"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def nmwdi_ose_nmwdiose__4_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nmwdi_ose_nmwdiose__4"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def nmwdi_ose_nmwdiose__4_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nmwdi_ose_nmwdiose__4"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def nmwdi_ose_nmwdiose__4_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def nmwdi_ose_nmwdiose__4_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def nmwdi_ose_nmwdiose__4_upload_summarize(context):
    returned_value = post_to_graph(
        "nmwdi_ose_nmwdiose__4",
        path=SUMMARY_PATH,
        extension="ttl",
...
@graph
def harvest_nmwdi_ose_nmwdiose__4():
    # check for containers
    containers = nmwdi_ose_nmwdiose__4_getImage()

    # conduct the harvest
    harvest = nmwdi_ose_nmwdiose__4_gleaner(start=containers)

    # data branch
    load_release = nmwdi_ose_nmwdiose__4_naburelease(start=harvest)
    load_uploadrelease = nmwdi_ose_nmwdiose__4_uploadrelease(start=load_release)
    load_prune = nmwdi_ose_nmwdiose__4_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = nmwdi_ose_nmwdiose__4_nabu_provrelease(start=harvest)
    prov_clear = nmwdi_ose_nmwdiose__4_nabu_provclear(start=prov_release)
    prov_object = nmwdi_ose_nmwdiose__4_nabu_provobject(start=prov_clear)
    prov_drain = nmwdi_ose_nmwdiose__4_nabu_provdrain(start=prov_object)

    # org branch
    org_release = nmwdi_ose_nmwdiose__4_nabu_orgsrelease(start=harvest)
    load_org = nmwdi_ose_nmwdiose__4_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = nmwdi_ose_nmwdiose__4_missingreport_s3(start=harvest)
    # report_idstat = nmwdi_ose_nmwdiose__4_identifier_stats(start=report_ms3)
    # report_bucketurl = nmwdi_ose_nmwdiose__4_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = nmwdi_ose_nmwdiose__4_summarize(start=load_uploadrelease)
    # upload_summarize = nmwdi_ose_nmwdiose__4_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = nmwdi_ose_nmwdiose__4_missingreport_graph(start=summarize)
    # report_graph = nmwdi_ose_nmwdiose__4_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="nmwdi_ose_nmwdiose__4"):
#
