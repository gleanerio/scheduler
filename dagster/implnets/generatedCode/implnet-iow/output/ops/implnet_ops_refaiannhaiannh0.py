...
# #########################  Ops Section
@op
def refaiannhaiannh0_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def refaiannhaiannh0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "refaiannhaiannh0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refaiannhaiannh0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "refaiannhaiannh0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refaiannhaiannh0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "refaiannhaiannh0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refaiannhaiannh0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "refaiannhaiannh0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refaiannhaiannh0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "refaiannhaiannh0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refaiannhaiannh0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "refaiannhaiannh0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refaiannhaiannh0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "refaiannhaiannh0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refaiannhaiannh0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "refaiannhaiannh0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refaiannhaiannh0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "refaiannhaiannh0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refaiannhaiannh0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "refaiannhaiannh0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def refaiannhaiannh0_uploadrelease(context):
#     returned_value = post_to_graph("refaiannhaiannh0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def refaiannhaiannh0_uploadrelease(context):
#     returned_value = postRelease("refaiannhaiannh0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def refaiannhaiannh0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refaiannhaiannh0"
    )
...
@op(ins={"start": In(Nothing)})
def refaiannhaiannh0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refaiannhaiannh0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def refaiannhaiannh0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refaiannhaiannh0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def refaiannhaiannh0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refaiannhaiannh0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def refaiannhaiannh0_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def refaiannhaiannh0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def refaiannhaiannh0_upload_summarize(context):
...
@graph
def harvest_refaiannhaiannh0():
    # check for containers
    containers = refaiannhaiannh0_getImage()

    # conduct the harvest
    harvest = refaiannhaiannh0_gleaner(start=containers)

    # data branch
    load_release = refaiannhaiannh0_naburelease(start=harvest)
    load_uploadrelease = refaiannhaiannh0_uploadrelease(start=load_release)
    load_prune = refaiannhaiannh0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = refaiannhaiannh0_nabu_provrelease(start=harvest)
    prov_clear = refaiannhaiannh0_nabu_provclear(start=prov_release)
    prov_object = refaiannhaiannh0_nabu_provobject(start=prov_clear)
    prov_drain = refaiannhaiannh0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = refaiannhaiannh0_nabu_orgsrelease(start=harvest)
    load_org = refaiannhaiannh0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = refaiannhaiannh0_missingreport_s3(start=harvest)
    # report_idstat = refaiannhaiannh0_identifier_stats(start=report_ms3)
    # report_bucketurl = refaiannhaiannh0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = refaiannhaiannh0_summarize(start=load_uploadrelease)
    # upload_summarize = refaiannhaiannh0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = refaiannhaiannh0_missingreport_graph(start=summarize)
    # report_graph = refaiannhaiannh0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="refaiannhaiannh0"):
#
