...
# #########################  Ops Section
@op
def nmwdiosenmwdiose2_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def nmwdiosenmwdiose2_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "nmwdiosenmwdiose2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose2_naburelease(context):
    returned_value = gleanerio(context, ("release"), "nmwdiosenmwdiose2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose2_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "nmwdiosenmwdiose2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose2_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "nmwdiosenmwdiose2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose2_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "nmwdiosenmwdiose2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose2_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "nmwdiosenmwdiose2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose2_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "nmwdiosenmwdiose2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose2_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "nmwdiosenmwdiose2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose2_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "nmwdiosenmwdiose2")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose2_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "nmwdiosenmwdiose2")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def nmwdiosenmwdiose2_uploadrelease(context):
#     returned_value = post_to_graph("nmwdiosenmwdiose2", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def nmwdiosenmwdiose2_uploadrelease(context):
#     returned_value = postRelease("nmwdiosenmwdiose2")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose2_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nmwdiosenmwdiose2"
    )
...
@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose2_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nmwdiosenmwdiose2"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose2_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nmwdiosenmwdiose2"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose2_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nmwdiosenmwdiose2"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose2_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose2_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose2_upload_summarize(context):
...
@graph
def harvest_nmwdiosenmwdiose2():
    # check for containers
    containers = nmwdiosenmwdiose2_getImage()

    # conduct the harvest
    harvest = nmwdiosenmwdiose2_gleaner(start=containers)

    # data branch
    load_release = nmwdiosenmwdiose2_naburelease(start=harvest)
    load_uploadrelease = nmwdiosenmwdiose2_uploadrelease(start=load_release)
    load_prune = nmwdiosenmwdiose2_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = nmwdiosenmwdiose2_nabu_provrelease(start=harvest)
    prov_clear = nmwdiosenmwdiose2_nabu_provclear(start=prov_release)
    prov_object = nmwdiosenmwdiose2_nabu_provobject(start=prov_clear)
    prov_drain = nmwdiosenmwdiose2_nabu_provdrain(start=prov_object)

    # org branch
    org_release = nmwdiosenmwdiose2_nabu_orgsrelease(start=harvest)
    load_org = nmwdiosenmwdiose2_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = nmwdiosenmwdiose2_missingreport_s3(start=harvest)
    # report_idstat = nmwdiosenmwdiose2_identifier_stats(start=report_ms3)
    # report_bucketurl = nmwdiosenmwdiose2_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = nmwdiosenmwdiose2_summarize(start=load_uploadrelease)
    # upload_summarize = nmwdiosenmwdiose2_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = nmwdiosenmwdiose2_missingreport_graph(start=summarize)
    # report_graph = nmwdiosenmwdiose2_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="nmwdiosenmwdiose2"):
#
