...
@op
def wade11_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "wade11")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade11_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "wade11")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade11_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "wade11")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade11_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "wade11")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def wade11_uploadrelease(context, msg: str):
...
@op
def wade11_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade11")
    ...
    return msg + r
@op
def wade11_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade11")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def wade11_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade11")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def wade11_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade11")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade11"
...
def wade11_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade11"
...
@graph
def harvest_wade11():
    harvest = wade11_gleaner()

    report_ms3 = wade11_missingreport_s3(harvest)
    report_idstat = wade11_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = wade11_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="wade11")
    load_release = wade11_naburelease(harvest)
    load_uploadrelease = wade11_uploadrelease(load_release)

    load_prune = wade11_nabu_prune(load_uploadrelease)
    load_prov = wade11_nabuprov(load_prune)
    load_org = wade11_nabuorg(load_prov)

# run after load
    report_msgraph=wade11_missingreport_graph(load_org)
    report_graph=wade11_graph_reports(report_msgraph)
