...
def wade11_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wade11")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def wade11_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wade11")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade11_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wade11")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade11_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wade11")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade11_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wade11")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade11_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wade11")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade11_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def wade11_uploadrelease(context):
#     returned_value = post_to_graph("wade11", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def wade11_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade11"
    )
...
@op(ins={"start": In(Nothing)})
def wade11_upload_summarize(context):
...
def harvest_wade11():
    ...
    # prov branch
    prov_release = wade11_nabu_provrelease(start=harvest)
    prov_clear = wade11_nabu_provclear(start=prov_release)
    prov_object = wade11_nabu_provobject(start=prov_clear)
    prov_drain = wade11_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wade11_nabu_orgsrelease(start=harvest)
    load_org = wade11_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = wade11_summarize(start=load_uploadrelease)
    # upload_summarize = wade11_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wade11_missingreport_graph(start=summarize)
