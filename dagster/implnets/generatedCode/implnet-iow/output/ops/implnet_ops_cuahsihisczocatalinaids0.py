...
def cuahsihisczocatalinaids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsihisczocatalinaids0")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def cuahsihisczocatalinaids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsihisczocatalinaids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisczocatalinaids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsihisczocatalinaids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisczocatalinaids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsihisczocatalinaids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisczocatalinaids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsihisczocatalinaids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisczocatalinaids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsihisczocatalinaids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisczocatalinaids0_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def cuahsihisczocatalinaids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsihisczocatalinaids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def cuahsihisczocatalinaids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsihisczocatalinaids0"
    )
...
@op(ins={"start": In(Nothing)})
def cuahsihisczocatalinaids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsihisczocatalinaids0_upload_summarize(context):
    returned_value = post_to_graph(
        "cuahsihisczocatalinaids0",
        path=SUMMARY_PATH,
        extension="ttl",
...
def harvest_cuahsihisczocatalinaids0():
    ...
    # prov branch
    prov_release = cuahsihisczocatalinaids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsihisczocatalinaids0_nabu_provclear(start=prov_release)
    prov_object = cuahsihisczocatalinaids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsihisczocatalinaids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsihisczocatalinaids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsihisczocatalinaids0_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = cuahsihisczocatalinaids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsihisczocatalinaids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsihisczocatalinaids0_missingreport_graph(start=summarize)
