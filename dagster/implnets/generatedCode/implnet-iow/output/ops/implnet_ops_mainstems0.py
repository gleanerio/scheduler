...
@op
def mainstems0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "mainstems0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def mainstems0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "mainstems0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def mainstems0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "mainstems0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def mainstems0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "mainstems0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def mainstems0_uploadrelease(context, msg: str):
...
@op
def mainstems0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="mainstems0")
    ...
    return msg + r
@op
def mainstems0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="mainstems0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def mainstems0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="mainstems0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def mainstems0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="mainstems0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "mainstems0"
...
def mainstems0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "mainstems0"
...
def harvest_mainstems0():
    harvest = mainstems0_gleaner()

    report_ms3 = mainstems0_missingreport_s3(harvest)
    report_idstat = mainstems0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = mainstems0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="mainstems0")
    load_release = mainstems0_naburelease(harvest)
    load_uploadrelease = mainstems0_uploadrelease(load_release)

    load_prune = mainstems0_nabu_prune(load_uploadrelease)
    load_prov = mainstems0_nabuprov(load_prune)
    load_org = mainstems0_nabuorg(load_prov)

# run after load
    report_msgraph=mainstems0_missingreport_graph(load_org)
    report_graph=mainstems0_graph_reports(report_msgraph)
