...
def mainstems0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "mainstems0")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def mainstems0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "mainstems0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def mainstems0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "mainstems0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def mainstems0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "mainstems0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def mainstems0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "mainstems0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def mainstems0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "mainstems0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def mainstems0_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def mainstems0_uploadrelease(context):
#     returned_value = post_to_graph("mainstems0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def mainstems0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="mainstems0"
    )
...
@op(ins={"start": In(Nothing)})
def mainstems0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def mainstems0_upload_summarize(context):
...
def harvest_mainstems0():
    ...
    # prov branch
    prov_release = mainstems0_nabu_provrelease(start=harvest)
    prov_clear = mainstems0_nabu_provclear(start=prov_release)
    prov_object = mainstems0_nabu_provobject(start=prov_clear)
    prov_drain = mainstems0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = mainstems0_nabu_orgsrelease(start=harvest)
    load_org = mainstems0_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = mainstems0_summarize(start=load_uploadrelease)
    # upload_summarize = mainstems0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = mainstems0_missingreport_graph(start=summarize)
