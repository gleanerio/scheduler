...
# #########################  Ops Section
@op
def wadewade3_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def wadewade3_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "wadewade3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade3_naburelease(context):
    returned_value = gleanerio(context, ("release"), "wadewade3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade3_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wadewade3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade3_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "wadewade3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade3_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wadewade3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade3_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wadewade3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade3_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wadewade3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade3_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wadewade3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade3_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wadewade3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade3_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "wadewade3")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wadewade3_uploadrelease(context):
#     returned_value = post_to_graph("wadewade3", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wadewade3_uploadrelease(context):
#     returned_value = postRelease("wadewade3")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def wadewade3_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade3"
    )
...
@op(ins={"start": In(Nothing)})
def wadewade3_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade3"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def wadewade3_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade3"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def wadewade3_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade3"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def wadewade3_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wadewade3_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wadewade3_upload_summarize(context):
...
@graph
def harvest_wadewade3():
    # check for containers
    containers = wadewade3_getImage()

    # conduct the harvest
    harvest = wadewade3_gleaner(start=containers)

    # data branch
    load_release = wadewade3_naburelease(start=harvest)
    load_uploadrelease = wadewade3_uploadrelease(start=load_release)
    load_prune = wadewade3_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = wadewade3_nabu_provrelease(start=harvest)
    prov_clear = wadewade3_nabu_provclear(start=prov_release)
    prov_object = wadewade3_nabu_provobject(start=prov_clear)
    prov_drain = wadewade3_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wadewade3_nabu_orgsrelease(start=harvest)
    load_org = wadewade3_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = wadewade3_missingreport_s3(start=harvest)
    # report_idstat = wadewade3_identifier_stats(start=report_ms3)
    # report_bucketurl = wadewade3_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = wadewade3_summarize(start=load_uploadrelease)
    # upload_summarize = wadewade3_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wadewade3_missingreport_graph(start=summarize)
    # report_graph = wadewade3_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="wadewade3"):
#
