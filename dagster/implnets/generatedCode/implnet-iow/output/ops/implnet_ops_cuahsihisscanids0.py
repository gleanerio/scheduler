...
@op
def cuahsihisscanids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisscanids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisscanids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisscanids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisscanids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisscanids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisscanids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisscanids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisscanids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisscanids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisscanids0")
    ...
    return msg + r
@op
def cuahsihisscanids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisscanids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisscanids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisscanids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisscanids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisscanids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisscanids0"
...
def cuahsihisscanids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisscanids0"
...
def harvest_cuahsihisscanids0():
    harvest = cuahsihisscanids0_gleaner()

    report_ms3 = cuahsihisscanids0_missingreport_s3(harvest)
    report_idstat = cuahsihisscanids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisscanids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisscanids0")
    load_release = cuahsihisscanids0_naburelease(harvest)
    load_uploadrelease = cuahsihisscanids0_uploadrelease(load_release)

    load_prune = cuahsihisscanids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisscanids0_nabuprov(load_prune)
    load_org = cuahsihisscanids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisscanids0_missingreport_graph(load_org)
    report_graph=cuahsihisscanids0_graph_reports(report_msgraph)
