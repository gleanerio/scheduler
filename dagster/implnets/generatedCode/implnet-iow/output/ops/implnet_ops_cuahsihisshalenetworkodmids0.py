...
@op
def cuahsihisshalenetworkodmids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisshalenetworkodmids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisshalenetworkodmids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisshalenetworkodmids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisshalenetworkodmids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisshalenetworkodmids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisshalenetworkodmids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisshalenetworkodmids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisshalenetworkodmids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisshalenetworkodmids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisshalenetworkodmids0")
    ...
    return msg + r
@op
def cuahsihisshalenetworkodmids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisshalenetworkodmids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisshalenetworkodmids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisshalenetworkodmids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisshalenetworkodmids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisshalenetworkodmids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisshalenetworkodmids0"
...
def cuahsihisshalenetworkodmids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisshalenetworkodmids0"
...
def harvest_cuahsihisshalenetworkodmids0():
    harvest = cuahsihisshalenetworkodmids0_gleaner()

    report_ms3 = cuahsihisshalenetworkodmids0_missingreport_s3(harvest)
    report_idstat = cuahsihisshalenetworkodmids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisshalenetworkodmids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisshalenetworkodmids0")
    load_release = cuahsihisshalenetworkodmids0_naburelease(harvest)
    load_uploadrelease = cuahsihisshalenetworkodmids0_uploadrelease(load_release)

    load_prune = cuahsihisshalenetworkodmids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisshalenetworkodmids0_nabuprov(load_prune)
    load_org = cuahsihisshalenetworkodmids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisshalenetworkodmids0_missingreport_graph(load_org)
    report_graph=cuahsihisshalenetworkodmids0_graph_reports(report_msgraph)
