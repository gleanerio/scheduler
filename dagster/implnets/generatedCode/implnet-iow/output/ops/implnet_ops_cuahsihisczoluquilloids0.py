...
@op
def cuahsihisczoluquilloids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisczoluquilloids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisczoluquilloids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisczoluquilloids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisczoluquilloids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisczoluquilloids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisczoluquilloids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisczoluquilloids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisczoluquilloids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisczoluquilloids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisczoluquilloids0")
    ...
    return msg + r
@op
def cuahsihisczoluquilloids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisczoluquilloids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisczoluquilloids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisczoluquilloids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisczoluquilloids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisczoluquilloids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisczoluquilloids0"
...
def cuahsihisczoluquilloids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisczoluquilloids0"
...
def harvest_cuahsihisczoluquilloids0():
    harvest = cuahsihisczoluquilloids0_gleaner()

    report_ms3 = cuahsihisczoluquilloids0_missingreport_s3(harvest)
    report_idstat = cuahsihisczoluquilloids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisczoluquilloids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisczoluquilloids0")
    load_release = cuahsihisczoluquilloids0_naburelease(harvest)
    load_uploadrelease = cuahsihisczoluquilloids0_uploadrelease(load_release)

    load_prune = cuahsihisczoluquilloids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisczoluquilloids0_nabuprov(load_prune)
    load_org = cuahsihisczoluquilloids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisczoluquilloids0_missingreport_graph(load_org)
    report_graph=cuahsihisczoluquilloids0_graph_reports(report_msgraph)
