...
@op(ins={"start": In(Nothing)})
def wade42_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade42")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade42_nabuorg(context):
...
def wade42_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade42")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade42")
    ...
    load_prov = wade42_nabuprov(start=load_prune)
    load_org = wade42_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade42_missingreport_graph(start=load_org)
