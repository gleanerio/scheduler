...
def wade42_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "wade42")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade42_naburelease(context):
    returned_value = gleanerio(context, ("release"), "wade42")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade42_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wade42")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade42_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "wade42")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade42_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wade42")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade42_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wade42")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade42_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wade42")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade42_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wade42")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade42_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wade42")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade42_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "wade42")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wade42_uploadrelease(context):
#     returned_value = post_to_graph("wade42", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wade42_uploadrelease(context):
#     returned_value = postRelease("wade42")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def wade42_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade42"
    )
...
@op(ins={"start": In(Nothing)})
def wade42_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade42"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def wade42_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade42"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def wade42_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade42"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def wade42_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wade42_upload_summarize(context):
...
def harvest_wade42():
    ...
    containers = wade42_getImage()

    # conduct the harvest
    harvest = wade42_gleaner(start=containers)

    # data branch
    load_release = wade42_naburelease(start=harvest)
    load_uploadrelease = wade42_uploadrelease(start=load_release)
    load_prune = wade42_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = wade42_nabu_provrelease(start=harvest)
    prov_clear = wade42_nabu_provclear(start=prov_release)
    prov_object = wade42_nabu_provobject(start=prov_clear)
    prov_drain = wade42_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wade42_nabu_orgsrelease(start=harvest)
    load_org = wade42_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = wade42_missingreport_s3(start=harvest)
    # report_idstat = wade42_identifier_stats(start=report_ms3)
    # report_bucketurl = wade42_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = wade42_summarize(start=load_uploadrelease)
    # upload_summarize = wade42_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wade42_missingreport_graph(start=summarize)
    # report_graph = wade42_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="wade42"):
#
