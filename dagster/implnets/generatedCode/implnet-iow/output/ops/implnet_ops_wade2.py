...
@op
def wade2_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "wade2")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade2_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "wade2")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade2_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "wade2")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade2_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "wade2")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def wade2_uploadrelease(context, msg: str):
...
@op
def wade2_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade2")
    ...
    return msg + r
@op
def wade2_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade2")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def wade2_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade2")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def wade2_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade2")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade2"
...
def wade2_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade2"
...
@graph
def harvest_wade2():
    harvest = wade2_gleaner()

    report_ms3 = wade2_missingreport_s3(harvest)
    report_idstat = wade2_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = wade2_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="wade2")
    load_release = wade2_naburelease(harvest)
    load_uploadrelease = wade2_uploadrelease(load_release)

    load_prune = wade2_nabu_prune(load_uploadrelease)
    load_prov = wade2_nabuprov(load_prune)
    load_org = wade2_nabuorg(load_prov)

# run after load
    report_msgraph=wade2_missingreport_graph(load_org)
    report_graph=wade2_graph_reports(report_msgraph)
