...
@op
def cuahsihisfrcwqmids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisfrcwqmids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisfrcwqmids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisfrcwqmids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisfrcwqmids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisfrcwqmids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisfrcwqmids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisfrcwqmids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisfrcwqmids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisfrcwqmids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisfrcwqmids0")
    ...
    return msg + r
@op
def cuahsihisfrcwqmids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisfrcwqmids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisfrcwqmids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisfrcwqmids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisfrcwqmids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisfrcwqmids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisfrcwqmids0"
...
def cuahsihisfrcwqmids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisfrcwqmids0"
...
def harvest_cuahsihisfrcwqmids0():
    harvest = cuahsihisfrcwqmids0_gleaner()

    report_ms3 = cuahsihisfrcwqmids0_missingreport_s3(harvest)
    report_idstat = cuahsihisfrcwqmids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisfrcwqmids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisfrcwqmids0")
    load_release = cuahsihisfrcwqmids0_naburelease(harvest)
    load_uploadrelease = cuahsihisfrcwqmids0_uploadrelease(load_release)

    load_prune = cuahsihisfrcwqmids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisfrcwqmids0_nabuprov(load_prune)
    load_org = cuahsihisfrcwqmids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisfrcwqmids0_missingreport_graph(load_org)
    report_graph=cuahsihisfrcwqmids0_graph_reports(report_msgraph)
