...
# #########################  Ops Section
@op
def cuahsicuahsihisnevadosids0_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def cuahsicuahsihisnevadosids0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "cuahsicuahsihisnevadosids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisnevadosids0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "cuahsicuahsihisnevadosids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisnevadosids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsicuahsihisnevadosids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisnevadosids0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "cuahsicuahsihisnevadosids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisnevadosids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsicuahsihisnevadosids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisnevadosids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsicuahsihisnevadosids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisnevadosids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsicuahsihisnevadosids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisnevadosids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsicuahsihisnevadosids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisnevadosids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsicuahsihisnevadosids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisnevadosids0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "cuahsicuahsihisnevadosids0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihisnevadosids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsicuahsihisnevadosids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihisnevadosids0_uploadrelease(context):
#     returned_value = postRelease("cuahsicuahsihisnevadosids0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisnevadosids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisnevadosids0"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsicuahsihisnevadosids0"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisnevadosids0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisnevadosids0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisnevadosids0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisnevadosids0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisnevadosids0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisnevadosids0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisnevadosids0_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisnevadosids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisnevadosids0_upload_summarize(context):
    returned_value = post_to_graph(
        "cuahsicuahsihisnevadosids0",
        path=SUMMARY_PATH,
        extension="ttl",
...
@graph
def harvest_cuahsicuahsihisnevadosids0():
    # check for containers
    containers = cuahsicuahsihisnevadosids0_getImage()

    # conduct the harvest
    harvest = cuahsicuahsihisnevadosids0_gleaner(start=containers)

    # data branch
    load_release = cuahsicuahsihisnevadosids0_naburelease(start=harvest)
    load_uploadrelease = cuahsicuahsihisnevadosids0_uploadrelease(start=load_release)
    load_prune = cuahsicuahsihisnevadosids0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = cuahsicuahsihisnevadosids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsicuahsihisnevadosids0_nabu_provclear(start=prov_release)
    prov_object = cuahsicuahsihisnevadosids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsicuahsihisnevadosids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsicuahsihisnevadosids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsicuahsihisnevadosids0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = cuahsicuahsihisnevadosids0_missingreport_s3(start=harvest)
    # report_idstat = cuahsicuahsihisnevadosids0_identifier_stats(start=report_ms3)
    # report_bucketurl = cuahsicuahsihisnevadosids0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = cuahsicuahsihisnevadosids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsicuahsihisnevadosids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsicuahsihisnevadosids0_missingreport_graph(start=summarize)
    # report_graph = cuahsicuahsihisnevadosids0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="cuahsicuahsihisnevadosids0"):
#
