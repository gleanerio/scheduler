...
@op
def sechydrgreg0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "sechydrgreg0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def sechydrgreg0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "sechydrgreg0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def sechydrgreg0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "sechydrgreg0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def sechydrgreg0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "sechydrgreg0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def sechydrgreg0_uploadrelease(context, msg: str):
...
@op
def sechydrgreg0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="sechydrgreg0")
    ...
    return msg + r
@op
def sechydrgreg0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="sechydrgreg0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def sechydrgreg0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="sechydrgreg0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def sechydrgreg0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="sechydrgreg0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "sechydrgreg0"
...
def sechydrgreg0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "sechydrgreg0"
...
def harvest_sechydrgreg0():
    harvest = sechydrgreg0_gleaner()

    report_ms3 = sechydrgreg0_missingreport_s3(harvest)
    report_idstat = sechydrgreg0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = sechydrgreg0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="sechydrgreg0")
    load_release = sechydrgreg0_naburelease(harvest)
    load_uploadrelease = sechydrgreg0_uploadrelease(load_release)

    load_prune = sechydrgreg0_nabu_prune(load_uploadrelease)
    load_prov = sechydrgreg0_nabuprov(load_prune)
    load_org = sechydrgreg0_nabuorg(load_prov)

# run after load
    report_msgraph=sechydrgreg0_missingreport_graph(load_org)
    report_graph=sechydrgreg0_graph_reports(report_msgraph)
