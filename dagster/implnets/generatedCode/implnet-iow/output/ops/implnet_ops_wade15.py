...
def wade15_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wade15")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def wade15_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wade15")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade15_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wade15")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade15_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wade15")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade15_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wade15")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade15_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wade15")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade15_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def wade15_uploadrelease(context):
#     returned_value = post_to_graph("wade15", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def wade15_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade15"
    )
...
@op(ins={"start": In(Nothing)})
def wade15_upload_summarize(context):
...
def harvest_wade15():
    ...
    # prov branch
    prov_release = wade15_nabu_provrelease(start=harvest)
    prov_clear = wade15_nabu_provclear(start=prov_release)
    prov_object = wade15_nabu_provobject(start=prov_clear)
    prov_drain = wade15_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wade15_nabu_orgsrelease(start=harvest)
    load_org = wade15_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = wade15_summarize(start=load_uploadrelease)
    # upload_summarize = wade15_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wade15_missingreport_graph(start=summarize)
