...
@op
def wade15_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "wade15")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade15_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "wade15")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade15_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "wade15")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade15_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "wade15")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def wade15_uploadrelease(context, msg: str):
...
@op
def wade15_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade15")
    ...
    return msg + r
@op
def wade15_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade15")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def wade15_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade15")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def wade15_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade15")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade15"
...
def wade15_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade15"
...
@graph
def harvest_wade15():
    harvest = wade15_gleaner()

    report_ms3 = wade15_missingreport_s3(harvest)
    report_idstat = wade15_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = wade15_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="wade15")
    load_release = wade15_naburelease(harvest)
    load_uploadrelease = wade15_uploadrelease(load_release)

    load_prune = wade15_nabu_prune(load_uploadrelease)
    load_prov = wade15_nabuprov(load_prune)
    load_org = wade15_nabuorg(load_prov)

# run after load
    report_msgraph=wade15_missingreport_graph(load_org)
    report_graph=wade15_graph_reports(report_msgraph)
