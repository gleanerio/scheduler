...
@op
def cuahsihistrwaids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihistrwaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihistrwaids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihistrwaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihistrwaids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihistrwaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihistrwaids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihistrwaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihistrwaids0_uploadrelease(context, msg: str):
...
@op
def cuahsihistrwaids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihistrwaids0")
    ...
    return msg + r
@op
def cuahsihistrwaids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihistrwaids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihistrwaids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihistrwaids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihistrwaids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihistrwaids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihistrwaids0"
...
def cuahsihistrwaids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihistrwaids0"
...
def harvest_cuahsihistrwaids0():
    harvest = cuahsihistrwaids0_gleaner()

    report_ms3 = cuahsihistrwaids0_missingreport_s3(harvest)
    report_idstat = cuahsihistrwaids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihistrwaids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihistrwaids0")
    load_release = cuahsihistrwaids0_naburelease(harvest)
    load_uploadrelease = cuahsihistrwaids0_uploadrelease(load_release)

    load_prune = cuahsihistrwaids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihistrwaids0_nabuprov(load_prune)
    load_org = cuahsihistrwaids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihistrwaids0_missingreport_graph(load_org)
    report_graph=cuahsihistrwaids0_graph_reports(report_msgraph)
