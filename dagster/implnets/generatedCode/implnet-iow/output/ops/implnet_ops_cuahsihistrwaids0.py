...
def cuahsihistrwaids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsihistrwaids0")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def cuahsihistrwaids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsihistrwaids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihistrwaids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsihistrwaids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihistrwaids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsihistrwaids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihistrwaids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsihistrwaids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihistrwaids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsihistrwaids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihistrwaids0_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def cuahsihistrwaids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsihistrwaids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def cuahsihistrwaids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsihistrwaids0"
    )
...
@op(ins={"start": In(Nothing)})
def cuahsihistrwaids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsihistrwaids0_upload_summarize(context):
...
def harvest_cuahsihistrwaids0():
    ...
    # prov branch
    prov_release = cuahsihistrwaids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsihistrwaids0_nabu_provclear(start=prov_release)
    prov_object = cuahsihistrwaids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsihistrwaids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsihistrwaids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsihistrwaids0_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = cuahsihistrwaids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsihistrwaids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsihistrwaids0_missingreport_graph(start=summarize)
