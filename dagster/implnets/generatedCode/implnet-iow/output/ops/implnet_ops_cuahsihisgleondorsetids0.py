...
@op
def cuahsihisgleondorsetids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisgleondorsetids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisgleondorsetids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisgleondorsetids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisgleondorsetids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisgleondorsetids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisgleondorsetids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisgleondorsetids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisgleondorsetids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisgleondorsetids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisgleondorsetids0")
    ...
    return msg + r
@op
def cuahsihisgleondorsetids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisgleondorsetids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisgleondorsetids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisgleondorsetids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisgleondorsetids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisgleondorsetids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisgleondorsetids0"
...
def cuahsihisgleondorsetids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisgleondorsetids0"
...
def harvest_cuahsihisgleondorsetids0():
    harvest = cuahsihisgleondorsetids0_gleaner()

    report_ms3 = cuahsihisgleondorsetids0_missingreport_s3(harvest)
    report_idstat = cuahsihisgleondorsetids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisgleondorsetids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisgleondorsetids0")
    load_release = cuahsihisgleondorsetids0_naburelease(harvest)
    load_uploadrelease = cuahsihisgleondorsetids0_uploadrelease(load_release)

    load_prune = cuahsihisgleondorsetids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisgleondorsetids0_nabuprov(load_prune)
    load_org = cuahsihisgleondorsetids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisgleondorsetids0_missingreport_graph(load_org)
    report_graph=cuahsihisgleondorsetids0_graph_reports(report_msgraph)
