...
def cuahsihisclarksburgspids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsihisclarksburgspids0")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def cuahsihisclarksburgspids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsihisclarksburgspids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisclarksburgspids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsihisclarksburgspids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisclarksburgspids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsihisclarksburgspids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisclarksburgspids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsihisclarksburgspids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisclarksburgspids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsihisclarksburgspids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisclarksburgspids0_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def cuahsihisclarksburgspids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsihisclarksburgspids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def cuahsihisclarksburgspids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsihisclarksburgspids0"
    )
...
@op(ins={"start": In(Nothing)})
def cuahsihisclarksburgspids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsihisclarksburgspids0_upload_summarize(context):
    returned_value = post_to_graph(
        "cuahsihisclarksburgspids0",
        path=SUMMARY_PATH,
        extension="ttl",
...
def harvest_cuahsihisclarksburgspids0():
    ...
    # prov branch
    prov_release = cuahsihisclarksburgspids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsihisclarksburgspids0_nabu_provclear(start=prov_release)
    prov_object = cuahsihisclarksburgspids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsihisclarksburgspids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsihisclarksburgspids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsihisclarksburgspids0_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = cuahsihisclarksburgspids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsihisclarksburgspids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsihisclarksburgspids0_missingreport_graph(start=summarize)
