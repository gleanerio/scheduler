...
@op
def cuahsihisclarksburgspids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisclarksburgspids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisclarksburgspids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisclarksburgspids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisclarksburgspids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisclarksburgspids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisclarksburgspids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisclarksburgspids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisclarksburgspids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisclarksburgspids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisclarksburgspids0")
    ...
    return msg + r
@op
def cuahsihisclarksburgspids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisclarksburgspids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisclarksburgspids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisclarksburgspids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisclarksburgspids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisclarksburgspids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisclarksburgspids0"
...
def cuahsihisclarksburgspids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisclarksburgspids0"
...
def harvest_cuahsihisclarksburgspids0():
    harvest = cuahsihisclarksburgspids0_gleaner()

    report_ms3 = cuahsihisclarksburgspids0_missingreport_s3(harvest)
    report_idstat = cuahsihisclarksburgspids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisclarksburgspids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisclarksburgspids0")
    load_release = cuahsihisclarksburgspids0_naburelease(harvest)
    load_uploadrelease = cuahsihisclarksburgspids0_uploadrelease(load_release)

    load_prune = cuahsihisclarksburgspids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisclarksburgspids0_nabuprov(load_prune)
    load_org = cuahsihisclarksburgspids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisclarksburgspids0_missingreport_graph(load_org)
    report_graph=cuahsihisclarksburgspids0_graph_reports(report_msgraph)
