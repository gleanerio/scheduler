...
def nwisgw27_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "nwisgw27")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def nwisgw27_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "nwisgw27")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw27_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "nwisgw27")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw27_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "nwisgw27")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw27_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "nwisgw27")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw27_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "nwisgw27")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw27_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def nwisgw27_uploadrelease(context):
#     returned_value = post_to_graph("nwisgw27", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def nwisgw27_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nwisgw27"
    )
...
@op(ins={"start": In(Nothing)})
def nwisgw27_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def nwisgw27_upload_summarize(context):
...
def harvest_nwisgw27():
    ...
    # prov branch
    prov_release = nwisgw27_nabu_provrelease(start=harvest)
    prov_clear = nwisgw27_nabu_provclear(start=prov_release)
    prov_object = nwisgw27_nabu_provobject(start=prov_clear)
    prov_drain = nwisgw27_nabu_provdrain(start=prov_object)

    # org branch
    org_release = nwisgw27_nabu_orgsrelease(start=harvest)
    load_org = nwisgw27_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = nwisgw27_summarize(start=load_uploadrelease)
    # upload_summarize = nwisgw27_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = nwisgw27_missingreport_graph(start=summarize)
