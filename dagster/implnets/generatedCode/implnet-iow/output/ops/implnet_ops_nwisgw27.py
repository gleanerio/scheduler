...
@op
def nwisgw27_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw27")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw27_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw27")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw27_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw27")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw27_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw27")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw27_uploadrelease(context, msg: str):
...
@op
def nwisgw27_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw27")
    ...
    return msg + r
@op
def nwisgw27_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw27")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw27_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw27")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw27_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw27")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw27"
...
def nwisgw27_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw27"
...
def harvest_nwisgw27():
    harvest = nwisgw27_gleaner()

    report_ms3 = nwisgw27_missingreport_s3(harvest)
    report_idstat = nwisgw27_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw27_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw27")
    load_release = nwisgw27_naburelease(harvest)
    load_uploadrelease = nwisgw27_uploadrelease(load_release)

    load_prune = nwisgw27_nabu_prune(load_uploadrelease)
    load_prov = nwisgw27_nabuprov(load_prune)
    load_org = nwisgw27_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw27_missingreport_graph(load_org)
    report_graph=nwisgw27_graph_reports(report_msgraph)
