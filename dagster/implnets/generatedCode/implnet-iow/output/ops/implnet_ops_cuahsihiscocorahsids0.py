...
@op
def cuahsihiscocorahsids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihiscocorahsids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihiscocorahsids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihiscocorahsids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihiscocorahsids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihiscocorahsids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihiscocorahsids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihiscocorahsids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihiscocorahsids0_uploadrelease(context, msg: str):
...
@op
def cuahsihiscocorahsids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihiscocorahsids0")
    ...
    return msg + r
@op
def cuahsihiscocorahsids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihiscocorahsids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihiscocorahsids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihiscocorahsids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihiscocorahsids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihiscocorahsids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihiscocorahsids0"
...
def cuahsihiscocorahsids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihiscocorahsids0"
...
def harvest_cuahsihiscocorahsids0():
    harvest = cuahsihiscocorahsids0_gleaner()

    report_ms3 = cuahsihiscocorahsids0_missingreport_s3(harvest)
    report_idstat = cuahsihiscocorahsids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihiscocorahsids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihiscocorahsids0")
    load_release = cuahsihiscocorahsids0_naburelease(harvest)
    load_uploadrelease = cuahsihiscocorahsids0_uploadrelease(load_release)

    load_prune = cuahsihiscocorahsids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihiscocorahsids0_nabuprov(load_prune)
    load_org = cuahsihiscocorahsids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihiscocorahsids0_missingreport_graph(load_org)
    report_graph=cuahsihiscocorahsids0_graph_reports(report_msgraph)
