...
@op
def cuahsihishydrodataczdids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihishydrodataczdids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihishydrodataczdids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihishydrodataczdids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihishydrodataczdids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihishydrodataczdids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihishydrodataczdids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihishydrodataczdids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihishydrodataczdids0_uploadrelease(context, msg: str):
...
@op
def cuahsihishydrodataczdids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihishydrodataczdids0")
    ...
    return msg + r
@op
def cuahsihishydrodataczdids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihishydrodataczdids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihishydrodataczdids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihishydrodataczdids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihishydrodataczdids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihishydrodataczdids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihishydrodataczdids0"
...
def cuahsihishydrodataczdids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihishydrodataczdids0"
...
def harvest_cuahsihishydrodataczdids0():
    harvest = cuahsihishydrodataczdids0_gleaner()

    report_ms3 = cuahsihishydrodataczdids0_missingreport_s3(harvest)
    report_idstat = cuahsihishydrodataczdids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihishydrodataczdids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihishydrodataczdids0")
    load_release = cuahsihishydrodataczdids0_naburelease(harvest)
    load_uploadrelease = cuahsihishydrodataczdids0_uploadrelease(load_release)

    load_prune = cuahsihishydrodataczdids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihishydrodataczdids0_nabuprov(load_prune)
    load_org = cuahsihishydrodataczdids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihishydrodataczdids0_missingreport_graph(load_org)
    report_graph=cuahsihishydrodataczdids0_graph_reports(report_msgraph)
