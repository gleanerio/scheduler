...
# #########################  Ops Section
@op
def cuahsicuahsihisczocatalinaids0_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def cuahsicuahsihisczocatalinaids0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "cuahsicuahsihisczocatalinaids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczocatalinaids0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "cuahsicuahsihisczocatalinaids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczocatalinaids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsicuahsihisczocatalinaids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczocatalinaids0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "cuahsicuahsihisczocatalinaids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczocatalinaids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsicuahsihisczocatalinaids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczocatalinaids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsicuahsihisczocatalinaids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczocatalinaids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsicuahsihisczocatalinaids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczocatalinaids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsicuahsihisczocatalinaids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczocatalinaids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsicuahsihisczocatalinaids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczocatalinaids0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "cuahsicuahsihisczocatalinaids0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihisczocatalinaids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsicuahsihisczocatalinaids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihisczocatalinaids0_uploadrelease(context):
#     returned_value = postRelease("cuahsicuahsihisczocatalinaids0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczocatalinaids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisczocatalinaids0"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsicuahsihisczocatalinaids0"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczocatalinaids0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisczocatalinaids0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczocatalinaids0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisczocatalinaids0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczocatalinaids0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisczocatalinaids0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczocatalinaids0_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczocatalinaids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczocatalinaids0_upload_summarize(context):
    returned_value = post_to_graph(
        "cuahsicuahsihisczocatalinaids0",
        path=SUMMARY_PATH,
        extension="ttl",
...
@graph
def harvest_cuahsicuahsihisczocatalinaids0():
    # check for containers
    containers = cuahsicuahsihisczocatalinaids0_getImage()

    # conduct the harvest
    harvest = cuahsicuahsihisczocatalinaids0_gleaner(start=containers)

    # data branch
    load_release = cuahsicuahsihisczocatalinaids0_naburelease(start=harvest)
    load_uploadrelease = cuahsicuahsihisczocatalinaids0_uploadrelease(start=load_release)
    load_prune = cuahsicuahsihisczocatalinaids0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = cuahsicuahsihisczocatalinaids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsicuahsihisczocatalinaids0_nabu_provclear(start=prov_release)
    prov_object = cuahsicuahsihisczocatalinaids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsicuahsihisczocatalinaids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsicuahsihisczocatalinaids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsicuahsihisczocatalinaids0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = cuahsicuahsihisczocatalinaids0_missingreport_s3(start=harvest)
    # report_idstat = cuahsicuahsihisczocatalinaids0_identifier_stats(start=report_ms3)
    # report_bucketurl = cuahsicuahsihisczocatalinaids0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = cuahsicuahsihisczocatalinaids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsicuahsihisczocatalinaids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsicuahsihisczocatalinaids0_missingreport_graph(start=summarize)
    # report_graph = cuahsicuahsihisczocatalinaids0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="cuahsicuahsihisczocatalinaids0"):
#
