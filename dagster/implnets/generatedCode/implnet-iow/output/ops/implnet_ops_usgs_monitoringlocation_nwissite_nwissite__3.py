...
# #########################  Ops Section
@op
def usgs_monitoringlocation_nwissite_nwissite__3_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def usgs_monitoringlocation_nwissite_nwissite__3_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "usgs_monitoringlocation_nwissite_nwissite__3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgs_monitoringlocation_nwissite_nwissite__3_naburelease(context):
    returned_value = gleanerio(context, ("release"), "usgs_monitoringlocation_nwissite_nwissite__3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgs_monitoringlocation_nwissite_nwissite__3_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "usgs_monitoringlocation_nwissite_nwissite__3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgs_monitoringlocation_nwissite_nwissite__3_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "usgs_monitoringlocation_nwissite_nwissite__3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgs_monitoringlocation_nwissite_nwissite__3_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "usgs_monitoringlocation_nwissite_nwissite__3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgs_monitoringlocation_nwissite_nwissite__3_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "usgs_monitoringlocation_nwissite_nwissite__3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgs_monitoringlocation_nwissite_nwissite__3_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "usgs_monitoringlocation_nwissite_nwissite__3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgs_monitoringlocation_nwissite_nwissite__3_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "usgs_monitoringlocation_nwissite_nwissite__3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgs_monitoringlocation_nwissite_nwissite__3_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "usgs_monitoringlocation_nwissite_nwissite__3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgs_monitoringlocation_nwissite_nwissite__3_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "usgs_monitoringlocation_nwissite_nwissite__3")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def usgs_monitoringlocation_nwissite_nwissite__3_uploadrelease(context):
#     returned_value = post_to_graph("usgs_monitoringlocation_nwissite_nwissite__3", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def usgs_monitoringlocation_nwissite_nwissite__3_uploadrelease(context):
#     returned_value = postRelease("usgs_monitoringlocation_nwissite_nwissite__3")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def usgs_monitoringlocation_nwissite_nwissite__3_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="usgs_monitoringlocation_nwissite_nwissite__3"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "usgs_monitoringlocation_nwissite_nwissite__3"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def usgs_monitoringlocation_nwissite_nwissite__3_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="usgs_monitoringlocation_nwissite_nwissite__3"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def usgs_monitoringlocation_nwissite_nwissite__3_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="usgs_monitoringlocation_nwissite_nwissite__3"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def usgs_monitoringlocation_nwissite_nwissite__3_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="usgs_monitoringlocation_nwissite_nwissite__3"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def usgs_monitoringlocation_nwissite_nwissite__3_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def usgs_monitoringlocation_nwissite_nwissite__3_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def usgs_monitoringlocation_nwissite_nwissite__3_upload_summarize(context):
    returned_value = post_to_graph(
        "usgs_monitoringlocation_nwissite_nwissite__3",
        path=SUMMARY_PATH,
        extension="ttl",
...
@graph
def harvest_usgs_monitoringlocation_nwissite_nwissite__3():
    # check for containers
    containers = usgs_monitoringlocation_nwissite_nwissite__3_getImage()

    # conduct the harvest
    harvest = usgs_monitoringlocation_nwissite_nwissite__3_gleaner(start=containers)

    # data branch
    load_release = usgs_monitoringlocation_nwissite_nwissite__3_naburelease(start=harvest)
    load_uploadrelease = usgs_monitoringlocation_nwissite_nwissite__3_uploadrelease(start=load_release)
    load_prune = usgs_monitoringlocation_nwissite_nwissite__3_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = usgs_monitoringlocation_nwissite_nwissite__3_nabu_provrelease(start=harvest)
    prov_clear = usgs_monitoringlocation_nwissite_nwissite__3_nabu_provclear(start=prov_release)
    prov_object = usgs_monitoringlocation_nwissite_nwissite__3_nabu_provobject(start=prov_clear)
    prov_drain = usgs_monitoringlocation_nwissite_nwissite__3_nabu_provdrain(start=prov_object)

    # org branch
    org_release = usgs_monitoringlocation_nwissite_nwissite__3_nabu_orgsrelease(start=harvest)
    load_org = usgs_monitoringlocation_nwissite_nwissite__3_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = usgs_monitoringlocation_nwissite_nwissite__3_missingreport_s3(start=harvest)
    # report_idstat = usgs_monitoringlocation_nwissite_nwissite__3_identifier_stats(start=report_ms3)
    # report_bucketurl = usgs_monitoringlocation_nwissite_nwissite__3_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = usgs_monitoringlocation_nwissite_nwissite__3_summarize(start=load_uploadrelease)
    # upload_summarize = usgs_monitoringlocation_nwissite_nwissite__3_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = usgs_monitoringlocation_nwissite_nwissite__3_missingreport_graph(start=summarize)
    # report_graph = usgs_monitoringlocation_nwissite_nwissite__3_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="usgs_monitoringlocation_nwissite_nwissite__3"):
#
