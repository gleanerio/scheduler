...
@op(ins={"start": In(Nothing)})
def wade47_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade47")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade47_nabuorg(context):
...
def wade47_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade47")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade47")
    ...
    load_prov = wade47_nabuprov(start=load_prune)
    load_org = wade47_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade47_missingreport_graph(start=load_org)
