...
@op(ins={"start": In(Nothing)})
def wade44_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade44")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade44_nabuorg(context):
...
def wade44_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade44")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade44")
    ...
    load_prov = wade44_nabuprov(start=load_prune)
    load_org = wade44_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade44_missingreport_graph(start=load_org)
