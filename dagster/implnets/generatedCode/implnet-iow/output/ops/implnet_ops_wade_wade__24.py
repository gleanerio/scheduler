...
# #########################  Ops Section
@op
def wade_wade__24_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def wade_wade__24_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "wade_wade__24")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade_wade__24_naburelease(context):
    returned_value = gleanerio(context, ("release"), "wade_wade__24")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade_wade__24_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wade_wade__24")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade_wade__24_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "wade_wade__24")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade_wade__24_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wade_wade__24")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade_wade__24_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wade_wade__24")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade_wade__24_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wade_wade__24")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade_wade__24_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wade_wade__24")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade_wade__24_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wade_wade__24")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade_wade__24_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "wade_wade__24")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wade_wade__24_uploadrelease(context):
#     returned_value = post_to_graph("wade_wade__24", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wade_wade__24_uploadrelease(context):
#     returned_value = postRelease("wade_wade__24")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def wade_wade__24_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade_wade__24"
    )
...
@op(ins={"start": In(Nothing)})
def wade_wade__24_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade_wade__24"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def wade_wade__24_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade_wade__24"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def wade_wade__24_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade_wade__24"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def wade_wade__24_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wade_wade__24_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wade_wade__24_upload_summarize(context):
...
@graph
def harvest_wade_wade__24():
    # check for containers
    containers = wade_wade__24_getImage()

    # conduct the harvest
    harvest = wade_wade__24_gleaner(start=containers)

    # data branch
    load_release = wade_wade__24_naburelease(start=harvest)
    load_uploadrelease = wade_wade__24_uploadrelease(start=load_release)
    load_prune = wade_wade__24_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = wade_wade__24_nabu_provrelease(start=harvest)
    prov_clear = wade_wade__24_nabu_provclear(start=prov_release)
    prov_object = wade_wade__24_nabu_provobject(start=prov_clear)
    prov_drain = wade_wade__24_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wade_wade__24_nabu_orgsrelease(start=harvest)
    load_org = wade_wade__24_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = wade_wade__24_missingreport_s3(start=harvest)
    # report_idstat = wade_wade__24_identifier_stats(start=report_ms3)
    # report_bucketurl = wade_wade__24_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = wade_wade__24_summarize(start=load_uploadrelease)
    # upload_summarize = wade_wade__24_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wade_wade__24_missingreport_graph(start=summarize)
    # report_graph = wade_wade__24_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="wade_wade__24"):
#
