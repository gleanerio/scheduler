...
@op
def cuahsihisccbepdapids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisccbepdapids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisccbepdapids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisccbepdapids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisccbepdapids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisccbepdapids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisccbepdapids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisccbepdapids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisccbepdapids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisccbepdapids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisccbepdapids0")
    ...
    return msg + r
@op
def cuahsihisccbepdapids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisccbepdapids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisccbepdapids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisccbepdapids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisccbepdapids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisccbepdapids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisccbepdapids0"
...
def cuahsihisccbepdapids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisccbepdapids0"
...
def harvest_cuahsihisccbepdapids0():
    harvest = cuahsihisccbepdapids0_gleaner()

    report_ms3 = cuahsihisccbepdapids0_missingreport_s3(harvest)
    report_idstat = cuahsihisccbepdapids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisccbepdapids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisccbepdapids0")
    load_release = cuahsihisccbepdapids0_naburelease(harvest)
    load_uploadrelease = cuahsihisccbepdapids0_uploadrelease(load_release)

    load_prune = cuahsihisccbepdapids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisccbepdapids0_nabuprov(load_prune)
    load_org = cuahsihisccbepdapids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisccbepdapids0_missingreport_graph(load_org)
    report_graph=cuahsihisccbepdapids0_graph_reports(report_msgraph)
