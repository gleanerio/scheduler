...
def hu100_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "hu100")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def hu100_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "hu100")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def hu100_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "hu100")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def hu100_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "hu100")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def hu100_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "hu100")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def hu100_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "hu100")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def hu100_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def hu100_uploadrelease(context):
#     returned_value = post_to_graph("hu100", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def hu100_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="hu100"
    )
...
@op(ins={"start": In(Nothing)})
def hu100_upload_summarize(context):
...
def harvest_hu100():
    ...
    # prov branch
    prov_release = hu100_nabu_provrelease(start=harvest)
    prov_clear = hu100_nabu_provclear(start=prov_release)
    prov_object = hu100_nabu_provobject(start=prov_clear)
    prov_drain = hu100_nabu_provdrain(start=prov_object)

    # org branch
    org_release = hu100_nabu_orgsrelease(start=harvest)
    load_org = hu100_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = hu100_summarize(start=load_uploadrelease)
    # upload_summarize = hu100_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = hu100_missingreport_graph(start=summarize)
