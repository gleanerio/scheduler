...
@op
def hu100_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "hu100")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def hu100_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "hu100")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def hu100_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "hu100")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def hu100_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "hu100")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def hu100_uploadrelease(context, msg: str):
...
@op
def hu100_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hu100")
    ...
    return msg + r
@op
def hu100_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hu100")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def hu100_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hu100")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def hu100_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hu100")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "hu100"
...
def hu100_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "hu100"
...
@graph
def harvest_hu100():
    harvest = hu100_gleaner()

    report_ms3 = hu100_missingreport_s3(harvest)
    report_idstat = hu100_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = hu100_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="hu100")
    load_release = hu100_naburelease(harvest)
    load_uploadrelease = hu100_uploadrelease(load_release)

    load_prune = hu100_nabu_prune(load_uploadrelease)
    load_prov = hu100_nabuprov(load_prune)
    load_org = hu100_nabuorg(load_prov)

# run after load
    report_msgraph=hu100_missingreport_graph(load_org)
    report_graph=hu100_graph_reports(report_msgraph)
