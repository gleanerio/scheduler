...
def wade39_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "wade39")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade39_naburelease(context):
    returned_value = gleanerio(context, ("release"), "wade39")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade39_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wade39")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade39_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "wade39")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade39_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wade39")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade39_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wade39")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade39_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wade39")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade39_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wade39")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade39_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wade39")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade39_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "wade39")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wade39_uploadrelease(context):
#     returned_value = post_to_graph("wade39", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wade39_uploadrelease(context):
#     returned_value = postRelease("wade39")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def wade39_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade39"
    )
...
@op(ins={"start": In(Nothing)})
def wade39_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade39"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def wade39_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade39"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def wade39_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade39"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def wade39_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wade39_upload_summarize(context):
...
def harvest_wade39():
    ...
    containers = wade39_getImage()

    # conduct the harvest
    harvest = wade39_gleaner(start=containers)

    # data branch
    load_release = wade39_naburelease(start=harvest)
    load_uploadrelease = wade39_uploadrelease(start=load_release)
    load_prune = wade39_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = wade39_nabu_provrelease(start=harvest)
    prov_clear = wade39_nabu_provclear(start=prov_release)
    prov_object = wade39_nabu_provobject(start=prov_clear)
    prov_drain = wade39_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wade39_nabu_orgsrelease(start=harvest)
    load_org = wade39_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = wade39_missingreport_s3(start=harvest)
    # report_idstat = wade39_identifier_stats(start=report_ms3)
    # report_bucketurl = wade39_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = wade39_summarize(start=load_uploadrelease)
    # upload_summarize = wade39_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wade39_missingreport_graph(start=summarize)
    # report_graph = wade39_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="wade39"):
#
