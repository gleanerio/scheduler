...
@op(ins={"start": In(Nothing)})
def wade39_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade39")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade39_nabuorg(context):
...
def wade39_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade39")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade39")
    ...
    load_prov = wade39_nabuprov(start=load_prune)
    load_org = wade39_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade39_missingreport_graph(start=load_org)
