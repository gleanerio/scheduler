...
# #########################  Ops Section
@op
def cuahsicuahsihispanolaodmids0_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def cuahsicuahsihispanolaodmids0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "cuahsicuahsihispanolaodmids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihispanolaodmids0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "cuahsicuahsihispanolaodmids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihispanolaodmids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsicuahsihispanolaodmids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihispanolaodmids0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "cuahsicuahsihispanolaodmids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihispanolaodmids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsicuahsihispanolaodmids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihispanolaodmids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsicuahsihispanolaodmids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihispanolaodmids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsicuahsihispanolaodmids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihispanolaodmids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsicuahsihispanolaodmids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihispanolaodmids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsicuahsihispanolaodmids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihispanolaodmids0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "cuahsicuahsihispanolaodmids0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihispanolaodmids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsicuahsihispanolaodmids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihispanolaodmids0_uploadrelease(context):
#     returned_value = postRelease("cuahsicuahsihispanolaodmids0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihispanolaodmids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihispanolaodmids0"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsicuahsihispanolaodmids0"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihispanolaodmids0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihispanolaodmids0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihispanolaodmids0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihispanolaodmids0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihispanolaodmids0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihispanolaodmids0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihispanolaodmids0_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihispanolaodmids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihispanolaodmids0_upload_summarize(context):
    returned_value = post_to_graph(
        "cuahsicuahsihispanolaodmids0",
        path=SUMMARY_PATH,
        extension="ttl",
...
@graph
def harvest_cuahsicuahsihispanolaodmids0():
    # check for containers
    containers = cuahsicuahsihispanolaodmids0_getImage()

    # conduct the harvest
    harvest = cuahsicuahsihispanolaodmids0_gleaner(start=containers)

    # data branch
    load_release = cuahsicuahsihispanolaodmids0_naburelease(start=harvest)
    load_uploadrelease = cuahsicuahsihispanolaodmids0_uploadrelease(start=load_release)
    load_prune = cuahsicuahsihispanolaodmids0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = cuahsicuahsihispanolaodmids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsicuahsihispanolaodmids0_nabu_provclear(start=prov_release)
    prov_object = cuahsicuahsihispanolaodmids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsicuahsihispanolaodmids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsicuahsihispanolaodmids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsicuahsihispanolaodmids0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = cuahsicuahsihispanolaodmids0_missingreport_s3(start=harvest)
    # report_idstat = cuahsicuahsihispanolaodmids0_identifier_stats(start=report_ms3)
    # report_bucketurl = cuahsicuahsihispanolaodmids0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = cuahsicuahsihispanolaodmids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsicuahsihispanolaodmids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsicuahsihispanolaodmids0_missingreport_graph(start=summarize)
    # report_graph = cuahsicuahsihispanolaodmids0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="cuahsicuahsihispanolaodmids0"):
#
