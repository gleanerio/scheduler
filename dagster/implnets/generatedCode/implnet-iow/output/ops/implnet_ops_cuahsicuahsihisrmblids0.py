...
# #########################  Ops Section
@op
def cuahsicuahsihisrmblids0_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def cuahsicuahsihisrmblids0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "cuahsicuahsihisrmblids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisrmblids0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "cuahsicuahsihisrmblids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisrmblids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsicuahsihisrmblids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisrmblids0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "cuahsicuahsihisrmblids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisrmblids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsicuahsihisrmblids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisrmblids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsicuahsihisrmblids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisrmblids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsicuahsihisrmblids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisrmblids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsicuahsihisrmblids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisrmblids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsicuahsihisrmblids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisrmblids0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "cuahsicuahsihisrmblids0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihisrmblids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsicuahsihisrmblids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihisrmblids0_uploadrelease(context):
#     returned_value = postRelease("cuahsicuahsihisrmblids0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisrmblids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisrmblids0"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsicuahsihisrmblids0"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisrmblids0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisrmblids0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisrmblids0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisrmblids0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisrmblids0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisrmblids0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisrmblids0_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisrmblids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisrmblids0_upload_summarize(context):
    returned_value = post_to_graph(
        "cuahsicuahsihisrmblids0",
        path=SUMMARY_PATH,
        extension="ttl",
...
@graph
def harvest_cuahsicuahsihisrmblids0():
    # check for containers
    containers = cuahsicuahsihisrmblids0_getImage()

    # conduct the harvest
    harvest = cuahsicuahsihisrmblids0_gleaner(start=containers)

    # data branch
    load_release = cuahsicuahsihisrmblids0_naburelease(start=harvest)
    load_uploadrelease = cuahsicuahsihisrmblids0_uploadrelease(start=load_release)
    load_prune = cuahsicuahsihisrmblids0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = cuahsicuahsihisrmblids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsicuahsihisrmblids0_nabu_provclear(start=prov_release)
    prov_object = cuahsicuahsihisrmblids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsicuahsihisrmblids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsicuahsihisrmblids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsicuahsihisrmblids0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = cuahsicuahsihisrmblids0_missingreport_s3(start=harvest)
    # report_idstat = cuahsicuahsihisrmblids0_identifier_stats(start=report_ms3)
    # report_bucketurl = cuahsicuahsihisrmblids0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = cuahsicuahsihisrmblids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsicuahsihisrmblids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsicuahsihisrmblids0_missingreport_graph(start=summarize)
    # report_graph = cuahsicuahsihisrmblids0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="cuahsicuahsihisrmblids0"):
#
