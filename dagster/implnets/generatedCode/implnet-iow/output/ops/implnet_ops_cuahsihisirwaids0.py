...
@op
def cuahsihisirwaids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisirwaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisirwaids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisirwaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisirwaids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisirwaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisirwaids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisirwaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisirwaids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisirwaids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisirwaids0")
    ...
    return msg + r
@op
def cuahsihisirwaids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisirwaids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisirwaids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisirwaids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisirwaids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisirwaids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisirwaids0"
...
def cuahsihisirwaids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisirwaids0"
...
def harvest_cuahsihisirwaids0():
    harvest = cuahsihisirwaids0_gleaner()

    report_ms3 = cuahsihisirwaids0_missingreport_s3(harvest)
    report_idstat = cuahsihisirwaids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisirwaids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisirwaids0")
    load_release = cuahsihisirwaids0_naburelease(harvest)
    load_uploadrelease = cuahsihisirwaids0_uploadrelease(load_release)

    load_prune = cuahsihisirwaids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisirwaids0_nabuprov(load_prune)
    load_org = cuahsihisirwaids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisirwaids0_missingreport_graph(load_org)
    report_graph=cuahsihisirwaids0_graph_reports(report_msgraph)
