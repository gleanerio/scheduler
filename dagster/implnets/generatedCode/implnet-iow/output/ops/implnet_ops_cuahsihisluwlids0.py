...
@op
def cuahsihisluwlids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisluwlids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisluwlids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisluwlids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisluwlids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisluwlids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisluwlids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisluwlids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisluwlids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisluwlids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisluwlids0")
    ...
    return msg + r
@op
def cuahsihisluwlids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisluwlids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisluwlids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisluwlids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisluwlids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisluwlids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisluwlids0"
...
def cuahsihisluwlids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisluwlids0"
...
def harvest_cuahsihisluwlids0():
    harvest = cuahsihisluwlids0_gleaner()

    report_ms3 = cuahsihisluwlids0_missingreport_s3(harvest)
    report_idstat = cuahsihisluwlids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisluwlids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisluwlids0")
    load_release = cuahsihisluwlids0_naburelease(harvest)
    load_uploadrelease = cuahsihisluwlids0_uploadrelease(load_release)

    load_prune = cuahsihisluwlids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisluwlids0_nabuprov(load_prune)
    load_org = cuahsihisluwlids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisluwlids0_missingreport_graph(load_org)
    report_graph=cuahsihisluwlids0_graph_reports(report_msgraph)
