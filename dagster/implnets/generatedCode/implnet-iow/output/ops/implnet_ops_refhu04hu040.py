...
# #########################  Ops Section
@op
def refhu04hu040_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def refhu04hu040_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "refhu04hu040")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu04hu040_naburelease(context):
    returned_value = gleanerio(context, ("release"), "refhu04hu040")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu04hu040_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "refhu04hu040")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu04hu040_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "refhu04hu040")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu04hu040_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "refhu04hu040")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu04hu040_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "refhu04hu040")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu04hu040_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "refhu04hu040")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu04hu040_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "refhu04hu040")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu04hu040_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "refhu04hu040")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu04hu040_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "refhu04hu040")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def refhu04hu040_uploadrelease(context):
#     returned_value = post_to_graph("refhu04hu040", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def refhu04hu040_uploadrelease(context):
#     returned_value = postRelease("refhu04hu040")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def refhu04hu040_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refhu04hu040"
    )
...
@op(ins={"start": In(Nothing)})
def refhu04hu040_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refhu04hu040"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def refhu04hu040_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refhu04hu040"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def refhu04hu040_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refhu04hu040"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def refhu04hu040_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def refhu04hu040_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def refhu04hu040_upload_summarize(context):
...
@graph
def harvest_refhu04hu040():
    # check for containers
    containers = refhu04hu040_getImage()

    # conduct the harvest
    harvest = refhu04hu040_gleaner(start=containers)

    # data branch
    load_release = refhu04hu040_naburelease(start=harvest)
    load_uploadrelease = refhu04hu040_uploadrelease(start=load_release)
    load_prune = refhu04hu040_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = refhu04hu040_nabu_provrelease(start=harvest)
    prov_clear = refhu04hu040_nabu_provclear(start=prov_release)
    prov_object = refhu04hu040_nabu_provobject(start=prov_clear)
    prov_drain = refhu04hu040_nabu_provdrain(start=prov_object)

    # org branch
    org_release = refhu04hu040_nabu_orgsrelease(start=harvest)
    load_org = refhu04hu040_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = refhu04hu040_missingreport_s3(start=harvest)
    # report_idstat = refhu04hu040_identifier_stats(start=report_ms3)
    # report_bucketurl = refhu04hu040_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = refhu04hu040_summarize(start=load_uploadrelease)
    # upload_summarize = refhu04hu040_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = refhu04hu040_missingreport_graph(start=summarize)
    # report_graph = refhu04hu040_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="refhu04hu040"):
#
