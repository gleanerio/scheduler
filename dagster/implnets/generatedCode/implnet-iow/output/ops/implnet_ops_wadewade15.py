...
# #########################  Ops Section
@op
def wadewade15_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def wadewade15_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "wadewade15")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade15_naburelease(context):
    returned_value = gleanerio(context, ("release"), "wadewade15")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade15_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wadewade15")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade15_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "wadewade15")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade15_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wadewade15")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade15_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wadewade15")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade15_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wadewade15")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade15_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wadewade15")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade15_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wadewade15")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade15_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "wadewade15")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wadewade15_uploadrelease(context):
#     returned_value = post_to_graph("wadewade15", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wadewade15_uploadrelease(context):
#     returned_value = postRelease("wadewade15")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def wadewade15_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade15"
    )
...
@op(ins={"start": In(Nothing)})
def wadewade15_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade15"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def wadewade15_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade15"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def wadewade15_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade15"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def wadewade15_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wadewade15_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wadewade15_upload_summarize(context):
...
@graph
def harvest_wadewade15():
    # check for containers
    containers = wadewade15_getImage()

    # conduct the harvest
    harvest = wadewade15_gleaner(start=containers)

    # data branch
    load_release = wadewade15_naburelease(start=harvest)
    load_uploadrelease = wadewade15_uploadrelease(start=load_release)
    load_prune = wadewade15_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = wadewade15_nabu_provrelease(start=harvest)
    prov_clear = wadewade15_nabu_provclear(start=prov_release)
    prov_object = wadewade15_nabu_provobject(start=prov_clear)
    prov_drain = wadewade15_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wadewade15_nabu_orgsrelease(start=harvest)
    load_org = wadewade15_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = wadewade15_missingreport_s3(start=harvest)
    # report_idstat = wadewade15_identifier_stats(start=report_ms3)
    # report_bucketurl = wadewade15_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = wadewade15_summarize(start=load_uploadrelease)
    # upload_summarize = wadewade15_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wadewade15_missingreport_graph(start=summarize)
    # report_graph = wadewade15_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="wadewade15"):
#
