...
@op
def hu060_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "hu060")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def hu060_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "hu060")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def hu060_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "hu060")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def hu060_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "hu060")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def hu060_uploadrelease(context, msg: str):
...
@op
def hu060_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hu060")
    ...
    return msg + r
@op
def hu060_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hu060")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def hu060_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hu060")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def hu060_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hu060")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "hu060"
...
def hu060_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "hu060"
...
@graph
def harvest_hu060():
    harvest = hu060_gleaner()

    report_ms3 = hu060_missingreport_s3(harvest)
    report_idstat = hu060_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = hu060_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="hu060")
    load_release = hu060_naburelease(harvest)
    load_uploadrelease = hu060_uploadrelease(load_release)

    load_prune = hu060_nabu_prune(load_uploadrelease)
    load_prov = hu060_nabuprov(load_prune)
    load_org = hu060_nabuorg(load_prov)

# run after load
    report_msgraph=hu060_missingreport_graph(load_org)
    report_graph=hu060_graph_reports(report_msgraph)
