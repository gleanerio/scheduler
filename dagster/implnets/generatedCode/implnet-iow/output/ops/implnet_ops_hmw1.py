...
def hmw1_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "hmw1")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def hmw1_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "hmw1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def hmw1_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "hmw1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def hmw1_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "hmw1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def hmw1_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "hmw1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def hmw1_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "hmw1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def hmw1_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def hmw1_uploadrelease(context):
#     returned_value = post_to_graph("hmw1", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def hmw1_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="hmw1"
    )
...
@op(ins={"start": In(Nothing)})
def hmw1_upload_summarize(context):
...
def harvest_hmw1():
    ...
    # prov branch
    prov_release = hmw1_nabu_provrelease(start=harvest)
    prov_clear = hmw1_nabu_provclear(start=prov_release)
    prov_object = hmw1_nabu_provobject(start=prov_clear)
    prov_drain = hmw1_nabu_provdrain(start=prov_object)

    # org branch
    org_release = hmw1_nabu_orgsrelease(start=harvest)
    load_org = hmw1_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = hmw1_summarize(start=load_uploadrelease)
    # upload_summarize = hmw1_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = hmw1_missingreport_graph(start=summarize)
