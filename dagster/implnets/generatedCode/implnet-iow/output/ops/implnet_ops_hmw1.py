...
@op
def hmw1_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "hmw1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
...
def hmw1_naburelease(context, msg: str):
    ...
    return msg + r
@op
def hmw1_uploadrelease(context, msg: str):
...
@op
def hmw1_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hmw1")
    ...
    return msg + r
@op
def hmw1_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hmw1")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def hmw1_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hmw1")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def hmw1_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hmw1")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "hmw1"
...
def hmw1_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "hmw1"
...
def harvest_hmw1():
    ...
    report_ms3 = hmw1_missingreport_s3(harvest)
    report_idstat = hmw1_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = hmw1_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="hmw1")
    load_release = hmw1_naburelease(harvest)
    load_uploadrelease = hmw1_uploadrelease(load_release)

    load_prune = hmw1_nabu_prune(load_uploadrelease)
    load_prov = hmw1_nabuprov(load_prune)
    load_org = hmw1_nabuorg(load_prov)

# run after load
    report_msgraph=hmw1_missingreport_graph(load_org)
    report_graph=hmw1_graph_reports(report_msgraph)
