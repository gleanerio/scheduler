...
@op
def nwisgw7_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw7")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw7_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw7")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw7_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw7")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw7_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw7")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw7_uploadrelease(context, msg: str):
...
@op
def nwisgw7_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw7")
    ...
    return msg + r
@op
def nwisgw7_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw7")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw7_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw7")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw7_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw7")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw7"
...
def nwisgw7_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw7"
...
@graph
def harvest_nwisgw7():
    harvest = nwisgw7_gleaner()

    report_ms3 = nwisgw7_missingreport_s3(harvest)
    report_idstat = nwisgw7_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw7_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw7")
    load_release = nwisgw7_naburelease(harvest)
    load_uploadrelease = nwisgw7_uploadrelease(load_release)

    load_prune = nwisgw7_nabu_prune(load_uploadrelease)
    load_prov = nwisgw7_nabuprov(load_prune)
    load_org = nwisgw7_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw7_missingreport_graph(load_org)
    report_graph=nwisgw7_graph_reports(report_msgraph)
