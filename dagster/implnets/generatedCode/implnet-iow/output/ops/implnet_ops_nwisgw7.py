...
def nwisgw7_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "nwisgw7")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def nwisgw7_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "nwisgw7")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw7_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "nwisgw7")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw7_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "nwisgw7")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw7_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "nwisgw7")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw7_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "nwisgw7")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw7_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def nwisgw7_uploadrelease(context):
#     returned_value = post_to_graph("nwisgw7", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def nwisgw7_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nwisgw7"
    )
...
@op(ins={"start": In(Nothing)})
def nwisgw7_upload_summarize(context):
...
def harvest_nwisgw7():
    ...
    # prov branch
    prov_release = nwisgw7_nabu_provrelease(start=harvest)
    prov_clear = nwisgw7_nabu_provclear(start=prov_release)
    prov_object = nwisgw7_nabu_provobject(start=prov_clear)
    prov_drain = nwisgw7_nabu_provdrain(start=prov_object)

    # org branch
    org_release = nwisgw7_nabu_orgsrelease(start=harvest)
    load_org = nwisgw7_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = nwisgw7_summarize(start=load_uploadrelease)
    # upload_summarize = nwisgw7_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = nwisgw7_missingreport_graph(start=summarize)
