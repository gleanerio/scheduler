...
@op
def cuahsihiscuisoids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihiscuisoids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihiscuisoids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihiscuisoids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihiscuisoids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihiscuisoids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihiscuisoids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihiscuisoids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihiscuisoids0_uploadrelease(context, msg: str):
...
@op
def cuahsihiscuisoids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihiscuisoids0")
    ...
    return msg + r
@op
def cuahsihiscuisoids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihiscuisoids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihiscuisoids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihiscuisoids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihiscuisoids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihiscuisoids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihiscuisoids0"
...
def cuahsihiscuisoids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihiscuisoids0"
...
def harvest_cuahsihiscuisoids0():
    harvest = cuahsihiscuisoids0_gleaner()

    report_ms3 = cuahsihiscuisoids0_missingreport_s3(harvest)
    report_idstat = cuahsihiscuisoids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihiscuisoids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihiscuisoids0")
    load_release = cuahsihiscuisoids0_naburelease(harvest)
    load_uploadrelease = cuahsihiscuisoids0_uploadrelease(load_release)

    load_prune = cuahsihiscuisoids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihiscuisoids0_nabuprov(load_prune)
    load_org = cuahsihiscuisoids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihiscuisoids0_missingreport_graph(load_org)
    report_graph=cuahsihiscuisoids0_graph_reports(report_msgraph)
