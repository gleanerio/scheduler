...
def cuahsihisandrewsforestlterids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsihisandrewsforestlterids0")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def cuahsihisandrewsforestlterids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsihisandrewsforestlterids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisandrewsforestlterids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsihisandrewsforestlterids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisandrewsforestlterids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsihisandrewsforestlterids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisandrewsforestlterids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsihisandrewsforestlterids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisandrewsforestlterids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsihisandrewsforestlterids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisandrewsforestlterids0_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def cuahsihisandrewsforestlterids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsihisandrewsforestlterids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def cuahsihisandrewsforestlterids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsihisandrewsforestlterids0"
    )
...
@op(ins={"start": In(Nothing)})
def cuahsihisandrewsforestlterids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsihisandrewsforestlterids0_upload_summarize(context):
    returned_value = post_to_graph(
        "cuahsihisandrewsforestlterids0",
        path=SUMMARY_PATH,
        extension="ttl",
...
def harvest_cuahsihisandrewsforestlterids0():
    ...
    # prov branch
    prov_release = cuahsihisandrewsforestlterids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsihisandrewsforestlterids0_nabu_provclear(start=prov_release)
    prov_object = cuahsihisandrewsforestlterids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsihisandrewsforestlterids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsihisandrewsforestlterids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsihisandrewsforestlterids0_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = cuahsihisandrewsforestlterids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsihisandrewsforestlterids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsihisandrewsforestlterids0_missingreport_graph(start=summarize)
