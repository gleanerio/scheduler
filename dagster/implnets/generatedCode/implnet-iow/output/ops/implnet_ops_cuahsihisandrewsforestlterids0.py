...
@op
def cuahsihisandrewsforestlterids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisandrewsforestlterids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisandrewsforestlterids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisandrewsforestlterids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisandrewsforestlterids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisandrewsforestlterids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisandrewsforestlterids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisandrewsforestlterids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisandrewsforestlterids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisandrewsforestlterids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisandrewsforestlterids0")
    ...
    return msg + r
@op
def cuahsihisandrewsforestlterids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisandrewsforestlterids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisandrewsforestlterids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisandrewsforestlterids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisandrewsforestlterids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisandrewsforestlterids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisandrewsforestlterids0"
...
def cuahsihisandrewsforestlterids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisandrewsforestlterids0"
...
def harvest_cuahsihisandrewsforestlterids0():
    harvest = cuahsihisandrewsforestlterids0_gleaner()

    report_ms3 = cuahsihisandrewsforestlterids0_missingreport_s3(harvest)
    report_idstat = cuahsihisandrewsforestlterids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisandrewsforestlterids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisandrewsforestlterids0")
    load_release = cuahsihisandrewsforestlterids0_naburelease(harvest)
    load_uploadrelease = cuahsihisandrewsforestlterids0_uploadrelease(load_release)

    load_prune = cuahsihisandrewsforestlterids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisandrewsforestlterids0_nabuprov(load_prune)
    load_org = cuahsihisandrewsforestlterids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisandrewsforestlterids0_missingreport_graph(load_org)
    report_graph=cuahsihisandrewsforestlterids0_graph_reports(report_msgraph)
