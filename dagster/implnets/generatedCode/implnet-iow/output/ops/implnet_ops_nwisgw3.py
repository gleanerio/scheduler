...
@op
def nwisgw3_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw3")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw3_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw3")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw3_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw3")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw3_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw3")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw3_uploadrelease(context, msg: str):
...
@op
def nwisgw3_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw3")
    ...
    return msg + r
@op
def nwisgw3_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw3")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw3_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw3")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw3_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw3")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw3"
...
def nwisgw3_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw3"
...
@graph
def harvest_nwisgw3():
    harvest = nwisgw3_gleaner()

    report_ms3 = nwisgw3_missingreport_s3(harvest)
    report_idstat = nwisgw3_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw3_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw3")
    load_release = nwisgw3_naburelease(harvest)
    load_uploadrelease = nwisgw3_uploadrelease(load_release)

    load_prune = nwisgw3_nabu_prune(load_uploadrelease)
    load_prov = nwisgw3_nabuprov(load_prune)
    load_org = nwisgw3_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw3_missingreport_graph(load_org)
    report_graph=nwisgw3_graph_reports(report_msgraph)
