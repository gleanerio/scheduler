...
def nwisgw3_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "nwisgw3")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def nwisgw3_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "nwisgw3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw3_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "nwisgw3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw3_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "nwisgw3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw3_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "nwisgw3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw3_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "nwisgw3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw3_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def nwisgw3_uploadrelease(context):
#     returned_value = post_to_graph("nwisgw3", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def nwisgw3_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nwisgw3"
    )
...
@op(ins={"start": In(Nothing)})
def nwisgw3_upload_summarize(context):
...
def harvest_nwisgw3():
    ...
    # prov branch
    prov_release = nwisgw3_nabu_provrelease(start=harvest)
    prov_clear = nwisgw3_nabu_provclear(start=prov_release)
    prov_object = nwisgw3_nabu_provobject(start=prov_clear)
    prov_drain = nwisgw3_nabu_provdrain(start=prov_object)

    # org branch
    org_release = nwisgw3_nabu_orgsrelease(start=harvest)
    load_org = nwisgw3_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = nwisgw3_summarize(start=load_uploadrelease)
    # upload_summarize = nwisgw3_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = nwisgw3_missingreport_graph(start=summarize)
