...
# #########################  Ops Section
@op
def cuahsicuahsihislaselvastreamdischargeids0_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def cuahsicuahsihislaselvastreamdischargeids0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "cuahsicuahsihislaselvastreamdischargeids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihislaselvastreamdischargeids0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "cuahsicuahsihislaselvastreamdischargeids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihislaselvastreamdischargeids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsicuahsihislaselvastreamdischargeids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihislaselvastreamdischargeids0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "cuahsicuahsihislaselvastreamdischargeids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihislaselvastreamdischargeids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsicuahsihislaselvastreamdischargeids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihislaselvastreamdischargeids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsicuahsihislaselvastreamdischargeids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihislaselvastreamdischargeids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsicuahsihislaselvastreamdischargeids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihislaselvastreamdischargeids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsicuahsihislaselvastreamdischargeids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihislaselvastreamdischargeids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsicuahsihislaselvastreamdischargeids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihislaselvastreamdischargeids0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "cuahsicuahsihislaselvastreamdischargeids0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihislaselvastreamdischargeids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsicuahsihislaselvastreamdischargeids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihislaselvastreamdischargeids0_uploadrelease(context):
#     returned_value = postRelease("cuahsicuahsihislaselvastreamdischargeids0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihislaselvastreamdischargeids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihislaselvastreamdischargeids0"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsicuahsihislaselvastreamdischargeids0"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihislaselvastreamdischargeids0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihislaselvastreamdischargeids0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihislaselvastreamdischargeids0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihislaselvastreamdischargeids0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihislaselvastreamdischargeids0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihislaselvastreamdischargeids0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihislaselvastreamdischargeids0_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihislaselvastreamdischargeids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihislaselvastreamdischargeids0_upload_summarize(context):
    returned_value = post_to_graph(
        "cuahsicuahsihislaselvastreamdischargeids0",
        path=SUMMARY_PATH,
        extension="ttl",
...
@graph
def harvest_cuahsicuahsihislaselvastreamdischargeids0():
    # check for containers
    containers = cuahsicuahsihislaselvastreamdischargeids0_getImage()

    # conduct the harvest
    harvest = cuahsicuahsihislaselvastreamdischargeids0_gleaner(start=containers)

    # data branch
    load_release = cuahsicuahsihislaselvastreamdischargeids0_naburelease(start=harvest)
    load_uploadrelease = cuahsicuahsihislaselvastreamdischargeids0_uploadrelease(start=load_release)
    load_prune = cuahsicuahsihislaselvastreamdischargeids0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = cuahsicuahsihislaselvastreamdischargeids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsicuahsihislaselvastreamdischargeids0_nabu_provclear(start=prov_release)
    prov_object = cuahsicuahsihislaselvastreamdischargeids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsicuahsihislaselvastreamdischargeids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsicuahsihislaselvastreamdischargeids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsicuahsihislaselvastreamdischargeids0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = cuahsicuahsihislaselvastreamdischargeids0_missingreport_s3(start=harvest)
    # report_idstat = cuahsicuahsihislaselvastreamdischargeids0_identifier_stats(start=report_ms3)
    # report_bucketurl = cuahsicuahsihislaselvastreamdischargeids0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = cuahsicuahsihislaselvastreamdischargeids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsicuahsihislaselvastreamdischargeids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsicuahsihislaselvastreamdischargeids0_missingreport_graph(start=summarize)
    # report_graph = cuahsicuahsihislaselvastreamdischargeids0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="cuahsicuahsihislaselvastreamdischargeids0"):
#
