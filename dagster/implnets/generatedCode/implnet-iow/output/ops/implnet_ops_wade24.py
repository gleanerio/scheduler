...
@op(ins={"start": In(Nothing)})
def wade24_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade24")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade24_nabuorg(context):
...
def wade24_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade24")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade24")
    ...
    load_prov = wade24_nabuprov(start=load_prune)
    load_org = wade24_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade24_missingreport_graph(start=load_org)
