...
@op
def cuahsihisumbcgwids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisumbcgwids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisumbcgwids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisumbcgwids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisumbcgwids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisumbcgwids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisumbcgwids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisumbcgwids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisumbcgwids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisumbcgwids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisumbcgwids0")
    ...
    return msg + r
@op
def cuahsihisumbcgwids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisumbcgwids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisumbcgwids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisumbcgwids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisumbcgwids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisumbcgwids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisumbcgwids0"
...
def cuahsihisumbcgwids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisumbcgwids0"
...
def harvest_cuahsihisumbcgwids0():
    harvest = cuahsihisumbcgwids0_gleaner()

    report_ms3 = cuahsihisumbcgwids0_missingreport_s3(harvest)
    report_idstat = cuahsihisumbcgwids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisumbcgwids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisumbcgwids0")
    load_release = cuahsihisumbcgwids0_naburelease(harvest)
    load_uploadrelease = cuahsihisumbcgwids0_uploadrelease(load_release)

    load_prune = cuahsihisumbcgwids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisumbcgwids0_nabuprov(load_prune)
    load_org = cuahsihisumbcgwids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisumbcgwids0_missingreport_graph(load_org)
    report_graph=cuahsihisumbcgwids0_graph_reports(report_msgraph)
