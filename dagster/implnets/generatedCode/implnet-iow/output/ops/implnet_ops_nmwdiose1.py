...
@op
def nmwdiose1_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nmwdiose1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nmwdiose1_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nmwdiose1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nmwdiose1_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nmwdiose1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nmwdiose1_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nmwdiose1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nmwdiose1_uploadrelease(context, msg: str):
...
@op
def nmwdiose1_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nmwdiose1")
    ...
    return msg + r
@op
def nmwdiose1_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nmwdiose1")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nmwdiose1_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nmwdiose1")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nmwdiose1_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nmwdiose1")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nmwdiose1"
...
def nmwdiose1_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nmwdiose1"
...
def harvest_nmwdiose1():
    harvest = nmwdiose1_gleaner()

    report_ms3 = nmwdiose1_missingreport_s3(harvest)
    report_idstat = nmwdiose1_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nmwdiose1_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nmwdiose1")
    load_release = nmwdiose1_naburelease(harvest)
    load_uploadrelease = nmwdiose1_uploadrelease(load_release)

    load_prune = nmwdiose1_nabu_prune(load_uploadrelease)
    load_prov = nmwdiose1_nabuprov(load_prune)
    load_org = nmwdiose1_nabuorg(load_prov)

# run after load
    report_msgraph=nmwdiose1_missingreport_graph(load_org)
    report_graph=nmwdiose1_graph_reports(report_msgraph)
