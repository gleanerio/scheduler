...
@op
def cuahsihisisbenaids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisisbenaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisisbenaids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisisbenaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisisbenaids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisisbenaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisisbenaids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisisbenaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisisbenaids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisisbenaids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisisbenaids0")
    ...
    return msg + r
@op
def cuahsihisisbenaids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisisbenaids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisisbenaids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisisbenaids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisisbenaids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisisbenaids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisisbenaids0"
...
def cuahsihisisbenaids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisisbenaids0"
...
def harvest_cuahsihisisbenaids0():
    harvest = cuahsihisisbenaids0_gleaner()

    report_ms3 = cuahsihisisbenaids0_missingreport_s3(harvest)
    report_idstat = cuahsihisisbenaids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisisbenaids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisisbenaids0")
    load_release = cuahsihisisbenaids0_naburelease(harvest)
    load_uploadrelease = cuahsihisisbenaids0_uploadrelease(load_release)

    load_prune = cuahsihisisbenaids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisisbenaids0_nabuprov(load_prune)
    load_org = cuahsihisisbenaids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisisbenaids0_missingreport_graph(load_org)
    report_graph=cuahsihisisbenaids0_graph_reports(report_msgraph)
