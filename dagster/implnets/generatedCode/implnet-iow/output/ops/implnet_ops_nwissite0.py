...
@op
def nwissite0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwissite0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwissite0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwissite0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwissite0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwissite0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwissite0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwissite0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwissite0_uploadrelease(context, msg: str):
...
@op
def nwissite0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwissite0")
    ...
    return msg + r
@op
def nwissite0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwissite0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwissite0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwissite0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwissite0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwissite0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwissite0"
...
def nwissite0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwissite0"
...
def harvest_nwissite0():
    harvest = nwissite0_gleaner()

    report_ms3 = nwissite0_missingreport_s3(harvest)
    report_idstat = nwissite0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwissite0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwissite0")
    load_release = nwissite0_naburelease(harvest)
    load_uploadrelease = nwissite0_uploadrelease(load_release)

    load_prune = nwissite0_nabu_prune(load_uploadrelease)
    load_prov = nwissite0_nabuprov(load_prune)
    load_org = nwissite0_nabuorg(load_prov)

# run after load
    report_msgraph=nwissite0_missingreport_graph(load_org)
    report_graph=nwissite0_graph_reports(report_msgraph)
