...
# #########################  Ops Section
@op
def refgagesgages3_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def refgagesgages3_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "refgagesgages3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refgagesgages3_naburelease(context):
    returned_value = gleanerio(context, ("release"), "refgagesgages3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refgagesgages3_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "refgagesgages3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refgagesgages3_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "refgagesgages3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refgagesgages3_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "refgagesgages3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refgagesgages3_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "refgagesgages3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refgagesgages3_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "refgagesgages3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refgagesgages3_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "refgagesgages3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refgagesgages3_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "refgagesgages3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refgagesgages3_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "refgagesgages3")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def refgagesgages3_uploadrelease(context):
#     returned_value = post_to_graph("refgagesgages3", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def refgagesgages3_uploadrelease(context):
#     returned_value = postRelease("refgagesgages3")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def refgagesgages3_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refgagesgages3"
    )
...
@op(ins={"start": In(Nothing)})
def refgagesgages3_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refgagesgages3"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def refgagesgages3_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refgagesgages3"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def refgagesgages3_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refgagesgages3"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def refgagesgages3_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def refgagesgages3_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def refgagesgages3_upload_summarize(context):
...
@graph
def harvest_refgagesgages3():
    # check for containers
    containers = refgagesgages3_getImage()

    # conduct the harvest
    harvest = refgagesgages3_gleaner(start=containers)

    # data branch
    load_release = refgagesgages3_naburelease(start=harvest)
    load_uploadrelease = refgagesgages3_uploadrelease(start=load_release)
    load_prune = refgagesgages3_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = refgagesgages3_nabu_provrelease(start=harvest)
    prov_clear = refgagesgages3_nabu_provclear(start=prov_release)
    prov_object = refgagesgages3_nabu_provobject(start=prov_clear)
    prov_drain = refgagesgages3_nabu_provdrain(start=prov_object)

    # org branch
    org_release = refgagesgages3_nabu_orgsrelease(start=harvest)
    load_org = refgagesgages3_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = refgagesgages3_missingreport_s3(start=harvest)
    # report_idstat = refgagesgages3_identifier_stats(start=report_ms3)
    # report_bucketurl = refgagesgages3_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = refgagesgages3_summarize(start=load_uploadrelease)
    # upload_summarize = refgagesgages3_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = refgagesgages3_missingreport_graph(start=summarize)
    # report_graph = refgagesgages3_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="refgagesgages3"):
#
