...
@op
def damspids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "damspids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def damspids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "damspids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def damspids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "damspids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def damspids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "damspids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def damspids0_uploadrelease(context, msg: str):
...
@op
def damspids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="damspids0")
    ...
    return msg + r
@op
def damspids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="damspids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def damspids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="damspids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def damspids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="damspids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "damspids0"
...
def damspids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "damspids0"
...
def harvest_damspids0():
    harvest = damspids0_gleaner()

    report_ms3 = damspids0_missingreport_s3(harvest)
    report_idstat = damspids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = damspids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="damspids0")
    load_release = damspids0_naburelease(harvest)
    load_uploadrelease = damspids0_uploadrelease(load_release)

    load_prune = damspids0_nabu_prune(load_uploadrelease)
    load_prov = damspids0_nabuprov(load_prune)
    load_org = damspids0_nabuorg(load_prov)

# run after load
    report_msgraph=damspids0_missingreport_graph(load_org)
    report_graph=damspids0_graph_reports(report_msgraph)
