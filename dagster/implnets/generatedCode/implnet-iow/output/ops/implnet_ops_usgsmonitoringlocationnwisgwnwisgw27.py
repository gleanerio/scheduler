...
# #########################  Ops Section
@op
def usgsmonitoringlocationnwisgwnwisgw27_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def usgsmonitoringlocationnwisgwnwisgw27_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "usgsmonitoringlocationnwisgwnwisgw27")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw27_naburelease(context):
    returned_value = gleanerio(context, ("release"), "usgsmonitoringlocationnwisgwnwisgw27")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw27_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "usgsmonitoringlocationnwisgwnwisgw27")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw27_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "usgsmonitoringlocationnwisgwnwisgw27")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw27_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "usgsmonitoringlocationnwisgwnwisgw27")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw27_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "usgsmonitoringlocationnwisgwnwisgw27")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw27_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "usgsmonitoringlocationnwisgwnwisgw27")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw27_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "usgsmonitoringlocationnwisgwnwisgw27")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw27_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "usgsmonitoringlocationnwisgwnwisgw27")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw27_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "usgsmonitoringlocationnwisgwnwisgw27")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def usgsmonitoringlocationnwisgwnwisgw27_uploadrelease(context):
#     returned_value = post_to_graph("usgsmonitoringlocationnwisgwnwisgw27", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def usgsmonitoringlocationnwisgwnwisgw27_uploadrelease(context):
#     returned_value = postRelease("usgsmonitoringlocationnwisgwnwisgw27")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw27_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="usgsmonitoringlocationnwisgwnwisgw27"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "usgsmonitoringlocationnwisgwnwisgw27"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw27_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="usgsmonitoringlocationnwisgwnwisgw27"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw27_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="usgsmonitoringlocationnwisgwnwisgw27"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw27_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="usgsmonitoringlocationnwisgwnwisgw27"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw27_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw27_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw27_upload_summarize(context):
    returned_value = post_to_graph(
        "usgsmonitoringlocationnwisgwnwisgw27",
        path=SUMMARY_PATH,
        extension="ttl",
...
@graph
def harvest_usgsmonitoringlocationnwisgwnwisgw27():
    # check for containers
    containers = usgsmonitoringlocationnwisgwnwisgw27_getImage()

    # conduct the harvest
    harvest = usgsmonitoringlocationnwisgwnwisgw27_gleaner(start=containers)

    # data branch
    load_release = usgsmonitoringlocationnwisgwnwisgw27_naburelease(start=harvest)
    load_uploadrelease = usgsmonitoringlocationnwisgwnwisgw27_uploadrelease(start=load_release)
    load_prune = usgsmonitoringlocationnwisgwnwisgw27_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = usgsmonitoringlocationnwisgwnwisgw27_nabu_provrelease(start=harvest)
    prov_clear = usgsmonitoringlocationnwisgwnwisgw27_nabu_provclear(start=prov_release)
    prov_object = usgsmonitoringlocationnwisgwnwisgw27_nabu_provobject(start=prov_clear)
    prov_drain = usgsmonitoringlocationnwisgwnwisgw27_nabu_provdrain(start=prov_object)

    # org branch
    org_release = usgsmonitoringlocationnwisgwnwisgw27_nabu_orgsrelease(start=harvest)
    load_org = usgsmonitoringlocationnwisgwnwisgw27_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = usgsmonitoringlocationnwisgwnwisgw27_missingreport_s3(start=harvest)
    # report_idstat = usgsmonitoringlocationnwisgwnwisgw27_identifier_stats(start=report_ms3)
    # report_bucketurl = usgsmonitoringlocationnwisgwnwisgw27_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = usgsmonitoringlocationnwisgwnwisgw27_summarize(start=load_uploadrelease)
    # upload_summarize = usgsmonitoringlocationnwisgwnwisgw27_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = usgsmonitoringlocationnwisgwnwisgw27_missingreport_graph(start=summarize)
    # report_graph = usgsmonitoringlocationnwisgwnwisgw27_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="usgsmonitoringlocationnwisgwnwisgw27"):
#
