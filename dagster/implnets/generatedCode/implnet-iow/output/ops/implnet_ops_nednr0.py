...
def nednr0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "nednr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nednr0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "nednr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nednr0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "nednr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nednr0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "nednr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nednr0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "nednr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nednr0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "nednr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nednr0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "nednr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nednr0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "nednr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nednr0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "nednr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nednr0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "nednr0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def nednr0_uploadrelease(context):
#     returned_value = post_to_graph("nednr0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def nednr0_uploadrelease(context):
#     returned_value = postRelease("nednr0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def nednr0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nednr0"
    )
...
@op(ins={"start": In(Nothing)})
def nednr0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nednr0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def nednr0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nednr0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def nednr0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nednr0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def nednr0_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def nednr0_upload_summarize(context):
...
def harvest_nednr0():
    ...
    containers = nednr0_getImage()

    # conduct the harvest
    harvest = nednr0_gleaner(start=containers)

    # data branch
    load_release = nednr0_naburelease(start=harvest)
    load_uploadrelease = nednr0_uploadrelease(start=load_release)
    load_prune = nednr0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = nednr0_nabu_provrelease(start=harvest)
    prov_clear = nednr0_nabu_provclear(start=prov_release)
    prov_object = nednr0_nabu_provobject(start=prov_clear)
    prov_drain = nednr0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = nednr0_nabu_orgsrelease(start=harvest)
    load_org = nednr0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = nednr0_missingreport_s3(start=harvest)
    # report_idstat = nednr0_identifier_stats(start=report_ms3)
    # report_bucketurl = nednr0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = nednr0_summarize(start=load_uploadrelease)
    # upload_summarize = nednr0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = nednr0_missingreport_graph(start=summarize)
    # report_graph = nednr0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="nednr0"):
#
