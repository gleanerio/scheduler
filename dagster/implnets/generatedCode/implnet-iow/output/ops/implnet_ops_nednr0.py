...
@op(ins={"start": In(Nothing)})
def nednr0_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "nednr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nednr0_nabuorg(context):
...
def nednr0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="nednr0")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="nednr0")
    ...
    load_prov = nednr0_nabuprov(start=load_prune)
    load_org = nednr0_nabuorg(start=load_prov)

# run after load
    report_msgraph=nednr0_missingreport_graph(start=load_org)
