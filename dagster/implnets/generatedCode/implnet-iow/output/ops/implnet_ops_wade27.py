...
@op(ins={"start": In(Nothing)})
def wade27_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade27")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade27_nabuorg(context):
...
def wade27_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade27")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade27")
    ...
    load_prov = wade27_nabuprov(start=load_prune)
    load_org = wade27_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade27_missingreport_graph(start=load_org)
