...
@op(ins={"start": In(Nothing)})
def wade34_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade34")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade34_nabuorg(context):
...
def wade34_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade34")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade34")
    ...
    load_prov = wade34_nabuprov(start=load_prune)
    load_org = wade34_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade34_missingreport_graph(start=load_org)
