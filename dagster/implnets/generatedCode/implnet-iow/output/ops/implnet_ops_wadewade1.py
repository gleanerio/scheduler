...
# #########################  Ops Section
@op
def wadewade1_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def wadewade1_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "wadewade1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade1_naburelease(context):
    returned_value = gleanerio(context, ("release"), "wadewade1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade1_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wadewade1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade1_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "wadewade1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade1_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wadewade1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade1_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wadewade1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade1_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wadewade1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade1_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wadewade1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade1_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wadewade1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade1_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "wadewade1")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wadewade1_uploadrelease(context):
#     returned_value = post_to_graph("wadewade1", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wadewade1_uploadrelease(context):
#     returned_value = postRelease("wadewade1")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def wadewade1_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade1"
    )
...
@op(ins={"start": In(Nothing)})
def wadewade1_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade1"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def wadewade1_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade1"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def wadewade1_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade1"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def wadewade1_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wadewade1_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wadewade1_upload_summarize(context):
...
@graph
def harvest_wadewade1():
    # check for containers
    containers = wadewade1_getImage()

    # conduct the harvest
    harvest = wadewade1_gleaner(start=containers)

    # data branch
    load_release = wadewade1_naburelease(start=harvest)
    load_uploadrelease = wadewade1_uploadrelease(start=load_release)
    load_prune = wadewade1_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = wadewade1_nabu_provrelease(start=harvest)
    prov_clear = wadewade1_nabu_provclear(start=prov_release)
    prov_object = wadewade1_nabu_provobject(start=prov_clear)
    prov_drain = wadewade1_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wadewade1_nabu_orgsrelease(start=harvest)
    load_org = wadewade1_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = wadewade1_missingreport_s3(start=harvest)
    # report_idstat = wadewade1_identifier_stats(start=report_ms3)
    # report_bucketurl = wadewade1_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = wadewade1_summarize(start=load_uploadrelease)
    # upload_summarize = wadewade1_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wadewade1_missingreport_graph(start=summarize)
    # report_graph = wadewade1_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="wadewade1"):
#
