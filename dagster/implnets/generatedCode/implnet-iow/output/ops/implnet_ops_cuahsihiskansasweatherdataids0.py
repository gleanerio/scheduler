...
@op
def cuahsihiskansasweatherdataids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihiskansasweatherdataids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihiskansasweatherdataids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihiskansasweatherdataids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihiskansasweatherdataids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihiskansasweatherdataids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihiskansasweatherdataids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihiskansasweatherdataids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihiskansasweatherdataids0_uploadrelease(context, msg: str):
...
@op
def cuahsihiskansasweatherdataids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihiskansasweatherdataids0")
    ...
    return msg + r
@op
def cuahsihiskansasweatherdataids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihiskansasweatherdataids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihiskansasweatherdataids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihiskansasweatherdataids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihiskansasweatherdataids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihiskansasweatherdataids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihiskansasweatherdataids0"
...
def cuahsihiskansasweatherdataids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihiskansasweatherdataids0"
...
def harvest_cuahsihiskansasweatherdataids0():
    harvest = cuahsihiskansasweatherdataids0_gleaner()

    report_ms3 = cuahsihiskansasweatherdataids0_missingreport_s3(harvest)
    report_idstat = cuahsihiskansasweatherdataids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihiskansasweatherdataids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihiskansasweatherdataids0")
    load_release = cuahsihiskansasweatherdataids0_naburelease(harvest)
    load_uploadrelease = cuahsihiskansasweatherdataids0_uploadrelease(load_release)

    load_prune = cuahsihiskansasweatherdataids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihiskansasweatherdataids0_nabuprov(load_prune)
    load_org = cuahsihiskansasweatherdataids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihiskansasweatherdataids0_missingreport_graph(load_org)
    report_graph=cuahsihiskansasweatherdataids0_graph_reports(report_msgraph)
