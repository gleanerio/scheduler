...
@op
def cuahsihisparalanaturalezaids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisparalanaturalezaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisparalanaturalezaids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisparalanaturalezaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisparalanaturalezaids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisparalanaturalezaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisparalanaturalezaids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisparalanaturalezaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisparalanaturalezaids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisparalanaturalezaids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisparalanaturalezaids0")
    ...
    return msg + r
@op
def cuahsihisparalanaturalezaids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisparalanaturalezaids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisparalanaturalezaids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisparalanaturalezaids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisparalanaturalezaids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisparalanaturalezaids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisparalanaturalezaids0"
...
def cuahsihisparalanaturalezaids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisparalanaturalezaids0"
...
def harvest_cuahsihisparalanaturalezaids0():
    harvest = cuahsihisparalanaturalezaids0_gleaner()

    report_ms3 = cuahsihisparalanaturalezaids0_missingreport_s3(harvest)
    report_idstat = cuahsihisparalanaturalezaids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisparalanaturalezaids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisparalanaturalezaids0")
    load_release = cuahsihisparalanaturalezaids0_naburelease(harvest)
    load_uploadrelease = cuahsihisparalanaturalezaids0_uploadrelease(load_release)

    load_prune = cuahsihisparalanaturalezaids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisparalanaturalezaids0_nabuprov(load_prune)
    load_org = cuahsihisparalanaturalezaids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisparalanaturalezaids0_missingreport_graph(load_org)
    report_graph=cuahsihisparalanaturalezaids0_graph_reports(report_msgraph)
