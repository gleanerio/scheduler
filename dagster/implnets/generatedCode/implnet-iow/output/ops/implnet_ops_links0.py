...
def links0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "links0")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def links0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "links0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def links0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "links0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def links0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "links0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def links0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "links0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def links0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "links0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def links0_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def links0_uploadrelease(context):
#     returned_value = post_to_graph("links0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def links0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="links0"
    )
...
@op(ins={"start": In(Nothing)})
def links0_upload_summarize(context):
...
def harvest_links0():
    ...
    # prov branch
    prov_release = links0_nabu_provrelease(start=harvest)
    prov_clear = links0_nabu_provclear(start=prov_release)
    prov_object = links0_nabu_provobject(start=prov_clear)
    prov_drain = links0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = links0_nabu_orgsrelease(start=harvest)
    load_org = links0_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = links0_summarize(start=load_uploadrelease)
    # upload_summarize = links0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = links0_missingreport_graph(start=summarize)
