...
@op
def links0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "links0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def links0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "links0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def links0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "links0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def links0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "links0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def links0_uploadrelease(context, msg: str):
...
@op
def links0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="links0")
    ...
    return msg + r
@op
def links0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="links0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def links0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="links0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def links0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="links0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "links0"
...
def links0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "links0"
...
@graph
def harvest_links0():
    harvest = links0_gleaner()

    report_ms3 = links0_missingreport_s3(harvest)
    report_idstat = links0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = links0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="links0")
    load_release = links0_naburelease(harvest)
    load_uploadrelease = links0_uploadrelease(load_release)

    load_prune = links0_nabu_prune(load_uploadrelease)
    load_prov = links0_nabuprov(load_prune)
    load_org = links0_nabuorg(load_prov)

# run after load
    report_msgraph=links0_missingreport_graph(load_org)
    report_graph=links0_graph_reports(report_msgraph)
