...
@op(ins={"start": In(Nothing)})
def exampleids0_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "exampleids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def exampleids0_nabuorg(context):
...
def exampleids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="exampleids0")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="exampleids0")
    ...
    load_prov = exampleids0_nabuprov(start=load_prune)
    load_org = exampleids0_nabuorg(start=load_prov)

# run after load
    report_msgraph=exampleids0_missingreport_graph(start=load_org)
