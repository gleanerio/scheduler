...
def counties0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "counties0")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def counties0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "counties0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def counties0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "counties0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def counties0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "counties0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def counties0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "counties0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def counties0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "counties0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def counties0_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def counties0_uploadrelease(context):
#     returned_value = post_to_graph("counties0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def counties0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="counties0"
    )
...
@op(ins={"start": In(Nothing)})
def counties0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def counties0_upload_summarize(context):
...
def harvest_counties0():
    ...
    # prov branch
    prov_release = counties0_nabu_provrelease(start=harvest)
    prov_clear = counties0_nabu_provclear(start=prov_release)
    prov_object = counties0_nabu_provobject(start=prov_clear)
    prov_drain = counties0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = counties0_nabu_orgsrelease(start=harvest)
    load_org = counties0_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = counties0_summarize(start=load_uploadrelease)
    # upload_summarize = counties0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = counties0_missingreport_graph(start=summarize)
