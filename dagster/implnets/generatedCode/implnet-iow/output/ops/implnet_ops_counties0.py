...
@op
def counties0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "counties0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def counties0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "counties0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def counties0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "counties0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def counties0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "counties0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def counties0_uploadrelease(context, msg: str):
...
@op
def counties0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="counties0")
    ...
    return msg + r
@op
def counties0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="counties0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def counties0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="counties0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def counties0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="counties0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "counties0"
...
def counties0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "counties0"
...
def harvest_counties0():
    harvest = counties0_gleaner()

    report_ms3 = counties0_missingreport_s3(harvest)
    report_idstat = counties0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = counties0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="counties0")
    load_release = counties0_naburelease(harvest)
    load_uploadrelease = counties0_uploadrelease(load_release)

    load_prune = counties0_nabu_prune(load_uploadrelease)
    load_prov = counties0_nabuprov(load_prune)
    load_org = counties0_nabuorg(load_prov)

# run after load
    report_msgraph=counties0_missingreport_graph(load_org)
    report_graph=counties0_graph_reports(report_msgraph)
