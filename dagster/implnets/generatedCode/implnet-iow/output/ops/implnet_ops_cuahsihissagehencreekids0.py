...
@op
def cuahsihissagehencreekids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihissagehencreekids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihissagehencreekids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihissagehencreekids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihissagehencreekids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihissagehencreekids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihissagehencreekids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihissagehencreekids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihissagehencreekids0_uploadrelease(context, msg: str):
...
@op
def cuahsihissagehencreekids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihissagehencreekids0")
    ...
    return msg + r
@op
def cuahsihissagehencreekids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihissagehencreekids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihissagehencreekids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihissagehencreekids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihissagehencreekids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihissagehencreekids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihissagehencreekids0"
...
def cuahsihissagehencreekids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihissagehencreekids0"
...
def harvest_cuahsihissagehencreekids0():
    harvest = cuahsihissagehencreekids0_gleaner()

    report_ms3 = cuahsihissagehencreekids0_missingreport_s3(harvest)
    report_idstat = cuahsihissagehencreekids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihissagehencreekids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihissagehencreekids0")
    load_release = cuahsihissagehencreekids0_naburelease(harvest)
    load_uploadrelease = cuahsihissagehencreekids0_uploadrelease(load_release)

    load_prune = cuahsihissagehencreekids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihissagehencreekids0_nabuprov(load_prune)
    load_org = cuahsihissagehencreekids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihissagehencreekids0_missingreport_graph(load_org)
    report_graph=cuahsihissagehencreekids0_graph_reports(report_msgraph)
