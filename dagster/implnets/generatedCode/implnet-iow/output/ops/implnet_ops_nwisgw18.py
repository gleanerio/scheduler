...
@op
def nwisgw18_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw18")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw18_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw18")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw18_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw18")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw18_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw18")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw18_uploadrelease(context, msg: str):
...
@op
def nwisgw18_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw18")
    ...
    return msg + r
@op
def nwisgw18_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw18")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw18_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw18")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw18_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw18")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw18"
...
def nwisgw18_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw18"
...
def harvest_nwisgw18():
    harvest = nwisgw18_gleaner()

    report_ms3 = nwisgw18_missingreport_s3(harvest)
    report_idstat = nwisgw18_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw18_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw18")
    load_release = nwisgw18_naburelease(harvest)
    load_uploadrelease = nwisgw18_uploadrelease(load_release)

    load_prune = nwisgw18_nabu_prune(load_uploadrelease)
    load_prov = nwisgw18_nabuprov(load_prune)
    load_org = nwisgw18_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw18_missingreport_graph(load_org)
    report_graph=nwisgw18_graph_reports(report_msgraph)
