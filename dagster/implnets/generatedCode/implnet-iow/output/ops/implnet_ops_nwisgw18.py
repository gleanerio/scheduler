...
def nwisgw18_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "nwisgw18")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def nwisgw18_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "nwisgw18")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw18_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "nwisgw18")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw18_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "nwisgw18")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw18_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "nwisgw18")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw18_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "nwisgw18")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw18_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def nwisgw18_uploadrelease(context):
#     returned_value = post_to_graph("nwisgw18", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def nwisgw18_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nwisgw18"
    )
...
@op(ins={"start": In(Nothing)})
def nwisgw18_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def nwisgw18_upload_summarize(context):
...
def harvest_nwisgw18():
    ...
    # prov branch
    prov_release = nwisgw18_nabu_provrelease(start=harvest)
    prov_clear = nwisgw18_nabu_provclear(start=prov_release)
    prov_object = nwisgw18_nabu_provobject(start=prov_clear)
    prov_drain = nwisgw18_nabu_provdrain(start=prov_object)

    # org branch
    org_release = nwisgw18_nabu_orgsrelease(start=harvest)
    load_org = nwisgw18_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = nwisgw18_summarize(start=load_uploadrelease)
    # upload_summarize = nwisgw18_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = nwisgw18_missingreport_graph(start=summarize)
