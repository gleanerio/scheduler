...
@op
def nwisgw20_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw20")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw20_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw20")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw20_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw20")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw20_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw20")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw20_uploadrelease(context, msg: str):
...
@op
def nwisgw20_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw20")
    ...
    return msg + r
@op
def nwisgw20_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw20")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw20_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw20")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw20_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw20")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw20"
...
def nwisgw20_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw20"
...
def harvest_nwisgw20():
    harvest = nwisgw20_gleaner()

    report_ms3 = nwisgw20_missingreport_s3(harvest)
    report_idstat = nwisgw20_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw20_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw20")
    load_release = nwisgw20_naburelease(harvest)
    load_uploadrelease = nwisgw20_uploadrelease(load_release)

    load_prune = nwisgw20_nabu_prune(load_uploadrelease)
    load_prov = nwisgw20_nabuprov(load_prune)
    load_org = nwisgw20_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw20_missingreport_graph(load_org)
    report_graph=nwisgw20_graph_reports(report_msgraph)
