...
# #########################  Ops Section
@op
def wadewade12_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def wadewade12_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "wadewade12")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade12_naburelease(context):
    returned_value = gleanerio(context, ("release"), "wadewade12")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade12_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wadewade12")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade12_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "wadewade12")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade12_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wadewade12")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade12_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wadewade12")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade12_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wadewade12")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade12_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wadewade12")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade12_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wadewade12")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade12_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "wadewade12")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wadewade12_uploadrelease(context):
#     returned_value = post_to_graph("wadewade12", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wadewade12_uploadrelease(context):
#     returned_value = postRelease("wadewade12")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def wadewade12_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade12"
    )
...
@op(ins={"start": In(Nothing)})
def wadewade12_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade12"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def wadewade12_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade12"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def wadewade12_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade12"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def wadewade12_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wadewade12_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wadewade12_upload_summarize(context):
...
@graph
def harvest_wadewade12():
    # check for containers
    containers = wadewade12_getImage()

    # conduct the harvest
    harvest = wadewade12_gleaner(start=containers)

    # data branch
    load_release = wadewade12_naburelease(start=harvest)
    load_uploadrelease = wadewade12_uploadrelease(start=load_release)
    load_prune = wadewade12_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = wadewade12_nabu_provrelease(start=harvest)
    prov_clear = wadewade12_nabu_provclear(start=prov_release)
    prov_object = wadewade12_nabu_provobject(start=prov_clear)
    prov_drain = wadewade12_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wadewade12_nabu_orgsrelease(start=harvest)
    load_org = wadewade12_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = wadewade12_missingreport_s3(start=harvest)
    # report_idstat = wadewade12_identifier_stats(start=report_ms3)
    # report_bucketurl = wadewade12_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = wadewade12_summarize(start=load_uploadrelease)
    # upload_summarize = wadewade12_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wadewade12_missingreport_graph(start=summarize)
    # report_graph = wadewade12_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="wadewade12"):
#
