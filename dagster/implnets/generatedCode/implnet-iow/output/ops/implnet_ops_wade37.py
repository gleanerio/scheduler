...
@op(ins={"start": In(Nothing)})
def wade37_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade37")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade37_nabuorg(context):
...
def wade37_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade37")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade37")
    ...
    load_prov = wade37_nabuprov(start=load_prune)
    load_org = wade37_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade37_missingreport_graph(start=load_org)
