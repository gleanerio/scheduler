...
def gages1_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "gages1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages1_naburelease(context):
    returned_value = gleanerio(context, ("release"), "gages1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages1_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "gages1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages1_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "gages1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages1_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "gages1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages1_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "gages1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages1_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "gages1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages1_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "gages1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages1_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "gages1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages1_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "gages1")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def gages1_uploadrelease(context):
#     returned_value = post_to_graph("gages1", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def gages1_uploadrelease(context):
#     returned_value = postRelease("gages1")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def gages1_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="gages1"
    )
...
@op(ins={"start": In(Nothing)})
def gages1_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="gages1"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def gages1_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="gages1"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def gages1_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="gages1"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def gages1_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def gages1_upload_summarize(context):
...
def harvest_gages1():
    ...
    containers = gages1_getImage()

    # conduct the harvest
    harvest = gages1_gleaner(start=containers)

    # data branch
    load_release = gages1_naburelease(start=harvest)
    load_uploadrelease = gages1_uploadrelease(start=load_release)
    load_prune = gages1_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = gages1_nabu_provrelease(start=harvest)
    prov_clear = gages1_nabu_provclear(start=prov_release)
    prov_object = gages1_nabu_provobject(start=prov_clear)
    prov_drain = gages1_nabu_provdrain(start=prov_object)

    # org branch
    org_release = gages1_nabu_orgsrelease(start=harvest)
    load_org = gages1_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = gages1_missingreport_s3(start=harvest)
    # report_idstat = gages1_identifier_stats(start=report_ms3)
    # report_bucketurl = gages1_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = gages1_summarize(start=load_uploadrelease)
    # upload_summarize = gages1_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = gages1_missingreport_graph(start=summarize)
    # report_graph = gages1_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="gages1"):
#
