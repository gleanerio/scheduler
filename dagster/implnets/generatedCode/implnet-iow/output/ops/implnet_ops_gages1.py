...
@op(ins={"start": In(Nothing)})
def gages1_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "gages1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages1_nabuorg(context):
...
def gages1_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="gages1")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="gages1")
    ...
    load_prov = gages1_nabuprov(start=load_prune)
    load_org = gages1_nabuorg(start=load_prov)

# run after load
    report_msgraph=gages1_missingreport_graph(start=load_org)
