...
# #########################  Ops Section
@op
def refdamsdams1_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def refdamsdams1_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "refdamsdams1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refdamsdams1_naburelease(context):
    returned_value = gleanerio(context, ("release"), "refdamsdams1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refdamsdams1_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "refdamsdams1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refdamsdams1_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "refdamsdams1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refdamsdams1_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "refdamsdams1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refdamsdams1_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "refdamsdams1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refdamsdams1_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "refdamsdams1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refdamsdams1_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "refdamsdams1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refdamsdams1_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "refdamsdams1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refdamsdams1_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "refdamsdams1")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def refdamsdams1_uploadrelease(context):
#     returned_value = post_to_graph("refdamsdams1", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def refdamsdams1_uploadrelease(context):
#     returned_value = postRelease("refdamsdams1")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def refdamsdams1_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refdamsdams1"
    )
...
@op(ins={"start": In(Nothing)})
def refdamsdams1_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refdamsdams1"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def refdamsdams1_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refdamsdams1"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def refdamsdams1_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refdamsdams1"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def refdamsdams1_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def refdamsdams1_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def refdamsdams1_upload_summarize(context):
...
@graph
def harvest_refdamsdams1():
    # check for containers
    containers = refdamsdams1_getImage()

    # conduct the harvest
    harvest = refdamsdams1_gleaner(start=containers)

    # data branch
    load_release = refdamsdams1_naburelease(start=harvest)
    load_uploadrelease = refdamsdams1_uploadrelease(start=load_release)
    load_prune = refdamsdams1_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = refdamsdams1_nabu_provrelease(start=harvest)
    prov_clear = refdamsdams1_nabu_provclear(start=prov_release)
    prov_object = refdamsdams1_nabu_provobject(start=prov_clear)
    prov_drain = refdamsdams1_nabu_provdrain(start=prov_object)

    # org branch
    org_release = refdamsdams1_nabu_orgsrelease(start=harvest)
    load_org = refdamsdams1_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = refdamsdams1_missingreport_s3(start=harvest)
    # report_idstat = refdamsdams1_identifier_stats(start=report_ms3)
    # report_bucketurl = refdamsdams1_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = refdamsdams1_summarize(start=load_uploadrelease)
    # upload_summarize = refdamsdams1_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = refdamsdams1_missingreport_graph(start=summarize)
    # report_graph = refdamsdams1_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="refdamsdams1"):
#
