...
@op
def cuahsihisredbuttecreekgamutids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisredbuttecreekgamutids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisredbuttecreekgamutids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisredbuttecreekgamutids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisredbuttecreekgamutids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisredbuttecreekgamutids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisredbuttecreekgamutids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisredbuttecreekgamutids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisredbuttecreekgamutids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisredbuttecreekgamutids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisredbuttecreekgamutids0")
    ...
    return msg + r
@op
def cuahsihisredbuttecreekgamutids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisredbuttecreekgamutids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisredbuttecreekgamutids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisredbuttecreekgamutids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisredbuttecreekgamutids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisredbuttecreekgamutids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisredbuttecreekgamutids0"
...
def cuahsihisredbuttecreekgamutids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisredbuttecreekgamutids0"
...
def harvest_cuahsihisredbuttecreekgamutids0():
    harvest = cuahsihisredbuttecreekgamutids0_gleaner()

    report_ms3 = cuahsihisredbuttecreekgamutids0_missingreport_s3(harvest)
    report_idstat = cuahsihisredbuttecreekgamutids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisredbuttecreekgamutids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisredbuttecreekgamutids0")
    load_release = cuahsihisredbuttecreekgamutids0_naburelease(harvest)
    load_uploadrelease = cuahsihisredbuttecreekgamutids0_uploadrelease(load_release)

    load_prune = cuahsihisredbuttecreekgamutids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisredbuttecreekgamutids0_nabuprov(load_prune)
    load_org = cuahsihisredbuttecreekgamutids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisredbuttecreekgamutids0_missingreport_graph(load_org)
    report_graph=cuahsihisredbuttecreekgamutids0_graph_reports(report_msgraph)
