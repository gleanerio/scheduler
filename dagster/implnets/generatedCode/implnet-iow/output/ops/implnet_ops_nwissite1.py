...
@op
def nwissite1_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwissite1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwissite1_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwissite1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwissite1_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwissite1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwissite1_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwissite1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwissite1_uploadrelease(context, msg: str):
...
@op
def nwissite1_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwissite1")
    ...
    return msg + r
@op
def nwissite1_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwissite1")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwissite1_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwissite1")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwissite1_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwissite1")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwissite1"
...
def nwissite1_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwissite1"
...
def harvest_nwissite1():
    harvest = nwissite1_gleaner()

    report_ms3 = nwissite1_missingreport_s3(harvest)
    report_idstat = nwissite1_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwissite1_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwissite1")
    load_release = nwissite1_naburelease(harvest)
    load_uploadrelease = nwissite1_uploadrelease(load_release)

    load_prune = nwissite1_nabu_prune(load_uploadrelease)
    load_prov = nwissite1_nabuprov(load_prune)
    load_org = nwissite1_nabuorg(load_prov)

# run after load
    report_msgraph=nwissite1_missingreport_graph(load_org)
    report_graph=nwissite1_graph_reports(report_msgraph)
