...
# #########################  Ops Section
@op
def cuahsicuahsihisubwpadids0_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def cuahsicuahsihisubwpadids0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "cuahsicuahsihisubwpadids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisubwpadids0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "cuahsicuahsihisubwpadids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisubwpadids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsicuahsihisubwpadids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisubwpadids0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "cuahsicuahsihisubwpadids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisubwpadids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsicuahsihisubwpadids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisubwpadids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsicuahsihisubwpadids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisubwpadids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsicuahsihisubwpadids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisubwpadids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsicuahsihisubwpadids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisubwpadids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsicuahsihisubwpadids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisubwpadids0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "cuahsicuahsihisubwpadids0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihisubwpadids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsicuahsihisubwpadids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihisubwpadids0_uploadrelease(context):
#     returned_value = postRelease("cuahsicuahsihisubwpadids0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisubwpadids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisubwpadids0"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsicuahsihisubwpadids0"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisubwpadids0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisubwpadids0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisubwpadids0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisubwpadids0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisubwpadids0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisubwpadids0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisubwpadids0_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisubwpadids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisubwpadids0_upload_summarize(context):
    returned_value = post_to_graph(
        "cuahsicuahsihisubwpadids0",
        path=SUMMARY_PATH,
        extension="ttl",
...
@graph
def harvest_cuahsicuahsihisubwpadids0():
    # check for containers
    containers = cuahsicuahsihisubwpadids0_getImage()

    # conduct the harvest
    harvest = cuahsicuahsihisubwpadids0_gleaner(start=containers)

    # data branch
    load_release = cuahsicuahsihisubwpadids0_naburelease(start=harvest)
    load_uploadrelease = cuahsicuahsihisubwpadids0_uploadrelease(start=load_release)
    load_prune = cuahsicuahsihisubwpadids0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = cuahsicuahsihisubwpadids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsicuahsihisubwpadids0_nabu_provclear(start=prov_release)
    prov_object = cuahsicuahsihisubwpadids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsicuahsihisubwpadids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsicuahsihisubwpadids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsicuahsihisubwpadids0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = cuahsicuahsihisubwpadids0_missingreport_s3(start=harvest)
    # report_idstat = cuahsicuahsihisubwpadids0_identifier_stats(start=report_ms3)
    # report_bucketurl = cuahsicuahsihisubwpadids0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = cuahsicuahsihisubwpadids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsicuahsihisubwpadids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsicuahsihisubwpadids0_missingreport_graph(start=summarize)
    # report_graph = cuahsicuahsihisubwpadids0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="cuahsicuahsihisubwpadids0"):
#
