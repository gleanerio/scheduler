...
@op
def cuahsihisnevcanids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisnevcanids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisnevcanids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisnevcanids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisnevcanids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisnevcanids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisnevcanids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisnevcanids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisnevcanids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisnevcanids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisnevcanids0")
    ...
    return msg + r
@op
def cuahsihisnevcanids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisnevcanids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisnevcanids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisnevcanids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisnevcanids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisnevcanids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisnevcanids0"
...
def cuahsihisnevcanids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisnevcanids0"
...
def harvest_cuahsihisnevcanids0():
    harvest = cuahsihisnevcanids0_gleaner()

    report_ms3 = cuahsihisnevcanids0_missingreport_s3(harvest)
    report_idstat = cuahsihisnevcanids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisnevcanids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisnevcanids0")
    load_release = cuahsihisnevcanids0_naburelease(harvest)
    load_uploadrelease = cuahsihisnevcanids0_uploadrelease(load_release)

    load_prune = cuahsihisnevcanids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisnevcanids0_nabuprov(load_prune)
    load_org = cuahsihisnevcanids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisnevcanids0_missingreport_graph(load_org)
    report_graph=cuahsihisnevcanids0_graph_reports(report_msgraph)
