...
def ndwr0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "ndwr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ndwr0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "ndwr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ndwr0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "ndwr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ndwr0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "ndwr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ndwr0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "ndwr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ndwr0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "ndwr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ndwr0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "ndwr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ndwr0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "ndwr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ndwr0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "ndwr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ndwr0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "ndwr0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def ndwr0_uploadrelease(context):
#     returned_value = post_to_graph("ndwr0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def ndwr0_uploadrelease(context):
#     returned_value = postRelease("ndwr0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def ndwr0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="ndwr0"
    )
...
@op(ins={"start": In(Nothing)})
def ndwr0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="ndwr0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def ndwr0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="ndwr0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def ndwr0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="ndwr0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def ndwr0_upload_summarize(context):
...
def harvest_ndwr0():
    ...
    containers = ndwr0_getImage()

    # conduct the harvest
    harvest = ndwr0_gleaner(start=containers)

    # data branch
    load_release = ndwr0_naburelease(start=harvest)
    load_uploadrelease = ndwr0_uploadrelease(start=load_release)
    load_prune = ndwr0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = ndwr0_nabu_provrelease(start=harvest)
    prov_clear = ndwr0_nabu_provclear(start=prov_release)
    prov_object = ndwr0_nabu_provobject(start=prov_clear)
    prov_drain = ndwr0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = ndwr0_nabu_orgsrelease(start=harvest)
    load_org = ndwr0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = ndwr0_missingreport_s3(start=harvest)
    # report_idstat = ndwr0_identifier_stats(start=report_ms3)
    # report_bucketurl = ndwr0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = ndwr0_summarize(start=load_uploadrelease)
    # upload_summarize = ndwr0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = ndwr0_missingreport_graph(start=summarize)
    # report_graph = ndwr0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="ndwr0"):
#
