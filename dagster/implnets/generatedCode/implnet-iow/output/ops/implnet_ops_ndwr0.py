...
@op(ins={"start": In(Nothing)})
def ndwr0_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "ndwr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def ndwr0_nabuorg(context):
...
def ndwr0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="ndwr0")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="ndwr0")
    ...
    load_prov = ndwr0_nabuprov(start=load_prune)
    load_org = ndwr0_nabuorg(start=load_prov)

# run after load
    report_msgraph=ndwr0_missingreport_graph(start=load_org)
