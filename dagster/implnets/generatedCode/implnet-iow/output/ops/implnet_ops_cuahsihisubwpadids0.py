...
@op
def cuahsihisubwpadids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisubwpadids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisubwpadids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisubwpadids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisubwpadids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisubwpadids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisubwpadids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisubwpadids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisubwpadids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisubwpadids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisubwpadids0")
    ...
    return msg + r
@op
def cuahsihisubwpadids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisubwpadids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisubwpadids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisubwpadids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisubwpadids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisubwpadids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisubwpadids0"
...
def cuahsihisubwpadids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisubwpadids0"
...
def harvest_cuahsihisubwpadids0():
    harvest = cuahsihisubwpadids0_gleaner()

    report_ms3 = cuahsihisubwpadids0_missingreport_s3(harvest)
    report_idstat = cuahsihisubwpadids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisubwpadids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisubwpadids0")
    load_release = cuahsihisubwpadids0_naburelease(harvest)
    load_uploadrelease = cuahsihisubwpadids0_uploadrelease(load_release)

    load_prune = cuahsihisubwpadids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisubwpadids0_nabuprov(load_prune)
    load_org = cuahsihisubwpadids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisubwpadids0_missingreport_graph(load_org)
    report_graph=cuahsihisubwpadids0_graph_reports(report_msgraph)
