...
def cuahsihisubwpadids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsihisubwpadids0")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def cuahsihisubwpadids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsihisubwpadids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisubwpadids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsihisubwpadids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisubwpadids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsihisubwpadids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisubwpadids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsihisubwpadids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisubwpadids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsihisubwpadids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisubwpadids0_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def cuahsihisubwpadids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsihisubwpadids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def cuahsihisubwpadids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsihisubwpadids0"
    )
...
@op(ins={"start": In(Nothing)})
def cuahsihisubwpadids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsihisubwpadids0_upload_summarize(context):
...
def harvest_cuahsihisubwpadids0():
    ...
    # prov branch
    prov_release = cuahsihisubwpadids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsihisubwpadids0_nabu_provclear(start=prov_release)
    prov_object = cuahsihisubwpadids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsihisubwpadids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsihisubwpadids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsihisubwpadids0_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = cuahsihisubwpadids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsihisubwpadids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsihisubwpadids0_missingreport_graph(start=summarize)
