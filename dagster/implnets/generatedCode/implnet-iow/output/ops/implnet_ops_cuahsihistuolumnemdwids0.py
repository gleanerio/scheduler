...
@op
def cuahsihistuolumnemdwids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihistuolumnemdwids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihistuolumnemdwids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihistuolumnemdwids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihistuolumnemdwids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihistuolumnemdwids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihistuolumnemdwids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihistuolumnemdwids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihistuolumnemdwids0_uploadrelease(context, msg: str):
...
@op
def cuahsihistuolumnemdwids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihistuolumnemdwids0")
    ...
    return msg + r
@op
def cuahsihistuolumnemdwids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihistuolumnemdwids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihistuolumnemdwids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihistuolumnemdwids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihistuolumnemdwids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihistuolumnemdwids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihistuolumnemdwids0"
...
def cuahsihistuolumnemdwids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihistuolumnemdwids0"
...
def harvest_cuahsihistuolumnemdwids0():
    harvest = cuahsihistuolumnemdwids0_gleaner()

    report_ms3 = cuahsihistuolumnemdwids0_missingreport_s3(harvest)
    report_idstat = cuahsihistuolumnemdwids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihistuolumnemdwids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihistuolumnemdwids0")
    load_release = cuahsihistuolumnemdwids0_naburelease(harvest)
    load_uploadrelease = cuahsihistuolumnemdwids0_uploadrelease(load_release)

    load_prune = cuahsihistuolumnemdwids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihistuolumnemdwids0_nabuprov(load_prune)
    load_org = cuahsihistuolumnemdwids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihistuolumnemdwids0_missingreport_graph(load_org)
    report_graph=cuahsihistuolumnemdwids0_graph_reports(report_msgraph)
