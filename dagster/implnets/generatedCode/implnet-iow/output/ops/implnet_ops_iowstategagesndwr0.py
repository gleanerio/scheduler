...
# #########################  Ops Section
@op
def iowstategagesndwr0_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def iowstategagesndwr0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "iowstategagesndwr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowstategagesndwr0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "iowstategagesndwr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowstategagesndwr0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "iowstategagesndwr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowstategagesndwr0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "iowstategagesndwr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowstategagesndwr0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "iowstategagesndwr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowstategagesndwr0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "iowstategagesndwr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowstategagesndwr0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "iowstategagesndwr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowstategagesndwr0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "iowstategagesndwr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowstategagesndwr0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "iowstategagesndwr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowstategagesndwr0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "iowstategagesndwr0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def iowstategagesndwr0_uploadrelease(context):
#     returned_value = post_to_graph("iowstategagesndwr0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def iowstategagesndwr0_uploadrelease(context):
#     returned_value = postRelease("iowstategagesndwr0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def iowstategagesndwr0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="iowstategagesndwr0"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "iowstategagesndwr0"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def iowstategagesndwr0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="iowstategagesndwr0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def iowstategagesndwr0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="iowstategagesndwr0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def iowstategagesndwr0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="iowstategagesndwr0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def iowstategagesndwr0_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def iowstategagesndwr0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def iowstategagesndwr0_upload_summarize(context):
...
@graph
def harvest_iowstategagesndwr0():
    # check for containers
    containers = iowstategagesndwr0_getImage()

    # conduct the harvest
    harvest = iowstategagesndwr0_gleaner(start=containers)

    # data branch
    load_release = iowstategagesndwr0_naburelease(start=harvest)
    load_uploadrelease = iowstategagesndwr0_uploadrelease(start=load_release)
    load_prune = iowstategagesndwr0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = iowstategagesndwr0_nabu_provrelease(start=harvest)
    prov_clear = iowstategagesndwr0_nabu_provclear(start=prov_release)
    prov_object = iowstategagesndwr0_nabu_provobject(start=prov_clear)
    prov_drain = iowstategagesndwr0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = iowstategagesndwr0_nabu_orgsrelease(start=harvest)
    load_org = iowstategagesndwr0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = iowstategagesndwr0_missingreport_s3(start=harvest)
    # report_idstat = iowstategagesndwr0_identifier_stats(start=report_ms3)
    # report_bucketurl = iowstategagesndwr0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = iowstategagesndwr0_summarize(start=load_uploadrelease)
    # upload_summarize = iowstategagesndwr0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = iowstategagesndwr0_missingreport_graph(start=summarize)
    # report_graph = iowstategagesndwr0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="iowstategagesndwr0"):
#
