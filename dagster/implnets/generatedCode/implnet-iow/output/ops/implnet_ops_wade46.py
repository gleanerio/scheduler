...
@op(ins={"start": In(Nothing)})
def wade46_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade46")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade46_nabuorg(context):
...
def wade46_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade46")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade46")
    ...
    load_prov = wade46_nabuprov(start=load_prune)
    load_org = wade46_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade46_missingreport_graph(start=load_org)
