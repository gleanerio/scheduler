...
@op
def dams0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "dams0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def dams0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "dams0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def dams0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "dams0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def dams0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "dams0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def dams0_uploadrelease(context, msg: str):
...
@op
def dams0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="dams0")
    ...
    return msg + r
@op
def dams0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="dams0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def dams0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="dams0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def dams0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="dams0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "dams0"
...
def dams0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "dams0"
...
@graph
def harvest_dams0():
    harvest = dams0_gleaner()

    report_ms3 = dams0_missingreport_s3(harvest)
    report_idstat = dams0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = dams0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="dams0")
    load_release = dams0_naburelease(harvest)
    load_uploadrelease = dams0_uploadrelease(load_release)

    load_prune = dams0_nabu_prune(load_uploadrelease)
    load_prov = dams0_nabuprov(load_prune)
    load_org = dams0_nabuorg(load_prov)

# run after load
    report_msgraph=dams0_missingreport_graph(load_org)
    report_graph=dams0_graph_reports(report_msgraph)
