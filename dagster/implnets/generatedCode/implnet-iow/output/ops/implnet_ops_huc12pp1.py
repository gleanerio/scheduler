...
def huc12pp1_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "huc12pp1")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def huc12pp1_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "huc12pp1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def huc12pp1_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "huc12pp1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def huc12pp1_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "huc12pp1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def huc12pp1_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "huc12pp1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def huc12pp1_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "huc12pp1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def huc12pp1_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def huc12pp1_uploadrelease(context):
#     returned_value = post_to_graph("huc12pp1", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def huc12pp1_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="huc12pp1"
    )
...
@op(ins={"start": In(Nothing)})
def huc12pp1_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def huc12pp1_upload_summarize(context):
...
def harvest_huc12pp1():
    ...
    # prov branch
    prov_release = huc12pp1_nabu_provrelease(start=harvest)
    prov_clear = huc12pp1_nabu_provclear(start=prov_release)
    prov_object = huc12pp1_nabu_provobject(start=prov_clear)
    prov_drain = huc12pp1_nabu_provdrain(start=prov_object)

    # org branch
    org_release = huc12pp1_nabu_orgsrelease(start=harvest)
    load_org = huc12pp1_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = huc12pp1_summarize(start=load_uploadrelease)
    # upload_summarize = huc12pp1_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = huc12pp1_missingreport_graph(start=summarize)
