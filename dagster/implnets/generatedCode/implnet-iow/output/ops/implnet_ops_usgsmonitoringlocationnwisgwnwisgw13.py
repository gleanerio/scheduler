...
# #########################  Ops Section
@op
def usgsmonitoringlocationnwisgwnwisgw13_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def usgsmonitoringlocationnwisgwnwisgw13_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "usgsmonitoringlocationnwisgwnwisgw13")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw13_naburelease(context):
    returned_value = gleanerio(context, ("release"), "usgsmonitoringlocationnwisgwnwisgw13")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw13_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "usgsmonitoringlocationnwisgwnwisgw13")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw13_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "usgsmonitoringlocationnwisgwnwisgw13")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw13_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "usgsmonitoringlocationnwisgwnwisgw13")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw13_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "usgsmonitoringlocationnwisgwnwisgw13")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw13_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "usgsmonitoringlocationnwisgwnwisgw13")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw13_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "usgsmonitoringlocationnwisgwnwisgw13")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw13_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "usgsmonitoringlocationnwisgwnwisgw13")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw13_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "usgsmonitoringlocationnwisgwnwisgw13")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def usgsmonitoringlocationnwisgwnwisgw13_uploadrelease(context):
#     returned_value = post_to_graph("usgsmonitoringlocationnwisgwnwisgw13", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def usgsmonitoringlocationnwisgwnwisgw13_uploadrelease(context):
#     returned_value = postRelease("usgsmonitoringlocationnwisgwnwisgw13")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw13_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="usgsmonitoringlocationnwisgwnwisgw13"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "usgsmonitoringlocationnwisgwnwisgw13"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw13_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="usgsmonitoringlocationnwisgwnwisgw13"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw13_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="usgsmonitoringlocationnwisgwnwisgw13"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw13_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="usgsmonitoringlocationnwisgwnwisgw13"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw13_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw13_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw13_upload_summarize(context):
    returned_value = post_to_graph(
        "usgsmonitoringlocationnwisgwnwisgw13",
        path=SUMMARY_PATH,
        extension="ttl",
...
@graph
def harvest_usgsmonitoringlocationnwisgwnwisgw13():
    # check for containers
    containers = usgsmonitoringlocationnwisgwnwisgw13_getImage()

    # conduct the harvest
    harvest = usgsmonitoringlocationnwisgwnwisgw13_gleaner(start=containers)

    # data branch
    load_release = usgsmonitoringlocationnwisgwnwisgw13_naburelease(start=harvest)
    load_uploadrelease = usgsmonitoringlocationnwisgwnwisgw13_uploadrelease(start=load_release)
    load_prune = usgsmonitoringlocationnwisgwnwisgw13_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = usgsmonitoringlocationnwisgwnwisgw13_nabu_provrelease(start=harvest)
    prov_clear = usgsmonitoringlocationnwisgwnwisgw13_nabu_provclear(start=prov_release)
    prov_object = usgsmonitoringlocationnwisgwnwisgw13_nabu_provobject(start=prov_clear)
    prov_drain = usgsmonitoringlocationnwisgwnwisgw13_nabu_provdrain(start=prov_object)

    # org branch
    org_release = usgsmonitoringlocationnwisgwnwisgw13_nabu_orgsrelease(start=harvest)
    load_org = usgsmonitoringlocationnwisgwnwisgw13_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = usgsmonitoringlocationnwisgwnwisgw13_missingreport_s3(start=harvest)
    # report_idstat = usgsmonitoringlocationnwisgwnwisgw13_identifier_stats(start=report_ms3)
    # report_bucketurl = usgsmonitoringlocationnwisgwnwisgw13_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = usgsmonitoringlocationnwisgwnwisgw13_summarize(start=load_uploadrelease)
    # upload_summarize = usgsmonitoringlocationnwisgwnwisgw13_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = usgsmonitoringlocationnwisgwnwisgw13_missingreport_graph(start=summarize)
    # report_graph = usgsmonitoringlocationnwisgwnwisgw13_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="usgsmonitoringlocationnwisgwnwisgw13"):
#
