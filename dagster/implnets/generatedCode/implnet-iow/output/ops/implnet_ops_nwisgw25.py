...
def nwisgw25_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "nwisgw25")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def nwisgw25_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "nwisgw25")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw25_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "nwisgw25")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw25_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "nwisgw25")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw25_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "nwisgw25")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw25_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "nwisgw25")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw25_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def nwisgw25_uploadrelease(context):
#     returned_value = post_to_graph("nwisgw25", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def nwisgw25_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nwisgw25"
    )
...
@op(ins={"start": In(Nothing)})
def nwisgw25_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def nwisgw25_upload_summarize(context):
...
def harvest_nwisgw25():
    ...
    # prov branch
    prov_release = nwisgw25_nabu_provrelease(start=harvest)
    prov_clear = nwisgw25_nabu_provclear(start=prov_release)
    prov_object = nwisgw25_nabu_provobject(start=prov_clear)
    prov_drain = nwisgw25_nabu_provdrain(start=prov_object)

    # org branch
    org_release = nwisgw25_nabu_orgsrelease(start=harvest)
    load_org = nwisgw25_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = nwisgw25_summarize(start=load_uploadrelease)
    # upload_summarize = nwisgw25_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = nwisgw25_missingreport_graph(start=summarize)
