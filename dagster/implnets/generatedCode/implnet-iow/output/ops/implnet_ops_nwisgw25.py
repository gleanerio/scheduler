...
@op
def nwisgw25_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw25")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw25_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw25")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw25_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw25")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw25_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw25")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw25_uploadrelease(context, msg: str):
...
@op
def nwisgw25_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw25")
    ...
    return msg + r
@op
def nwisgw25_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw25")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw25_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw25")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw25_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw25")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw25"
...
def nwisgw25_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw25"
...
def harvest_nwisgw25():
    harvest = nwisgw25_gleaner()

    report_ms3 = nwisgw25_missingreport_s3(harvest)
    report_idstat = nwisgw25_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw25_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw25")
    load_release = nwisgw25_naburelease(harvest)
    load_uploadrelease = nwisgw25_uploadrelease(load_release)

    load_prune = nwisgw25_nabu_prune(load_uploadrelease)
    load_prov = nwisgw25_nabuprov(load_prune)
    load_org = nwisgw25_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw25_missingreport_graph(load_org)
    report_graph=nwisgw25_graph_reports(report_msgraph)
