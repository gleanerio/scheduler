...
# #########################  Ops Section
@op
def cuahsicuahsihismobilecrowdhydrologyids0_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def cuahsicuahsihismobilecrowdhydrologyids0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "cuahsicuahsihismobilecrowdhydrologyids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihismobilecrowdhydrologyids0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "cuahsicuahsihismobilecrowdhydrologyids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihismobilecrowdhydrologyids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsicuahsihismobilecrowdhydrologyids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihismobilecrowdhydrologyids0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "cuahsicuahsihismobilecrowdhydrologyids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihismobilecrowdhydrologyids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsicuahsihismobilecrowdhydrologyids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihismobilecrowdhydrologyids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsicuahsihismobilecrowdhydrologyids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihismobilecrowdhydrologyids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsicuahsihismobilecrowdhydrologyids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihismobilecrowdhydrologyids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsicuahsihismobilecrowdhydrologyids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihismobilecrowdhydrologyids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsicuahsihismobilecrowdhydrologyids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihismobilecrowdhydrologyids0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "cuahsicuahsihismobilecrowdhydrologyids0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihismobilecrowdhydrologyids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsicuahsihismobilecrowdhydrologyids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihismobilecrowdhydrologyids0_uploadrelease(context):
#     returned_value = postRelease("cuahsicuahsihismobilecrowdhydrologyids0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihismobilecrowdhydrologyids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihismobilecrowdhydrologyids0"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsicuahsihismobilecrowdhydrologyids0"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihismobilecrowdhydrologyids0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihismobilecrowdhydrologyids0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihismobilecrowdhydrologyids0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihismobilecrowdhydrologyids0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihismobilecrowdhydrologyids0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihismobilecrowdhydrologyids0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihismobilecrowdhydrologyids0_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihismobilecrowdhydrologyids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihismobilecrowdhydrologyids0_upload_summarize(context):
    returned_value = post_to_graph(
        "cuahsicuahsihismobilecrowdhydrologyids0",
        path=SUMMARY_PATH,
        extension="ttl",
...
@graph
def harvest_cuahsicuahsihismobilecrowdhydrologyids0():
    # check for containers
    containers = cuahsicuahsihismobilecrowdhydrologyids0_getImage()

    # conduct the harvest
    harvest = cuahsicuahsihismobilecrowdhydrologyids0_gleaner(start=containers)

    # data branch
    load_release = cuahsicuahsihismobilecrowdhydrologyids0_naburelease(start=harvest)
    load_uploadrelease = cuahsicuahsihismobilecrowdhydrologyids0_uploadrelease(start=load_release)
    load_prune = cuahsicuahsihismobilecrowdhydrologyids0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = cuahsicuahsihismobilecrowdhydrologyids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsicuahsihismobilecrowdhydrologyids0_nabu_provclear(start=prov_release)
    prov_object = cuahsicuahsihismobilecrowdhydrologyids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsicuahsihismobilecrowdhydrologyids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsicuahsihismobilecrowdhydrologyids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsicuahsihismobilecrowdhydrologyids0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = cuahsicuahsihismobilecrowdhydrologyids0_missingreport_s3(start=harvest)
    # report_idstat = cuahsicuahsihismobilecrowdhydrologyids0_identifier_stats(start=report_ms3)
    # report_bucketurl = cuahsicuahsihismobilecrowdhydrologyids0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = cuahsicuahsihismobilecrowdhydrologyids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsicuahsihismobilecrowdhydrologyids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsicuahsihismobilecrowdhydrologyids0_missingreport_graph(start=summarize)
    # report_graph = cuahsicuahsihismobilecrowdhydrologyids0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="cuahsicuahsihismobilecrowdhydrologyids0"):
#
