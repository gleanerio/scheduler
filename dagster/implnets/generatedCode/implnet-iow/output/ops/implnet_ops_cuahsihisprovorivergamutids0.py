...
@op
def cuahsihisprovorivergamutids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisprovorivergamutids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisprovorivergamutids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisprovorivergamutids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisprovorivergamutids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisprovorivergamutids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisprovorivergamutids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisprovorivergamutids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisprovorivergamutids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisprovorivergamutids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisprovorivergamutids0")
    ...
    return msg + r
@op
def cuahsihisprovorivergamutids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisprovorivergamutids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisprovorivergamutids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisprovorivergamutids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisprovorivergamutids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisprovorivergamutids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisprovorivergamutids0"
...
def cuahsihisprovorivergamutids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisprovorivergamutids0"
...
def harvest_cuahsihisprovorivergamutids0():
    harvest = cuahsihisprovorivergamutids0_gleaner()

    report_ms3 = cuahsihisprovorivergamutids0_missingreport_s3(harvest)
    report_idstat = cuahsihisprovorivergamutids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisprovorivergamutids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisprovorivergamutids0")
    load_release = cuahsihisprovorivergamutids0_naburelease(harvest)
    load_uploadrelease = cuahsihisprovorivergamutids0_uploadrelease(load_release)

    load_prune = cuahsihisprovorivergamutids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisprovorivergamutids0_nabuprov(load_prune)
    load_org = cuahsihisprovorivergamutids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisprovorivergamutids0_missingreport_graph(load_org)
    report_graph=cuahsihisprovorivergamutids0_graph_reports(report_msgraph)
