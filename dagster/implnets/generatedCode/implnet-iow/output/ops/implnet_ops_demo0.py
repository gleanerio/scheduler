...
def demo0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "demo0")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def demo0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "demo0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def demo0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "demo0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def demo0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "demo0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def demo0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "demo0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def demo0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "demo0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def demo0_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def demo0_uploadrelease(context):
#     returned_value = post_to_graph("demo0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def demo0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="demo0"
    )
...
@op(ins={"start": In(Nothing)})
def demo0_upload_summarize(context):
...
def harvest_demo0():
    ...
    # prov branch
    prov_release = demo0_nabu_provrelease(start=harvest)
    prov_clear = demo0_nabu_provclear(start=prov_release)
    prov_object = demo0_nabu_provobject(start=prov_clear)
    prov_drain = demo0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = demo0_nabu_orgsrelease(start=harvest)
    load_org = demo0_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = demo0_summarize(start=load_uploadrelease)
    # upload_summarize = demo0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = demo0_missingreport_graph(start=summarize)
