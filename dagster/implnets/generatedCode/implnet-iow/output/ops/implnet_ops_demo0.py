...
@op
def demo0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "demo0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def demo0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "demo0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def demo0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "demo0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def demo0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "demo0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def demo0_uploadrelease(context, msg: str):
...
@op
def demo0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="demo0")
    ...
    return msg + r
@op
def demo0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="demo0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def demo0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="demo0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def demo0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="demo0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "demo0"
...
def demo0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "demo0"
...
@graph
def harvest_demo0():
    harvest = demo0_gleaner()

    report_ms3 = demo0_missingreport_s3(harvest)
    report_idstat = demo0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = demo0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="demo0")
    load_release = demo0_naburelease(harvest)
    load_uploadrelease = demo0_uploadrelease(load_release)

    load_prune = demo0_nabu_prune(load_uploadrelease)
    load_prov = demo0_nabuprov(load_prune)
    load_org = demo0_nabuorg(load_prov)

# run after load
    report_msgraph=demo0_missingreport_graph(load_org)
    report_graph=demo0_graph_reports(report_msgraph)
