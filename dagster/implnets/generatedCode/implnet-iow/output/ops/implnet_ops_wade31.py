...
@op(ins={"start": In(Nothing)})
def wade31_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade31")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade31_nabuorg(context):
...
def wade31_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade31")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade31")
    ...
    load_prov = wade31_nabuprov(start=load_prune)
    load_org = wade31_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade31_missingreport_graph(start=load_org)
