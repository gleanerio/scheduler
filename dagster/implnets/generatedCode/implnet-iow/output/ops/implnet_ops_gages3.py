...
@op(ins={"start": In(Nothing)})
def gages3_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "gages3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages3_nabuorg(context):
...
def gages3_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="gages3")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="gages3")
    ...
    load_prov = gages3_nabuprov(start=load_prune)
    load_org = gages3_nabuorg(start=load_prov)

# run after load
    report_msgraph=gages3_missingreport_graph(start=load_org)
