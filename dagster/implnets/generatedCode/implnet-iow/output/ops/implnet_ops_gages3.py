...
def gages3_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "gages3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages3_naburelease(context):
    returned_value = gleanerio(context, ("release"), "gages3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages3_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "gages3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages3_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "gages3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages3_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "gages3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages3_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "gages3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages3_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "gages3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages3_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "gages3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages3_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "gages3")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages3_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "gages3")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def gages3_uploadrelease(context):
#     returned_value = post_to_graph("gages3", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def gages3_uploadrelease(context):
#     returned_value = postRelease("gages3")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def gages3_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="gages3"
    )
...
@op(ins={"start": In(Nothing)})
def gages3_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="gages3"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def gages3_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="gages3"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def gages3_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="gages3"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def gages3_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def gages3_upload_summarize(context):
...
def harvest_gages3():
    ...
    containers = gages3_getImage()

    # conduct the harvest
    harvest = gages3_gleaner(start=containers)

    # data branch
    load_release = gages3_naburelease(start=harvest)
    load_uploadrelease = gages3_uploadrelease(start=load_release)
    load_prune = gages3_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = gages3_nabu_provrelease(start=harvest)
    prov_clear = gages3_nabu_provclear(start=prov_release)
    prov_object = gages3_nabu_provobject(start=prov_clear)
    prov_drain = gages3_nabu_provdrain(start=prov_object)

    # org branch
    org_release = gages3_nabu_orgsrelease(start=harvest)
    load_org = gages3_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = gages3_missingreport_s3(start=harvest)
    # report_idstat = gages3_identifier_stats(start=report_ms3)
    # report_bucketurl = gages3_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = gages3_summarize(start=load_uploadrelease)
    # upload_summarize = gages3_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = gages3_missingreport_graph(start=summarize)
    # report_graph = gages3_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="gages3"):
#
