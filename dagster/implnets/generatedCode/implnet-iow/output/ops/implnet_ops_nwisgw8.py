...
@op
def nwisgw8_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw8")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw8_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw8")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw8_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw8")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw8_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw8")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw8_uploadrelease(context, msg: str):
...
@op
def nwisgw8_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw8")
    ...
    return msg + r
@op
def nwisgw8_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw8")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw8_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw8")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw8_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw8")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw8"
...
def nwisgw8_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw8"
...
@graph
def harvest_nwisgw8():
    harvest = nwisgw8_gleaner()

    report_ms3 = nwisgw8_missingreport_s3(harvest)
    report_idstat = nwisgw8_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw8_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw8")
    load_release = nwisgw8_naburelease(harvest)
    load_uploadrelease = nwisgw8_uploadrelease(load_release)

    load_prune = nwisgw8_nabu_prune(load_uploadrelease)
    load_prov = nwisgw8_nabuprov(load_prune)
    load_org = nwisgw8_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw8_missingreport_graph(load_org)
    report_graph=nwisgw8_graph_reports(report_msgraph)
