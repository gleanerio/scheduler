...
def nwisgw8_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "nwisgw8")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def nwisgw8_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "nwisgw8")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw8_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "nwisgw8")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw8_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "nwisgw8")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw8_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "nwisgw8")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw8_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "nwisgw8")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw8_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def nwisgw8_uploadrelease(context):
#     returned_value = post_to_graph("nwisgw8", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def nwisgw8_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nwisgw8"
    )
...
@op(ins={"start": In(Nothing)})
def nwisgw8_upload_summarize(context):
...
def harvest_nwisgw8():
    ...
    # prov branch
    prov_release = nwisgw8_nabu_provrelease(start=harvest)
    prov_clear = nwisgw8_nabu_provclear(start=prov_release)
    prov_object = nwisgw8_nabu_provobject(start=prov_clear)
    prov_drain = nwisgw8_nabu_provdrain(start=prov_object)

    # org branch
    org_release = nwisgw8_nabu_orgsrelease(start=harvest)
    load_org = nwisgw8_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = nwisgw8_summarize(start=load_uploadrelease)
    # upload_summarize = nwisgw8_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = nwisgw8_missingreport_graph(start=summarize)
