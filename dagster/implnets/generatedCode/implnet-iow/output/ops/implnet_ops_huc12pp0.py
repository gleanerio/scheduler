...
@op
def huc12pp0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "huc12pp0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def huc12pp0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "huc12pp0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def huc12pp0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "huc12pp0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def huc12pp0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "huc12pp0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def huc12pp0_uploadrelease(context, msg: str):
...
@op
def huc12pp0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="huc12pp0")
    ...
    return msg + r
@op
def huc12pp0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="huc12pp0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def huc12pp0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="huc12pp0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def huc12pp0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="huc12pp0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "huc12pp0"
...
def huc12pp0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "huc12pp0"
...
def harvest_huc12pp0():
    harvest = huc12pp0_gleaner()

    report_ms3 = huc12pp0_missingreport_s3(harvest)
    report_idstat = huc12pp0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = huc12pp0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="huc12pp0")
    load_release = huc12pp0_naburelease(harvest)
    load_uploadrelease = huc12pp0_uploadrelease(load_release)

    load_prune = huc12pp0_nabu_prune(load_uploadrelease)
    load_prov = huc12pp0_nabuprov(load_prune)
    load_org = huc12pp0_nabuorg(load_prov)

# run after load
    report_msgraph=huc12pp0_missingreport_graph(load_org)
    report_graph=huc12pp0_graph_reports(report_msgraph)
