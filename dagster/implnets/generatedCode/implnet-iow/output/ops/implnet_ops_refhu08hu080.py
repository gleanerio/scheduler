...
# #########################  Ops Section
@op
def refhu08hu080_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def refhu08hu080_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "refhu08hu080")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu08hu080_naburelease(context):
    returned_value = gleanerio(context, ("release"), "refhu08hu080")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu08hu080_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "refhu08hu080")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu08hu080_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "refhu08hu080")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu08hu080_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "refhu08hu080")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu08hu080_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "refhu08hu080")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu08hu080_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "refhu08hu080")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu08hu080_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "refhu08hu080")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu08hu080_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "refhu08hu080")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu08hu080_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "refhu08hu080")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def refhu08hu080_uploadrelease(context):
#     returned_value = post_to_graph("refhu08hu080", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def refhu08hu080_uploadrelease(context):
#     returned_value = postRelease("refhu08hu080")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def refhu08hu080_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refhu08hu080"
    )
...
@op(ins={"start": In(Nothing)})
def refhu08hu080_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refhu08hu080"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def refhu08hu080_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refhu08hu080"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def refhu08hu080_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refhu08hu080"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def refhu08hu080_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def refhu08hu080_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def refhu08hu080_upload_summarize(context):
...
@graph
def harvest_refhu08hu080():
    # check for containers
    containers = refhu08hu080_getImage()

    # conduct the harvest
    harvest = refhu08hu080_gleaner(start=containers)

    # data branch
    load_release = refhu08hu080_naburelease(start=harvest)
    load_uploadrelease = refhu08hu080_uploadrelease(start=load_release)
    load_prune = refhu08hu080_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = refhu08hu080_nabu_provrelease(start=harvest)
    prov_clear = refhu08hu080_nabu_provclear(start=prov_release)
    prov_object = refhu08hu080_nabu_provobject(start=prov_clear)
    prov_drain = refhu08hu080_nabu_provdrain(start=prov_object)

    # org branch
    org_release = refhu08hu080_nabu_orgsrelease(start=harvest)
    load_org = refhu08hu080_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = refhu08hu080_missingreport_s3(start=harvest)
    # report_idstat = refhu08hu080_identifier_stats(start=report_ms3)
    # report_bucketurl = refhu08hu080_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = refhu08hu080_summarize(start=load_uploadrelease)
    # upload_summarize = refhu08hu080_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = refhu08hu080_missingreport_graph(start=summarize)
    # report_graph = refhu08hu080_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="refhu08hu080"):
#
