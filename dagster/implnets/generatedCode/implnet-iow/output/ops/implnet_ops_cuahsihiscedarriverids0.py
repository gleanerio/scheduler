...
@op
def cuahsihiscedarriverids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihiscedarriverids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihiscedarriverids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihiscedarriverids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihiscedarriverids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihiscedarriverids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihiscedarriverids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihiscedarriverids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihiscedarriverids0_uploadrelease(context, msg: str):
...
@op
def cuahsihiscedarriverids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihiscedarriverids0")
    ...
    return msg + r
@op
def cuahsihiscedarriverids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihiscedarriverids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihiscedarriverids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihiscedarriverids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihiscedarriverids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihiscedarriverids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihiscedarriverids0"
...
def cuahsihiscedarriverids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihiscedarriverids0"
...
def harvest_cuahsihiscedarriverids0():
    harvest = cuahsihiscedarriverids0_gleaner()

    report_ms3 = cuahsihiscedarriverids0_missingreport_s3(harvest)
    report_idstat = cuahsihiscedarriverids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihiscedarriverids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihiscedarriverids0")
    load_release = cuahsihiscedarriverids0_naburelease(harvest)
    load_uploadrelease = cuahsihiscedarriverids0_uploadrelease(load_release)

    load_prune = cuahsihiscedarriverids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihiscedarriverids0_nabuprov(load_prune)
    load_org = cuahsihiscedarriverids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihiscedarriverids0_missingreport_graph(load_org)
    report_graph=cuahsihiscedarriverids0_graph_reports(report_msgraph)
