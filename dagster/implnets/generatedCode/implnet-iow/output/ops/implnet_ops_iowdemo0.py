...
def iowdemo0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "iowdemo0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowdemo0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "iowdemo0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowdemo0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "iowdemo0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowdemo0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "iowdemo0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowdemo0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "iowdemo0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowdemo0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "iowdemo0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowdemo0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "iowdemo0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowdemo0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "iowdemo0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowdemo0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "iowdemo0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowdemo0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "iowdemo0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def iowdemo0_uploadrelease(context):
#     returned_value = post_to_graph("iowdemo0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def iowdemo0_uploadrelease(context):
#     returned_value = postRelease("iowdemo0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def iowdemo0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="iowdemo0"
    )
...
@op(ins={"start": In(Nothing)})
def iowdemo0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="iowdemo0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def iowdemo0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="iowdemo0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def iowdemo0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="iowdemo0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def iowdemo0_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def iowdemo0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def iowdemo0_upload_summarize(context):
...
@graph
def harvest_iowdemo0():
    # check for containers
    containers = iowdemo0_getImage()

    # conduct the harvest
    harvest = iowdemo0_gleaner(start=containers)

    # data branch
    load_release = iowdemo0_naburelease(start=harvest)
    load_uploadrelease = iowdemo0_uploadrelease(start=load_release)
    load_prune = iowdemo0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = iowdemo0_nabu_provrelease(start=harvest)
    prov_clear = iowdemo0_nabu_provclear(start=prov_release)
    prov_object = iowdemo0_nabu_provobject(start=prov_clear)
    prov_drain = iowdemo0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = iowdemo0_nabu_orgsrelease(start=harvest)
    load_org = iowdemo0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = iowdemo0_missingreport_s3(start=harvest)
    # report_idstat = iowdemo0_identifier_stats(start=report_ms3)
    # report_bucketurl = iowdemo0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = iowdemo0_summarize(start=load_uploadrelease)
    # upload_summarize = iowdemo0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = iowdemo0_missingreport_graph(start=summarize)
    # report_graph = iowdemo0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="iowdemo0"):
#
