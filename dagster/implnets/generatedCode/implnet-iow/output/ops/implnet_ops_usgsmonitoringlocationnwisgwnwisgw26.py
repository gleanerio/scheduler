...
# #########################  Ops Section
@op
def usgsmonitoringlocationnwisgwnwisgw26_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def usgsmonitoringlocationnwisgwnwisgw26_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "usgsmonitoringlocationnwisgwnwisgw26")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw26_naburelease(context):
    returned_value = gleanerio(context, ("release"), "usgsmonitoringlocationnwisgwnwisgw26")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw26_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "usgsmonitoringlocationnwisgwnwisgw26")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw26_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "usgsmonitoringlocationnwisgwnwisgw26")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw26_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "usgsmonitoringlocationnwisgwnwisgw26")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw26_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "usgsmonitoringlocationnwisgwnwisgw26")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw26_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "usgsmonitoringlocationnwisgwnwisgw26")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw26_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "usgsmonitoringlocationnwisgwnwisgw26")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw26_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "usgsmonitoringlocationnwisgwnwisgw26")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw26_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "usgsmonitoringlocationnwisgwnwisgw26")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def usgsmonitoringlocationnwisgwnwisgw26_uploadrelease(context):
#     returned_value = post_to_graph("usgsmonitoringlocationnwisgwnwisgw26", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def usgsmonitoringlocationnwisgwnwisgw26_uploadrelease(context):
#     returned_value = postRelease("usgsmonitoringlocationnwisgwnwisgw26")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw26_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="usgsmonitoringlocationnwisgwnwisgw26"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "usgsmonitoringlocationnwisgwnwisgw26"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw26_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="usgsmonitoringlocationnwisgwnwisgw26"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw26_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="usgsmonitoringlocationnwisgwnwisgw26"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw26_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="usgsmonitoringlocationnwisgwnwisgw26"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw26_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw26_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def usgsmonitoringlocationnwisgwnwisgw26_upload_summarize(context):
    returned_value = post_to_graph(
        "usgsmonitoringlocationnwisgwnwisgw26",
        path=SUMMARY_PATH,
        extension="ttl",
...
@graph
def harvest_usgsmonitoringlocationnwisgwnwisgw26():
    # check for containers
    containers = usgsmonitoringlocationnwisgwnwisgw26_getImage()

    # conduct the harvest
    harvest = usgsmonitoringlocationnwisgwnwisgw26_gleaner(start=containers)

    # data branch
    load_release = usgsmonitoringlocationnwisgwnwisgw26_naburelease(start=harvest)
    load_uploadrelease = usgsmonitoringlocationnwisgwnwisgw26_uploadrelease(start=load_release)
    load_prune = usgsmonitoringlocationnwisgwnwisgw26_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = usgsmonitoringlocationnwisgwnwisgw26_nabu_provrelease(start=harvest)
    prov_clear = usgsmonitoringlocationnwisgwnwisgw26_nabu_provclear(start=prov_release)
    prov_object = usgsmonitoringlocationnwisgwnwisgw26_nabu_provobject(start=prov_clear)
    prov_drain = usgsmonitoringlocationnwisgwnwisgw26_nabu_provdrain(start=prov_object)

    # org branch
    org_release = usgsmonitoringlocationnwisgwnwisgw26_nabu_orgsrelease(start=harvest)
    load_org = usgsmonitoringlocationnwisgwnwisgw26_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = usgsmonitoringlocationnwisgwnwisgw26_missingreport_s3(start=harvest)
    # report_idstat = usgsmonitoringlocationnwisgwnwisgw26_identifier_stats(start=report_ms3)
    # report_bucketurl = usgsmonitoringlocationnwisgwnwisgw26_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = usgsmonitoringlocationnwisgwnwisgw26_summarize(start=load_uploadrelease)
    # upload_summarize = usgsmonitoringlocationnwisgwnwisgw26_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = usgsmonitoringlocationnwisgwnwisgw26_missingreport_graph(start=summarize)
    # report_graph = usgsmonitoringlocationnwisgwnwisgw26_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="usgsmonitoringlocationnwisgwnwisgw26"):
#
