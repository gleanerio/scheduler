...
@op
def cuahsihisczoboulderids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisczoboulderids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisczoboulderids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisczoboulderids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisczoboulderids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisczoboulderids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisczoboulderids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisczoboulderids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisczoboulderids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisczoboulderids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisczoboulderids0")
    ...
    return msg + r
@op
def cuahsihisczoboulderids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisczoboulderids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisczoboulderids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisczoboulderids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisczoboulderids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisczoboulderids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisczoboulderids0"
...
def cuahsihisczoboulderids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisczoboulderids0"
...
def harvest_cuahsihisczoboulderids0():
    harvest = cuahsihisczoboulderids0_gleaner()

    report_ms3 = cuahsihisczoboulderids0_missingreport_s3(harvest)
    report_idstat = cuahsihisczoboulderids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisczoboulderids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisczoboulderids0")
    load_release = cuahsihisczoboulderids0_naburelease(harvest)
    load_uploadrelease = cuahsihisczoboulderids0_uploadrelease(load_release)

    load_prune = cuahsihisczoboulderids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisczoboulderids0_nabuprov(load_prune)
    load_org = cuahsihisczoboulderids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisczoboulderids0_missingreport_graph(load_org)
    report_graph=cuahsihisczoboulderids0_graph_reports(report_msgraph)
