...
def cuahsihisczoboulderids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsihisczoboulderids0")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def cuahsihisczoboulderids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsihisczoboulderids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisczoboulderids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsihisczoboulderids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisczoboulderids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsihisczoboulderids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisczoboulderids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsihisczoboulderids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisczoboulderids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsihisczoboulderids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisczoboulderids0_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def cuahsihisczoboulderids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsihisczoboulderids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def cuahsihisczoboulderids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsihisczoboulderids0"
    )
...
@op(ins={"start": In(Nothing)})
def cuahsihisczoboulderids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsihisczoboulderids0_upload_summarize(context):
    returned_value = post_to_graph(
        "cuahsihisczoboulderids0",
        path=SUMMARY_PATH,
        extension="ttl",
...
def harvest_cuahsihisczoboulderids0():
    ...
    # prov branch
    prov_release = cuahsihisczoboulderids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsihisczoboulderids0_nabu_provclear(start=prov_release)
    prov_object = cuahsihisczoboulderids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsihisczoboulderids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsihisczoboulderids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsihisczoboulderids0_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = cuahsihisczoboulderids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsihisczoboulderids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsihisczoboulderids0_missingreport_graph(start=summarize)
