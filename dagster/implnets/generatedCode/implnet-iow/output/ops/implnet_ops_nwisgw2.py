...
@op
def nwisgw2_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw2")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw2_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw2")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw2_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw2")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw2_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw2")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw2_uploadrelease(context, msg: str):
...
@op
def nwisgw2_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw2")
    ...
    return msg + r
@op
def nwisgw2_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw2")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw2_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw2")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw2_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw2")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw2"
...
def nwisgw2_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw2"
...
@graph
def harvest_nwisgw2():
    harvest = nwisgw2_gleaner()

    report_ms3 = nwisgw2_missingreport_s3(harvest)
    report_idstat = nwisgw2_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw2_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw2")
    load_release = nwisgw2_naburelease(harvest)
    load_uploadrelease = nwisgw2_uploadrelease(load_release)

    load_prune = nwisgw2_nabu_prune(load_uploadrelease)
    load_prov = nwisgw2_nabuprov(load_prune)
    load_org = nwisgw2_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw2_missingreport_graph(load_org)
    report_graph=nwisgw2_graph_reports(report_msgraph)
