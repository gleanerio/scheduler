...
@op(ins={"start": In(Nothing)})
def wade32_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade32")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade32_nabuorg(context):
...
def wade32_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade32")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade32")
    ...
    load_prov = wade32_nabuprov(start=load_prune)
    load_org = wade32_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade32_missingreport_graph(start=load_org)
