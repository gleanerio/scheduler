...
@op
def refgage0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "refgage0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def refgage0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "refgage0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def refgage0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "refgage0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def refgage0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "refgage0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def refgage0_uploadrelease(context, msg: str):
...
@op
def refgage0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="refgage0")
    ...
    return msg + r
@op
def refgage0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="refgage0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def refgage0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="refgage0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def refgage0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="refgage0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "refgage0"
...
def refgage0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "refgage0"
...
def harvest_refgage0():
    ...
    report_ms3 = refgage0_missingreport_s3(harvest)
    report_idstat = refgage0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = refgage0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="refgage0")
    load_release = refgage0_naburelease(harvest)
    load_uploadrelease = refgage0_uploadrelease(load_release)

    load_prune = refgage0_nabu_prune(load_uploadrelease)
    load_prov = refgage0_nabuprov(load_prune)
    load_org = refgage0_nabuorg(load_prov)

# run after load
    report_msgraph=refgage0_missingreport_graph(load_org)
    report_graph=refgage0_graph_reports(report_msgraph)
