...
def hmw0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "hmw0")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def hmw0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "hmw0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def hmw0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "hmw0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def hmw0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "hmw0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def hmw0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "hmw0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def hmw0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "hmw0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def hmw0_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def hmw0_uploadrelease(context):
#     returned_value = post_to_graph("hmw0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def hmw0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="hmw0"
    )
...
@op(ins={"start": In(Nothing)})
def hmw0_upload_summarize(context):
...
def harvest_hmw0():
    ...
    # prov branch
    prov_release = hmw0_nabu_provrelease(start=harvest)
    prov_clear = hmw0_nabu_provclear(start=prov_release)
    prov_object = hmw0_nabu_provobject(start=prov_clear)
    prov_drain = hmw0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = hmw0_nabu_orgsrelease(start=harvest)
    load_org = hmw0_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = hmw0_summarize(start=load_uploadrelease)
    # upload_summarize = hmw0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = hmw0_missingreport_graph(start=summarize)
