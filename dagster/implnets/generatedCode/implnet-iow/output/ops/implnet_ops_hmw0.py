...
@op
def hmw0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "hmw0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def hmw0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "hmw0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def hmw0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "hmw0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def hmw0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "hmw0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def hmw0_uploadrelease(context, msg: str):
...
@op
def hmw0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hmw0")
    ...
    return msg + r
@op
def hmw0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hmw0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def hmw0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hmw0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def hmw0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hmw0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "hmw0"
...
def hmw0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "hmw0"
...
@graph
def harvest_hmw0():
    harvest = hmw0_gleaner()

    report_ms3 = hmw0_missingreport_s3(harvest)
    report_idstat = hmw0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = hmw0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="hmw0")
    load_release = hmw0_naburelease(harvest)
    load_uploadrelease = hmw0_uploadrelease(load_release)

    load_prune = hmw0_nabu_prune(load_uploadrelease)
    load_prov = hmw0_nabuprov(load_prune)
    load_org = hmw0_nabuorg(load_prov)

# run after load
    report_msgraph=hmw0_missingreport_graph(load_org)
    report_graph=hmw0_graph_reports(report_msgraph)
