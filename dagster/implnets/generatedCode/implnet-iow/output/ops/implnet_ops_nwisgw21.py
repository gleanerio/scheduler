...
@op
def nwisgw21_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw21")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw21_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw21")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw21_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw21")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw21_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw21")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw21_uploadrelease(context, msg: str):
...
@op
def nwisgw21_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw21")
    ...
    return msg + r
@op
def nwisgw21_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw21")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw21_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw21")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw21_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw21")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw21"
...
def nwisgw21_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw21"
...
def harvest_nwisgw21():
    harvest = nwisgw21_gleaner()

    report_ms3 = nwisgw21_missingreport_s3(harvest)
    report_idstat = nwisgw21_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw21_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw21")
    load_release = nwisgw21_naburelease(harvest)
    load_uploadrelease = nwisgw21_uploadrelease(load_release)

    load_prune = nwisgw21_nabu_prune(load_uploadrelease)
    load_prov = nwisgw21_nabuprov(load_prune)
    load_org = nwisgw21_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw21_missingreport_graph(load_org)
    report_graph=nwisgw21_graph_reports(report_msgraph)
