...
@op
def nwisgw24_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw24")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw24_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw24")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw24_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw24")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw24_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw24")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw24_uploadrelease(context, msg: str):
...
@op
def nwisgw24_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw24")
    ...
    return msg + r
@op
def nwisgw24_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw24")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw24_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw24")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw24_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw24")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw24"
...
def nwisgw24_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw24"
...
def harvest_nwisgw24():
    harvest = nwisgw24_gleaner()

    report_ms3 = nwisgw24_missingreport_s3(harvest)
    report_idstat = nwisgw24_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw24_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw24")
    load_release = nwisgw24_naburelease(harvest)
    load_uploadrelease = nwisgw24_uploadrelease(load_release)

    load_prune = nwisgw24_nabu_prune(load_uploadrelease)
    load_prov = nwisgw24_nabuprov(load_prune)
    load_org = nwisgw24_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw24_missingreport_graph(load_org)
    report_graph=nwisgw24_graph_reports(report_msgraph)
