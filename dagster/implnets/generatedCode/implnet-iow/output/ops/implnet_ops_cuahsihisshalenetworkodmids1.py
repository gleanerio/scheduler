...
@op
def cuahsihisshalenetworkodmids1_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisshalenetworkodmids1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisshalenetworkodmids1_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisshalenetworkodmids1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisshalenetworkodmids1_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisshalenetworkodmids1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisshalenetworkodmids1_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisshalenetworkodmids1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisshalenetworkodmids1_uploadrelease(context, msg: str):
...
@op
def cuahsihisshalenetworkodmids1_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisshalenetworkodmids1")
    ...
    return msg + r
@op
def cuahsihisshalenetworkodmids1_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisshalenetworkodmids1")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisshalenetworkodmids1_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisshalenetworkodmids1")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisshalenetworkodmids1_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisshalenetworkodmids1")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisshalenetworkodmids1"
...
def cuahsihisshalenetworkodmids1_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisshalenetworkodmids1"
...
def harvest_cuahsihisshalenetworkodmids1():
    harvest = cuahsihisshalenetworkodmids1_gleaner()

    report_ms3 = cuahsihisshalenetworkodmids1_missingreport_s3(harvest)
    report_idstat = cuahsihisshalenetworkodmids1_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisshalenetworkodmids1_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisshalenetworkodmids1")
    load_release = cuahsihisshalenetworkodmids1_naburelease(harvest)
    load_uploadrelease = cuahsihisshalenetworkodmids1_uploadrelease(load_release)

    load_prune = cuahsihisshalenetworkodmids1_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisshalenetworkodmids1_nabuprov(load_prune)
    load_org = cuahsihisshalenetworkodmids1_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisshalenetworkodmids1_missingreport_graph(load_org)
    report_graph=cuahsihisshalenetworkodmids1_graph_reports(report_msgraph)
