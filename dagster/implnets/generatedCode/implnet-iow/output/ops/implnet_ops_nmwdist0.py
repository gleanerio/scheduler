...
@op
def nmwdist0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nmwdist0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nmwdist0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nmwdist0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nmwdist0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nmwdist0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nmwdist0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nmwdist0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nmwdist0_uploadrelease(context, msg: str):
...
@op
def nmwdist0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nmwdist0")
    ...
    return msg + r
@op
def nmwdist0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nmwdist0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nmwdist0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nmwdist0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nmwdist0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nmwdist0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nmwdist0"
...
def nmwdist0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nmwdist0"
...
def harvest_nmwdist0():
    harvest = nmwdist0_gleaner()

    report_ms3 = nmwdist0_missingreport_s3(harvest)
    report_idstat = nmwdist0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nmwdist0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nmwdist0")
    load_release = nmwdist0_naburelease(harvest)
    load_uploadrelease = nmwdist0_uploadrelease(load_release)

    load_prune = nmwdist0_nabu_prune(load_uploadrelease)
    load_prov = nmwdist0_nabuprov(load_prune)
    load_org = nmwdist0_nabuorg(load_prov)

# run after load
    report_msgraph=nmwdist0_missingreport_graph(load_org)
    report_graph=nmwdist0_graph_reports(report_msgraph)
