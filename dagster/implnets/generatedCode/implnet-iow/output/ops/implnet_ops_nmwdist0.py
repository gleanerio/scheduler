...
def nmwdist0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "nmwdist0")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def nmwdist0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "nmwdist0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdist0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "nmwdist0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdist0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "nmwdist0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdist0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "nmwdist0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdist0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "nmwdist0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdist0_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def nmwdist0_uploadrelease(context):
#     returned_value = post_to_graph("nmwdist0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def nmwdist0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nmwdist0"
    )
...
@op(ins={"start": In(Nothing)})
def nmwdist0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def nmwdist0_upload_summarize(context):
...
def harvest_nmwdist0():
    ...
    # prov branch
    prov_release = nmwdist0_nabu_provrelease(start=harvest)
    prov_clear = nmwdist0_nabu_provclear(start=prov_release)
    prov_object = nmwdist0_nabu_provobject(start=prov_clear)
    prov_drain = nmwdist0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = nmwdist0_nabu_orgsrelease(start=harvest)
    load_org = nmwdist0_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = nmwdist0_summarize(start=load_uploadrelease)
    # upload_summarize = nmwdist0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = nmwdist0_missingreport_graph(start=summarize)
