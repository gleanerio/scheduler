...
@op
def cuahsihismaaeriids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihismaaeriids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismaaeriids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihismaaeriids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismaaeriids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihismaaeriids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismaaeriids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihismaaeriids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihismaaeriids0_uploadrelease(context, msg: str):
...
@op
def cuahsihismaaeriids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismaaeriids0")
    ...
    return msg + r
@op
def cuahsihismaaeriids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismaaeriids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihismaaeriids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismaaeriids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihismaaeriids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismaaeriids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihismaaeriids0"
...
def cuahsihismaaeriids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihismaaeriids0"
...
def harvest_cuahsihismaaeriids0():
    harvest = cuahsihismaaeriids0_gleaner()

    report_ms3 = cuahsihismaaeriids0_missingreport_s3(harvest)
    report_idstat = cuahsihismaaeriids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihismaaeriids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihismaaeriids0")
    load_release = cuahsihismaaeriids0_naburelease(harvest)
    load_uploadrelease = cuahsihismaaeriids0_uploadrelease(load_release)

    load_prune = cuahsihismaaeriids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihismaaeriids0_nabuprov(load_prune)
    load_org = cuahsihismaaeriids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihismaaeriids0_missingreport_graph(load_org)
    report_graph=cuahsihismaaeriids0_graph_reports(report_msgraph)
