...
def autotest20_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "autotest20")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def autotest20_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "autotest20")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def autotest20_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "autotest20")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def autotest20_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "autotest20")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def autotest20_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "autotest20")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def autotest20_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "autotest20")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def autotest20_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def autotest20_uploadrelease(context):
#     returned_value = post_to_graph("autotest20", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def autotest20_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="autotest20"
    )
...
@op(ins={"start": In(Nothing)})
def autotest20_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def autotest20_upload_summarize(context):
...
def harvest_autotest20():
    ...
    # prov branch
    prov_release = autotest20_nabu_provrelease(start=harvest)
    prov_clear = autotest20_nabu_provclear(start=prov_release)
    prov_object = autotest20_nabu_provobject(start=prov_clear)
    prov_drain = autotest20_nabu_provdrain(start=prov_object)

    # org branch
    org_release = autotest20_nabu_orgsrelease(start=harvest)
    load_org = autotest20_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = autotest20_summarize(start=load_uploadrelease)
    # upload_summarize = autotest20_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = autotest20_missingreport_graph(start=summarize)
