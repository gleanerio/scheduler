...
@op
def autotest20_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "autotest20")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def autotest20_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "autotest20")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def autotest20_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "autotest20")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def autotest20_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "autotest20")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def autotest20_uploadrelease(context, msg: str):
...
@op
def autotest20_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="autotest20")
    ...
    return msg + r
@op
def autotest20_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="autotest20")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def autotest20_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="autotest20")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def autotest20_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="autotest20")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "autotest20"
...
def autotest20_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "autotest20"
...
def harvest_autotest20():
    harvest = autotest20_gleaner()

    report_ms3 = autotest20_missingreport_s3(harvest)
    report_idstat = autotest20_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = autotest20_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="autotest20")
    load_release = autotest20_naburelease(harvest)
    load_uploadrelease = autotest20_uploadrelease(load_release)

    load_prune = autotest20_nabu_prune(load_uploadrelease)
    load_prov = autotest20_nabuprov(load_prune)
    load_org = autotest20_nabuorg(load_prov)

# run after load
    report_msgraph=autotest20_missingreport_graph(load_org)
    report_graph=autotest20_graph_reports(report_msgraph)
