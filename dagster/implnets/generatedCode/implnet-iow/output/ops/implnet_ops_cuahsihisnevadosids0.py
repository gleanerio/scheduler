...
@op
def cuahsihisnevadosids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisnevadosids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisnevadosids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisnevadosids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisnevadosids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisnevadosids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisnevadosids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisnevadosids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisnevadosids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisnevadosids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisnevadosids0")
    ...
    return msg + r
@op
def cuahsihisnevadosids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisnevadosids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisnevadosids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisnevadosids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisnevadosids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisnevadosids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisnevadosids0"
...
def cuahsihisnevadosids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisnevadosids0"
...
def harvest_cuahsihisnevadosids0():
    harvest = cuahsihisnevadosids0_gleaner()

    report_ms3 = cuahsihisnevadosids0_missingreport_s3(harvest)
    report_idstat = cuahsihisnevadosids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisnevadosids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisnevadosids0")
    load_release = cuahsihisnevadosids0_naburelease(harvest)
    load_uploadrelease = cuahsihisnevadosids0_uploadrelease(load_release)

    load_prune = cuahsihisnevadosids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisnevadosids0_nabuprov(load_prune)
    load_org = cuahsihisnevadosids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisnevadosids0_missingreport_graph(load_org)
    report_graph=cuahsihisnevadosids0_graph_reports(report_msgraph)
