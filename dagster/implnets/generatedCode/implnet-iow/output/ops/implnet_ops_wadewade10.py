...
# #########################  Ops Section
@op
def wadewade10_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def wadewade10_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "wadewade10")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade10_naburelease(context):
    returned_value = gleanerio(context, ("release"), "wadewade10")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade10_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wadewade10")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade10_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "wadewade10")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade10_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wadewade10")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade10_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wadewade10")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade10_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wadewade10")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade10_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wadewade10")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade10_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wadewade10")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade10_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "wadewade10")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wadewade10_uploadrelease(context):
#     returned_value = post_to_graph("wadewade10", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wadewade10_uploadrelease(context):
#     returned_value = postRelease("wadewade10")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def wadewade10_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade10"
    )
...
@op(ins={"start": In(Nothing)})
def wadewade10_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade10"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def wadewade10_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade10"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def wadewade10_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade10"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def wadewade10_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wadewade10_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wadewade10_upload_summarize(context):
...
@graph
def harvest_wadewade10():
    # check for containers
    containers = wadewade10_getImage()

    # conduct the harvest
    harvest = wadewade10_gleaner(start=containers)

    # data branch
    load_release = wadewade10_naburelease(start=harvest)
    load_uploadrelease = wadewade10_uploadrelease(start=load_release)
    load_prune = wadewade10_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = wadewade10_nabu_provrelease(start=harvest)
    prov_clear = wadewade10_nabu_provclear(start=prov_release)
    prov_object = wadewade10_nabu_provobject(start=prov_clear)
    prov_drain = wadewade10_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wadewade10_nabu_orgsrelease(start=harvest)
    load_org = wadewade10_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = wadewade10_missingreport_s3(start=harvest)
    # report_idstat = wadewade10_identifier_stats(start=report_ms3)
    # report_bucketurl = wadewade10_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = wadewade10_summarize(start=load_uploadrelease)
    # upload_summarize = wadewade10_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wadewade10_missingreport_graph(start=summarize)
    # report_graph = wadewade10_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="wadewade10"):
#
