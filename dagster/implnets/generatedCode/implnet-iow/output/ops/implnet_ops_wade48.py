...
@op(ins={"start": In(Nothing)})
def wade48_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade48")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade48_nabuorg(context):
...
def wade48_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade48")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade48")
    ...
    load_prov = wade48_nabuprov(start=load_prune)
    load_org = wade48_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade48_missingreport_graph(start=load_org)
