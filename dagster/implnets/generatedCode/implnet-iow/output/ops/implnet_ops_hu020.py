...
@op
def hu020_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "hu020")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def hu020_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "hu020")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def hu020_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "hu020")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def hu020_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "hu020")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def hu020_uploadrelease(context, msg: str):
...
@op
def hu020_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hu020")
    ...
    return msg + r
@op
def hu020_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hu020")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def hu020_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hu020")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def hu020_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hu020")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "hu020"
...
def hu020_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "hu020"
...
@graph
def harvest_hu020():
    harvest = hu020_gleaner()

    report_ms3 = hu020_missingreport_s3(harvest)
    report_idstat = hu020_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = hu020_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="hu020")
    load_release = hu020_naburelease(harvest)
    load_uploadrelease = hu020_uploadrelease(load_release)

    load_prune = hu020_nabu_prune(load_uploadrelease)
    load_prov = hu020_nabuprov(load_prune)
    load_org = hu020_nabuorg(load_prov)

# run after load
    report_msgraph=hu020_missingreport_graph(load_org)
    report_graph=hu020_graph_reports(report_msgraph)
