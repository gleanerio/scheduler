...
def hu020_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "hu020")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def hu020_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "hu020")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def hu020_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "hu020")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def hu020_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "hu020")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def hu020_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "hu020")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def hu020_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "hu020")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def hu020_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def hu020_uploadrelease(context):
#     returned_value = post_to_graph("hu020", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def hu020_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="hu020"
    )
...
@op(ins={"start": In(Nothing)})
def hu020_upload_summarize(context):
...
def harvest_hu020():
    ...
    # prov branch
    prov_release = hu020_nabu_provrelease(start=harvest)
    prov_clear = hu020_nabu_provclear(start=prov_release)
    prov_object = hu020_nabu_provobject(start=prov_clear)
    prov_drain = hu020_nabu_provdrain(start=prov_object)

    # org branch
    org_release = hu020_nabu_orgsrelease(start=harvest)
    load_org = hu020_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = hu020_summarize(start=load_uploadrelease)
    # upload_summarize = hu020_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = hu020_missingreport_graph(start=summarize)
