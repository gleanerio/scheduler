...
@op
def cuahsihismwraids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihismwraids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismwraids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihismwraids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismwraids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihismwraids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismwraids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihismwraids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihismwraids0_uploadrelease(context, msg: str):
...
@op
def cuahsihismwraids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismwraids0")
    ...
    return msg + r
@op
def cuahsihismwraids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismwraids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihismwraids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismwraids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihismwraids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismwraids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihismwraids0"
...
def cuahsihismwraids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihismwraids0"
...
def harvest_cuahsihismwraids0():
    harvest = cuahsihismwraids0_gleaner()

    report_ms3 = cuahsihismwraids0_missingreport_s3(harvest)
    report_idstat = cuahsihismwraids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihismwraids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihismwraids0")
    load_release = cuahsihismwraids0_naburelease(harvest)
    load_uploadrelease = cuahsihismwraids0_uploadrelease(load_release)

    load_prune = cuahsihismwraids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihismwraids0_nabuprov(load_prune)
    load_org = cuahsihismwraids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihismwraids0_missingreport_graph(load_org)
    report_graph=cuahsihismwraids0_graph_reports(report_msgraph)
