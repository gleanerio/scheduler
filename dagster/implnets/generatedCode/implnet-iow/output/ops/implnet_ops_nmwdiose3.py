...
@op
def nmwdiose3_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nmwdiose3")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nmwdiose3_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nmwdiose3")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nmwdiose3_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nmwdiose3")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nmwdiose3_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nmwdiose3")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nmwdiose3_uploadrelease(context, msg: str):
...
@op
def nmwdiose3_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nmwdiose3")
    ...
    return msg + r
@op
def nmwdiose3_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nmwdiose3")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nmwdiose3_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nmwdiose3")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nmwdiose3_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nmwdiose3")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nmwdiose3"
...
def nmwdiose3_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nmwdiose3"
...
def harvest_nmwdiose3():
    harvest = nmwdiose3_gleaner()

    report_ms3 = nmwdiose3_missingreport_s3(harvest)
    report_idstat = nmwdiose3_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nmwdiose3_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nmwdiose3")
    load_release = nmwdiose3_naburelease(harvest)
    load_uploadrelease = nmwdiose3_uploadrelease(load_release)

    load_prune = nmwdiose3_nabu_prune(load_uploadrelease)
    load_prov = nmwdiose3_nabuprov(load_prune)
    load_org = nmwdiose3_nabuorg(load_prov)

# run after load
    report_msgraph=nmwdiose3_missingreport_graph(load_org)
    report_graph=nmwdiose3_graph_reports(report_msgraph)
