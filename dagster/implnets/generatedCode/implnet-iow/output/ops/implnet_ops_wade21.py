...
@op(ins={"start": In(Nothing)})
def wade21_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade21")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade21_nabuorg(context):
...
def wade21_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade21")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade21")
    ...
    load_prov = wade21_nabuprov(start=load_prune)
    load_org = wade21_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade21_missingreport_graph(start=load_org)
