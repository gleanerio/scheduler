...
def gages0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "gages0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "gages0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "gages0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "gages0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "gages0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "gages0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "gages0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "gages0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "gages0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "gages0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def gages0_uploadrelease(context):
#     returned_value = post_to_graph("gages0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def gages0_uploadrelease(context):
#     returned_value = postRelease("gages0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def gages0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="gages0"
    )
...
@op(ins={"start": In(Nothing)})
def gages0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="gages0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def gages0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="gages0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def gages0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="gages0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def gages0_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def gages0_upload_summarize(context):
...
def harvest_gages0():
    ...
    containers = gages0_getImage()

    # conduct the harvest
    harvest = gages0_gleaner(start=containers)

    # data branch
    load_release = gages0_naburelease(start=harvest)
    load_uploadrelease = gages0_uploadrelease(start=load_release)
    load_prune = gages0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = gages0_nabu_provrelease(start=harvest)
    prov_clear = gages0_nabu_provclear(start=prov_release)
    prov_object = gages0_nabu_provobject(start=prov_clear)
    prov_drain = gages0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = gages0_nabu_orgsrelease(start=harvest)
    load_org = gages0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = gages0_missingreport_s3(start=harvest)
    # report_idstat = gages0_identifier_stats(start=report_ms3)
    # report_bucketurl = gages0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = gages0_summarize(start=load_uploadrelease)
    # upload_summarize = gages0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = gages0_missingreport_graph(start=summarize)
    # report_graph = gages0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="gages0"):
#
