...
@op(ins={"start": In(Nothing)})
def gages0_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "gages0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def gages0_nabuorg(context):
...
def gages0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="gages0")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="gages0")
    ...
    load_prov = gages0_nabuprov(start=load_prune)
    load_org = gages0_nabuorg(start=load_prov)

# run after load
    report_msgraph=gages0_missingreport_graph(start=load_org)
