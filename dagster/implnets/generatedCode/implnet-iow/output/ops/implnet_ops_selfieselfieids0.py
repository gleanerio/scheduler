...
# #########################  Ops Section
@op
def selfieselfieids0_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def selfieselfieids0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "selfieselfieids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def selfieselfieids0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "selfieselfieids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def selfieselfieids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "selfieselfieids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def selfieselfieids0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "selfieselfieids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def selfieselfieids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "selfieselfieids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def selfieselfieids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "selfieselfieids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def selfieselfieids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "selfieselfieids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def selfieselfieids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "selfieselfieids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def selfieselfieids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "selfieselfieids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def selfieselfieids0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "selfieselfieids0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def selfieselfieids0_uploadrelease(context):
#     returned_value = post_to_graph("selfieselfieids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def selfieselfieids0_uploadrelease(context):
#     returned_value = postRelease("selfieselfieids0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def selfieselfieids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="selfieselfieids0"
    )
...
@op(ins={"start": In(Nothing)})
def selfieselfieids0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="selfieselfieids0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def selfieselfieids0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="selfieselfieids0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def selfieselfieids0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="selfieselfieids0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def selfieselfieids0_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def selfieselfieids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def selfieselfieids0_upload_summarize(context):
...
@graph
def harvest_selfieselfieids0():
    # check for containers
    containers = selfieselfieids0_getImage()

    # conduct the harvest
    harvest = selfieselfieids0_gleaner(start=containers)

    # data branch
    load_release = selfieselfieids0_naburelease(start=harvest)
    load_uploadrelease = selfieselfieids0_uploadrelease(start=load_release)
    load_prune = selfieselfieids0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = selfieselfieids0_nabu_provrelease(start=harvest)
    prov_clear = selfieselfieids0_nabu_provclear(start=prov_release)
    prov_object = selfieselfieids0_nabu_provobject(start=prov_clear)
    prov_drain = selfieselfieids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = selfieselfieids0_nabu_orgsrelease(start=harvest)
    load_org = selfieselfieids0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = selfieselfieids0_missingreport_s3(start=harvest)
    # report_idstat = selfieselfieids0_identifier_stats(start=report_ms3)
    # report_bucketurl = selfieselfieids0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = selfieselfieids0_summarize(start=load_uploadrelease)
    # upload_summarize = selfieselfieids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = selfieselfieids0_missingreport_graph(start=summarize)
    # report_graph = selfieselfieids0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="selfieselfieids0"):
#
