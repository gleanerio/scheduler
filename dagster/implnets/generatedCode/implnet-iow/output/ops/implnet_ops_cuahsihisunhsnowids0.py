...
@op
def cuahsihisunhsnowids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisunhsnowids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisunhsnowids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisunhsnowids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisunhsnowids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisunhsnowids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisunhsnowids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisunhsnowids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisunhsnowids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisunhsnowids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisunhsnowids0")
    ...
    return msg + r
@op
def cuahsihisunhsnowids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisunhsnowids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisunhsnowids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisunhsnowids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisunhsnowids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisunhsnowids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisunhsnowids0"
...
def cuahsihisunhsnowids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisunhsnowids0"
...
def harvest_cuahsihisunhsnowids0():
    harvest = cuahsihisunhsnowids0_gleaner()

    report_ms3 = cuahsihisunhsnowids0_missingreport_s3(harvest)
    report_idstat = cuahsihisunhsnowids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisunhsnowids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisunhsnowids0")
    load_release = cuahsihisunhsnowids0_naburelease(harvest)
    load_uploadrelease = cuahsihisunhsnowids0_uploadrelease(load_release)

    load_prune = cuahsihisunhsnowids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisunhsnowids0_nabuprov(load_prune)
    load_org = cuahsihisunhsnowids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisunhsnowids0_missingreport_graph(load_org)
    report_graph=cuahsihisunhsnowids0_graph_reports(report_msgraph)
