...
def places0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "places0")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def places0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "places0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def places0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "places0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def places0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "places0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def places0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "places0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def places0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "places0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def places0_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def places0_uploadrelease(context):
#     returned_value = post_to_graph("places0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def places0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="places0"
    )
...
@op(ins={"start": In(Nothing)})
def places0_upload_summarize(context):
...
def harvest_places0():
    ...
    # prov branch
    prov_release = places0_nabu_provrelease(start=harvest)
    prov_clear = places0_nabu_provclear(start=prov_release)
    prov_object = places0_nabu_provobject(start=prov_clear)
    prov_drain = places0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = places0_nabu_orgsrelease(start=harvest)
    load_org = places0_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = places0_summarize(start=load_uploadrelease)
    # upload_summarize = places0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = places0_missingreport_graph(start=summarize)
