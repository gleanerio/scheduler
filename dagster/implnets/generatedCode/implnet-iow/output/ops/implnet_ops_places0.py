...
@op
def places0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "places0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def places0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "places0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def places0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "places0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def places0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "places0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def places0_uploadrelease(context, msg: str):
...
@op
def places0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="places0")
    ...
    return msg + r
@op
def places0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="places0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def places0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="places0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def places0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="places0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "places0"
...
def places0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "places0"
...
@graph
def harvest_places0():
    harvest = places0_gleaner()

    report_ms3 = places0_missingreport_s3(harvest)
    report_idstat = places0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = places0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="places0")
    load_release = places0_naburelease(harvest)
    load_uploadrelease = places0_uploadrelease(load_release)

    load_prune = places0_nabu_prune(load_uploadrelease)
    load_prov = places0_nabuprov(load_prune)
    load_org = places0_nabuorg(load_prov)

# run after load
    report_msgraph=places0_missingreport_graph(load_org)
    report_graph=places0_graph_reports(report_msgraph)
