...
@op
def nwisgw23_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw23")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw23_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw23")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw23_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw23")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw23_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw23")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw23_uploadrelease(context, msg: str):
...
@op
def nwisgw23_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw23")
    ...
    return msg + r
@op
def nwisgw23_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw23")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw23_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw23")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw23_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw23")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw23"
...
def nwisgw23_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw23"
...
def harvest_nwisgw23():
    harvest = nwisgw23_gleaner()

    report_ms3 = nwisgw23_missingreport_s3(harvest)
    report_idstat = nwisgw23_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw23_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw23")
    load_release = nwisgw23_naburelease(harvest)
    load_uploadrelease = nwisgw23_uploadrelease(load_release)

    load_prune = nwisgw23_nabu_prune(load_uploadrelease)
    load_prov = nwisgw23_nabuprov(load_prune)
    load_org = nwisgw23_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw23_missingreport_graph(load_org)
    report_graph=nwisgw23_graph_reports(report_msgraph)
