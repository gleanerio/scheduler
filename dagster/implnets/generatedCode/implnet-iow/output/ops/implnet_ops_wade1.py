...
def wade1_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wade1")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def wade1_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wade1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade1_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wade1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade1_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wade1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade1_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wade1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade1_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wade1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade1_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def wade1_uploadrelease(context):
#     returned_value = post_to_graph("wade1", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def wade1_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade1"
    )
...
@op(ins={"start": In(Nothing)})
def wade1_upload_summarize(context):
...
def harvest_wade1():
    ...
    # prov branch
    prov_release = wade1_nabu_provrelease(start=harvest)
    prov_clear = wade1_nabu_provclear(start=prov_release)
    prov_object = wade1_nabu_provobject(start=prov_clear)
    prov_drain = wade1_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wade1_nabu_orgsrelease(start=harvest)
    load_org = wade1_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = wade1_summarize(start=load_uploadrelease)
    # upload_summarize = wade1_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wade1_missingreport_graph(start=summarize)
