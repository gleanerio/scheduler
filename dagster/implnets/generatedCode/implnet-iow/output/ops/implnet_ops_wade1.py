...
@op
def wade1_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "wade1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade1_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "wade1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade1_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "wade1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade1_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "wade1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def wade1_uploadrelease(context, msg: str):
...
@op
def wade1_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade1")
    ...
    return msg + r
@op
def wade1_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade1")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def wade1_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade1")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def wade1_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade1")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade1"
...
def wade1_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade1"
...
def harvest_wade1():
    ...
    report_ms3 = wade1_missingreport_s3(harvest)
    report_idstat = wade1_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = wade1_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="wade1")
    load_release = wade1_naburelease(harvest)
    load_uploadrelease = wade1_uploadrelease(load_release)

    load_prune = wade1_nabu_prune(load_uploadrelease)
    load_prov = wade1_nabuprov(load_prune)
    load_org = wade1_nabuorg(load_prov)

# run after load
    report_msgraph=wade1_missingreport_graph(load_org)
    report_graph=wade1_graph_reports(report_msgraph)
