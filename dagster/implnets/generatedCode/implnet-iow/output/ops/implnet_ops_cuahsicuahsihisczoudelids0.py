...
# #########################  Ops Section
@op
def cuahsicuahsihisczoudelids0_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def cuahsicuahsihisczoudelids0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "cuahsicuahsihisczoudelids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoudelids0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "cuahsicuahsihisczoudelids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoudelids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsicuahsihisczoudelids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoudelids0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "cuahsicuahsihisczoudelids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoudelids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsicuahsihisczoudelids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoudelids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsicuahsihisczoudelids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoudelids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsicuahsihisczoudelids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoudelids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsicuahsihisczoudelids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoudelids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsicuahsihisczoudelids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoudelids0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "cuahsicuahsihisczoudelids0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihisczoudelids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsicuahsihisczoudelids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihisczoudelids0_uploadrelease(context):
#     returned_value = postRelease("cuahsicuahsihisczoudelids0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoudelids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisczoudelids0"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsicuahsihisczoudelids0"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoudelids0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisczoudelids0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoudelids0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisczoudelids0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoudelids0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisczoudelids0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoudelids0_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoudelids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisczoudelids0_upload_summarize(context):
    returned_value = post_to_graph(
        "cuahsicuahsihisczoudelids0",
        path=SUMMARY_PATH,
        extension="ttl",
...
@graph
def harvest_cuahsicuahsihisczoudelids0():
    # check for containers
    containers = cuahsicuahsihisczoudelids0_getImage()

    # conduct the harvest
    harvest = cuahsicuahsihisczoudelids0_gleaner(start=containers)

    # data branch
    load_release = cuahsicuahsihisczoudelids0_naburelease(start=harvest)
    load_uploadrelease = cuahsicuahsihisczoudelids0_uploadrelease(start=load_release)
    load_prune = cuahsicuahsihisczoudelids0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = cuahsicuahsihisczoudelids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsicuahsihisczoudelids0_nabu_provclear(start=prov_release)
    prov_object = cuahsicuahsihisczoudelids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsicuahsihisczoudelids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsicuahsihisczoudelids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsicuahsihisczoudelids0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = cuahsicuahsihisczoudelids0_missingreport_s3(start=harvest)
    # report_idstat = cuahsicuahsihisczoudelids0_identifier_stats(start=report_ms3)
    # report_bucketurl = cuahsicuahsihisczoudelids0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = cuahsicuahsihisczoudelids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsicuahsihisczoudelids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsicuahsihisczoudelids0_missingreport_graph(start=summarize)
    # report_graph = cuahsicuahsihisczoudelids0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="cuahsicuahsihisczoudelids0"):
#
