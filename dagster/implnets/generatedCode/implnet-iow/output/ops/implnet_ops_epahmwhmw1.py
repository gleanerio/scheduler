...
# #########################  Ops Section
@op
def epahmwhmw1_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def epahmwhmw1_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "epahmwhmw1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def epahmwhmw1_naburelease(context):
    returned_value = gleanerio(context, ("release"), "epahmwhmw1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def epahmwhmw1_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "epahmwhmw1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def epahmwhmw1_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "epahmwhmw1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def epahmwhmw1_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "epahmwhmw1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def epahmwhmw1_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "epahmwhmw1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def epahmwhmw1_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "epahmwhmw1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def epahmwhmw1_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "epahmwhmw1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def epahmwhmw1_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "epahmwhmw1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def epahmwhmw1_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "epahmwhmw1")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def epahmwhmw1_uploadrelease(context):
#     returned_value = post_to_graph("epahmwhmw1", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def epahmwhmw1_uploadrelease(context):
#     returned_value = postRelease("epahmwhmw1")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def epahmwhmw1_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="epahmwhmw1"
    )
...
@op(ins={"start": In(Nothing)})
def epahmwhmw1_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="epahmwhmw1"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def epahmwhmw1_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="epahmwhmw1"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def epahmwhmw1_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="epahmwhmw1"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def epahmwhmw1_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def epahmwhmw1_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def epahmwhmw1_upload_summarize(context):
...
@graph
def harvest_epahmwhmw1():
    # check for containers
    containers = epahmwhmw1_getImage()

    # conduct the harvest
    harvest = epahmwhmw1_gleaner(start=containers)

    # data branch
    load_release = epahmwhmw1_naburelease(start=harvest)
    load_uploadrelease = epahmwhmw1_uploadrelease(start=load_release)
    load_prune = epahmwhmw1_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = epahmwhmw1_nabu_provrelease(start=harvest)
    prov_clear = epahmwhmw1_nabu_provclear(start=prov_release)
    prov_object = epahmwhmw1_nabu_provobject(start=prov_clear)
    prov_drain = epahmwhmw1_nabu_provdrain(start=prov_object)

    # org branch
    org_release = epahmwhmw1_nabu_orgsrelease(start=harvest)
    load_org = epahmwhmw1_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = epahmwhmw1_missingreport_s3(start=harvest)
    # report_idstat = epahmwhmw1_identifier_stats(start=report_ms3)
    # report_bucketurl = epahmwhmw1_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = epahmwhmw1_summarize(start=load_uploadrelease)
    # upload_summarize = epahmwhmw1_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = epahmwhmw1_missingreport_graph(start=summarize)
    # report_graph = epahmwhmw1_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="epahmwhmw1"):
#
