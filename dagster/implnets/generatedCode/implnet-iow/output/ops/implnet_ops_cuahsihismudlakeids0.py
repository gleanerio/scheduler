...
@op
def cuahsihismudlakeids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihismudlakeids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismudlakeids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihismudlakeids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismudlakeids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihismudlakeids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismudlakeids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihismudlakeids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihismudlakeids0_uploadrelease(context, msg: str):
...
@op
def cuahsihismudlakeids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismudlakeids0")
    ...
    return msg + r
@op
def cuahsihismudlakeids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismudlakeids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihismudlakeids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismudlakeids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihismudlakeids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismudlakeids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihismudlakeids0"
...
def cuahsihismudlakeids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihismudlakeids0"
...
def harvest_cuahsihismudlakeids0():
    harvest = cuahsihismudlakeids0_gleaner()

    report_ms3 = cuahsihismudlakeids0_missingreport_s3(harvest)
    report_idstat = cuahsihismudlakeids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihismudlakeids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihismudlakeids0")
    load_release = cuahsihismudlakeids0_naburelease(harvest)
    load_uploadrelease = cuahsihismudlakeids0_uploadrelease(load_release)

    load_prune = cuahsihismudlakeids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihismudlakeids0_nabuprov(load_prune)
    load_org = cuahsihismudlakeids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihismudlakeids0_missingreport_graph(load_org)
    report_graph=cuahsihismudlakeids0_graph_reports(report_msgraph)
