...
@op
def nwisgw17_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw17")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw17_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw17")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw17_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw17")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw17_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw17")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw17_uploadrelease(context, msg: str):
...
@op
def nwisgw17_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw17")
    ...
    return msg + r
@op
def nwisgw17_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw17")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw17_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw17")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw17_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw17")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw17"
...
def nwisgw17_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw17"
...
def harvest_nwisgw17():
    harvest = nwisgw17_gleaner()

    report_ms3 = nwisgw17_missingreport_s3(harvest)
    report_idstat = nwisgw17_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw17_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw17")
    load_release = nwisgw17_naburelease(harvest)
    load_uploadrelease = nwisgw17_uploadrelease(load_release)

    load_prune = nwisgw17_nabu_prune(load_uploadrelease)
    load_prov = nwisgw17_nabuprov(load_prune)
    load_org = nwisgw17_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw17_missingreport_graph(load_org)
    report_graph=nwisgw17_graph_reports(report_msgraph)
