...
def nwisgw17_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "nwisgw17")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def nwisgw17_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "nwisgw17")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw17_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "nwisgw17")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw17_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "nwisgw17")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw17_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "nwisgw17")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw17_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "nwisgw17")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw17_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def nwisgw17_uploadrelease(context):
#     returned_value = post_to_graph("nwisgw17", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def nwisgw17_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nwisgw17"
    )
...
@op(ins={"start": In(Nothing)})
def nwisgw17_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def nwisgw17_upload_summarize(context):
...
def harvest_nwisgw17():
    ...
    # prov branch
    prov_release = nwisgw17_nabu_provrelease(start=harvest)
    prov_clear = nwisgw17_nabu_provclear(start=prov_release)
    prov_object = nwisgw17_nabu_provobject(start=prov_clear)
    prov_drain = nwisgw17_nabu_provdrain(start=prov_object)

    # org branch
    org_release = nwisgw17_nabu_orgsrelease(start=harvest)
    load_org = nwisgw17_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = nwisgw17_summarize(start=load_uploadrelease)
    # upload_summarize = nwisgw17_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = nwisgw17_missingreport_graph(start=summarize)
