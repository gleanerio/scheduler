...
# #########################  Ops Section
@op
def cuahsi_cuahsi_his_globalriversobservatory_ids__0_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def cuahsi_cuahsi_his_globalriversobservatory_ids__0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "cuahsi_cuahsi_his_globalriversobservatory_ids__0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsi_cuahsi_his_globalriversobservatory_ids__0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "cuahsi_cuahsi_his_globalriversobservatory_ids__0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsi_cuahsi_his_globalriversobservatory_ids__0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsi_cuahsi_his_globalriversobservatory_ids__0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsi_cuahsi_his_globalriversobservatory_ids__0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "cuahsi_cuahsi_his_globalriversobservatory_ids__0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsi_cuahsi_his_globalriversobservatory_ids__0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsi_cuahsi_his_globalriversobservatory_ids__0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsi_cuahsi_his_globalriversobservatory_ids__0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsi_cuahsi_his_globalriversobservatory_ids__0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsi_cuahsi_his_globalriversobservatory_ids__0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsi_cuahsi_his_globalriversobservatory_ids__0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsi_cuahsi_his_globalriversobservatory_ids__0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsi_cuahsi_his_globalriversobservatory_ids__0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsi_cuahsi_his_globalriversobservatory_ids__0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsi_cuahsi_his_globalriversobservatory_ids__0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsi_cuahsi_his_globalriversobservatory_ids__0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "cuahsi_cuahsi_his_globalriversobservatory_ids__0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsi_cuahsi_his_globalriversobservatory_ids__0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsi_cuahsi_his_globalriversobservatory_ids__0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsi_cuahsi_his_globalriversobservatory_ids__0_uploadrelease(context):
#     returned_value = postRelease("cuahsi_cuahsi_his_globalriversobservatory_ids__0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def cuahsi_cuahsi_his_globalriversobservatory_ids__0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsi_cuahsi_his_globalriversobservatory_ids__0"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsi_cuahsi_his_globalriversobservatory_ids__0"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def cuahsi_cuahsi_his_globalriversobservatory_ids__0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsi_cuahsi_his_globalriversobservatory_ids__0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def cuahsi_cuahsi_his_globalriversobservatory_ids__0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsi_cuahsi_his_globalriversobservatory_ids__0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def cuahsi_cuahsi_his_globalriversobservatory_ids__0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsi_cuahsi_his_globalriversobservatory_ids__0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def cuahsi_cuahsi_his_globalriversobservatory_ids__0_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsi_cuahsi_his_globalriversobservatory_ids__0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsi_cuahsi_his_globalriversobservatory_ids__0_upload_summarize(context):
    returned_value = post_to_graph(
        "cuahsi_cuahsi_his_globalriversobservatory_ids__0",
        path=SUMMARY_PATH,
        extension="ttl",
...
@graph
def harvest_cuahsi_cuahsi_his_globalriversobservatory_ids__0():
    # check for containers
    containers = cuahsi_cuahsi_his_globalriversobservatory_ids__0_getImage()

    # conduct the harvest
    harvest = cuahsi_cuahsi_his_globalriversobservatory_ids__0_gleaner(start=containers)

    # data branch
    load_release = cuahsi_cuahsi_his_globalriversobservatory_ids__0_naburelease(start=harvest)
    load_uploadrelease = cuahsi_cuahsi_his_globalriversobservatory_ids__0_uploadrelease(start=load_release)
    load_prune = cuahsi_cuahsi_his_globalriversobservatory_ids__0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = cuahsi_cuahsi_his_globalriversobservatory_ids__0_nabu_provrelease(start=harvest)
    prov_clear = cuahsi_cuahsi_his_globalriversobservatory_ids__0_nabu_provclear(start=prov_release)
    prov_object = cuahsi_cuahsi_his_globalriversobservatory_ids__0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsi_cuahsi_his_globalriversobservatory_ids__0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsi_cuahsi_his_globalriversobservatory_ids__0_nabu_orgsrelease(start=harvest)
    load_org = cuahsi_cuahsi_his_globalriversobservatory_ids__0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = cuahsi_cuahsi_his_globalriversobservatory_ids__0_missingreport_s3(start=harvest)
    # report_idstat = cuahsi_cuahsi_his_globalriversobservatory_ids__0_identifier_stats(start=report_ms3)
    # report_bucketurl = cuahsi_cuahsi_his_globalriversobservatory_ids__0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = cuahsi_cuahsi_his_globalriversobservatory_ids__0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsi_cuahsi_his_globalriversobservatory_ids__0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsi_cuahsi_his_globalriversobservatory_ids__0_missingreport_graph(start=summarize)
    # report_graph = cuahsi_cuahsi_his_globalriversobservatory_ids__0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="cuahsi_cuahsi_his_globalriversobservatory_ids__0"):
#
