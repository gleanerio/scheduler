...
@op
def cuahsihisczopsuids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisczopsuids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisczopsuids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisczopsuids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisczopsuids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisczopsuids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisczopsuids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisczopsuids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisczopsuids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisczopsuids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisczopsuids0")
    ...
    return msg + r
@op
def cuahsihisczopsuids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisczopsuids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisczopsuids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisczopsuids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisczopsuids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisczopsuids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisczopsuids0"
...
def cuahsihisczopsuids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisczopsuids0"
...
def harvest_cuahsihisczopsuids0():
    harvest = cuahsihisczopsuids0_gleaner()

    report_ms3 = cuahsihisczopsuids0_missingreport_s3(harvest)
    report_idstat = cuahsihisczopsuids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisczopsuids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisczopsuids0")
    load_release = cuahsihisczopsuids0_naburelease(harvest)
    load_uploadrelease = cuahsihisczopsuids0_uploadrelease(load_release)

    load_prune = cuahsihisczopsuids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisczopsuids0_nabuprov(load_prune)
    load_org = cuahsihisczopsuids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisczopsuids0_missingreport_graph(load_org)
    report_graph=cuahsihisczopsuids0_graph_reports(report_msgraph)
