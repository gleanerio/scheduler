...
# #########################  Ops Section
@op
def iowstategagesnednr0_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def iowstategagesnednr0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "iowstategagesnednr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowstategagesnednr0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "iowstategagesnednr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowstategagesnednr0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "iowstategagesnednr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowstategagesnednr0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "iowstategagesnednr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowstategagesnednr0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "iowstategagesnednr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowstategagesnednr0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "iowstategagesnednr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowstategagesnednr0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "iowstategagesnednr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowstategagesnednr0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "iowstategagesnednr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowstategagesnednr0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "iowstategagesnednr0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def iowstategagesnednr0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "iowstategagesnednr0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def iowstategagesnednr0_uploadrelease(context):
#     returned_value = post_to_graph("iowstategagesnednr0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def iowstategagesnednr0_uploadrelease(context):
#     returned_value = postRelease("iowstategagesnednr0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def iowstategagesnednr0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="iowstategagesnednr0"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "iowstategagesnednr0"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def iowstategagesnednr0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="iowstategagesnednr0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def iowstategagesnednr0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="iowstategagesnednr0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def iowstategagesnednr0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="iowstategagesnednr0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def iowstategagesnednr0_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def iowstategagesnednr0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def iowstategagesnednr0_upload_summarize(context):
...
@graph
def harvest_iowstategagesnednr0():
    # check for containers
    containers = iowstategagesnednr0_getImage()

    # conduct the harvest
    harvest = iowstategagesnednr0_gleaner(start=containers)

    # data branch
    load_release = iowstategagesnednr0_naburelease(start=harvest)
    load_uploadrelease = iowstategagesnednr0_uploadrelease(start=load_release)
    load_prune = iowstategagesnednr0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = iowstategagesnednr0_nabu_provrelease(start=harvest)
    prov_clear = iowstategagesnednr0_nabu_provclear(start=prov_release)
    prov_object = iowstategagesnednr0_nabu_provobject(start=prov_clear)
    prov_drain = iowstategagesnednr0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = iowstategagesnednr0_nabu_orgsrelease(start=harvest)
    load_org = iowstategagesnednr0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = iowstategagesnednr0_missingreport_s3(start=harvest)
    # report_idstat = iowstategagesnednr0_identifier_stats(start=report_ms3)
    # report_bucketurl = iowstategagesnednr0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = iowstategagesnednr0_summarize(start=load_uploadrelease)
    # upload_summarize = iowstategagesnednr0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = iowstategagesnednr0_missingreport_graph(start=summarize)
    # report_graph = iowstategagesnednr0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="iowstategagesnednr0"):
#
