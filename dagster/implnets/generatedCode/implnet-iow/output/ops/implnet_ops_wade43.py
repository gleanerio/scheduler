...
@op(ins={"start": In(Nothing)})
def wade43_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade43")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade43_nabuorg(context):
...
def wade43_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade43")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade43")
    ...
    load_prov = wade43_nabuprov(start=load_prune)
    load_org = wade43_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade43_missingreport_graph(start=load_org)
