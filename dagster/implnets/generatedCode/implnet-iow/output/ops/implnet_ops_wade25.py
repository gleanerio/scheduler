...
@op(ins={"start": In(Nothing)})
def wade25_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade25")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade25_nabuorg(context):
...
def wade25_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade25")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade25")
    ...
    load_prov = wade25_nabuprov(start=load_prune)
    load_org = wade25_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade25_missingreport_graph(start=load_org)
