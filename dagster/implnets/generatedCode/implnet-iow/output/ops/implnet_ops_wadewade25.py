...
# #########################  Ops Section
@op
def wadewade25_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def wadewade25_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "wadewade25")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade25_naburelease(context):
    returned_value = gleanerio(context, ("release"), "wadewade25")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade25_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wadewade25")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade25_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "wadewade25")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade25_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wadewade25")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade25_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wadewade25")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade25_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wadewade25")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade25_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wadewade25")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade25_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wadewade25")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wadewade25_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "wadewade25")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wadewade25_uploadrelease(context):
#     returned_value = post_to_graph("wadewade25", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wadewade25_uploadrelease(context):
#     returned_value = postRelease("wadewade25")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def wadewade25_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade25"
    )
...
@op(ins={"start": In(Nothing)})
def wadewade25_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade25"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def wadewade25_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade25"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def wadewade25_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wadewade25"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def wadewade25_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wadewade25_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wadewade25_upload_summarize(context):
...
@graph
def harvest_wadewade25():
    # check for containers
    containers = wadewade25_getImage()

    # conduct the harvest
    harvest = wadewade25_gleaner(start=containers)

    # data branch
    load_release = wadewade25_naburelease(start=harvest)
    load_uploadrelease = wadewade25_uploadrelease(start=load_release)
    load_prune = wadewade25_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = wadewade25_nabu_provrelease(start=harvest)
    prov_clear = wadewade25_nabu_provclear(start=prov_release)
    prov_object = wadewade25_nabu_provobject(start=prov_clear)
    prov_drain = wadewade25_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wadewade25_nabu_orgsrelease(start=harvest)
    load_org = wadewade25_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = wadewade25_missingreport_s3(start=harvest)
    # report_idstat = wadewade25_identifier_stats(start=report_ms3)
    # report_bucketurl = wadewade25_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = wadewade25_summarize(start=load_uploadrelease)
    # upload_summarize = wadewade25_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wadewade25_missingreport_graph(start=summarize)
    # report_graph = wadewade25_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="wadewade25"):
#
