...
# #########################  Ops Section
@op
def cuahsicuahsihisshalenetworkodmids0_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def cuahsicuahsihisshalenetworkodmids0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "cuahsicuahsihisshalenetworkodmids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisshalenetworkodmids0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "cuahsicuahsihisshalenetworkodmids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisshalenetworkodmids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsicuahsihisshalenetworkodmids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisshalenetworkodmids0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "cuahsicuahsihisshalenetworkodmids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisshalenetworkodmids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsicuahsihisshalenetworkodmids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisshalenetworkodmids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsicuahsihisshalenetworkodmids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisshalenetworkodmids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsicuahsihisshalenetworkodmids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisshalenetworkodmids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsicuahsihisshalenetworkodmids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisshalenetworkodmids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsicuahsihisshalenetworkodmids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisshalenetworkodmids0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "cuahsicuahsihisshalenetworkodmids0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihisshalenetworkodmids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsicuahsihisshalenetworkodmids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihisshalenetworkodmids0_uploadrelease(context):
#     returned_value = postRelease("cuahsicuahsihisshalenetworkodmids0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisshalenetworkodmids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisshalenetworkodmids0"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsicuahsihisshalenetworkodmids0"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisshalenetworkodmids0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisshalenetworkodmids0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisshalenetworkodmids0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisshalenetworkodmids0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisshalenetworkodmids0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisshalenetworkodmids0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisshalenetworkodmids0_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisshalenetworkodmids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisshalenetworkodmids0_upload_summarize(context):
    returned_value = post_to_graph(
        "cuahsicuahsihisshalenetworkodmids0",
        path=SUMMARY_PATH,
        extension="ttl",
...
@graph
def harvest_cuahsicuahsihisshalenetworkodmids0():
    # check for containers
    containers = cuahsicuahsihisshalenetworkodmids0_getImage()

    # conduct the harvest
    harvest = cuahsicuahsihisshalenetworkodmids0_gleaner(start=containers)

    # data branch
    load_release = cuahsicuahsihisshalenetworkodmids0_naburelease(start=harvest)
    load_uploadrelease = cuahsicuahsihisshalenetworkodmids0_uploadrelease(start=load_release)
    load_prune = cuahsicuahsihisshalenetworkodmids0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = cuahsicuahsihisshalenetworkodmids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsicuahsihisshalenetworkodmids0_nabu_provclear(start=prov_release)
    prov_object = cuahsicuahsihisshalenetworkodmids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsicuahsihisshalenetworkodmids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsicuahsihisshalenetworkodmids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsicuahsihisshalenetworkodmids0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = cuahsicuahsihisshalenetworkodmids0_missingreport_s3(start=harvest)
    # report_idstat = cuahsicuahsihisshalenetworkodmids0_identifier_stats(start=report_ms3)
    # report_bucketurl = cuahsicuahsihisshalenetworkodmids0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = cuahsicuahsihisshalenetworkodmids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsicuahsihisshalenetworkodmids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsicuahsihisshalenetworkodmids0_missingreport_graph(start=summarize)
    # report_graph = cuahsicuahsihisshalenetworkodmids0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="cuahsicuahsihisshalenetworkodmids0"):
#
