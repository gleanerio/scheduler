...
@op
def cuahsihisnashrwaids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisnashrwaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisnashrwaids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisnashrwaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisnashrwaids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisnashrwaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisnashrwaids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisnashrwaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisnashrwaids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisnashrwaids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisnashrwaids0")
    ...
    return msg + r
@op
def cuahsihisnashrwaids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisnashrwaids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisnashrwaids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisnashrwaids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisnashrwaids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisnashrwaids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisnashrwaids0"
...
def cuahsihisnashrwaids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisnashrwaids0"
...
def harvest_cuahsihisnashrwaids0():
    harvest = cuahsihisnashrwaids0_gleaner()

    report_ms3 = cuahsihisnashrwaids0_missingreport_s3(harvest)
    report_idstat = cuahsihisnashrwaids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisnashrwaids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisnashrwaids0")
    load_release = cuahsihisnashrwaids0_naburelease(harvest)
    load_uploadrelease = cuahsihisnashrwaids0_uploadrelease(load_release)

    load_prune = cuahsihisnashrwaids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisnashrwaids0_nabuprov(load_prune)
    load_org = cuahsihisnashrwaids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisnashrwaids0_missingreport_graph(load_org)
    report_graph=cuahsihisnashrwaids0_graph_reports(report_msgraph)
