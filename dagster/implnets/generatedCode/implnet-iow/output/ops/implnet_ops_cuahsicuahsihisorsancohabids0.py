...
# #########################  Ops Section
@op
def cuahsicuahsihisorsancohabids0_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def cuahsicuahsihisorsancohabids0_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "cuahsicuahsihisorsancohabids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisorsancohabids0_naburelease(context):
    returned_value = gleanerio(context, ("release"), "cuahsicuahsihisorsancohabids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisorsancohabids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsicuahsihisorsancohabids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisorsancohabids0_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "cuahsicuahsihisorsancohabids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisorsancohabids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsicuahsihisorsancohabids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisorsancohabids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsicuahsihisorsancohabids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisorsancohabids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsicuahsihisorsancohabids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisorsancohabids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsicuahsihisorsancohabids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisorsancohabids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsicuahsihisorsancohabids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsicuahsihisorsancohabids0_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "cuahsicuahsihisorsancohabids0")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihisorsancohabids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsicuahsihisorsancohabids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def cuahsicuahsihisorsancohabids0_uploadrelease(context):
#     returned_value = postRelease("cuahsicuahsihisorsancohabids0")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisorsancohabids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisorsancohabids0"
    )
    ...
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsicuahsihisorsancohabids0"
    graphendpoint = None
    milled = False
    summon = True
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisorsancohabids0_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisorsancohabids0"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisorsancohabids0_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisorsancohabids0"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisorsancohabids0_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsicuahsihisorsancohabids0"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisorsancohabids0_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisorsancohabids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsicuahsihisorsancohabids0_upload_summarize(context):
    returned_value = post_to_graph(
        "cuahsicuahsihisorsancohabids0",
        path=SUMMARY_PATH,
        extension="ttl",
...
@graph
def harvest_cuahsicuahsihisorsancohabids0():
    # check for containers
    containers = cuahsicuahsihisorsancohabids0_getImage()

    # conduct the harvest
    harvest = cuahsicuahsihisorsancohabids0_gleaner(start=containers)

    # data branch
    load_release = cuahsicuahsihisorsancohabids0_naburelease(start=harvest)
    load_uploadrelease = cuahsicuahsihisorsancohabids0_uploadrelease(start=load_release)
    load_prune = cuahsicuahsihisorsancohabids0_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = cuahsicuahsihisorsancohabids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsicuahsihisorsancohabids0_nabu_provclear(start=prov_release)
    prov_object = cuahsicuahsihisorsancohabids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsicuahsihisorsancohabids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsicuahsihisorsancohabids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsicuahsihisorsancohabids0_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = cuahsicuahsihisorsancohabids0_missingreport_s3(start=harvest)
    # report_idstat = cuahsicuahsihisorsancohabids0_identifier_stats(start=report_ms3)
    # report_bucketurl = cuahsicuahsihisorsancohabids0_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = cuahsicuahsihisorsancohabids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsicuahsihisorsancohabids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsicuahsihisorsancohabids0_missingreport_graph(start=summarize)
    # report_graph = cuahsicuahsihisorsancohabids0_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="cuahsicuahsihisorsancohabids0"):
#
