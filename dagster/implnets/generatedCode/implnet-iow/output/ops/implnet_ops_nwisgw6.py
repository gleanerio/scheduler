...
def nwisgw6_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "nwisgw6")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def nwisgw6_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "nwisgw6")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw6_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "nwisgw6")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw6_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "nwisgw6")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw6_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "nwisgw6")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw6_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "nwisgw6")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw6_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def nwisgw6_uploadrelease(context):
#     returned_value = post_to_graph("nwisgw6", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def nwisgw6_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nwisgw6"
    )
...
@op(ins={"start": In(Nothing)})
def nwisgw6_upload_summarize(context):
...
def harvest_nwisgw6():
    ...
    # prov branch
    prov_release = nwisgw6_nabu_provrelease(start=harvest)
    prov_clear = nwisgw6_nabu_provclear(start=prov_release)
    prov_object = nwisgw6_nabu_provobject(start=prov_clear)
    prov_drain = nwisgw6_nabu_provdrain(start=prov_object)

    # org branch
    org_release = nwisgw6_nabu_orgsrelease(start=harvest)
    load_org = nwisgw6_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = nwisgw6_summarize(start=load_uploadrelease)
    # upload_summarize = nwisgw6_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = nwisgw6_missingreport_graph(start=summarize)
