...
@op
def nwisgw6_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw6")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw6_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw6")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw6_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw6")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw6_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw6")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw6_uploadrelease(context, msg: str):
...
@op
def nwisgw6_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw6")
    ...
    return msg + r
@op
def nwisgw6_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw6")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw6_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw6")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw6_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw6")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw6"
...
def nwisgw6_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw6"
...
@graph
def harvest_nwisgw6():
    harvest = nwisgw6_gleaner()

    report_ms3 = nwisgw6_missingreport_s3(harvest)
    report_idstat = nwisgw6_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw6_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw6")
    load_release = nwisgw6_naburelease(harvest)
    load_uploadrelease = nwisgw6_uploadrelease(load_release)

    load_prune = nwisgw6_nabu_prune(load_uploadrelease)
    load_prov = nwisgw6_nabuprov(load_prune)
    load_org = nwisgw6_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw6_missingreport_graph(load_org)
    report_graph=nwisgw6_graph_reports(report_msgraph)
