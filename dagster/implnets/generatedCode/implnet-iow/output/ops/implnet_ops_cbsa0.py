...
def cbsa0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cbsa0")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def cbsa0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cbsa0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cbsa0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cbsa0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cbsa0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cbsa0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cbsa0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cbsa0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cbsa0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cbsa0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cbsa0_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def cbsa0_uploadrelease(context):
#     returned_value = post_to_graph("cbsa0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def cbsa0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cbsa0"
    )
...
@op(ins={"start": In(Nothing)})
def cbsa0_upload_summarize(context):
...
def harvest_cbsa0():
    ...
    # prov branch
    prov_release = cbsa0_nabu_provrelease(start=harvest)
    prov_clear = cbsa0_nabu_provclear(start=prov_release)
    prov_object = cbsa0_nabu_provobject(start=prov_clear)
    prov_drain = cbsa0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cbsa0_nabu_orgsrelease(start=harvest)
    load_org = cbsa0_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = cbsa0_summarize(start=load_uploadrelease)
    # upload_summarize = cbsa0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cbsa0_missingreport_graph(start=summarize)
