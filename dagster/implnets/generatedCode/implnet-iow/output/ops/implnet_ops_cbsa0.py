...
@op
def cbsa0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cbsa0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cbsa0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cbsa0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cbsa0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cbsa0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cbsa0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cbsa0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cbsa0_uploadrelease(context, msg: str):
...
@op
def cbsa0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cbsa0")
    ...
    return msg + r
@op
def cbsa0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cbsa0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cbsa0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cbsa0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cbsa0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cbsa0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cbsa0"
...
def cbsa0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cbsa0"
...
@graph
def harvest_cbsa0():
    harvest = cbsa0_gleaner()

    report_ms3 = cbsa0_missingreport_s3(harvest)
    report_idstat = cbsa0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cbsa0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cbsa0")
    load_release = cbsa0_naburelease(harvest)
    load_uploadrelease = cbsa0_uploadrelease(load_release)

    load_prune = cbsa0_nabu_prune(load_uploadrelease)
    load_prov = cbsa0_nabuprov(load_prune)
    load_org = cbsa0_nabuorg(load_prov)

# run after load
    report_msgraph=cbsa0_missingreport_graph(load_org)
    report_graph=cbsa0_graph_reports(report_msgraph)
