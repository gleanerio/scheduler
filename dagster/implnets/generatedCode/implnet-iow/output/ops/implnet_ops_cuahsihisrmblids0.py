...
@op
def cuahsihisrmblids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisrmblids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisrmblids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisrmblids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisrmblids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisrmblids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisrmblids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisrmblids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisrmblids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisrmblids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisrmblids0")
    ...
    return msg + r
@op
def cuahsihisrmblids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisrmblids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisrmblids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisrmblids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisrmblids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisrmblids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisrmblids0"
...
def cuahsihisrmblids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisrmblids0"
...
def harvest_cuahsihisrmblids0():
    harvest = cuahsihisrmblids0_gleaner()

    report_ms3 = cuahsihisrmblids0_missingreport_s3(harvest)
    report_idstat = cuahsihisrmblids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisrmblids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisrmblids0")
    load_release = cuahsihisrmblids0_naburelease(harvest)
    load_uploadrelease = cuahsihisrmblids0_uploadrelease(load_release)

    load_prune = cuahsihisrmblids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisrmblids0_nabuprov(load_prune)
    load_org = cuahsihisrmblids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisrmblids0_missingreport_graph(load_org)
    report_graph=cuahsihisrmblids0_graph_reports(report_msgraph)
