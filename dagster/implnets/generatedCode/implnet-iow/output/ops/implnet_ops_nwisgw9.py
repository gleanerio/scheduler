...
@op
def nwisgw9_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw9")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw9_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw9")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw9_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw9")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw9_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw9")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw9_uploadrelease(context, msg: str):
...
@op
def nwisgw9_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw9")
    ...
    return msg + r
@op
def nwisgw9_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw9")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw9_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw9")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw9_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw9")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw9"
...
def nwisgw9_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw9"
...
@graph
def harvest_nwisgw9():
    harvest = nwisgw9_gleaner()

    report_ms3 = nwisgw9_missingreport_s3(harvest)
    report_idstat = nwisgw9_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw9_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw9")
    load_release = nwisgw9_naburelease(harvest)
    load_uploadrelease = nwisgw9_uploadrelease(load_release)

    load_prune = nwisgw9_nabu_prune(load_uploadrelease)
    load_prov = nwisgw9_nabuprov(load_prune)
    load_org = nwisgw9_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw9_missingreport_graph(load_org)
    report_graph=nwisgw9_graph_reports(report_msgraph)
