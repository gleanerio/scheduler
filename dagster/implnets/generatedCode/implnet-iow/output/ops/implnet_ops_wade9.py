...
@op
def wade9_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "wade9")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade9_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "wade9")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade9_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "wade9")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade9_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "wade9")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def wade9_uploadrelease(context, msg: str):
...
@op
def wade9_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade9")
    ...
    return msg + r
@op
def wade9_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade9")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def wade9_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade9")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def wade9_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade9")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade9"
...
def wade9_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade9"
...
@graph
def harvest_wade9():
    harvest = wade9_gleaner()

    report_ms3 = wade9_missingreport_s3(harvest)
    report_idstat = wade9_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = wade9_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="wade9")
    load_release = wade9_naburelease(harvest)
    load_uploadrelease = wade9_uploadrelease(load_release)

    load_prune = wade9_nabu_prune(load_uploadrelease)
    load_prov = wade9_nabuprov(load_prune)
    load_org = wade9_nabuorg(load_prov)

# run after load
    report_msgraph=wade9_missingreport_graph(load_org)
    report_graph=wade9_graph_reports(report_msgraph)
