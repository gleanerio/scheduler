...
@op(ins={"start": In(Nothing)})
def wade29_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade29")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade29_nabuorg(context):
...
def wade29_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade29")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade29")
    ...
    load_prov = wade29_nabuprov(start=load_prune)
    load_org = wade29_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade29_missingreport_graph(start=load_org)
