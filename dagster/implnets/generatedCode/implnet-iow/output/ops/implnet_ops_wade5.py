...
@op
def wade5_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "wade5")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade5_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "wade5")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade5_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "wade5")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade5_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "wade5")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def wade5_uploadrelease(context, msg: str):
...
@op
def wade5_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade5")
    ...
    return msg + r
@op
def wade5_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade5")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def wade5_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade5")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def wade5_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade5")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade5"
...
def wade5_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade5"
...
@graph
def harvest_wade5():
    harvest = wade5_gleaner()

    report_ms3 = wade5_missingreport_s3(harvest)
    report_idstat = wade5_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = wade5_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="wade5")
    load_release = wade5_naburelease(harvest)
    load_uploadrelease = wade5_uploadrelease(load_release)

    load_prune = wade5_nabu_prune(load_uploadrelease)
    load_prov = wade5_nabuprov(load_prune)
    load_org = wade5_nabuorg(load_prov)

# run after load
    report_msgraph=wade5_missingreport_graph(load_org)
    report_graph=wade5_graph_reports(report_msgraph)
