...
def wade5_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wade5")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def wade5_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wade5")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade5_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wade5")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade5_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wade5")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade5_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wade5")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade5_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wade5")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade5_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def wade5_uploadrelease(context):
#     returned_value = post_to_graph("wade5", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def wade5_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade5"
    )
...
@op(ins={"start": In(Nothing)})
def wade5_upload_summarize(context):
...
def harvest_wade5():
    ...
    # prov branch
    prov_release = wade5_nabu_provrelease(start=harvest)
    prov_clear = wade5_nabu_provclear(start=prov_release)
    prov_object = wade5_nabu_provobject(start=prov_clear)
    prov_drain = wade5_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wade5_nabu_orgsrelease(start=harvest)
    load_org = wade5_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = wade5_summarize(start=load_uploadrelease)
    # upload_summarize = wade5_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wade5_missingreport_graph(start=summarize)
