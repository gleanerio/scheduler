...
@op
def cuahsihistarlandwaterqualityids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihistarlandwaterqualityids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihistarlandwaterqualityids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihistarlandwaterqualityids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihistarlandwaterqualityids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihistarlandwaterqualityids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihistarlandwaterqualityids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihistarlandwaterqualityids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihistarlandwaterqualityids0_uploadrelease(context, msg: str):
...
@op
def cuahsihistarlandwaterqualityids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihistarlandwaterqualityids0")
    ...
    return msg + r
@op
def cuahsihistarlandwaterqualityids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihistarlandwaterqualityids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihistarlandwaterqualityids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihistarlandwaterqualityids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihistarlandwaterqualityids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihistarlandwaterqualityids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihistarlandwaterqualityids0"
...
def cuahsihistarlandwaterqualityids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihistarlandwaterqualityids0"
...
def harvest_cuahsihistarlandwaterqualityids0():
    harvest = cuahsihistarlandwaterqualityids0_gleaner()

    report_ms3 = cuahsihistarlandwaterqualityids0_missingreport_s3(harvest)
    report_idstat = cuahsihistarlandwaterqualityids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihistarlandwaterqualityids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihistarlandwaterqualityids0")
    load_release = cuahsihistarlandwaterqualityids0_naburelease(harvest)
    load_uploadrelease = cuahsihistarlandwaterqualityids0_uploadrelease(load_release)

    load_prune = cuahsihistarlandwaterqualityids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihistarlandwaterqualityids0_nabuprov(load_prune)
    load_org = cuahsihistarlandwaterqualityids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihistarlandwaterqualityids0_missingreport_graph(load_org)
    report_graph=cuahsihistarlandwaterqualityids0_graph_reports(report_msgraph)
