...
@op(ins={"start": In(Nothing)})
def wade22_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade22")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade22_nabuorg(context):
...
def wade22_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade22")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade22")
    ...
    load_prov = wade22_nabuprov(start=load_prune)
    load_org = wade22_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade22_missingreport_graph(start=load_org)
