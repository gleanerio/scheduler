...
@op
def wade12_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "wade12")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade12_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "wade12")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade12_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "wade12")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade12_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "wade12")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def wade12_uploadrelease(context, msg: str):
...
@op
def wade12_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade12")
    ...
    return msg + r
@op
def wade12_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade12")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def wade12_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade12")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def wade12_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade12")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade12"
...
def wade12_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade12"
...
@graph
def harvest_wade12():
    harvest = wade12_gleaner()

    report_ms3 = wade12_missingreport_s3(harvest)
    report_idstat = wade12_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = wade12_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="wade12")
    load_release = wade12_naburelease(harvest)
    load_uploadrelease = wade12_uploadrelease(load_release)

    load_prune = wade12_nabu_prune(load_uploadrelease)
    load_prov = wade12_nabuprov(load_prune)
    load_org = wade12_nabuorg(load_prov)

# run after load
    report_msgraph=wade12_missingreport_graph(load_org)
    report_graph=wade12_graph_reports(report_msgraph)
