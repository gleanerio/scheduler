...
@op
def nwisgw0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw0_uploadrelease(context, msg: str):
...
@op
def nwisgw0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw0")
    ...
    return msg + r
@op
def nwisgw0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw0"
...
def nwisgw0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw0"
...
@graph
def harvest_nwisgw0():
    harvest = nwisgw0_gleaner()

    report_ms3 = nwisgw0_missingreport_s3(harvest)
    report_idstat = nwisgw0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw0")
    load_release = nwisgw0_naburelease(harvest)
    load_uploadrelease = nwisgw0_uploadrelease(load_release)

    load_prune = nwisgw0_nabu_prune(load_uploadrelease)
    load_prov = nwisgw0_nabuprov(load_prune)
    load_org = nwisgw0_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw0_missingreport_graph(load_org)
    report_graph=nwisgw0_graph_reports(report_msgraph)
