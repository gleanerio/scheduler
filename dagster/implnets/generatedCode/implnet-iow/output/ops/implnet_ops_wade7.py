...
@op
def wade7_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "wade7")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade7_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "wade7")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade7_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "wade7")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade7_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "wade7")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def wade7_uploadrelease(context, msg: str):
...
@op
def wade7_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade7")
    ...
    return msg + r
@op
def wade7_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade7")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def wade7_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade7")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def wade7_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade7")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade7"
...
def wade7_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade7"
...
@graph
def harvest_wade7():
    harvest = wade7_gleaner()

    report_ms3 = wade7_missingreport_s3(harvest)
    report_idstat = wade7_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = wade7_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="wade7")
    load_release = wade7_naburelease(harvest)
    load_uploadrelease = wade7_uploadrelease(load_release)

    load_prune = wade7_nabu_prune(load_uploadrelease)
    load_prov = wade7_nabuprov(load_prune)
    load_org = wade7_nabuorg(load_prov)

# run after load
    report_msgraph=wade7_missingreport_graph(load_org)
    report_graph=wade7_graph_reports(report_msgraph)
