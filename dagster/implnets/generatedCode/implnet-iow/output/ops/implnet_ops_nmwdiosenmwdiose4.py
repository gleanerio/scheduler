...
# #########################  Ops Section
@op
def nmwdiosenmwdiose4_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def nmwdiosenmwdiose4_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "nmwdiosenmwdiose4")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose4_naburelease(context):
    returned_value = gleanerio(context, ("release"), "nmwdiosenmwdiose4")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose4_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "nmwdiosenmwdiose4")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose4_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "nmwdiosenmwdiose4")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose4_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "nmwdiosenmwdiose4")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose4_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "nmwdiosenmwdiose4")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose4_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "nmwdiosenmwdiose4")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose4_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "nmwdiosenmwdiose4")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose4_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "nmwdiosenmwdiose4")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose4_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "nmwdiosenmwdiose4")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def nmwdiosenmwdiose4_uploadrelease(context):
#     returned_value = post_to_graph("nmwdiosenmwdiose4", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def nmwdiosenmwdiose4_uploadrelease(context):
#     returned_value = postRelease("nmwdiosenmwdiose4")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose4_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nmwdiosenmwdiose4"
    )
...
@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose4_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nmwdiosenmwdiose4"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose4_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nmwdiosenmwdiose4"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose4_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nmwdiosenmwdiose4"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose4_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose4_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def nmwdiosenmwdiose4_upload_summarize(context):
...
@graph
def harvest_nmwdiosenmwdiose4():
    # check for containers
    containers = nmwdiosenmwdiose4_getImage()

    # conduct the harvest
    harvest = nmwdiosenmwdiose4_gleaner(start=containers)

    # data branch
    load_release = nmwdiosenmwdiose4_naburelease(start=harvest)
    load_uploadrelease = nmwdiosenmwdiose4_uploadrelease(start=load_release)
    load_prune = nmwdiosenmwdiose4_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = nmwdiosenmwdiose4_nabu_provrelease(start=harvest)
    prov_clear = nmwdiosenmwdiose4_nabu_provclear(start=prov_release)
    prov_object = nmwdiosenmwdiose4_nabu_provobject(start=prov_clear)
    prov_drain = nmwdiosenmwdiose4_nabu_provdrain(start=prov_object)

    # org branch
    org_release = nmwdiosenmwdiose4_nabu_orgsrelease(start=harvest)
    load_org = nmwdiosenmwdiose4_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = nmwdiosenmwdiose4_missingreport_s3(start=harvest)
    # report_idstat = nmwdiosenmwdiose4_identifier_stats(start=report_ms3)
    # report_bucketurl = nmwdiosenmwdiose4_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = nmwdiosenmwdiose4_summarize(start=load_uploadrelease)
    # upload_summarize = nmwdiosenmwdiose4_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = nmwdiosenmwdiose4_missingreport_graph(start=summarize)
    # report_graph = nmwdiosenmwdiose4_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="nmwdiosenmwdiose4"):
#
