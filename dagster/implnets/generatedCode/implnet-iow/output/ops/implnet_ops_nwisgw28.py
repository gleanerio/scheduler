...
@op
def nwisgw28_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw28")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw28_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw28")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw28_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw28")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw28_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw28")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw28_uploadrelease(context, msg: str):
...
@op
def nwisgw28_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw28")
    ...
    return msg + r
@op
def nwisgw28_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw28")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw28_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw28")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw28_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw28")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw28"
...
def nwisgw28_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw28"
...
def harvest_nwisgw28():
    harvest = nwisgw28_gleaner()

    report_ms3 = nwisgw28_missingreport_s3(harvest)
    report_idstat = nwisgw28_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw28_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw28")
    load_release = nwisgw28_naburelease(harvest)
    load_uploadrelease = nwisgw28_uploadrelease(load_release)

    load_prune = nwisgw28_nabu_prune(load_uploadrelease)
    load_prov = nwisgw28_nabuprov(load_prune)
    load_org = nwisgw28_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw28_missingreport_graph(load_org)
    report_graph=nwisgw28_graph_reports(report_msgraph)
