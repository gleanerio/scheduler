...
@op
def cuahsihisneonids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisneonids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisneonids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisneonids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisneonids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisneonids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisneonids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisneonids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisneonids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisneonids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisneonids0")
    ...
    return msg + r
@op
def cuahsihisneonids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisneonids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisneonids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisneonids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisneonids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisneonids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisneonids0"
...
def cuahsihisneonids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisneonids0"
...
def harvest_cuahsihisneonids0():
    harvest = cuahsihisneonids0_gleaner()

    report_ms3 = cuahsihisneonids0_missingreport_s3(harvest)
    report_idstat = cuahsihisneonids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisneonids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisneonids0")
    load_release = cuahsihisneonids0_naburelease(harvest)
    load_uploadrelease = cuahsihisneonids0_uploadrelease(load_release)

    load_prune = cuahsihisneonids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisneonids0_nabuprov(load_prune)
    load_org = cuahsihisneonids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisneonids0_missingreport_graph(load_org)
    report_graph=cuahsihisneonids0_graph_reports(report_msgraph)
