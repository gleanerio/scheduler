...
def nwisgw5_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "nwisgw5")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def nwisgw5_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "nwisgw5")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw5_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "nwisgw5")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw5_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "nwisgw5")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw5_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "nwisgw5")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw5_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "nwisgw5")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw5_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def nwisgw5_uploadrelease(context):
#     returned_value = post_to_graph("nwisgw5", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def nwisgw5_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nwisgw5"
    )
...
@op(ins={"start": In(Nothing)})
def nwisgw5_upload_summarize(context):
...
def harvest_nwisgw5():
    ...
    # prov branch
    prov_release = nwisgw5_nabu_provrelease(start=harvest)
    prov_clear = nwisgw5_nabu_provclear(start=prov_release)
    prov_object = nwisgw5_nabu_provobject(start=prov_clear)
    prov_drain = nwisgw5_nabu_provdrain(start=prov_object)

    # org branch
    org_release = nwisgw5_nabu_orgsrelease(start=harvest)
    load_org = nwisgw5_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = nwisgw5_summarize(start=load_uploadrelease)
    # upload_summarize = nwisgw5_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = nwisgw5_missingreport_graph(start=summarize)
