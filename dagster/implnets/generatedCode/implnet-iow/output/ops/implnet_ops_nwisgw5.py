...
@op
def nwisgw5_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw5")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw5_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw5")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw5_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw5")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw5_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw5")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw5_uploadrelease(context, msg: str):
...
@op
def nwisgw5_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw5")
    ...
    return msg + r
@op
def nwisgw5_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw5")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw5_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw5")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw5_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw5")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw5"
...
def nwisgw5_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw5"
...
@graph
def harvest_nwisgw5():
    harvest = nwisgw5_gleaner()

    report_ms3 = nwisgw5_missingreport_s3(harvest)
    report_idstat = nwisgw5_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw5_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw5")
    load_release = nwisgw5_naburelease(harvest)
    load_uploadrelease = nwisgw5_uploadrelease(load_release)

    load_prune = nwisgw5_nabu_prune(load_uploadrelease)
    load_prov = nwisgw5_nabuprov(load_prune)
    load_org = nwisgw5_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw5_missingreport_graph(load_org)
    report_graph=nwisgw5_graph_reports(report_msgraph)
