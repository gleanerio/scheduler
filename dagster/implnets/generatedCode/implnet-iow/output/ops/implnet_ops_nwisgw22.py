...
@op
def nwisgw22_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw22")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw22_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw22")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw22_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw22")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw22_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw22")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw22_uploadrelease(context, msg: str):
...
@op
def nwisgw22_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw22")
    ...
    return msg + r
@op
def nwisgw22_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw22")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw22_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw22")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw22_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw22")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw22"
...
def nwisgw22_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw22"
...
def harvest_nwisgw22():
    harvest = nwisgw22_gleaner()

    report_ms3 = nwisgw22_missingreport_s3(harvest)
    report_idstat = nwisgw22_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw22_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw22")
    load_release = nwisgw22_naburelease(harvest)
    load_uploadrelease = nwisgw22_uploadrelease(load_release)

    load_prune = nwisgw22_nabu_prune(load_uploadrelease)
    load_prov = nwisgw22_nabuprov(load_prune)
    load_org = nwisgw22_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw22_missingreport_graph(load_org)
    report_graph=nwisgw22_graph_reports(report_msgraph)
