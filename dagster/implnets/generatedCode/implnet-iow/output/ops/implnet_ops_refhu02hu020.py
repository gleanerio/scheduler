...
# #########################  Ops Section
@op
def refhu02hu020_getImage(context):
    run_container_context = DockerContainerContext.create_for_run(
        context.dagster_run,
        context.instance.run_launcher
        if isinstance(context.instance.run_launcher, DockerRunLauncher)
...
def refhu02hu020_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "refhu02hu020")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu02hu020_naburelease(context):
    returned_value = gleanerio(context, ("release"), "refhu02hu020")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu02hu020_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "refhu02hu020")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu02hu020_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "refhu02hu020")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu02hu020_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "refhu02hu020")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu02hu020_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "refhu02hu020")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu02hu020_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "refhu02hu020")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu02hu020_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "refhu02hu020")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu02hu020_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "refhu02hu020")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def refhu02hu020_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "refhu02hu020")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def refhu02hu020_uploadrelease(context):
#     returned_value = post_to_graph("refhu02hu020", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def refhu02hu020_uploadrelease(context):
#     returned_value = postRelease("refhu02hu020")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def refhu02hu020_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refhu02hu020"
    )
...
@op(ins={"start": In(Nothing)})
def refhu02hu020_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refhu02hu020"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def refhu02hu020_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refhu02hu020"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def refhu02hu020_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="refhu02hu020"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def refhu02hu020_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def refhu02hu020_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def refhu02hu020_upload_summarize(context):
...
@graph
def harvest_refhu02hu020():
    # check for containers
    containers = refhu02hu020_getImage()

    # conduct the harvest
    harvest = refhu02hu020_gleaner(start=containers)

    # data branch
    load_release = refhu02hu020_naburelease(start=harvest)
    load_uploadrelease = refhu02hu020_uploadrelease(start=load_release)
    load_prune = refhu02hu020_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = refhu02hu020_nabu_provrelease(start=harvest)
    prov_clear = refhu02hu020_nabu_provclear(start=prov_release)
    prov_object = refhu02hu020_nabu_provobject(start=prov_clear)
    prov_drain = refhu02hu020_nabu_provdrain(start=prov_object)

    # org branch
    org_release = refhu02hu020_nabu_orgsrelease(start=harvest)
    load_org = refhu02hu020_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = refhu02hu020_missingreport_s3(start=harvest)
    # report_idstat = refhu02hu020_identifier_stats(start=report_ms3)
    # report_bucketurl = refhu02hu020_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = refhu02hu020_summarize(start=load_uploadrelease)
    # upload_summarize = refhu02hu020_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = refhu02hu020_missingreport_graph(start=summarize)
    # report_graph = refhu02hu020_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="refhu02hu020"):
#
