...
def rise0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "rise0")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def rise0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "rise0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def rise0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "rise0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def rise0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "rise0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def rise0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "rise0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def rise0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "rise0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def rise0_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def rise0_uploadrelease(context):
#     returned_value = post_to_graph("rise0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def rise0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="rise0"
    )
...
@op(ins={"start": In(Nothing)})
def rise0_upload_summarize(context):
...
def harvest_rise0():
    ...
    # prov branch
    prov_release = rise0_nabu_provrelease(start=harvest)
    prov_clear = rise0_nabu_provclear(start=prov_release)
    prov_object = rise0_nabu_provobject(start=prov_clear)
    prov_drain = rise0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = rise0_nabu_orgsrelease(start=harvest)
    load_org = rise0_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = rise0_summarize(start=load_uploadrelease)
    # upload_summarize = rise0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = rise0_missingreport_graph(start=summarize)
