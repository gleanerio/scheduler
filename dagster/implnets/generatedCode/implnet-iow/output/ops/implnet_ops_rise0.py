...
@op
def rise0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "rise0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def rise0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "rise0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def rise0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "rise0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def rise0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "rise0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def rise0_uploadrelease(context, msg: str):
...
@op
def rise0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="rise0")
    ...
    return msg + r
@op
def rise0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="rise0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def rise0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="rise0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def rise0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="rise0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "rise0"
...
def rise0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "rise0"
...
@graph
def harvest_rise0():
    harvest = rise0_gleaner()

    report_ms3 = rise0_missingreport_s3(harvest)
    report_idstat = rise0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = rise0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="rise0")
    load_release = rise0_naburelease(harvest)
    load_uploadrelease = rise0_uploadrelease(load_release)

    load_prune = rise0_nabu_prune(load_uploadrelease)
    load_prov = rise0_nabuprov(load_prune)
    load_org = rise0_nabuorg(load_prov)

# run after load
    report_msgraph=rise0_missingreport_graph(load_org)
    report_graph=rise0_graph_reports(report_msgraph)
