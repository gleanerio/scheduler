...
@op
def cuahsihisbrazilucbids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisbrazilucbids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisbrazilucbids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisbrazilucbids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisbrazilucbids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisbrazilucbids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisbrazilucbids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisbrazilucbids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisbrazilucbids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisbrazilucbids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisbrazilucbids0")
    ...
    return msg + r
@op
def cuahsihisbrazilucbids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisbrazilucbids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisbrazilucbids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisbrazilucbids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisbrazilucbids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisbrazilucbids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisbrazilucbids0"
...
def cuahsihisbrazilucbids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisbrazilucbids0"
...
def harvest_cuahsihisbrazilucbids0():
    harvest = cuahsihisbrazilucbids0_gleaner()

    report_ms3 = cuahsihisbrazilucbids0_missingreport_s3(harvest)
    report_idstat = cuahsihisbrazilucbids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisbrazilucbids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisbrazilucbids0")
    load_release = cuahsihisbrazilucbids0_naburelease(harvest)
    load_uploadrelease = cuahsihisbrazilucbids0_uploadrelease(load_release)

    load_prune = cuahsihisbrazilucbids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisbrazilucbids0_nabuprov(load_prune)
    load_org = cuahsihisbrazilucbids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisbrazilucbids0_missingreport_graph(load_org)
    report_graph=cuahsihisbrazilucbids0_graph_reports(report_msgraph)
