...
def cuahsihisbrazilucbids0_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "cuahsihisbrazilucbids0")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def cuahsihisbrazilucbids0_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "cuahsihisbrazilucbids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisbrazilucbids0_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "cuahsihisbrazilucbids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisbrazilucbids0_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "cuahsihisbrazilucbids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisbrazilucbids0_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "cuahsihisbrazilucbids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisbrazilucbids0_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "cuahsihisbrazilucbids0")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def cuahsihisbrazilucbids0_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def cuahsihisbrazilucbids0_uploadrelease(context):
#     returned_value = post_to_graph("cuahsihisbrazilucbids0", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def cuahsihisbrazilucbids0_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="cuahsihisbrazilucbids0"
    )
...
@op(ins={"start": In(Nothing)})
def cuahsihisbrazilucbids0_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def cuahsihisbrazilucbids0_upload_summarize(context):
    returned_value = post_to_graph(
        "cuahsihisbrazilucbids0",
        path=SUMMARY_PATH,
        extension="ttl",
...
def harvest_cuahsihisbrazilucbids0():
    ...
    # prov branch
    prov_release = cuahsihisbrazilucbids0_nabu_provrelease(start=harvest)
    prov_clear = cuahsihisbrazilucbids0_nabu_provclear(start=prov_release)
    prov_object = cuahsihisbrazilucbids0_nabu_provobject(start=prov_clear)
    prov_drain = cuahsihisbrazilucbids0_nabu_provdrain(start=prov_object)

    # org branch
    org_release = cuahsihisbrazilucbids0_nabu_orgsrelease(start=harvest)
    load_org = cuahsihisbrazilucbids0_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = cuahsihisbrazilucbids0_summarize(start=load_uploadrelease)
    # upload_summarize = cuahsihisbrazilucbids0_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = cuahsihisbrazilucbids0_missingreport_graph(start=summarize)
