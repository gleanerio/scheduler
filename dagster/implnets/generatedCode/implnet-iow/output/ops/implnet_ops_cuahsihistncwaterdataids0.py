...
@op
def cuahsihistncwaterdataids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihistncwaterdataids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihistncwaterdataids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihistncwaterdataids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihistncwaterdataids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihistncwaterdataids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihistncwaterdataids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihistncwaterdataids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihistncwaterdataids0_uploadrelease(context, msg: str):
...
@op
def cuahsihistncwaterdataids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihistncwaterdataids0")
    ...
    return msg + r
@op
def cuahsihistncwaterdataids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihistncwaterdataids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihistncwaterdataids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihistncwaterdataids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihistncwaterdataids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihistncwaterdataids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihistncwaterdataids0"
...
def cuahsihistncwaterdataids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihistncwaterdataids0"
...
def harvest_cuahsihistncwaterdataids0():
    harvest = cuahsihistncwaterdataids0_gleaner()

    report_ms3 = cuahsihistncwaterdataids0_missingreport_s3(harvest)
    report_idstat = cuahsihistncwaterdataids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihistncwaterdataids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihistncwaterdataids0")
    load_release = cuahsihistncwaterdataids0_naburelease(harvest)
    load_uploadrelease = cuahsihistncwaterdataids0_uploadrelease(load_release)

    load_prune = cuahsihistncwaterdataids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihistncwaterdataids0_nabuprov(load_prune)
    load_org = cuahsihistncwaterdataids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihistncwaterdataids0_missingreport_graph(load_org)
    report_graph=cuahsihistncwaterdataids0_graph_reports(report_msgraph)
