...
@op(ins={"start": In(Nothing)})
def wade40_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade40")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade40_nabuorg(context):
...
def wade40_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade40")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade40")
    ...
    load_prov = wade40_nabuprov(start=load_prune)
    load_org = wade40_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade40_missingreport_graph(start=load_org)
