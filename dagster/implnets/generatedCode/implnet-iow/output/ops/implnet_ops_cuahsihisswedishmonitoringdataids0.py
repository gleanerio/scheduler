...
@op
def cuahsihisswedishmonitoringdataids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisswedishmonitoringdataids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisswedishmonitoringdataids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisswedishmonitoringdataids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisswedishmonitoringdataids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisswedishmonitoringdataids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisswedishmonitoringdataids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisswedishmonitoringdataids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisswedishmonitoringdataids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisswedishmonitoringdataids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisswedishmonitoringdataids0")
    ...
    return msg + r
@op
def cuahsihisswedishmonitoringdataids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisswedishmonitoringdataids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisswedishmonitoringdataids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisswedishmonitoringdataids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisswedishmonitoringdataids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisswedishmonitoringdataids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisswedishmonitoringdataids0"
...
def cuahsihisswedishmonitoringdataids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisswedishmonitoringdataids0"
...
def harvest_cuahsihisswedishmonitoringdataids0():
    harvest = cuahsihisswedishmonitoringdataids0_gleaner()

    report_ms3 = cuahsihisswedishmonitoringdataids0_missingreport_s3(harvest)
    report_idstat = cuahsihisswedishmonitoringdataids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisswedishmonitoringdataids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisswedishmonitoringdataids0")
    load_release = cuahsihisswedishmonitoringdataids0_naburelease(harvest)
    load_uploadrelease = cuahsihisswedishmonitoringdataids0_uploadrelease(load_release)

    load_prune = cuahsihisswedishmonitoringdataids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisswedishmonitoringdataids0_nabuprov(load_prune)
    load_org = cuahsihisswedishmonitoringdataids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisswedishmonitoringdataids0_missingreport_graph(load_org)
    report_graph=cuahsihisswedishmonitoringdataids0_graph_reports(report_msgraph)
