...
@op
def dams1_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "dams1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def dams1_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "dams1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def dams1_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "dams1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def dams1_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "dams1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def dams1_uploadrelease(context, msg: str):
...
@op
def dams1_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="dams1")
    ...
    return msg + r
@op
def dams1_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="dams1")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def dams1_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="dams1")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def dams1_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="dams1")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "dams1"
...
def dams1_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "dams1"
...
@graph
def harvest_dams1():
    harvest = dams1_gleaner()

    report_ms3 = dams1_missingreport_s3(harvest)
    report_idstat = dams1_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = dams1_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="dams1")
    load_release = dams1_naburelease(harvest)
    load_uploadrelease = dams1_uploadrelease(load_release)

    load_prune = dams1_nabu_prune(load_uploadrelease)
    load_prov = dams1_nabuprov(load_prune)
    load_org = dams1_nabuorg(load_prov)

# run after load
    report_msgraph=dams1_missingreport_graph(load_org)
    report_graph=dams1_graph_reports(report_msgraph)
