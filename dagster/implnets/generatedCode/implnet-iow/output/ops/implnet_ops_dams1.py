...
def dams1_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "dams1")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def dams1_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "dams1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def dams1_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "dams1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def dams1_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "dams1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def dams1_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "dams1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def dams1_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "dams1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def dams1_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def dams1_uploadrelease(context):
#     returned_value = post_to_graph("dams1", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def dams1_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="dams1"
    )
...
@op(ins={"start": In(Nothing)})
def dams1_upload_summarize(context):
...
def harvest_dams1():
    ...
    # prov branch
    prov_release = dams1_nabu_provrelease(start=harvest)
    prov_clear = dams1_nabu_provclear(start=prov_release)
    prov_object = dams1_nabu_provobject(start=prov_clear)
    prov_drain = dams1_nabu_provdrain(start=prov_object)

    # org branch
    org_release = dams1_nabu_orgsrelease(start=harvest)
    load_org = dams1_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = dams1_summarize(start=load_uploadrelease)
    # upload_summarize = dams1_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = dams1_missingreport_graph(start=summarize)
