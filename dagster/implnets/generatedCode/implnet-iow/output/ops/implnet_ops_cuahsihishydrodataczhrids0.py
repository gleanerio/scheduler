...
@op
def cuahsihishydrodataczhrids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihishydrodataczhrids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihishydrodataczhrids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihishydrodataczhrids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihishydrodataczhrids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihishydrodataczhrids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihishydrodataczhrids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihishydrodataczhrids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihishydrodataczhrids0_uploadrelease(context, msg: str):
...
@op
def cuahsihishydrodataczhrids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihishydrodataczhrids0")
    ...
    return msg + r
@op
def cuahsihishydrodataczhrids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihishydrodataczhrids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihishydrodataczhrids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihishydrodataczhrids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihishydrodataczhrids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihishydrodataczhrids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihishydrodataczhrids0"
...
def cuahsihishydrodataczhrids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihishydrodataczhrids0"
...
def harvest_cuahsihishydrodataczhrids0():
    ...
    report_ms3 = cuahsihishydrodataczhrids0_missingreport_s3(harvest)
    report_idstat = cuahsihishydrodataczhrids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihishydrodataczhrids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihishydrodataczhrids0")
    load_release = cuahsihishydrodataczhrids0_naburelease(harvest)
    load_uploadrelease = cuahsihishydrodataczhrids0_uploadrelease(load_release)

    load_prune = cuahsihishydrodataczhrids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihishydrodataczhrids0_nabuprov(load_prune)
    load_org = cuahsihishydrodataczhrids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihishydrodataczhrids0_missingreport_graph(load_org)
    report_graph=cuahsihishydrodataczhrids0_graph_reports(report_msgraph)
