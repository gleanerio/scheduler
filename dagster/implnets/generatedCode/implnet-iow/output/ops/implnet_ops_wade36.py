...
@op(ins={"start": In(Nothing)})
def wade36_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade36")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade36_nabuorg(context):
...
def wade36_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade36")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade36")
    ...
    load_prov = wade36_nabuprov(start=load_prune)
    load_org = wade36_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade36_missingreport_graph(start=load_org)
