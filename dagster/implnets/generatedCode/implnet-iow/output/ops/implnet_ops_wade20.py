...
@op(ins={"start": In(Nothing)})
def wade20_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade20")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade20_nabuorg(context):
...
def wade20_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade20")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade20")
    ...
    load_prov = wade20_nabuprov(start=load_prune)
    load_org = wade20_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade20_missingreport_graph(start=load_org)
