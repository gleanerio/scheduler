...
@op(ins={"start": In(Nothing)})
def wade23_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade23")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade23_nabuorg(context):
...
def wade23_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade23")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade23")
    ...
    load_prov = wade23_nabuprov(start=load_prune)
    load_org = wade23_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade23_missingreport_graph(start=load_org)
