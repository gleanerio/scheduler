...
@op
def wade3_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "wade3")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade3_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "wade3")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade3_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "wade3")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade3_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "wade3")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def wade3_uploadrelease(context, msg: str):
...
@op
def wade3_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade3")
    ...
    return msg + r
@op
def wade3_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade3")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def wade3_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade3")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def wade3_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade3")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade3"
...
def wade3_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade3"
...
@graph
def harvest_wade3():
    harvest = wade3_gleaner()

    report_ms3 = wade3_missingreport_s3(harvest)
    report_idstat = wade3_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = wade3_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="wade3")
    load_release = wade3_naburelease(harvest)
    load_uploadrelease = wade3_uploadrelease(load_release)

    load_prune = wade3_nabu_prune(load_uploadrelease)
    load_prov = wade3_nabuprov(load_prune)
    load_org = wade3_nabuorg(load_prov)

# run after load
    report_msgraph=wade3_missingreport_graph(load_org)
    report_graph=wade3_graph_reports(report_msgraph)
