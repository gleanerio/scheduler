...
@op
def cuahsihisgleonauburnids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisgleonauburnids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisgleonauburnids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisgleonauburnids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisgleonauburnids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisgleonauburnids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisgleonauburnids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisgleonauburnids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisgleonauburnids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisgleonauburnids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisgleonauburnids0")
    ...
    return msg + r
@op
def cuahsihisgleonauburnids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisgleonauburnids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisgleonauburnids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisgleonauburnids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisgleonauburnids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisgleonauburnids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisgleonauburnids0"
...
def cuahsihisgleonauburnids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisgleonauburnids0"
...
def harvest_cuahsihisgleonauburnids0():
    harvest = cuahsihisgleonauburnids0_gleaner()

    report_ms3 = cuahsihisgleonauburnids0_missingreport_s3(harvest)
    report_idstat = cuahsihisgleonauburnids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisgleonauburnids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisgleonauburnids0")
    load_release = cuahsihisgleonauburnids0_naburelease(harvest)
    load_uploadrelease = cuahsihisgleonauburnids0_uploadrelease(load_release)

    load_prune = cuahsihisgleonauburnids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisgleonauburnids0_nabuprov(load_prune)
    load_org = cuahsihisgleonauburnids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisgleonauburnids0_missingreport_graph(load_org)
    report_graph=cuahsihisgleonauburnids0_graph_reports(report_msgraph)
