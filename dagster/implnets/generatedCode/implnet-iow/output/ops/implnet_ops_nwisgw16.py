...
@op
def nwisgw16_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw16")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw16_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw16")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw16_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw16")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw16_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw16")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw16_uploadrelease(context, msg: str):
...
@op
def nwisgw16_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw16")
    ...
    return msg + r
@op
def nwisgw16_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw16")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw16_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw16")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw16_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw16")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw16"
...
def nwisgw16_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw16"
...
def harvest_nwisgw16():
    harvest = nwisgw16_gleaner()

    report_ms3 = nwisgw16_missingreport_s3(harvest)
    report_idstat = nwisgw16_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw16_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw16")
    load_release = nwisgw16_naburelease(harvest)
    load_uploadrelease = nwisgw16_uploadrelease(load_release)

    load_prune = nwisgw16_nabu_prune(load_uploadrelease)
    load_prov = nwisgw16_nabuprov(load_prune)
    load_org = nwisgw16_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw16_missingreport_graph(load_org)
    report_graph=nwisgw16_graph_reports(report_msgraph)
