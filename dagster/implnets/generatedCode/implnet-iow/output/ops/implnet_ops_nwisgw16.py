...
def nwisgw16_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "nwisgw16")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def nwisgw16_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "nwisgw16")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw16_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "nwisgw16")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw16_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "nwisgw16")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw16_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "nwisgw16")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw16_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "nwisgw16")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw16_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def nwisgw16_uploadrelease(context):
#     returned_value = post_to_graph("nwisgw16", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def nwisgw16_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nwisgw16"
    )
...
@op(ins={"start": In(Nothing)})
def nwisgw16_summarize(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def nwisgw16_upload_summarize(context):
...
def harvest_nwisgw16():
    ...
    # prov branch
    prov_release = nwisgw16_nabu_provrelease(start=harvest)
    prov_clear = nwisgw16_nabu_provclear(start=prov_release)
    prov_object = nwisgw16_nabu_provobject(start=prov_clear)
    prov_drain = nwisgw16_nabu_provdrain(start=prov_object)

    # org branch
    org_release = nwisgw16_nabu_orgsrelease(start=harvest)
    load_org = nwisgw16_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = nwisgw16_summarize(start=load_uploadrelease)
    # upload_summarize = nwisgw16_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = nwisgw16_missingreport_graph(start=summarize)
