...
@op
def nwisgw4_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw4")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw4_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "nwisgw4")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw4_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "nwisgw4")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def nwisgw4_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "nwisgw4")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def nwisgw4_uploadrelease(context, msg: str):
...
@op
def nwisgw4_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw4")
    ...
    return msg + r
@op
def nwisgw4_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw4")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw4_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw4")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw4_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw4")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw4"
...
def nwisgw4_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw4"
...
@graph
def harvest_nwisgw4():
    harvest = nwisgw4_gleaner()

    report_ms3 = nwisgw4_missingreport_s3(harvest)
    report_idstat = nwisgw4_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw4_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw4")
    load_release = nwisgw4_naburelease(harvest)
    load_uploadrelease = nwisgw4_uploadrelease(load_release)

    load_prune = nwisgw4_nabu_prune(load_uploadrelease)
    load_prov = nwisgw4_nabuprov(load_prune)
    load_org = nwisgw4_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw4_missingreport_graph(load_org)
    report_graph=nwisgw4_graph_reports(report_msgraph)
