...
def nwisgw4_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "nwisgw4")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def nwisgw4_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "nwisgw4")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw4_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "nwisgw4")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw4_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "nwisgw4")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw4_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "nwisgw4")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw4_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "nwisgw4")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def nwisgw4_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def nwisgw4_uploadrelease(context):
#     returned_value = post_to_graph("nwisgw4", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def nwisgw4_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="nwisgw4"
    )
...
@op(ins={"start": In(Nothing)})
def nwisgw4_upload_summarize(context):
...
def harvest_nwisgw4():
    ...
    # prov branch
    prov_release = nwisgw4_nabu_provrelease(start=harvest)
    prov_clear = nwisgw4_nabu_provclear(start=prov_release)
    prov_object = nwisgw4_nabu_provobject(start=prov_clear)
    prov_drain = nwisgw4_nabu_provdrain(start=prov_object)

    # org branch
    org_release = nwisgw4_nabu_orgsrelease(start=harvest)
    load_org = nwisgw4_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = nwisgw4_summarize(start=load_uploadrelease)
    # upload_summarize = nwisgw4_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = nwisgw4_missingreport_graph(start=summarize)
