...
def hu080_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "hu080")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def hu080_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "hu080")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def hu080_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "hu080")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def hu080_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "hu080")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def hu080_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "hu080")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def hu080_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "hu080")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def hu080_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def hu080_uploadrelease(context):
#     returned_value = post_to_graph("hu080", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def hu080_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="hu080"
    )
...
@op(ins={"start": In(Nothing)})
def hu080_upload_summarize(context):
...
def harvest_hu080():
    ...
    # prov branch
    prov_release = hu080_nabu_provrelease(start=harvest)
    prov_clear = hu080_nabu_provclear(start=prov_release)
    prov_object = hu080_nabu_provobject(start=prov_clear)
    prov_drain = hu080_nabu_provdrain(start=prov_object)

    # org branch
    org_release = hu080_nabu_orgsrelease(start=harvest)
    load_org = hu080_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = hu080_summarize(start=load_uploadrelease)
    # upload_summarize = hu080_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = hu080_missingreport_graph(start=summarize)
