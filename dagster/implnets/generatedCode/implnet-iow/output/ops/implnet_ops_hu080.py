...
@op
def hu080_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "hu080")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def hu080_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "hu080")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def hu080_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "hu080")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def hu080_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "hu080")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def hu080_uploadrelease(context, msg: str):
...
@op
def hu080_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hu080")
    ...
    return msg + r
@op
def hu080_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hu080")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def hu080_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hu080")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def hu080_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="hu080")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "hu080"
...
def hu080_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "hu080"
...
@graph
def harvest_hu080():
    harvest = hu080_gleaner()

    report_ms3 = hu080_missingreport_s3(harvest)
    report_idstat = hu080_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = hu080_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="hu080")
    load_release = hu080_naburelease(harvest)
    load_uploadrelease = hu080_uploadrelease(load_release)

    load_prune = hu080_nabu_prune(load_uploadrelease)
    load_prov = hu080_nabuprov(load_prune)
    load_org = hu080_nabuorg(load_prov)

# run after load
    report_msgraph=hu080_missingreport_graph(load_org)
    report_graph=hu080_graph_reports(report_msgraph)
