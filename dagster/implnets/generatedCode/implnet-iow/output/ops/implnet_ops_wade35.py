...
@op(ins={"start": In(Nothing)})
def wade35_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade35")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade35_nabuorg(context):
...
def wade35_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade35")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade35")
    ...
    load_prov = wade35_nabuprov(start=load_prune)
    load_org = wade35_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade35_missingreport_graph(start=load_org)
