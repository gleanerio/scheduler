...
def wade35_gleaner(context):
    returned_value = gleanerio(context, ("gleaner"), "wade35")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade35_naburelease(context):
    returned_value = gleanerio(context, ("release"), "wade35")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade35_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wade35")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu object call release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade35_nabu_prune(context):
    returned_value = gleanerio(context, ("prune"), "wade35")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prune returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade35_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wade35")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade35_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wade35")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade35_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wade35")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade35_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wade35")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade35_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wade35")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade35_nabuorgs(context):
    returned_value = gleanerio(context, ("orgs"), "wade35")
    r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wade35_uploadrelease(context):
#     returned_value = post_to_graph("wade35", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
# @op(ins={"start": In(Nothing)})
# def wade35_uploadrelease(context):
#     returned_value = postRelease("wade35")
#     r = str('returned value:{}'.format(returned_value))
#     get_dagster_logger().info(f"upload release returned  {r} ")
#     return
...
@op(ins={"start": In(Nothing)})
def wade35_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade35"
    )
...
@op(ins={"start": In(Nothing)})
def wade35_missingreport_graph(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade35"
    )
    source_url = source.get("url")
...
@op(ins={"start": In(Nothing)})
def wade35_graph_reports(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade35"
    )
    # source_url = source.get('url')
...
@op(ins={"start": In(Nothing)})
def wade35_identifier_stats(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade35"
    )
    s3Minio = s3.MinioDatastore(
...
@op(ins={"start": In(Nothing)})
def wade35_bucket_urls(context):
    s3Minio = s3.MinioDatastore(
        _pythonMinioAddress(GLEANER_MINIO_ADDRESS, GLEANER_MINIO_PORT), MINIO_OPTIONS
    )
    bucket = GLEANER_MINIO_BUCKET
...
@op(ins={"start": In(Nothing)})
def wade35_upload_summarize(context):
...
def harvest_wade35():
    ...
    containers = wade35_getImage()

    # conduct the harvest
    harvest = wade35_gleaner(start=containers)

    # data branch
    load_release = wade35_naburelease(start=harvest)
    load_uploadrelease = wade35_uploadrelease(start=load_release)
    load_prune = wade35_nabu_prune(start=load_uploadrelease)

    # prov branch
    prov_release = wade35_nabu_provrelease(start=harvest)
    prov_clear = wade35_nabu_provclear(start=prov_release)
    prov_object = wade35_nabu_provobject(start=prov_clear)
    prov_drain = wade35_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wade35_nabu_orgsrelease(start=harvest)
    load_org = wade35_nabuorgs(start=org_release)

    # reports branch
    # report_ms3 = wade35_missingreport_s3(start=harvest)
    # report_idstat = wade35_identifier_stats(start=report_ms3)
    # report_bucketurl = wade35_bucket_urls( start=report_idstat)
    # for some reason, this causes a msg parameter missing

    # summarize
    # summarize = wade35_summarize(start=load_uploadrelease)
    # upload_summarize = wade35_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wade35_missingreport_graph(start=summarize)
    # report_graph = wade35_graph_reports(start=report_msgraph)
...
# def missingreport_s3(context, msg: str, source="wade35"):
#
