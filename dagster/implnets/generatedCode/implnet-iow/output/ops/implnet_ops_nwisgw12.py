...
@op
def nwisgw12_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "nwisgw12")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
...
def nwisgw12_naburelease(context, msg: str):
    ...
    return msg + r
@op
def nwisgw12_uploadrelease(context, msg: str):
...
@op
def nwisgw12_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw12")
    ...
    return msg + r
@op
def nwisgw12_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw12")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def nwisgw12_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw12")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def nwisgw12_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="nwisgw12")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw12"
...
def nwisgw12_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "nwisgw12"
...
def harvest_nwisgw12():
    ...
    report_ms3 = nwisgw12_missingreport_s3(harvest)
    report_idstat = nwisgw12_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = nwisgw12_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="nwisgw12")
    load_release = nwisgw12_naburelease(harvest)
    load_uploadrelease = nwisgw12_uploadrelease(load_release)

    load_prune = nwisgw12_nabu_prune(load_uploadrelease)
    load_prov = nwisgw12_nabuprov(load_prune)
    load_org = nwisgw12_nabuorg(load_prov)

# run after load
    report_msgraph=nwisgw12_missingreport_graph(load_org)
    report_graph=nwisgw12_graph_reports(report_msgraph)
