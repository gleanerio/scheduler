...
@op
def cuahsihismopexids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihismopexids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismopexids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihismopexids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismopexids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihismopexids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismopexids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihismopexids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihismopexids0_uploadrelease(context, msg: str):
...
@op
def cuahsihismopexids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismopexids0")
    ...
    return msg + r
@op
def cuahsihismopexids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismopexids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihismopexids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismopexids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihismopexids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismopexids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihismopexids0"
...
def cuahsihismopexids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihismopexids0"
...
def harvest_cuahsihismopexids0():
    harvest = cuahsihismopexids0_gleaner()

    report_ms3 = cuahsihismopexids0_missingreport_s3(harvest)
    report_idstat = cuahsihismopexids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihismopexids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihismopexids0")
    load_release = cuahsihismopexids0_naburelease(harvest)
    load_uploadrelease = cuahsihismopexids0_uploadrelease(load_release)

    load_prune = cuahsihismopexids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihismopexids0_nabuprov(load_prune)
    load_org = cuahsihismopexids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihismopexids0_missingreport_graph(load_org)
    report_graph=cuahsihismopexids0_graph_reports(report_msgraph)
