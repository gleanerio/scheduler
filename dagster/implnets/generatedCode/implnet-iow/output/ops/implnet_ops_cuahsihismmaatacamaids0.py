...
@op
def cuahsihismmaatacamaids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihismmaatacamaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismmaatacamaids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihismmaatacamaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismmaatacamaids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihismmaatacamaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismmaatacamaids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihismmaatacamaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihismmaatacamaids0_uploadrelease(context, msg: str):
...
@op
def cuahsihismmaatacamaids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismmaatacamaids0")
    ...
    return msg + r
@op
def cuahsihismmaatacamaids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismmaatacamaids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihismmaatacamaids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismmaatacamaids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihismmaatacamaids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismmaatacamaids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihismmaatacamaids0"
...
def cuahsihismmaatacamaids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihismmaatacamaids0"
...
def harvest_cuahsihismmaatacamaids0():
    harvest = cuahsihismmaatacamaids0_gleaner()

    report_ms3 = cuahsihismmaatacamaids0_missingreport_s3(harvest)
    report_idstat = cuahsihismmaatacamaids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihismmaatacamaids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihismmaatacamaids0")
    load_release = cuahsihismmaatacamaids0_naburelease(harvest)
    load_uploadrelease = cuahsihismmaatacamaids0_uploadrelease(load_release)

    load_prune = cuahsihismmaatacamaids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihismmaatacamaids0_nabuprov(load_prune)
    load_org = cuahsihismmaatacamaids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihismmaatacamaids0_missingreport_graph(load_org)
    report_graph=cuahsihismmaatacamaids0_graph_reports(report_msgraph)
