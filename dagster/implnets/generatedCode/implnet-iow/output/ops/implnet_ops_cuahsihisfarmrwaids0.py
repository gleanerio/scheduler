...
@op
def cuahsihisfarmrwaids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihisfarmrwaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisfarmrwaids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihisfarmrwaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisfarmrwaids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihisfarmrwaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihisfarmrwaids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihisfarmrwaids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihisfarmrwaids0_uploadrelease(context, msg: str):
...
@op
def cuahsihisfarmrwaids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisfarmrwaids0")
    ...
    return msg + r
@op
def cuahsihisfarmrwaids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisfarmrwaids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihisfarmrwaids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisfarmrwaids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihisfarmrwaids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihisfarmrwaids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisfarmrwaids0"
...
def cuahsihisfarmrwaids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihisfarmrwaids0"
...
def harvest_cuahsihisfarmrwaids0():
    harvest = cuahsihisfarmrwaids0_gleaner()

    report_ms3 = cuahsihisfarmrwaids0_missingreport_s3(harvest)
    report_idstat = cuahsihisfarmrwaids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihisfarmrwaids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihisfarmrwaids0")
    load_release = cuahsihisfarmrwaids0_naburelease(harvest)
    load_uploadrelease = cuahsihisfarmrwaids0_uploadrelease(load_release)

    load_prune = cuahsihisfarmrwaids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihisfarmrwaids0_nabuprov(load_prune)
    load_org = cuahsihisfarmrwaids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihisfarmrwaids0_missingreport_graph(load_org)
    report_graph=cuahsihisfarmrwaids0_graph_reports(report_msgraph)
