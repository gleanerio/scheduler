...
def wade14_uploadrelease(context):
    returned_value = gleanerio(context, ("object"), "wade14")
    r = str('returned value:{}'.format(returned_value))
...
@op(ins={"start": In(Nothing)})
def wade14_nabu_provrelease(context):
    returned_value = gleanerio(context, ("prov-release"), "wade14")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade14_nabu_provclear(context):
    returned_value = gleanerio(context, ("prov-clear"), "wade14")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-clear returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade14_nabu_provobject(context):
    returned_value = gleanerio(context, ("prov-object"), "wade14")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-object returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade14_nabu_provdrain(context):
    returned_value = gleanerio(context, ("prov-drain"), "wade14")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov-drain returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade14_nabu_orgsrelease(context):
    returned_value = gleanerio(context, ("orgs-release"), "wade14")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu orgs-release returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade14_nabuorgs(context):
...
# @op(ins={"start": In(Nothing)})
# def wade14_uploadrelease(context):
#     returned_value = post_to_graph("wade14", extension="nq")
#     r = str('returned value:{}'.format(returned_value))
...
def wade14_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(
        DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade14"
    )
...
@op(ins={"start": In(Nothing)})
def wade14_upload_summarize(context):
...
def harvest_wade14():
    ...
    # prov branch
    prov_release = wade14_nabu_provrelease(start=harvest)
    prov_clear = wade14_nabu_provclear(start=prov_release)
    prov_object = wade14_nabu_provobject(start=prov_clear)
    prov_drain = wade14_nabu_provdrain(start=prov_object)

    # org branch
    org_release = wade14_nabu_orgsrelease(start=harvest)
    load_org = wade14_nabuorgs(start=org_release)
    ...
    # summarize
    # summarize = wade14_summarize(start=load_uploadrelease)
    # upload_summarize = wade14_upload_summarize(start=summarize)

    # run after load
    # report_msgraph = wade14_missingreport_graph(start=summarize)
