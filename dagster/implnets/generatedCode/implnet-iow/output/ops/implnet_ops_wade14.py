...
@op
def wade14_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "wade14")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade14_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "wade14")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade14_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "wade14")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def wade14_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "wade14")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def wade14_uploadrelease(context, msg: str):
...
@op
def wade14_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade14")
    ...
    return msg + r
@op
def wade14_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade14")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def wade14_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade14")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def wade14_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="wade14")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade14"
...
def wade14_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "wade14"
...
@graph
def harvest_wade14():
    harvest = wade14_gleaner()

    report_ms3 = wade14_missingreport_s3(harvest)
    report_idstat = wade14_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = wade14_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="wade14")
    load_release = wade14_naburelease(harvest)
    load_uploadrelease = wade14_uploadrelease(load_release)

    load_prune = wade14_nabu_prune(load_uploadrelease)
    load_prov = wade14_nabuprov(load_prune)
    load_org = wade14_nabuorg(load_prov)

# run after load
    report_msgraph=wade14_missingreport_graph(load_org)
    report_graph=wade14_graph_reports(report_msgraph)
