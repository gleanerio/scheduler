...
@op(ins={"start": In(Nothing)})
def wade30_nabuprov(context):
    returned_value = gleanerio(context,("prov"), "wade30")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"nabu prov returned  {r} ")
    return

@op(ins={"start": In(Nothing)})
def wade30_nabuorg(context):
...
def wade30_missingreport_s3(context):
    source = getSitemapSourcesFromGleaner(DAGSTER_GLEANER_CONFIG_PATH, sourcename="wade30")
...
# defingin nothing dependencies
    ...
    #report1 = missingreport_s3(harvest, source="wade30")
    ...
    load_prov = wade30_nabuprov(start=load_prune)
    load_org = wade30_nabuorg(start=load_prov)

# run after load
    report_msgraph=wade30_missingreport_graph(start=load_org)
