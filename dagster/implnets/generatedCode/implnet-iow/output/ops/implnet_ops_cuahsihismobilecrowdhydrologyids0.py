...
@op
def cuahsihismobilecrowdhydrologyids0_nabu_prune(context, msg: str):
    returned_value = gleanerio(context,("nabu"), "cuahsihismobilecrowdhydrologyids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismobilecrowdhydrologyids0_nabuprov(context, msg: str):
    returned_value = gleanerio(context,("prov"), "cuahsihismobilecrowdhydrologyids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismobilecrowdhydrologyids0_nabuorg(context, msg: str):
    returned_value = gleanerio(context,("orgs"), "cuahsihismobilecrowdhydrologyids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@op
def cuahsihismobilecrowdhydrologyids0_naburelease(context, msg: str):
    returned_value = gleanerio(context,("release"), "cuahsihismobilecrowdhydrologyids0")
    r = str('returned value:{}'.format(returned_value))
    return msg + r
@op
def cuahsihismobilecrowdhydrologyids0_uploadrelease(context, msg: str):
...
@op
def cuahsihismobilecrowdhydrologyids0_missingreport_s3(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismobilecrowdhydrologyids0")
    ...
    return msg + r
@op
def cuahsihismobilecrowdhydrologyids0_missingreport_graph(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismobilecrowdhydrologyids0")
    source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    ...
    return msg + r
@op
def cuahsihismobilecrowdhydrologyids0_graph_reports(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismobilecrowdhydrologyids0")
    #source_url = source.get('url')
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
...
@op
def cuahsihismobilecrowdhydrologyids0_identifier_stats(context, msg: str):
    source = getSitemapSourcesFromGleaner("/scheduler/gleanerconfig.yaml", sourcename="cuahsihismobilecrowdhydrologyids0")
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihismobilecrowdhydrologyids0"
...
def cuahsihismobilecrowdhydrologyids0_bucket_urls(context, msg: str):
    s3Minio = s3.MinioDatastore(_pythonMinioUrl(GLEANER_MINIO_ADDRESS), None)
    bucket = GLEANER_MINIO_BUCKET
    source_name = "cuahsihismobilecrowdhydrologyids0"
...
def harvest_cuahsihismobilecrowdhydrologyids0():
    harvest = cuahsihismobilecrowdhydrologyids0_gleaner()

    report_ms3 = cuahsihismobilecrowdhydrologyids0_missingreport_s3(harvest)
    report_idstat = cuahsihismobilecrowdhydrologyids0_identifier_stats(report_ms3)
    # for some reason, this causes a msg parameter missing
   # report_bucketurl = cuahsihismobilecrowdhydrologyids0_bucket_urls(report_idstat)

    #report1 = missingreport_s3(harvest, source="cuahsihismobilecrowdhydrologyids0")
    load_release = cuahsihismobilecrowdhydrologyids0_naburelease(harvest)
    load_uploadrelease = cuahsihismobilecrowdhydrologyids0_uploadrelease(load_release)

    load_prune = cuahsihismobilecrowdhydrologyids0_nabu_prune(load_uploadrelease)
    load_prov = cuahsihismobilecrowdhydrologyids0_nabuprov(load_prune)
    load_org = cuahsihismobilecrowdhydrologyids0_nabuorg(load_prov)

# run after load
    report_msgraph=cuahsihismobilecrowdhydrologyids0_missingreport_graph(load_org)
    report_graph=cuahsihismobilecrowdhydrologyids0_graph_reports(report_msgraph)
