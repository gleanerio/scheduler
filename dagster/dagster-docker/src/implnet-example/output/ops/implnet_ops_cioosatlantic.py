...
@op
def cioosatlantic_gleaner():
    returned_value = gleanerio(("gleaner"), "cioosatlantic")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cioosatlantic_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cioosatlantic")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cioosatlantic():
    harvest = cioosatlantic_gleaner()
    load1 = cioosatlantic_nabu(harvest)
...
#     returned_value = subprocess.run('./gleaner.bin -cfg gleanerconfig.yaml  --source cioosatlantic -rude', shell=True, cwd='/usr/src/app')
