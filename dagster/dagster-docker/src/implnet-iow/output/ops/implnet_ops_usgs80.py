...
@op
def usgs80_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs80")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs80_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs80")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs80():
    harvest = usgs80_gleaner()
    load1 = usgs80_nabu(harvest)
