...
@op
def usgs68_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs68")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs68_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs68")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs68():
    harvest = usgs68_gleaner()
    load1 = usgs68_nabu(harvest)
