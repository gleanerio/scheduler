...
@op
def usgs86_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs86")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs86_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs86")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs86():
    harvest = usgs86_gleaner()
    load1 = usgs86_nabu(harvest)
