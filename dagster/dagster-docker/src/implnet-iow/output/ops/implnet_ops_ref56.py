...
@op
def ref56_gleaner(context):
    returned_value = gleanerio(("gleaner"), "ref56")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def ref56_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "ref56")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_ref56():
    harvest = ref56_gleaner()
    load1 = ref56_nabu(harvest)
