...
@op
def ref53_gleaner(context):
    returned_value = gleanerio(("gleaner"), "ref53")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def ref53_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "ref53")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_ref53():
    harvest = ref53_gleaner()
    load1 = ref53_nabu(harvest)
