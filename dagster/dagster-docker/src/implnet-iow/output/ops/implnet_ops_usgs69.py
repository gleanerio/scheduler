...
@op
def usgs69_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs69")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs69_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs69")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs69():
    harvest = usgs69_gleaner()
    load1 = usgs69_nabu(harvest)
