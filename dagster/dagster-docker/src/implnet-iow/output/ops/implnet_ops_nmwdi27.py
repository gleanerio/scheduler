...
@op
def nmwdi27_gleaner(context):
    returned_value = gleanerio(("gleaner"), "nmwdi27")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def nmwdi27_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "nmwdi27")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_nmwdi27():
    harvest = nmwdi27_gleaner()
    load1 = nmwdi27_nabu(harvest)
