...
@op
def cuahsi118_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi118")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi118_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi118")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi118():
    harvest = cuahsi118_gleaner()
    load1 = cuahsi118_nabu(harvest)
