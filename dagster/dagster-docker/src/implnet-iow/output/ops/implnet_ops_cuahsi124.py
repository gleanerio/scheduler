...
@op
def cuahsi124_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi124")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi124_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi124")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi124():
    harvest = cuahsi124_gleaner()
    load1 = cuahsi124_nabu(harvest)
