...
@op
def nhdplusv232_gleaner(context):
    returned_value = gleanerio(("gleaner"), "nhdplusv232")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def nhdplusv232_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "nhdplusv232")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_nhdplusv232():
    harvest = nhdplusv232_gleaner()
    load1 = nhdplusv232_nabu(harvest)
