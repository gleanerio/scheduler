...
@op
def ref47_gleaner(context):
    returned_value = gleanerio(("gleaner"), "ref47")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def ref47_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "ref47")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_ref47():
    harvest = ref47_gleaner()
    load1 = ref47_nabu(harvest)
