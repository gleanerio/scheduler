...
@op
def cuahsi128_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi128")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi128_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi128")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi128():
    harvest = cuahsi128_gleaner()
    load1 = cuahsi128_nabu(harvest)
