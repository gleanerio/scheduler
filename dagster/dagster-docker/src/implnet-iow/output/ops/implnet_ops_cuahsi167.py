...
@op
def cuahsi167_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi167")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi167_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi167")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi167():
    harvest = cuahsi167_gleaner()
    load1 = cuahsi167_nabu(harvest)
    # load2 = cuahsi167_prov(load1)
