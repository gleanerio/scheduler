...
@op
def iow36_gleaner(context):
    returned_value = gleanerio(("gleaner"), "iow36")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def iow36_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "iow36")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_iow36():
    harvest = iow36_gleaner()
    load1 = iow36_nabu(harvest)
