...
@op
def cuahsi101_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi101")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi101_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi101")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi101():
    harvest = cuahsi101_gleaner()
    load1 = cuahsi101_nabu(harvest)
