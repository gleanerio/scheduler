...
@op
def ref54_gleaner(context):
    returned_value = gleanerio(("gleaner"), "ref54")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def ref54_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "ref54")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_ref54():
    harvest = ref54_gleaner()
    load1 = ref54_nabu(harvest)
