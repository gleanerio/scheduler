...
@op
def ref59_gleaner(context):
    returned_value = gleanerio(("gleaner"), "ref59")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def ref59_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "ref59")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_ref59():
    harvest = ref59_gleaner()
    load1 = ref59_nabu(harvest)
