...
@op
def usgs73_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs73")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs73_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs73")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs73():
    harvest = usgs73_gleaner()
    load1 = usgs73_nabu(harvest)
