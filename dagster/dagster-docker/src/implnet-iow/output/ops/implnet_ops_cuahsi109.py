...
@op
def cuahsi109_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi109")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi109_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi109")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi109():
    harvest = cuahsi109_gleaner()
    load1 = cuahsi109_nabu(harvest)
