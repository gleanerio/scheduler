...
@op
def cuahsi106_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi106")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi106_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi106")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi106():
    harvest = cuahsi106_gleaner()
    load1 = cuahsi106_nabu(harvest)
