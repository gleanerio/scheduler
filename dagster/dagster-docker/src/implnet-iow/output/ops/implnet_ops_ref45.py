...
@op
def ref45_gleaner(context):
    returned_value = gleanerio(("gleaner"), "ref45")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def ref45_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "ref45")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_ref45():
    harvest = ref45_gleaner()
    load1 = ref45_nabu(harvest)
