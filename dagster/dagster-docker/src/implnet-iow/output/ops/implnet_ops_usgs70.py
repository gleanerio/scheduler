...
@op
def usgs70_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs70")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs70_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs70")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs70():
    harvest = usgs70_gleaner()
    load1 = usgs70_nabu(harvest)
