...
@op
def cuahsi169_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi169")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi169_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi169")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi169():
    harvest = cuahsi169_gleaner()
    load1 = cuahsi169_nabu(harvest)
