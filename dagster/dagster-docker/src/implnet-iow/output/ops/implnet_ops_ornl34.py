...
@op
def ornl34_gleaner(context):
    returned_value = gleanerio(("gleaner"), "ornl34")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def ornl34_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "ornl34")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_ornl34():
    harvest = ornl34_gleaner()
    load1 = ornl34_nabu(harvest)
