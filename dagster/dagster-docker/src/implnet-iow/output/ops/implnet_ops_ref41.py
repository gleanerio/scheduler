...
@op
def ref41_gleaner(context):
    returned_value = gleanerio(("gleaner"), "ref41")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def ref41_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "ref41")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_ref41():
    harvest = ref41_gleaner()
    load1 = ref41_nabu(harvest)
