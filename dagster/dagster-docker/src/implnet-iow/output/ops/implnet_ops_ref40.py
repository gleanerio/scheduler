...
@op
def ref40_gleaner(context):
    returned_value = gleanerio(("gleaner"), "ref40")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def ref40_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "ref40")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_ref40():
    harvest = ref40_gleaner()
    load1 = ref40_nabu(harvest)
