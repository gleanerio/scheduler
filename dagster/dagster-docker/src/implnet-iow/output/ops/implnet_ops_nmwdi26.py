...
@op
def nmwdi26_gleaner(context):
    returned_value = gleanerio(("gleaner"), "nmwdi26")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def nmwdi26_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "nmwdi26")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_nmwdi26():
    harvest = nmwdi26_gleaner()
    load1 = nmwdi26_nabu(harvest)
