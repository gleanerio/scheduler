...
@op
def nmwdi25_gleaner(context):
    returned_value = gleanerio(("gleaner"), "nmwdi25")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def nmwdi25_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "nmwdi25")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_nmwdi25():
    harvest = nmwdi25_gleaner()
    load1 = nmwdi25_nabu(harvest)
