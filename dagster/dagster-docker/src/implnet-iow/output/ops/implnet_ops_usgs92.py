...
@op
def usgs92_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs92")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs92_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs92")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs92():
    harvest = usgs92_gleaner()
    load1 = usgs92_nabu(harvest)
