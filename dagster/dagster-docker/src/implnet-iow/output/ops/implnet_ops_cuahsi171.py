...
@op
def cuahsi171_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi171")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi171_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi171")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi171():
    harvest = cuahsi171_gleaner()
    load1 = cuahsi171_nabu(harvest)
