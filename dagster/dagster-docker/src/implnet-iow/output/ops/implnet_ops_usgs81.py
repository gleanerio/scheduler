...
@op
def usgs81_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs81")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs81_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs81")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs81():
    harvest = usgs81_gleaner()
    load1 = usgs81_nabu(harvest)
