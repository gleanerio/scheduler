...
@op
def usgs71_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs71")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs71_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs71")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs71():
    harvest = usgs71_gleaner()
    load1 = usgs71_nabu(harvest)
