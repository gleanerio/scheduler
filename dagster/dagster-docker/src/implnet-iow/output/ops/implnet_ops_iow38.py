...
@op
def iow38_gleaner(context):
    returned_value = gleanerio(("gleaner"), "iow38")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def iow38_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "iow38")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_iow38():
    harvest = iow38_gleaner()
    load1 = iow38_nabu(harvest)
