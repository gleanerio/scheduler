...
@op
def cuahsi170_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi170")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi170_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi170")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi170():
    harvest = cuahsi170_gleaner()
    load1 = cuahsi170_nabu(harvest)
