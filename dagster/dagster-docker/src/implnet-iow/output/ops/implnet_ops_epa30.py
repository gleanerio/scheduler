...
@op
def epa30_gleaner(context):
    returned_value = gleanerio(("gleaner"), "epa30")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def epa30_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "epa30")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_epa30():
    harvest = epa30_gleaner()
    load1 = epa30_nabu(harvest)
