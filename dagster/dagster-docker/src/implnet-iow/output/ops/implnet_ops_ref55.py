...
@op
def ref55_gleaner(context):
    returned_value = gleanerio(("gleaner"), "ref55")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def ref55_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "ref55")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_ref55():
    harvest = ref55_gleaner()
    load1 = ref55_nabu(harvest)
