...
@op
def cuahsi131_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi131")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi131_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi131")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi131():
    harvest = cuahsi131_gleaner()
    load1 = cuahsi131_nabu(harvest)
