...
@op
def nmwdi23_gleaner(context):
    returned_value = gleanerio(("gleaner"), "nmwdi23")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def nmwdi23_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "nmwdi23")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_nmwdi23():
    harvest = nmwdi23_gleaner()
    load1 = nmwdi23_nabu(harvest)
