...
@op
def usgs74_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs74")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs74_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs74")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs74():
    harvest = usgs74_gleaner()
    load1 = usgs74_nabu(harvest)
