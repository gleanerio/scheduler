...
@op
def usgs72_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs72")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs72_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs72")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs72():
    harvest = usgs72_gleaner()
    load1 = usgs72_nabu(harvest)
