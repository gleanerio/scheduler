...
@op
def usgs93_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs93")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs93_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs93")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs93():
    harvest = usgs93_gleaner()
    load1 = usgs93_nabu(harvest)
