...
@op
def cuahsi105_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi105")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi105_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi105")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi105():
    harvest = cuahsi105_gleaner()
    load1 = cuahsi105_nabu(harvest)
