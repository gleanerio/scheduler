...
@op
def ref60_gleaner(context):
    returned_value = gleanerio(("gleaner"), "ref60")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def ref60_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "ref60")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_ref60():
    harvest = ref60_gleaner()
    load1 = ref60_nabu(harvest)
