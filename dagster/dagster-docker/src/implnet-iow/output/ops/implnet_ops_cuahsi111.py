...
@op
def cuahsi111_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi111")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi111_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi111")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi111():
    harvest = cuahsi111_gleaner()
    load1 = cuahsi111_nabu(harvest)
