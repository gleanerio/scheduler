...
@op
def ref57_gleaner(context):
    returned_value = gleanerio(("gleaner"), "ref57")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def ref57_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "ref57")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_ref57():
    harvest = ref57_gleaner()
    load1 = ref57_nabu(harvest)
