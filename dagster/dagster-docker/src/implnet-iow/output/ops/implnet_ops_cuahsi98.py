...
@op
def cuahsi98_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi98")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi98_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi98")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi98():
    harvest = cuahsi98_gleaner()
    load1 = cuahsi98_nabu(harvest)
