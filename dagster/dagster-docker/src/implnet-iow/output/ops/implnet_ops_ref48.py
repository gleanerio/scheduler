...
@op
def ref48_gleaner(context):
    returned_value = gleanerio(("gleaner"), "ref48")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def ref48_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "ref48")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_ref48():
    harvest = ref48_gleaner()
    load1 = ref48_nabu(harvest)
