...
@op
def usgs94_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs94")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs94_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs94")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs94():
    harvest = usgs94_gleaner()
    load1 = usgs94_nabu(harvest)
