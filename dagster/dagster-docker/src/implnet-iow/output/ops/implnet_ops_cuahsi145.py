...
@op
def cuahsi145_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi145")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi145_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi145")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi145():
    harvest = cuahsi145_gleaner()
    load1 = cuahsi145_nabu(harvest)
