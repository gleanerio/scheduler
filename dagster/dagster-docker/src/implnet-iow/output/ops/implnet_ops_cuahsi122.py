...
@op
def cuahsi122_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi122")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi122_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi122")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi122():
    harvest = cuahsi122_gleaner()
    load1 = cuahsi122_nabu(harvest)
