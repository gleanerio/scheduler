...
@op
def cuahsi141_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi141")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi141_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi141")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi141():
    harvest = cuahsi141_gleaner()
    load1 = cuahsi141_nabu(harvest)
