...
@op
def chyld1_gleaner(context):
    returned_value = gleanerio(("gleaner"), "chyld1")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def chyld1_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "chyld1")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_chyld1():
    harvest = chyld1_gleaner()
    load1 = chyld1_nabu(harvest)
