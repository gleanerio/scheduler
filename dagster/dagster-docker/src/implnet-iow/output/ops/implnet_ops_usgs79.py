...
@op
def usgs79_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs79")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs79_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs79")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs79():
    harvest = usgs79_gleaner()
    load1 = usgs79_nabu(harvest)
