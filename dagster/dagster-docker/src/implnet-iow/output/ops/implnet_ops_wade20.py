...
def wade20_gleaner(context):
    returned_value = gleanerio(("gleaner"), "wade20")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def wade20_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "wade20")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_wade20():
    harvest = wade20_gleaner()
    load1 = wade20_nabu(harvest)
