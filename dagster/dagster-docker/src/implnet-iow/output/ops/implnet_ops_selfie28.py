...
@op
def selfie28_gleaner(context):
    returned_value = gleanerio(("gleaner"), "selfie28")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def selfie28_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "selfie28")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_selfie28():
    harvest = selfie28_gleaner()
    load1 = selfie28_nabu(harvest)
