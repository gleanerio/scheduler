...
@op
def cuahsi100_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi100")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi100_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi100")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi100():
    harvest = cuahsi100_gleaner()
    load1 = cuahsi100_nabu(harvest)
