...
@op
def cuahsi181_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi181")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi181_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi181")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi181():
    harvest = cuahsi181_gleaner()
    load1 = cuahsi181_nabu(harvest)
    # load2 = cuahsi181_prov(load1)
