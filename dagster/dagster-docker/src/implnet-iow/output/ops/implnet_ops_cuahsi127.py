...
@op
def cuahsi127_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi127")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi127_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi127")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi127():
    harvest = cuahsi127_gleaner()
    load1 = cuahsi127_nabu(harvest)
