...
@op
def usgs77_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs77")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs77_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs77")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs77():
    harvest = usgs77_gleaner()
    load1 = usgs77_nabu(harvest)
