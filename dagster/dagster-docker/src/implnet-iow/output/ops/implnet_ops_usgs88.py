...
@op
def usgs88_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs88")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs88_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs88")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs88():
    harvest = usgs88_gleaner()
    load1 = usgs88_nabu(harvest)
