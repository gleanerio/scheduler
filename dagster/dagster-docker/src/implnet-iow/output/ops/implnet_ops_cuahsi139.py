...
@op
def cuahsi139_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi139")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi139_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi139")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi139():
    harvest = cuahsi139_gleaner()
    load1 = cuahsi139_nabu(harvest)
