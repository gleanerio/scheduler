...
@op
def cuahsi113_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi113")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi113_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi113")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi113():
    harvest = cuahsi113_gleaner()
    load1 = cuahsi113_nabu(harvest)
