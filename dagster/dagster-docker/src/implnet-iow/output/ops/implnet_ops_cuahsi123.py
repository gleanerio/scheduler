...
@op
def cuahsi123_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi123")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi123_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi123")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi123():
    harvest = cuahsi123_gleaner()
    load1 = cuahsi123_nabu(harvest)
