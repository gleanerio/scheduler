...
@op
def cuahsi164_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi164")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi164_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi164")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi164():
    harvest = cuahsi164_gleaner()
    load1 = cuahsi164_nabu(harvest)
