...
@op
def usgs87_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs87")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs87_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs87")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs87():
    harvest = usgs87_gleaner()
    load1 = usgs87_nabu(harvest)
