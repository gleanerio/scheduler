...
@op
def usgs91_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs91")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs91_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs91")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs91():
    harvest = usgs91_gleaner()
    load1 = usgs91_nabu(harvest)
