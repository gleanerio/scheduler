...
@op
def usgs75_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs75")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs75_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs75")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs75():
    harvest = usgs75_gleaner()
    load1 = usgs75_nabu(harvest)
