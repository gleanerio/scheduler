...
@op
def cuahsi126_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi126")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi126_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi126")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi126():
    harvest = cuahsi126_gleaner()
    load1 = cuahsi126_nabu(harvest)
