...
@op
def nhdplusv231_gleaner(context):
    returned_value = gleanerio(("gleaner"), "nhdplusv231")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def nhdplusv231_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "nhdplusv231")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_nhdplusv231():
    harvest = nhdplusv231_gleaner()
    load1 = nhdplusv231_nabu(harvest)
