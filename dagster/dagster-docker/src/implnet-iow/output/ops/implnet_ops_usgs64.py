...
@op
def usgs64_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs64")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs64_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs64")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs64():
    harvest = usgs64_gleaner()
    load1 = usgs64_nabu(harvest)
