...
@op
def ref61_gleaner(context):
    returned_value = gleanerio(("gleaner"), "ref61")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def ref61_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "ref61")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_ref61():
    harvest = ref61_gleaner()
    load1 = ref61_nabu(harvest)
