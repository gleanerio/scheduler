...
@op
def usgs85_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs85")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs85_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs85")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs85():
    harvest = usgs85_gleaner()
    load1 = usgs85_nabu(harvest)
