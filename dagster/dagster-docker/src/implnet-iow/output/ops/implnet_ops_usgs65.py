...
@op
def usgs65_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs65")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs65_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs65")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs65():
    harvest = usgs65_gleaner()
    load1 = usgs65_nabu(harvest)
