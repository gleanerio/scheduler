...
@op
def cuahsi175_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi175")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi175_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi175")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi175():
    harvest = cuahsi175_gleaner()
    load1 = cuahsi175_nabu(harvest)
    # load2 = cuahsi175_prov(load1)
