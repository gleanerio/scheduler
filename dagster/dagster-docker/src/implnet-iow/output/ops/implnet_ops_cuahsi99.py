...
@op
def cuahsi99_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi99")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi99_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi99")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi99():
    harvest = cuahsi99_gleaner()
    load1 = cuahsi99_nabu(harvest)
