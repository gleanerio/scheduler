...
@op
def cuahsi166_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi166")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi166_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi166")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi166():
    harvest = cuahsi166_gleaner()
    load1 = cuahsi166_nabu(harvest)
