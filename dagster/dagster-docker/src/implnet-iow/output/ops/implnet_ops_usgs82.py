...
@op
def usgs82_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs82")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs82_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs82")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs82():
    harvest = usgs82_gleaner()
    load1 = usgs82_nabu(harvest)
