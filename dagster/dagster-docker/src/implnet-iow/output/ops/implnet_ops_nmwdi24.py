...
@op
def nmwdi24_gleaner(context):
    returned_value = gleanerio(("gleaner"), "nmwdi24")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def nmwdi24_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "nmwdi24")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_nmwdi24():
    harvest = nmwdi24_gleaner()
    load1 = nmwdi24_nabu(harvest)
