...
@op
def usgs67_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs67")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs67_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs67")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs67():
    harvest = usgs67_gleaner()
    load1 = usgs67_nabu(harvest)
