...
@op
def cuahsi160_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi160")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi160_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi160")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi160():
    harvest = cuahsi160_gleaner()
    load1 = cuahsi160_nabu(harvest)
