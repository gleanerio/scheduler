...
@op
def usgs96_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs96")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs96_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs96")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs96():
    harvest = usgs96_gleaner()
    load1 = usgs96_nabu(harvest)
