...
@op
def cuahsi125_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi125")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi125_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi125")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi125():
    harvest = cuahsi125_gleaner()
    load1 = cuahsi125_nabu(harvest)
