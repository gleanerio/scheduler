...
@op
def usgs95_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs95")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs95_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs95")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs95():
    harvest = usgs95_gleaner()
    load1 = usgs95_nabu(harvest)
