...
@op
def usgs90_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs90")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs90_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs90")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs90():
    harvest = usgs90_gleaner()
    load1 = usgs90_nabu(harvest)
