...
@op
def ref42_gleaner(context):
    returned_value = gleanerio(("gleaner"), "ref42")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def ref42_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "ref42")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_ref42():
    harvest = ref42_gleaner()
    load1 = ref42_nabu(harvest)
