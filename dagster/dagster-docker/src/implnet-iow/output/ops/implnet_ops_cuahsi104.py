...
@op
def cuahsi104_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi104")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi104_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi104")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi104():
    harvest = cuahsi104_gleaner()
    load1 = cuahsi104_nabu(harvest)
