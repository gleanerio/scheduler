...
@op
def cuahsi135_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi135")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi135_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi135")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi135():
    harvest = cuahsi135_gleaner()
    load1 = cuahsi135_nabu(harvest)
