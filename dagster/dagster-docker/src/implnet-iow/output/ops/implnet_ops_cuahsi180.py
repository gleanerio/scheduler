...
@op
def cuahsi180_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi180")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi180_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi180")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi180():
    harvest = cuahsi180_gleaner()
    load1 = cuahsi180_nabu(harvest)
