...
@op
def iow35_gleaner(context):
    returned_value = gleanerio(("gleaner"), "iow35")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def iow35_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "iow35")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_iow35():
    harvest = iow35_gleaner()
    load1 = iow35_nabu(harvest)
