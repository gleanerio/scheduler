...
@op
def cuahsi112_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi112")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi112_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi112")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi112():
    harvest = cuahsi112_gleaner()
    load1 = cuahsi112_nabu(harvest)
