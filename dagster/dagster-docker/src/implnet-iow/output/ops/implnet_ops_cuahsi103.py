...
@op
def cuahsi103_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi103")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi103_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi103")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi103():
    harvest = cuahsi103_gleaner()
    load1 = cuahsi103_nabu(harvest)
    # load2 = cuahsi103_prov(load1)
