...
@op
def ref43_gleaner(context):
    returned_value = gleanerio(("gleaner"), "ref43")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def ref43_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "ref43")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_ref43():
    harvest = ref43_gleaner()
    load1 = ref43_nabu(harvest)
