...
@op
def ref52_gleaner(context):
    returned_value = gleanerio(("gleaner"), "ref52")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def ref52_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "ref52")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_ref52():
    harvest = ref52_gleaner()
    load1 = ref52_nabu(harvest)
