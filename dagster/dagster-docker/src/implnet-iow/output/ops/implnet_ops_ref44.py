...
@op
def ref44_gleaner(context):
    returned_value = gleanerio(("gleaner"), "ref44")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def ref44_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "ref44")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_ref44():
    harvest = ref44_gleaner()
    load1 = ref44_nabu(harvest)
