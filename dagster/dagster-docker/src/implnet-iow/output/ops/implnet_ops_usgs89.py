...
@op
def usgs89_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs89")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs89_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs89")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs89():
    harvest = usgs89_gleaner()
    load1 = usgs89_nabu(harvest)
