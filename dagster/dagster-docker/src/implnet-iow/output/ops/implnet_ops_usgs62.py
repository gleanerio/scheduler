...
@op
def usgs62_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs62")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs62_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs62")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs62():
    harvest = usgs62_gleaner()
    load1 = usgs62_nabu(harvest)
