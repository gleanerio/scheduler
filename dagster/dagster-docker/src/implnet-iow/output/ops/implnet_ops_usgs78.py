...
@op
def usgs78_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs78")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs78_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs78")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs78():
    harvest = usgs78_gleaner()
    load1 = usgs78_nabu(harvest)
