...
@op
def usgs66_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs66")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs66_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs66")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs66():
    harvest = usgs66_gleaner()
    load1 = usgs66_nabu(harvest)
