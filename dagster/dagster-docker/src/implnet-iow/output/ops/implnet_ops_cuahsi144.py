...
@op
def cuahsi144_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi144")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi144_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi144")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi144():
    harvest = cuahsi144_gleaner()
    load1 = cuahsi144_nabu(harvest)
