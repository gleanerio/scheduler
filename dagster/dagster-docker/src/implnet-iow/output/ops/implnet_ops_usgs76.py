...
@op
def usgs76_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs76")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs76_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs76")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs76():
    harvest = usgs76_gleaner()
    load1 = usgs76_nabu(harvest)
