...
@op
def cuahsi159_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi159")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi159_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi159")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi159():
    harvest = cuahsi159_gleaner()
    load1 = cuahsi159_nabu(harvest)
