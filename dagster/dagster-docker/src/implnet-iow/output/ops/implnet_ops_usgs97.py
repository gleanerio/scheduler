...
@op
def usgs97_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs97")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs97_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs97")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs97():
    harvest = usgs97_gleaner()
    load1 = usgs97_nabu(harvest)
