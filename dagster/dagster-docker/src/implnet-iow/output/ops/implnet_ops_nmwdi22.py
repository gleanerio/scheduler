...
@op
def nmwdi22_gleaner(context):
    returned_value = gleanerio(("gleaner"), "nmwdi22")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def nmwdi22_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "nmwdi22")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_nmwdi22():
    harvest = nmwdi22_gleaner()
    load1 = nmwdi22_nabu(harvest)
