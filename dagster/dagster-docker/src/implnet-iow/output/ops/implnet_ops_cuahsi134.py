...
@op
def cuahsi134_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi134")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi134_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi134")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi134():
    harvest = cuahsi134_gleaner()
    load1 = cuahsi134_nabu(harvest)
