...
@op
def usgs63_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs63")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs63_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs63")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs63():
    harvest = usgs63_gleaner()
    load1 = usgs63_nabu(harvest)
