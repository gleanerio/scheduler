...
@op
def cuahsi162_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi162")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi162_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi162")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi162():
    harvest = cuahsi162_gleaner()
    load1 = cuahsi162_nabu(harvest)
