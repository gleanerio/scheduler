...
@op
def epa29_gleaner(context):
    returned_value = gleanerio(("gleaner"), "epa29")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def epa29_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "epa29")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_epa29():
    harvest = epa29_gleaner()
    load1 = epa29_nabu(harvest)
