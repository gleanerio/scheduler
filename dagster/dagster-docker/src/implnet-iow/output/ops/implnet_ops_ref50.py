...
@op
def ref50_gleaner(context):
    returned_value = gleanerio(("gleaner"), "ref50")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def ref50_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "ref50")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_ref50():
    harvest = ref50_gleaner()
    load1 = ref50_nabu(harvest)
