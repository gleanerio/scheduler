...
@op
def cuahsi114_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi114")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi114_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi114")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi114():
    harvest = cuahsi114_gleaner()
    load1 = cuahsi114_nabu(harvest)
