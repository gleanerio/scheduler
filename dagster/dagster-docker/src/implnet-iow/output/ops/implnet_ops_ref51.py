...
@op
def ref51_gleaner(context):
    returned_value = gleanerio(("gleaner"), "ref51")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def ref51_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "ref51")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_ref51():
    harvest = ref51_gleaner()
    load1 = ref51_nabu(harvest)
