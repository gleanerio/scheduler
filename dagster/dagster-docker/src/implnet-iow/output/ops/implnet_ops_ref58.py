...
@op
def ref58_gleaner(context):
    returned_value = gleanerio(("gleaner"), "ref58")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def ref58_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "ref58")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_ref58():
    harvest = ref58_gleaner()
    load1 = ref58_nabu(harvest)
