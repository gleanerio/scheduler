...
def wade21_gleaner(context):
    returned_value = gleanerio(("gleaner"), "wade21")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def wade21_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "wade21")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_wade21():
    harvest = wade21_gleaner()
    load1 = wade21_nabu(harvest)
