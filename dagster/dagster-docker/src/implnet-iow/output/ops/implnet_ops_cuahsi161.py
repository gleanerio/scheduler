...
@op
def cuahsi161_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi161")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi161_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi161")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi161():
    harvest = cuahsi161_gleaner()
    load1 = cuahsi161_nabu(harvest)
