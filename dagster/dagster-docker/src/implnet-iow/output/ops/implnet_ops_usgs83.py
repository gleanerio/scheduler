...
@op
def usgs83_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs83")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs83_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs83")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs83():
    harvest = usgs83_gleaner()
    load1 = usgs83_nabu(harvest)
