...
@op
def cuahsi172_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi172")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi172_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi172")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi172():
    harvest = cuahsi172_gleaner()
    load1 = cuahsi172_nabu(harvest)
