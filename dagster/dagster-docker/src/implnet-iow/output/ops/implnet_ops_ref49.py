...
@op
def ref49_gleaner(context):
    returned_value = gleanerio(("gleaner"), "ref49")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def ref49_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "ref49")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_ref49():
    harvest = ref49_gleaner()
    load1 = ref49_nabu(harvest)
