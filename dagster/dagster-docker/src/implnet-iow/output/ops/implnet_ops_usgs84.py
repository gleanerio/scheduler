...
@op
def usgs84_gleaner(context):
    returned_value = gleanerio(("gleaner"), "usgs84")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def usgs84_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "usgs84")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_usgs84():
    harvest = usgs84_gleaner()
    load1 = usgs84_nabu(harvest)
