...
@op
def cuahsi137_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi137")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi137_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi137")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi137():
    harvest = cuahsi137_gleaner()
    load1 = cuahsi137_nabu(harvest)
