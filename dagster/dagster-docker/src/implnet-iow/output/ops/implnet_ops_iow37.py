...
@op
def iow37_gleaner(context):
    returned_value = gleanerio(("gleaner"), "iow37")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def iow37_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "iow37")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_iow37():
    harvest = iow37_gleaner()
    load1 = iow37_nabu(harvest)
