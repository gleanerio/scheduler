...
@op
def cuahsi132_gleaner(context):
    returned_value = gleanerio(("gleaner"), "cuahsi132")
    r = str('returned value:{}'.format(returned_value))
    get_dagster_logger().info(f"Gleaner notes are  {r} ")
    return r

@op
def cuahsi132_nabu(context, msg: str):
    returned_value = gleanerio(("nabu"), "cuahsi132")
    r = str('returned value:{}'.format(returned_value))
    return msg + r

@graph
def harvest_cuahsi132():
    harvest = cuahsi132_gleaner()
    load1 = cuahsi132_nabu(harvest)
